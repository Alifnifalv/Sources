...
namespace Eduegate.Domain.Mappers.Mutual
{
    public class GeoLocationLogMapper : DTOEntityDynamicMapper
    {
        ...
        public List<GeoLocationLogDTO> GetGeoLocationByParentLoginID(long loginID)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var currentAcademicYearStatus = dbContext.Settings.FirstOrDefault(x => x.SettingCode == "CURRENT_ACADEMIC_YEAR_STATUSID").SettingValue;
                int? currentAcademicYearStatusID = int.Parse(currentAcademicYearStatus);

                var parentDet = dbContext.Parents.FirstOrDefault(x => x.LoginID == loginID);

                //var studentDet = parentDet != null ? dbContext.Students.Where(y => y.ParentID == parentDet.ParentIID) : null;

                //var studentList = parentDet != null ? dbContext.Students.Where(y => y.ParentID == parentDet.ParentIID && y.IsActive == true && y.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID).ToList() : null;

                var studentDet = parentDet != null ? dbContext.Students.FirstOrDefault(y => y.ParentID == parentDet.ParentIID && y.IsActive == true && y.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID) : null;
                ...
                if (studentDet != null)
                {
                    ...
                    var transportDet = dbContext.StudentRouteStopMaps.Where(a => a.StudentID == studentDet.StudentIID && a.IsActive == true && a.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID).ToList().LastOrDefault();

                    if (transportDet != null)
                    {
                        var route = transportDet.Routes11 != null ? dbContext.Routes1.FirstOrDefault(b => b.RouteID == transportDet.Routes11.RouteID && b.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID) : transportDet.Routes1 != null ? dbContext.Routes1.FirstOrDefault(b => b.RouteID == transportDet.Routes1.RouteID) : null;
                        ...
                        var assign = route != null ? dbContext.AssignVehicleMaps.FirstOrDefault(d => d.RouteID == route.RouteID && d.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID) : null;

                        var employeeDet = assign != null ? dbContext.Employees.FirstOrDefault(e => e.EmployeeIID == assign.EmployeeID) : null;

                        var logs = employeeDet != null ? dbContext.GeoLocationLogs.OrderByDescending(x => x.GeoLocationLogIID).FirstOrDefault(x => x.ReferenceID2 == employeeDet.EmployeeIID.ToString()) : null;
        ...
        public override string GetEntity(long IID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var entitydto = dbContext.GeoLocationLogs.FirstOrDefault(x => x.GeoLocationLogIID == IID);
        ...
        public void UpdateUserGeoLocation(string geoLocation, bool? isDriver)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var geoLocationLastData = dbContext.GeoLocationLogs.OrderByDescending(x => x.GeoLocationLogIID).FirstOrDefault(x => x.ReferenceID1 == geoLocationDeails.ReferenceID1);
        ...
        public GeoLocationLog GetLastGeoLocationByLoginID(long loginID)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var geoLocationLastData = dbContext.GeoLocationLogs.OrderByDescending(x => x.GeoLocationLogIID).FirstOrDefault(x => x.ReferenceID1 == referenceID1);
                if(geoLocationLastData != null)
                {
