...
namespace Eduegate.Domain.Repository
{
    ...
    {
        #region Search
        public View GetViewInfo(SearchView view, string languageCode = null)
        {
            View viewDef = null;
            ...
            try
            {
                ...
                {
                    viewDef = dbContext.Views
                        .Include("ViewActions")
                        .Include(b => b.ViewCultureDatas)
                        .FirstOrDefault(x => x.ViewID == (long)view);
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    columns = dbContext.FilterColumns
                            .Include(x => x.FilterColumnCultureDatas)
        ...
        public List<FilterColumnUserValue> GetUserFilterMetadata(SearchView view, long? loginID, int companyID = 0)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return dbContext.FilterColumnUserValues.Where(x => x.ViewID == (long)view && x.LoginID == (loginID == 0 ? (long?)null : loginID) && x.CompanyID == companyID).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public bool HasUserFilter(SearchView view, long? loginID, long companyID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.FilterColumnUserValues.Any(x => x.ViewID == (long)view && (x.LoginID == (loginID == 0 ? (long?)null : loginID) && x.CompanyID == companyID));
            }
        }
        ...
        public bool SaveUserFilterMetadata(SearchView view, long? loginID, List<FilterColumnUserValue> values, int companyID = 0)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    dbContext.FilterColumnUserValues.RemoveRange(dbContext.FilterColumnUserValues.Where(x => x.ViewID == (long)view && x.LoginID == loginID));
        ...
        public List<DocumentReferenceType> GetDocumentReferenceTypes(Eduegate.Services.Contracts.Enums.Systems system)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.DocumentReferenceTypes.Where(a => a.System == system.ToString()).OrderBy(a => a.InventoryTypeName).ToList();
            }
        }
        ...
        public List<DocumentType> GetDocumentType(Eduegate.Services.Contracts.Enums.Systems system, int? CompanyID = 0)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.DocumentTypes.Where(a => a.System == system.ToString() && a.CompanyID == CompanyID).OrderBy(a => a.TransactionTypeName).ToList();
            }
        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    updatedEntity = dbContext.DocumentTypes
                        .Include(a => a.Workflow)
                        .Where(x => x.DocumentTypeID == entity.DocumentTypeID).FirstOrDefault();
                }
            }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    if (dbData.IsNotNull() && dbData.Count > 0)
                    {
                        ...
                        {
                            var dtm = dbContext.DocumentTypeTypes.Where(x => x.DocumentTypeID == data.DocumentTypeID).ToList();
        ...
        public List<DocumentTypeType> GetDocumentMap(int documentTypeID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.DocumentTypeTypes.Where(a => a.DocumentTypeID == documentTypeID).ToList();
            }
        }
        ...
        public Location GetLocation(long locationID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Locations.Where(a => a.LocationIID == locationID).FirstOrDefault();
            }
        }
        ...
        public Location SaveLocation(Location entity)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    dbContext.SaveChanges();
                    updatedEntity = dbContext.Locations.Where(x => x.LocationIID == entity.LocationIID).FirstOrDefault();
                }
            }
            catch (Exception exception)
            {
        ...
        public string GetDocumentTypeName(int documentTypeMapID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return (from dt in dbContext.DocumentTypes
                        ...
                        select dt.TransactionTypeName).FirstOrDefault();
            }
        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    foreach (var entity in entities)
                    {
                        ...
                        {
                            existingData = existingDocumentTypeTransactionNumbers.Where(x => x.Month == entity.Month && x.Year == entity.Year).FirstOrDefault();
                        }
        ...
        public List<DocumentTypeTransactionNumber> GetDocumentTypeTransactionNumber(long documentTypeID)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return dbContext.DocumentTypeTransactionNumbers.Where(x => x.DocumentTypeID == documentTypeID).ToList();
                }
            }
            catch (Exception exception)
            {
        ...
        public DocumentType SaveNextTransactionNumberByMonthYear(int documentTypeID, int month, int year)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var documentType = dbContext.DocumentTypes.Include(x => x.DocumentTypeTransactionNumbers).Where(a => a.DocumentTypeID == documentTypeID).FirstOrDefault();
