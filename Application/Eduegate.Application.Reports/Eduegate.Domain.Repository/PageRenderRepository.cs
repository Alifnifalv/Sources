...
namespace Eduegate.Domain.Repository
{
    public class PageRenderRepository
    {
        public List<BoilerPlate> GetBoilerPlatesByPageID(long pageID)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                boilerplates = (from a in db.PageBoilerplateMaps
                                ...
                                select b).OrderBy(a => a.BoilerPlateID).ToList();
        ...
        public BoilerPlate GetBoilerPlate(long boilerPlateID)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.BoilerPlates.Where(a => a.BoilerPlateID == boilerPlateID).FirstOrDefault();
        ...
        public Page GetPageDetails(long pageID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var page = db.Pages.Where(a => a.PageID == pageID).FirstOrDefault();
        ...
        public List<Site> GetSites()
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.Sites.OrderBy(a=> a.SiteName).ToList();
            }
        }
        ...
        public List<PageType> GetPageTypes()
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.PageTypes.OrderBy(a=> a.TypeName).ToList();
            }
        }
        ...
        public List<Page> GetPages(int? companyID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (companyID.HasValue)
                    return db.Pages.OrderBy(a => a.PageName).Where(a=> a.CompanyID == companyID.Value).ToList();
                else
                    return db.Pages.OrderBy(a=> a.PageName).ToList();
            }
        }
                    ...
                    using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                    {
                        ...
                        else
                        {
                            ...
                            if (paramReferenceID.HasValue || paramReferenceID > 0)
                                deletedMaps = dbContext.PageBoilerplateMaps.Include("PageBoilerplateMapParameters").Where(a => a.PageID == entity.PageID
                                && a.ReferenceID == paramReferenceID
                                && !pageboilerpageIDs.Contains(a.PageBoilerplateMapIID));
                            else
                                deletedMaps = dbContext.PageBoilerplateMaps.Include("PageBoilerplateMapParameters").Where(a => a.PageID == entity.PageID
                                && a.ReferenceID == null
                                && !pageboilerpageIDs.Contains(a.PageBoilerplateMapIID));
                            ...
                            foreach (var map in entity.PageBoilerplateMaps)
                            {
                                ...
                                foreach (var param in map.PageBoilerplateMapParameters)
                                {
                                    if (param.PageBoilerplateMapParameterIID == 0)
                                        dbContext.Entry(param).State = System.Data.Entity.EntityState.Added;
                                    else
                                        dbContext.Entry(param).State = System.Data.Entity.EntityState.Modified;
                                }
                            }
                        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (pageBoilerPlateMaps.IsNotNull() && pageBoilerPlateMaps.Count > 0)
                    {
                        ...
                        {
                            PageBoilerplateMap pageEntity = dbContext.PageBoilerplateMaps.Where(x => x.PageBoilerplateMapIID == pbMap.PageBoilerplateMapIID).FirstOrDefault();
                            ...
                            if (pbMap.PageBoilerplateMapParameters.IsNotNull() && pbMap.PageBoilerplateMapParameters.Count > 0)
                            {
                                ...
                                {
                                    PageBoilerplateMapParameter pageParamEntity = dbContext.PageBoilerplateMapParameters.Where(y => y.PageBoilerplateMapParameterIID == pbmParameter.PageBoilerplateMapParameterIID).FirstOrDefault();
