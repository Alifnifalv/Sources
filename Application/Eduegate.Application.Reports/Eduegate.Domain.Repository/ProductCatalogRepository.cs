...
using model = Eduegate.Domain.Entity.Models;
...
namespace Eduegate.Domain.Repository
{
    public class ProductCatalogRepository 
    {
        ...
        public List<ProductSKUDetail> GetProductListWithSKU(string searchText, int dataSize)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (!string.IsNullOrEmpty(searchText))
                {
                    ...
                    productList = dbContext.Database.SqlQuery<ProductSKUDetail>(searchQuery).ToList();
                }
                else
                {
        ...
        {
            model.dbEduegateERPContext db = new model.dbEduegateERPContext();
            ...
            switch (MenuId)
            {
                case (int)ProductListType.Latest:
                    ...
                    var subQry = (from p in db.Products
                                  orderby DbFunctions.TruncateTime(p.Created)
                                  select new
                                  {
                                      Created = DbFunctions.TruncateTime(p.Created)
                    ...
                    var qryProduct = (from p in db.Products
                                      ...
                                      where DbFunctions.TruncateTime(p.Created) >= last.Created && DbFunctions.TruncateTime(p.Created) <= zero.Created
        ...
        public List<SearchResult> GetProductsBySearchCriteria(string searchText, int pageNumber, int pageSize, string sortBy)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                productList = dbContext.Database.SqlQuery<SearchResult>(searchQuery).ToList().Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
            }
            return productList;
        }
        ...
        public Category GetCategoryTags(long categoryID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var entity = dbContext.Categories.Where(x => x.CategoryIID == categoryID).FirstOrDefault();
        ...
        public Category SaveCategory(Category entity,int companyID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var getCategoryDataByCode = dbContext.Categories.FirstOrDefault(c => c.CategoryCode == entity.CategoryCode);

                    if (getCategoryDataByCode != null/* && entity.CategoryIID == 0*/)
                    {
                        throw new Exception("Same category code already exist!");
                    }
                    ...
                    else
                    {
                        ...
                        dbContext.CategoryImageMaps.RemoveRange(dbContext.CategoryImageMaps.Where(a => a.CategoryID == entity.CategoryIID && a.CompanyID == companyID));
                        dbContext.CategoryTagMaps.RemoveRange(dbContext.CategoryTagMaps.Where(a => a.CategoryID == entity.CategoryIID));
                        dbContext.CategoryCultureDatas.RemoveRange(dbContext.CategoryCultureDatas.Where(a => a.CategoryID == entity.CategoryIID));
                    }
                    ...
                    {
                        dbContext.Entry(tagMap).State = System.Data.Entity.EntityState.Added;

                        if (tagMap.CategoryTagID == null || tagMap.CategoryTagID == 0)
                        {
                            dbContext.Entry(tagMap.CategoryTag).State = System.Data.Entity.EntityState.Added;
                        }
                        else
                        {
                            dbContext.Entry(tagMap.CategoryTag).State = System.Data.Entity.EntityState.Modified;
                        }
                    }
                    ...
                    foreach (var settingMap in entity.CategorySettings)
                    {
                        ...
                        { 
                            if (settingMap.CategorySettingsID > 0) { dbContext.Entry(settingMap).State = System.Data.Entity.EntityState.Modified; }
                            else { dbContext.Entry(settingMap).State = System.Data.Entity.EntityState.Added; }
                        }
                    }
                    dbContext.SaveChanges();
        ...
        public ProductSKUDetail GetProductAndSKUByID(long productSKUMapID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (productSKUMapID > 0)
                {
                    ...
                    productList = dbContext.Database.SqlQuery<ProductSKUDetail>(searchQuery).FirstOrDefault();
                }
            }
            return productList;
        }
        ...
        public List<ProductPriceListType> GetProductPriceListTypes()
        {
            ...
            {
                return db.ProductPriceListTypes.ToList();
            }
        }
        ...
        public List<ProductPriceListLevel> GetProductPriceListLevels()
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductPriceListLevels.ToList();
            }
        }
        ...
        public List<ProductImageMap> GetProductDetailImages(long skuID)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                productImageMapList = db.ProductImageMaps.OrderBy(a => a.Sequence).Where(a => a.ProductSKUMapID == skuID && a.ProductImageTypeID == 3).ToList();
            }
        ...
        public List<ProductPropertyMap> GetProductKeyFeatures(long skuID, int cultureID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                try
                {
                    ...
                    var productPropertyMaps = db.ProductPropertyMaps.Where(a => a.ProductSKUMapID == skuID && (a.PropertyID != null || (a.PropertyID == null && !string.IsNullOrEmpty(a.Value)))).ToList();
        ...
        public ProductInventoryConfigCultureData GetProductDescriptionCultureID(long skuID, long cultureID)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                if (exists)
                {
                    ...
                    if (db.ProductInventoryConfigCultureDatas.Where(a => a.ProductInventoryConfigID == config.ProductInventoryConfigIID).Any())
                    {
                        cultureConfig = db.ProductInventoryConfigCultureDatas.Where(a => a.ProductInventoryConfigID == config.ProductInventoryConfigIID && a.CultureID == cultureID).FirstOrDefault();
                    }
                }
            }
        ...
        public ProductSKUDetail GetProductSKUDetail(long skuIID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                return dbContext.Database.SqlQuery<ProductSKUDetail>(searchQuery).FirstOrDefault();
            }
        }
        ...
        public ProductDTO GetUnitIDs(long skuIID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var prdct = dbContext.Products.FirstOrDefault(x => x.ProductSKUMaps.Select(s => s.ProductSKUMapIID == skuIID).FirstOrDefault());
        ...
        public Location GetLocationDetail(long locationID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Locations.Where(l => l.LocationIID == locationID).FirstOrDefault();
            }
        }
        ...
        public Location GetLocationDetailByBarCode(string barcode)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                location = dbContext.Locations.Where(l => l.Barcode == barcode).FirstOrDefault();
            }
        ...
        public Location GetProductSKULocation(long skuID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var productLocationMap = dbContext.ProductLocationMaps.Where(l => l.ProductSKUMapID == skuID).FirstOrDefault();
        ...
        public ProductLocationMap GetProductLocationMap(long locationID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ProductLocationMaps.Where(l => l.LocationID == locationID).FirstOrDefault();
            }
        }
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (cultureID == 1)
                {
                    brand = (from p in db.Products
                             ...
                             join psm in db.ProductSKUMaps on p.ProductIID equals psm.ProductID
                             where psm.ProductSKUMapIID == skuID
                             select b.BrandName.ToString()).FirstOrDefault();
                }
                else
                {
                    brand = (from p in db.Products
                             ...
                             select bs.BrandName.ToString()).FirstOrDefault();
                }
            }
            return brand;
        }
        ...
        public List<ProductVideoMap> GetProductVideos(long skuID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductVideoMaps.Where(a => a.ProductSKUMapID == skuID).OrderByDescending(a => a.Sequence).ToList();
            }
        }
