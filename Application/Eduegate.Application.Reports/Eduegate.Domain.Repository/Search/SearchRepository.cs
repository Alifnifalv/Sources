   ...
   public class SearchRepository
    {
       public List<UserView> GetUserViews(SearchView view, long? userID, int companyID)
       {
        ...
        public DataTable SearchData(SearchView view, ref long totalRecords, int currentPage, int pageSize, string orderby, long loginId, string runtimeFilter, int companyID,char viewType, byte? schoolID, int? academicYearID)
        {
       ...
       private string GetOrderByDataType(long ViewID, string orderByColumn)
       {
           using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
           {
               var col = dbContext.ViewGridColumns.Where(y => y.ViewID == ViewID && y.PhysicalColumnName == orderByColumn).Select(x => x.DataType).FirstOrDefault();
