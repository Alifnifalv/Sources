    ...
    public class UserServiceRepository
    {
        ...
        public string UpdateProfileDetails(Login login)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (login.IsNotNull())
                    {
                        Login loginEntity = dbContext.Logins.Where(x => x.LoginEmailID == login.LoginEmailID).FirstOrDefault();
        ...
        public bool UpdatePersonalProfileDetails(Login login)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (login.IsNotNull())
                    {
                        Login loginEntity = login.LoginIID != 0 ? dbContext.Logins.Where(x => x.LoginIID == login.LoginIID).FirstOrDefault()  : dbContext.Logins.Where(x => x.LoginEmailID == login.LoginEmailID).FirstOrDefault();
                        ...
                        if (login.Customers.IsNotNull())
                        {
                            foreach (Customer customer in login.Customers)
                            {
                                ...
                                if (customerEntity.IsNotNull())
                                {
                                    ...
                                    customerEntity.IsSubscribeForNewsLetter = customer.IsSubscribeForNewsLetter;
                                    var billingAddressContact = dbContext.Contacts.Where(a => a.LoginID == login.LoginIID && a.IsBillingAddress == true).FirstOrDefault();
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (contact.IsNotNull())
                    {
                        ...
                        List<Contact> contacts = dbContext.Contacts.Where(x => x.LoginID == contact.LoginID).ToList();
        ...
        public bool DeleteAddressBook(decimal addressIID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    Contact contactEntity = dbContext.Contacts.Where(x => x.ContactIID == addressIID).FirstOrDefault();
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                if (orderID == default(long))
                {
                    historyDetails = (from transactionHead in dbContext.TransactionHeads
                                      ...
                                      select transactionHead).OrderByDescending(x => x.HeadIID).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
                }
                else
                {
                    historyDetails = (from transactionHead in dbContext.TransactionHeads
                ...
                if (historyDetails.IsNotNull() && historyDetails.Count > 0)
                {
                    foreach (var history in historyDetails)
                    {
                        ...
                        if (voucherOrder != null)
                        {
                            ...
                            var vouchers = dbContext.TransactionHeadEntitlementMaps.Where(a => a.EntitlementID == (short)Eduegate.Framework.Payment.EntitlementType.Voucher && a.TransactionHeadID == history.HeadIID).FirstOrDefault();
                        ...
                        {
                            var countryName = (from a in dbContext.Countries where a.CountryID == historyHeader.DeliveryAddress.CountryID select a.CountryName).FirstOrDefault();
                            var areaName = (from a in dbContext.Areas where a.AreaID == historyHeader.DeliveryAddress.AreaID select a.AreaName).FirstOrDefault();
                            var cityName = (from a in dbContext.Cities where a.CityID == historyHeader.DeliveryAddress.CityID select a.CityName).FirstOrDefault();
                        ...
                        {
                            var countryName = (from a in dbContext.Countries where a.CountryID == historyHeader.BillingAddress.CountryID select a.CountryName).FirstOrDefault();
                            var areaName = (from a in dbContext.Areas where a.AreaID == historyHeader.BillingAddress.AreaID select a.AreaName).FirstOrDefault();
                            var cityName = (from a in dbContext.Cities where a.CityID == historyHeader.BillingAddress.CityID select a.CityName).FirstOrDefault();
                        ...
                        if (history.TransactionDetails.IsNotNull() && history.TransactionDetails.Count > 0)
                        {
                            ...
                            foreach (TransactionDetail transactionDetail in history.TransactionDetails)
                            {
                                ...
                                if (lang == "ar")
                                {
                                    var prodtAr = dbContext.ProductSKUCultureDatas.Where(a => a.ProductSKUMapID == transactionDetail.ProductSKUMapID && a.CultureID == 2).Select(a => a.ProductSKUName).FirstOrDefault();
                                    productDetail.ProductName = prodtAr;
                                }
                                else
                                {
                                    productDetail.ProductName = dbContext.Products.Where(x => x.ProductIID == transactionDetail.ProductID).FirstOrDefault().ProductName;
                                }

                                productDetail.ProductImageUrl = (dbContext.ProductImageMaps.Where(p => p.ProductSKUMapID == transactionDetail.ProductSKUMapID && p.ProductImageTypeID == (long)Framework.Helper.Enums.ImageType.ThumbnailImage).FirstOrDefault() != null) ? (dbContext.ProductImageMaps.Where(p => p.ProductSKUMapID == transactionDetail.ProductSKUMapID && p.ProductImageTypeID == (long)Framework.Helper.Enums.ImageType.ThumbnailImage).FirstOrDefault().ImageFile) : null; ;
                                productDetail.ProductSKUMapID = transactionDetail.ProductSKUMapID;
                                productDetail.Quantity = transactionDetail.Quantity;
        ...
        public bool AddSaveForLater(string loginEmailID, long skuID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (dbContext.WishLists.Where(x => x.CustomerID == customerID && x.SKUID == skuID && x.IsWishList == true).Any())
                    return false; //Based on true/false we are showing the message whether whishlist is added or not existing item stopping to show returning false
        ...
        public TransactionHeadShoppingCartMap GetTransactionHeadShoppingCartMap(long transactionHeadID)
        {
            ...
            {
                TransactionHeadShoppingCartMap obj = db.TransactionHeadShoppingCartMaps.Where(x => x.TransactionHeadID == transactionHeadID).ToList().FirstOrDefault();
                return obj;
            }
        }
        ...
        public UserDeviceMap GetRegisteredActiveUserDeviceByLoginID(long loginID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.UserDeviceMaps
                    ...
                    .OrderByDescending(x => x.UserDeviceMapIID).FirstOrDefault();
            }
        }
        ...
        public UserDeviceMap GetLastRegisteredUserDeviceByLoginID(long loginID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.UserDeviceMaps
                    ...
                    .OrderByDescending(y => y.UserDeviceMapIID).FirstOrDefault();
            }
        }
        ...
        public List<long> GetAllUsersLoginIds()
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.Logins.Select(x => x.LoginIID).ToList();
            }
        }
