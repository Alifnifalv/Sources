...
namespace Eduegate.Domain
{
    public class BoilerPlateBL
    {
        ...
        public BoilerPlateDataSourceDTO GetBoilerPlates(BoilerPlateInfo boilerPlateInfo)
        {
            ...
            switch (boilerPlateInfo.boilerPlateID)
            {
                ...
                case BoilerPlates.ProductDetailWithAddToCart:
                    ...
                    //var settingDTO = new SettingBL().GetSettingDetail("ONLINEBRANCHID");
                    long customerID = 0;
                    try
                ...
                case BoilerPlates.WebsiteOneDealADay:
                    var settingDetail = new SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINEBRANCHID, _callContext.CompanyID.Value);
                ...
                case BoilerPlates.QuickLaunch:
                    ...
                    boilerPlateDataSourceDTO.BoilerPlateResultSetList.Add(
                      BoilerPlateMapper.Mapper.GetQuickLanuch(
                          new MenuRepository().GetMenusByType(Services.Contracts.Enums.MenuLinkTypes.QuickLaunch, parentMenuID, _callContext.LanguageCode)
                          ));
                    break;
        ...
        public DirectorsDashBoardDTO GetTeacherRelatedDataForDirectorsDashBoard()
        {
            ...
            {
                var empPermenant = dbContext.Settings.FirstOrDefault(x => x.SettingCode == "EMPLOYEE_PERMANANT").SettingValue;
                var empTemporary = dbContext.Settings.FirstOrDefault(x => x.SettingCode == "EMPLOYEE_TEMPORARY").SettingValue;
                var adminManDesigID = dbContext.Settings.FirstOrDefault(p => p.SettingCode == "EMP_ADMINMANAGER_DESIGNATION_ID").SettingValue;
                var administrationDesigID = dbContext.Settings.FirstOrDefault(p => p.SettingCode == "EMP_ADMINISTRATION_DESIGNATION_ID").SettingValue;

                var schools = dbContext.School.ToList();

                var getAcademic = dbContext.AcademicYears.FirstOrDefault(a => a.AcademicYearID == _callContext.AcademicYearID);
                var academic = getAcademic.StartDate.Value.Year + " - " + getAcademic.EndDate.Value.Year;

                foreach (var scl in schools)
                {
                    var employees = dbContext.Employees.Where(e => e.BranchID == scl.SchoolID && e.IsActive == true).ToList();
        ...
        public DirectorsDashBoardDTO GetClassTeachersDashBoard()
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var getAcademic = dbContext.AcademicYears.FirstOrDefault(a => a.AcademicYearID == _callContext.AcademicYearID);
                var academic = getAcademic.StartDate.Value.Year + " - " + getAcademic.EndDate.Value.Year;
                var classSec = dbContext.ClassClassTeacherMaps.Where(x => x.Employee1.LoginID == _callContext.LoginID).ToList();
        ...
        public EmployeeDTO UserProfileForDashBoard()
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var getUserDetails = dbContext.Employees.FirstOrDefault(a => a.LoginID == _callContext.LoginID);
                ...
                var attendance = dbContext.StaffAttendences.Where(x => x.EmployeeID == getUserDetails.EmployeeIID).ToList().LastOrDefault();
        ...
        public Services.Contracts.Notifications.PushNotificationDTO GetNotificationsForDashBoard()
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var getNotificationDetails = dbContext.NotificationAlerts.Where(a => a.ToLoginID == _callContext.LoginID && a.NotificationDate.Value >= checkDateBy.Value).ToList();
        ...
        public TimeTableDTO GetWeeklyTimeTableForDashBoard(int weekDayID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var getTimeTable = dbContext.TimeTableAllocations.Where(a => a.Employee.LoginID == _callContext.LoginID && a.WeekDayID == weekDayID).ToList();
