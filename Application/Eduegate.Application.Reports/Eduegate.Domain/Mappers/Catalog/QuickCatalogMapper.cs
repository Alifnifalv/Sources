...
namespace Eduegate.Domain.Mappers.Catalog
{
    public class QuickCatalogMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            var existingProductCategoryMapMapList = new List<ProductCategoryMap>();

            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                //var repository = new EntiyRepository<ProductCategoryMap, dbEduegateERPContext>(dbContext);
                var rakrepository = new EntiyRepository<ProductSKURackMap, dbEduegateERPContext>(dbContext);
                //repository.Delete(x => x.ProductID == entity.ProductIID);

                existingProductCategoryMapMapList = dbContext.ProductCategoryMaps
                    .Include("FeeDueInventoryMaps")
                    .Where(c => c.ProductID == entity.ProductIID).ToList();

                if (existingProductCategoryMapMapList.Any(x => x.FeeDueInventoryMaps.Count == 0))
                {
                    dbContext.ProductCategoryMaps.RemoveRange(existingProductCategoryMapMapList);
                }

                rakrepository.Delete(x => x.ProductID == entity.ProductIID);
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var repository = new EntiyRepository<ProductBundle, dbEduegateERPContext>(dbContext);
                ...
                var bundentities = dbContext.ProductBundles.Where(x =>
                    ...
                    !IIDs.Contains(x.BundleIID)).ToList();
            ...
            var categoriesentities = new List<ProductCategoryMap>();
            ...
            if (toDto.Categories.Count > 0)
            {
                ...
                {
                    categoriesentities.Add(new ProductCategoryMap()
                    {
            ...
            var Allergiesentities = new List<ProductAllergyMap>();
            ...
            if (toDto.Allergies.Count > 0)
            {
                ...
                {
                    Allergiesentities.Add(new ProductAllergyMap()
                    {
                        ProductID = entity.ProductIID,
            ...
            new PriceSettingsRepository().SaveSKUPrice(entities, toDto.ProductID, null);
            new PriceSettingsRepository().Saveproductbundlesettings(bendleentities, toDto.ProductID, null);
            if (existingProductCategoryMapMapList.Any(x => x.FeeDueInventoryMaps.Count == 0))
            {
                new PriceSettingsRepository().Savecategoeries(categoriesentities, toDto.ProductID, null);
            }
            else
            {
                new PriceSettingsRepository().Savecategoeries(categoriesentities, toDto.ProductID, existingProductCategoryMapMapList);
            }

            new PriceSettingsRepository().SaveAllergies(Allergiesentities, toDto.ProductID, null);
        ...
        public override string GetEntity(long IID)
        {
            ...
            var productbundleMap = _product.ProductBundles1.ToList();
            ...
            {
                foreach (var pBundle in productbundleMap)
                {
                    dto.ProductBundle.Add(new ProductBundleDTO()
                    {
                        ...
                        FromProduct = new KeyValueDTO()
                        {
                            ...
                            Value = pBundle.Product.ProductName
        ...
        public Product ToEntity(SKUDTO dto)
        {
            ...
            if (dto.ProductID != 0)
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var imageSkumapDataList = dbContext.ProductImageMaps.Where(x => x.ProductID == dto.ProductID).ToList();
        ...
        private bool ResizeProductImage(long userID, Eduegate.Framework.Helper.Enums.ImageType imageType, SKUDTO dto)
        {
            ...
            try
            {
                ...
                string destinationFolderPath = string.Format("{0}\\{1}\\{2}\\{3}", ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.Products, dto.ProductSKUMapID, imageType.ToString());
