    ...
    public class NotificationAlertMapper : IDTOEntityMapper<NotificationAlertsDTO, NotificationAlert>
    {
        ...
        public NotificationAlertsDTO ToDTO(NotificationAlert entity)
        {
            ...
            return new NotificationAlertsDTO()
            {
                ...
                NotificationDateString = entity.NotificationDate.HasValue ? entity.NotificationDate.Value.ToString(dateFormat) : null,
                ...
                FromEmployeeName = entity.Login.Employees.Count > 0 ? entity.Login.Employees.Select(e =>e.EmployeeCode+" - "+e.FirstName+" "+e.MiddleName+" "+e.LastName).FirstOrDefault() : "IT department" ,
        ...
        public List<MailBoxDTO> GetSendMailFromParent(long loginID)
        {
            ...
            {
                var mailBox = db.MailBoxes.Where(x => x.fromID == loginID).ToList();
                var getData = new List<MailBoxDTO>();
                foreach (var mails in mailBox)
                {
                    var employee = db.Employees.FirstOrDefault(e => e.LoginID == mails.toID);
        ...
        public string MarkNotificationAsRead(long loginID,long notificationAlertIID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    var singleData = dbContext.NotificationAlerts.FirstOrDefault(x => x.NotificationAlertIID == notificationAlertIID);
                    if (singleData != null)
                    {
                    ...
                    dbContext.Entry(singleData).State = EntityState.Modified;
                    dbContext.SaveChanges();
                }
                //Update All AlertStatusID to Read by LoginID
                else if (loginID != 0)
                {
                    var notifications = dbContext.NotificationAlerts.Where(x => x.ToLoginID == loginID).ToList();
