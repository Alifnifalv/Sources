    ...
    public class PropertyTypeMapper : IDTOEntityMapper<PropertyTypeDTO, PropertyType>
    {
        ...
        public PropertyTypeDTO ToDTO(PropertyType entity)
        {
            if (entity != null)
            {
                return new PropertyTypeDTO()
                {
                    ...
                    PropertyList = entity.PropertyTypePropertyMaps.Select(x => PropertyMapper.Mapper(_context).ToDTO(new ProductDetailRepository().GetProperty(x.PropertyID))).ToList(),
                };
            }
        ...
        public PropertyType ToEntity(PropertyTypeDTO dto)
        {
            return new PropertyType()
            {
                ...
                PropertyTypePropertyMaps = dto.PropertyList.Select(x => new PropertyTypePropertyMap() { PropertyID = x.PropertyIID, PropertyTypeID = dto.PropertyTypeID }).ToList()
            };
