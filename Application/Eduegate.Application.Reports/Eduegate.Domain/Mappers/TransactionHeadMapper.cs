...
namespace Eduegate.Domain.Mappers
{
    public class TransactionHeadMapper : IDTOEntityMapper<TransactionHeadDTO, TransactionHead>
    {
        ...
        public TransactionHead ToEntity(TransactionHeadDTO dto)
        {
            if (dto != null)
            {
                var transactionHead = new TransactionHead()
                {
                    ...
                    DocumentCancelledDate = dto.DocumentCancelledDate.IsNotNull() ? Convert.ToDateTime(dto.DocumentCancelledDate) : DateTime.Now,
                    SchoolID = dto.SchoolID == null ? (byte)_context.SchoolID : dto.SchoolID,
                    AcademicYearID = dto.AcademicYearID == null ? (int)_context.AcademicYearID : dto.AcademicYearID,
                    ...
                    TransactionHeadEntitlementMaps = dto.TransactionHeadEntitlementMaps.Select(x => TransactionHeadEntitlementMapMapper.Mapper(_context).ToEntity(x)).ToList(),
                };
        ...
        public TransactionHeadDTO ToDTO(TransactionHead entity)
        {
            if (entity != null)
            {
                ...
                if (employeeDto.IsNotNull())
                {
                    ...
                    transactionDTO.EmployeeName = employeeDto.EmployeeName;
                }
        ...
        private TransactionHeadDTO StudentDetail(TransactionHeadDTO transactionDTO, long? studentID)
        {
            ...
            {
                string defaultMail = dbContext.Settings.FirstOrDefault(s => s.SettingCode == "DEFAULT_MAIL_ID").SettingValue;
                var repository = new EntiyRepository<Student, dbEduegateERPContext>(dbContext);
                var application = repository.Get(a => a.StudentIID == studentID).LastOrDefault();

                var classDet = dbContext.Classes.FirstOrDefault(a => a.ClassID == application.ClassID);

                var secDet = dbContext.Sections.FirstOrDefault(a => a.SectionID == application.SectionID);

                transactionDTO.StudentName = application.AdmissionNumber + "-" + application.FirstName + " " + application.MiddleName + " " + application.LastName;
                transactionDTO.StudentClassSectionDescription = classDet.ClassDescription + " " + secDet.SectionName;
                transactionDTO.EmailID = application?.Parent?.GaurdianEmail?? defaultMail;
                transactionDTO.SchoolID = application.SchoolID;
                return transactionDTO;
            }
        }
