...
namespace Eduegate.Domain
{
    public class ShoppingCartBL
    {
            ...
            bool withCurrencyConversion = true, bool isDeliveryCharge = false, bool isValidation = true, ShoppingCartStatus cartStatus = ShoppingCartStatus.InProcess, long ShoppingCartIID = 0, long? customerIID = null)
        {
            ...
            //var settingDTO = new SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINEBRANCHID, (long)_callContext.CompanyID);
            ...
            contextualInformation.CurrencyCode = contextualInformation.CurrencyCode.IsNotNullOrEmpty() ? contextualInformation.CurrencyCode : ConfigurationExtensions.GetAppConfigValue("CurrencyCode");
        ...
        public AddToCartStatusDTO AddToCart(CartProductDTO product, CallContext contextualInformation)
        {
            ...
            var settingDTO = new SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINEBRANCHID,_callContext.CompanyID.HasValue?(long)_callContext.CompanyID.Value: default(long));
            ...
            if (voucher.IsNotNull() && !string.IsNullOrEmpty(voucher.VoucherNo))
            {
                ...
                if (voucher.ExpiryDate > DateTime.Now || voucher.ExpiryDate.IsNull())
                {
                    ...
                    if (Convert.ToDecimal(voucher.CurrentBalance) > 0)
                    {
                        ...
                        {
                            decimal.TryParse(new SettingBL().GetSettingDetail("MINIMUMVOUCHERAMOUNTKUW").SettingValue, out minimumAmount);
                        }
                        else
                        {
                        ...
                        {
                            voucherResult.Status = Eduegate.Framework.Helper.Enums.Status.Invalid;
                        ...
                        else
                        {
                            ...
                            if (voucherMap.IsNotNull())
                            {
                                shoppingCartRepository.RemoveVoucherMap(voucherMap.ShoppingCartVoucherMapID);
                            }
                            ...
                            newVoucherMap.StatusID = Convert.ToByte(Eduegate.Framework.Helper.Enums.Status.Valid);
                            ...
                            {
                                voucherResult.Status = Eduegate.Framework.Helper.Enums.Status.Valid;
                                voucherResult.IsVoucherValid = true;
                                voucherResult.VoucherValue = Utility.FormatDecimal(Convert.ToDecimal(newVoucherMap.Amount * ConversionRate), 3); ;
                            }
                            else
                            {
                                voucherResult.Status = Framework.Helper.Enums.Status.Invalid;
                    ...
                    {
                        voucherResult.Status = Eduegate.Framework.Helper.Enums.Status.Used;
                ...
                {
                    voucherResult.Status = Eduegate.Framework.Helper.Enums.Status.Expired;
        ...
        public bool RemoveVoucherMap(long shoppingCartID)
        {
            ...
            if (voucherMap.IsNotNull())
            {
                return shoppingCartRepository.RemoveVoucherMap(voucherMap.ShoppingCartVoucherMapID);
            }
            return true;
        }
        ...
        public List<PaymentMethodDTO> GetPaymentBySitePaymentGroup(int siteID, bool isCustomerVerified, bool isLocal, bool isDigitalCart)
        {
            ...
            if (paymentMethods == null || paymentMethods.Count() == 0)
            {
                var paymentMethodEntity = new Eduegate.Domain.Repository.ReferenceDataRepository()
                    .GetPaymentBySitePaymentGroup(siteID, isCustomerVerified, isLocal, isDigitalCart);
