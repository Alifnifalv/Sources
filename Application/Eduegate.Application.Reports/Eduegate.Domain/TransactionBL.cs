    ...
    public class TransactionBL
    {
        ...
        public List<ProductInventoryDTO> ProcessProductInventory(List<ProductInventoryDTO> dtos)
        {
            ...
            foreach (var item in dtos)
            {
                ...
                var costSetting = Eduegate.Framework.Extensions.ConfigurationExtensions.GetAppConfigValueWithDefault<CostSetting>("COSTSETTING", CostSetting.Average);
        ...
        public bool SaveInvetoryTransactions(List<InvetoryTransactionDTO> dtos)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var referenceHeadID = dbContext.TransactionHeads.FirstOrDefault(x => x.HeadIID == check.HeadID).ReferenceHeadID;

                var docTypeID = referenceHeadID != null ? dbContext.TransactionHeads.FirstOrDefault(r => r.HeadIID == referenceHeadID).DocumentTypeID : null;
            ...
            if (transaction.TransactionHead.DocumentReferenceTypeID != null)
            {
                switch (transaction.TransactionHead.DocumentReferenceTypeID.Value)
                {
                    ...
                    case Services.Contracts.Enums.DocumentReferenceTypes.FOCSales:
                        ...
                        if (monthlyClosingDate != null && monthlyClosingDate.Value.Year > 1900 && transactionDTO.TransactionHead.TransactionDate.Value.Date >= monthlyClosingDate.Value.Date)
                        {
                    ...
                    case Services.Contracts.Enums.DocumentReferenceTypes.PurchaseInvoice:
                        {
                            ...
                            {
                                var hasClaim = new SecurityRepository().HasClaimAccess(long.Parse(new SettingRepository().GetSettingDetail(Eduegate.Framework.Helper.Constants.WRITESERIALKEYCLAIM).SettingValue), _callContext.LoginID.Value);
                                if (!hasClaim)
                                {
        ...
        {
            decimal checkAmount = new CustomerBL(_callContext).CustomerVerificatonCheck(customerID) == true ? Convert.ToDecimal(new SettingBL().GetSettingDetail(Constants.TransactionSettings.CUSVERIFIEDAMTLIMIT, companyID).SettingValue) : Convert.ToDecimal(new SettingBL().GetSettingDetail(Constants.TransactionSettings.CUSBLOCKEDAMTLIMIT, companyID).SettingValue);
        ...
        public StockVerificationDTO SaveStockVerificationDatas(StockVerificationDTO stockDatas)
        {
            ...
            {
                var docType = dbContext.DocumentTypes.FirstOrDefault(s => s.TransactionTypeName == "Physical Stock Entry");
                ...
                if (stockDatas != null)
                {
                    ...
                    if (entity.HeadIID == 0)
                    {
                        ...
                        dbContext.Entry(docType).State = System.Data.Entity.EntityState.Modified;
                    ...
                    var getMapDatas = dbContext.StockCompareDetails.Where(x => x.HeadID == entity.HeadIID).ToList();
                    ...
                    foreach (var listDatas in stockDatas.StockVerificationMap)
                    {
                        ...
                        {
                            var getPrdctID = dbContext.ProductSKUMaps.FirstOrDefault(p => p.ProductSKUMapIID == listDatas.ProductSKUMapID);
                            ...
                            dbContext.Entry(mapEntity).State = System.Data.Entity.EntityState.Added;
                            dbContext.SaveChanges();
                        }
                    }
        ...
        public StockVerificationDTO GetStockVerificationDatas(long headIID)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var getStockData = dbContext.StockCompareHeads.FirstOrDefault(x => x.HeadIID == headIID);
                var employee = dbContext.Employees.FirstOrDefault(e => e.EmployeeIID == getStockData.EmployeeID);
                var branchName = dbContext.Branches.FirstOrDefault(b => b.BranchIID == getStockData.BranchID);
                var docTypes = dbContext.DocumentStatuses.FirstOrDefault(b => b.DocumentStatusID == getStockData.TransactionStatusID);
                ...
                {
                    var prdctItem = dbContext.ProductSKUMaps.FirstOrDefault(p => p.ProductSKUMapIID == listDat.ProductSKUMapID);
                    ...
                    returnData.StockVerificationMap.Add(new StockVerificationMapDTO()
                    {
                        ...
                        BookStock = prdctItem.ProductInventories.Where(s => s.ProductSKUMapID == listDat.ProductSKUMapID && s.BranchID == branchName.BranchIID).Sum(d => d.Quantity),
        ...
        long orderID = default(long), bool withCurrencyConversion = true)
        {
            ...
            if (context.IsNull())
            {
                ...
                context.CurrencyCode = new SettingBL(null).GetSettingValue<string>("CurrencyCode");
            }
        ...
        public List<StockVerificationMapDTO> GetProductListsByBranchID(long branchID, DateTime date)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                SqlConnectionStringBuilder _sBuilder = new SqlConnectionStringBuilder(ConfigurationExtensions.GetConnectionString("dbEduegateERPContext").ToString());
        ...
        public TransactionSummaryDetailDTO GetCountByDocumentTypeID(int docTypeID)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var transactions = dbContext.TransactionDetails.Where(x => x.TransactionHead.DocumentTypeID == docTypeID && x.TransactionHead.SchoolID == _callContext.SchoolID).ToList();
