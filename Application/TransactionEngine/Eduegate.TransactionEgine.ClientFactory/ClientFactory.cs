...
namespace Eduegate.TransactionEgine.ClientFactory
{
    public class ClientFactory
    {
        ...
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new AccountingTransactionServiceClient(_context).AddAccountTransactionReceivablesMaps(AccountTransactionReceivablesMapDTOs);
            else
                return new AccountingTransactionBL(_context).AddAccountTransactionReceivablesMaps(AccountTransactionReceivablesMapDTOs);
        }
        ...
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).CheckAssetAvailability(branchID, assetID);
            else
                return new AssetTransactionBL(_context).CheckAssetAvailability(branchID, assetID);
        }

        public List<AssetInventoryDTO> ProcessAssetInventory(List<AssetInventoryDTO> dto)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).ProcessAssetInventory(dto);
            else
                return new AssetTransactionBL(_context).ProcessAssetInventory(dto);
        }

        public bool SaveAssetInventoryTransactions(List<AssetInventoryTransactionDTO> dto)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).SaveAssetInventoryTransactions(dto);
            else
                return new AssetTransactionBL(_context).SaveAssetInventoryTransactions(dto);
        }

        public bool CancelAssetTransaction(long headID)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).CancelAssetTransaction(headID);
            else
                return new AssetTransactionBL(_context).CancelAssetTransaction(headID);
        }

        public AssetTransactionHeadDTO GetAssetTransactionDetail(long headID)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).GetAssetTransactionDetail(headID);
            else
                return new AssetTransactionBL(_context).GetAssetTransactionDetail(headID);
        }

        public List<AssetTransactionHeadDTO> GetAllAssetTransaction(DocumentReferenceTypes referenceTypes, TransactionStatus transactionStatus)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).GetAllAssetTransaction(referenceTypes, transactionStatus);
            else
                return new AssetTransactionBL(_context).GetAllAssetTransaction(referenceTypes, transactionStatus);
        }

        public bool UpdateAssetSerialMapsInventoryID(List<long> AssetSerialMapIDs, AssetInventoryDTO assetInventory)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).UpdateAssetSerialMapsInventoryID(AssetSerialMapIDs, assetInventory);
            else
                return new AssetTransactionBL(_context).UpdateAssetSerialMapsInventoryID(AssetSerialMapIDs, assetInventory);
        }

        public bool UpdateAssetSerialMapsStatus(List<long> AssetSerialMapIDs, AssetInventoryDTO assetInventory)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).UpdateAssetSerialMapsStatus(AssetSerialMapIDs, assetInventory);
            else
                return new AssetTransactionBL(_context).UpdateAssetSerialMapsStatus(AssetSerialMapIDs, assetInventory);
        }

        public bool InsertAssetSerialMapList(List<AssetSerialMapDTO> assetSerialMaps)
        {
            if (new Domain.Setting.SettingBL().GetSettingValue<Tiers>("Tiers") == Tiers.Multi)
                return new FixedAssetServiceClient(_context).InsertAssetSerialMapList(assetSerialMaps);
            else
                return new AssetTransactionBL(_context).InsertAssetSerialMapList(assetSerialMaps);
        }
