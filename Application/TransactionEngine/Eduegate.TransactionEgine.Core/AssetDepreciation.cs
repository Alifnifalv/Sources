...
{
    public class AssetDepreciation : AssetTransactionBase, IAssetTransactions
    {
        private CallContext _callContext;

        public AssetDepreciation(Action<string> logError, CallContext context = null)
        {
            _logError = logError;
        ...
        {
            WriteLog("Asset depreciation-Process started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                WriteLog("Processing depreciation:" + transaction.HeadIID.ToString());
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in asset depreciation-Process.:" + ex.Message, ex);
        ...
        {
            WriteLog("AssetDepreciation-Process started for TransactionID:" + transaction.HeadIID.ToString());
            ...
            try
            {
                ...
                {
                    isSuccess = RevertAssetDepreciation(transaction.HeadIID);
                    WriteLog("Asset depreciation reverted?: " + isSuccess.ToString());
                }
                ...
                if (isSuccess)
                {
                    //WriteLog("Calling AssetDepreciation-AssetDepreciationInventory");
                    //isSuccess = UpdateInAssetInventory(transaction);
                    WriteLog("Asset depreciation?: " + isSuccess.ToString());
                }
                else
                {
        ...
        public bool UpdateInAssetInventory(AssetTransactionHeadViewModel transactions)
        {
            try
            {
                UpdateAssetInventory(transactions, DocumentReferenceTypes.AssetDepreciation, transactions.BranchID.Value);
                return true;
            }
            catch
            {
        ...
        private bool RevertAssetDepreciation(long transactionHeadID)
        {
            try
            {
                return true;
