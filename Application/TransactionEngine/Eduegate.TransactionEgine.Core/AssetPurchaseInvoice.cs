...
{
    public class AssetPurchaseInvoice : AssetTransactionBase, IAssetTransactions
    {
        private CallContext _callContext;

        public AssetPurchaseInvoice(Action<string> logError, CallContext context = null)
        {
            _logError = logError;
        ...
        {
            get { return Framework.Enums.DocumentReferenceTypes.AssetEntryPurchase; }
        }

        public void Process(AssetTransactionHeadViewModel transaction)
        {
            WriteLog("Asset purchase invoice-Process started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                WriteLog("Processing asset purchase invoice:" + transaction.HeadIID.ToString());
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in asset purchase invoice-Process.:" + ex.Message, ex);
        ...
        {
            WriteLog("AssetPurchaseInvoice-Process started for TransactionID:" + transaction.HeadIID.ToString());
            ...
            try
            {
                ...
                {
                    isSuccess = RevertAssetPurchaseInvoice(transaction.HeadIID);
                    WriteLog("Asset purchase invoice reverted?: " + isSuccess.ToString());
                }
                ...
                if (isSuccess)
                {
                    WriteLog("Calling AssetPurchaseInvoice-AssetPurchaseInventory");
        ...
        public bool UpdateInAssetInventory(AssetTransactionHeadViewModel transactions)
        {
            try
            {
                UpdateAssetInventory(transactions, DocumentReferenceTypes.AssetEntryPurchase, transactions.BranchID.Value);
                return true;
            }
            catch
            {
        ...
        private bool RevertAssetPurchaseInvoice(long transactionHeadID)
        {
            try
            {
                return true;
