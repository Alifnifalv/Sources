...
{
    public class AssetTransferIssue : AssetTransactionBase, IAssetTransactions
    {
        public AssetTransferIssue(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        {
            get { return Framework.Enums.DocumentReferenceTypes.AssetTransferIssue; }
        }

        public void Process(AssetTransactionHeadViewModel transaction)
        {
            WriteLog("Asset transfer issue-Process started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                WriteLog("Processing asset transfer issue:" + transaction.HeadIID.ToString());
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in asset branch transfer issue-Process.:" + ex.Message, ex);
        ...
        {
            WriteLog("AssetTransferIssue-Process started for TransactionID:" + transaction.HeadIID.ToString());
            ...
            try
            {
                ...
                {
                    isSuccess = RevertAssetBranchTransfer(transaction.HeadIID);
                ...
                if (isSuccess)
                {
                    WriteLog("Calling AssetTransferIssue-TranferAssetInventory");
                    isSuccess = TransferAssetInventory(transaction);
                    WriteLog("Asset inventory transferred?: " + isSuccess.ToString());
                }
                else
                {
                ...
                //2. Save into asset inventory transactions
                if (isSuccess)
                {
                ...
                {
                    WriteLog("Method SaveAssetInventoryTransactions is Failed for HeadId= " + transaction.HeadIID);
                    return;
                }
            ...
            catch (Exception ex)
            {
                UpdateAssetTransactionHead(transaction, Eduegate.Framework.Enums.TransactionStatus.Failed,Services.Contracts.Enums.DocumentStatuses.Draft);
                WriteLog(ex.Message.ToString(), ex);
                WriteLog("Exception occured in AssetTransferIssue-Process.:" + ex.Message, ex);
            }
        }

        public bool TransferAssetInventory(AssetTransactionHeadViewModel transactions)
        {
            try
            {
                UpdateAssetInventory(transactions, DocumentReferenceTypes.AssetTransferIssue, transactions.ToBranchID.Value);
                return true;
            }
            catch
            {
        ...
        private bool RevertAssetBranchTransfer(long transactionHeadID)
        {
            try
            {
                return true;
