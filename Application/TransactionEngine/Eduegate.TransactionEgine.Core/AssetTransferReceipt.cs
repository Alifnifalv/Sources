...
{
    public class AssetTransferReceipt : AssetTransactionBase, IAssetTransactions
    {
        public AssetTransferReceipt(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        {
            WriteLog("Asset transfer receipt-Process started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                WriteLog("Processing asset transfer receipt:" + transaction.HeadIID.ToString());
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in asset transfer receipt-Process.:" + ex.Message, ex);
        ...
        {
            WriteLog("AssetTransferReceipt-Process started for TransactionID:" + transaction.HeadIID.ToString());
            ...
            try
            {
                ...
                if (isSuccess)
                {
                    WriteLog("Calling AssetTransferReceipt-TranferAssetInventory");
        ...
        public bool TransferAssetInventory(AssetTransactionHeadViewModel transactions)
        {
            try
            {
                UpdateAssetInventory(transactions, DocumentReferenceTypes.AssetTransferReceipt, transactions.ToBranchID.Value);
                return true;
            }
            catch
            {
