...
{
    public class BundleWrap : TransactionBase, ITransactions
    {
        public BundleWrap(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        {
            WriteLog("BundleWrap-Process started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                WriteLog("Processing Bundle Wrap :" + transaction.HeadIID.ToString());
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in BundleWrap-Process.:" + ex.Message, ex);
            }
        }
        ...
        private void ProcessNew(TransactionHeadViewModel transaction)
        {
            ...
            if (isSuccess)
            {
                WriteLog("Calling BundleWrap-UpdateProductInventory");
                foreach (var trans in transaction.TransactionDetails)
                {
                    int mainCount = 0;
                    ...
                    var bundleItems = new Eduegate.TransactionEgine.ClientFactory.ClientFactory(null)
                                .GetProductBundleItemDetail(trans.ProductSKUMapID.Value);
                    isSuccess = UpdateBundleProductInventory(transaction, trans.Quantity.Value, bundleItems, BundleItemReferenceTypes, mainCount);
                    if (isSuccess)
        ...
        private bool RevertSalesTransaction(long transactionHeadID)
        {
            ...
            WriteLog("Calling BundleWrap-RevertSalesTransaction for HeadID:" + transactionHeadID.ToString());
            ...
            isSuccess = new PurchaseInvoice(_logError).UpdateProductInventory(vm, DocumentReferenceTypes.BundleWrap);
            return isSuccess;
        }
