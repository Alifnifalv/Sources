...
PUBLISH_ARTIFACTS_ROOT="$AZURE_PIPELINE_PATH/sharedrepo/Demo"
...
MAIN_CATALOG="Demo_EduEgate"
CONTENT_CATALOG="Demo_EduEgate_Contents"
LOG_CATALOG="Demo_EduEgate_Log"
...
set_service_vars() {
    case $1 in
        erp)
            SERVICE_NAME="ERP"
            CONTAINER_NAME="demo_erp_portal"
        ...
        parent)
            ...
            CONTAINER_NAME="demo_parent_portal"
        ...
        api)
            SERVICE_NAME="API"
            CONTAINER_NAME="demo_mobile_api"
        ...
        chat)
            ...
            CONTAINER_NAME="demo_mobile_chat"
        ...
        signup)
            SERVICE_NAME="Signup"
            CONTAINER_NAME="demo_signup_portal"
        ...
        exam)
            SERVICE_NAME="Exam"
            CONTAINER_NAME="demo_exam_portal"
		...
		vendor)
            SERVICE_NAME="Vendor"
            CONTAINER_NAME="demo_vendor_portal"
		...
		recruitment)
            ...
            CONTAINER_NAME="demo_recruitment_portal"
...
# Validate --service
if [[ -z "$SERVICE" ]]; then
    echo "❌ Error: Missing required argument --service <name>"
    exit 1
fi

# Resolve server IP
if ! $SERVER_ARG_PROVIDED || [[ -z "$SERVER" || "$SERVER" == "localhost" || "$SERVER" == "127.0.0.1" ]]; then
    echo "🔍 Auto-detecting server IP..."
    SERVER=$(ip -4 addr show dev eth0 2>/dev/null | grep -oP 'inet \K[\d.]+')
    if [[ -z "$SERVER" ]]; then
        SERVER=$(ip -4 addr show dev ens4 2>/dev/null | grep -oP 'inet \K[\d.]+')
    fi
    if [[ -z "$SERVER" ]]; then
        echo "❌ Error: Could not auto-detect server IP from eth0 or ens4."
        exit 1
    fi
fi
