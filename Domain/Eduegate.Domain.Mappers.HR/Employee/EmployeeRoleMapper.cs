...
namespace Eduegate.Domain.Repository.HR.Employee
{
    public class EmployeeRoleMapper : DTOEntityDynamicMapper
    {
        ...
        public List<KeyValueDTO> GetEmployeesByDepartmentBranch(long? departmentID,int? branchID)
        {
            ...
            using (var dbContext = new dbEduegateHRContext())
            {
                var entities = departmentID > 0 ? dbContext.Employees.Where(x => (departmentID==null || x.DepartmentID == departmentID) && (branchID==null || x.BranchID== branchID) && x.IsActive == true).AsNoTracking().ToList() :
                    dbContext.Employees.Where(x => x.IsActive == true).AsNoTracking().ToList();
        ...
        public KeyValueDTO GetSchoolPrincipal(byte schoolID)
        {
            using (var dbContext = new dbEduegateHRContext())
            {
                ...
                var entity = dbContext.Employees.Where(x => x.BranchID == schoolID && x.IsActive == true && x.DesignationID == princiDesign)
        ...
        public KeyValueDTO GetSchoolHeadMistress(byte schoolID)
        {
            using (var dbContext = new dbEduegateHRContext())
            {
                ...
                var entity = dbContext.Employees.Where(x => x.BranchID == schoolID && x.IsActive == true && x.DesignationID == mistrsDesign)
                    .AsNoTracking().FirstOrDefault();
