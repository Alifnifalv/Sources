...
namespace Eduegate.Domain.Mappers.HR.Leaves
{
    public class LeaveRequestMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            var rejectStasus = settingBL.GetSettingValue<int>("LEAVE_REJECT_STATUS_ID");
            ...
            using (dbEduegateHRContext dbContext = new dbEduegateHRContext())
            {
                ...
                var prvLeaves = dbContext.LeaveApplications.Where(a => a.EmployeeID == toDto.EmployeeID && ((toDto.FromDate.Value.Date >= a.FromDate.Value.Date &&
                                                                                                            toDto.FromDate.Value.Date <= a.ToDate.Value.Date) ||
                                                                                                            (toDto.ToDate.Value.Date >= a.FromDate.Value.Date &&
                                                                                                            toDto.ToDate.Value.Date <= a.ToDate.Value.Date))).AsNoTracking().FirstOrDefault();

                if(prvLeaves != null && prvLeaves.LeaveStatusID != rejectStasus && toDto.LeaveApplicationIID == 0)
                {
                    throw new Exception("The selected employee has already applied for leave on the specified date (" + prvLeaves.FromDate.Value.Date.ToString(dateFormat, CultureInfo.InvariantCulture) + " - " + prvLeaves.ToDate.Value.Date.ToString(dateFormat, CultureInfo.InvariantCulture) + ") !");
                }
        ...
        public bool CheckLeaveDateConflict(LeaveRequestDTO leaveData)
            ...
            using (var dbContext = new dbEduegateHRContext())
            {
                if (!leaveData.FromDate.HasValue || !leaveData.ToDate.HasValue)
                    return false; // Or handle as invalid input

                var fromDate = leaveData.FromDate.Value.Date;
                var toDate = leaveData.ToDate.Value.Date;

                return dbContext.LeaveApplications.Any(x =>
          x.EmployeeID == leaveData.EmployeeID &&
          x.LeaveApplicationIID != leaveData.LeaveApplicationIID &&
          x.FromDate.HasValue && x.ToDate.HasValue && // ensure values are not null
          x.FromDate.Value.Date <= toDate &&
          x.ToDate.Value.Date >= fromDate
      );
