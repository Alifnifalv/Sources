        ...
        private LoanHeadDTO ToDTO(long IID)
        {
            using (var dbContext = new dbEduegateHRContext())
            {
                ...
                var entity = dbContext.LoanRequests.Where(X => X.LoanRequestIID == IID)
                                  .Include(i => i.Employee).ThenInclude(i => i.PassportVisaDetails).ThenInclude(i => i.Sponsor)
                                  .AsNoTracking()
                                  .FirstOrDefault();
                ...
                var loanHeadDTO = new LoanHeadDTO()
                {
                    ...
                    SponsorShip = entity.Employee?.PassportVisaDetails?.FirstOrDefault().Sponsor?.SponsorName,
                };
