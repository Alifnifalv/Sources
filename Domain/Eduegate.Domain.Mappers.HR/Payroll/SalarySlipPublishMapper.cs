    ...
    public class SalarySlipPublishMapper : DTOEntityDynamicMapper
    {
            ...
            using (var dbContext = new dbEduegateHRContext())
            {
                ...
                try
                {
                    ...
                    if (salarySlips != null && salarySlips.Count() > 0)
                    {
                        ...
                        {
                            var dedAmount = salarySlips.Where(x => x.Amount < 0).Sum(x => x.Amount);
                            var allowAmount = salarySlips.Where(x => x.Amount > 0).Sum(x => x.Amount);
                            if (allowAmount < dedAmount || (dedAmount + allowAmount) < 0)
                            {
                                message = new OperationResultDTO()
                                {
                                    operationResult = OperationResult.Error,
                                    Message = "The total salary slip amount must be positive.Cannot be saved!"
                                };
                                return message;
                            }
                        }
        ...
        public OperationResultDTO UpdateSlipData(SalarySlipDTO salarySlipDTO)
        {
            using (var dbContext = new dbEduegateHRContext())
            {
                ...
                var vacationSalaryComponentID = settingBL.GetSettingValue<byte>("VACATION_SALARY_COMPONENT");
                var leaveSalaryComponentID = settingBL.GetSettingValue<byte>("LEAVE_SALARY");

                //var approvedAirfareAllowance = settingBL.GetSettingValue<byte>("AIRFARE_APPROVAL_STATUS");
                try
                {
                    ...
                    if (salarySlips != null && salarySlips.Count() > 0)
                    {
                        if (salarySlipDTO.SalarySlipStatusID == salarySlipApproved)
                        {
                            var dedAmount = salarySlips.Where(x => x.Amount < 0).Sum(x => x.Amount);
                            var allowAmount = salarySlips.Where(x => x.Amount > 0).Sum(x => x.Amount);
                            if (allowAmount < dedAmount || (dedAmount + allowAmount)<0)
                        ...
                        foreach (var slipData in salarySlips)
                        {
                            ...
                            #region Update LastTicketGivenDate
                            if ((slipData.SalaryComponentID == vacationSalaryComponentID || slipData.SalaryComponentID == leaveSalaryComponentID) && salarySlipDTO.SalarySlipStatusID == salarySlipApproved)
                            {
                                var settlementDate = dbContext.EmployeeSalarySettlements.Where(x => x.EmployeeID == slipData.EmployeeID).Max(x => x.LeaveStartDate);

                                if (settlementDate != null)
                                {
                                    var employee = (from t in dbContext.Employees
                                                    where t.EmployeeIID == slipData.EmployeeID
                                                    select t)
                                  .FirstOrDefault();
                                    employee.LastTicketGivenDate = settlementDate;
                                    dbContext.Entry(employee).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                                    dbContext.SaveChanges();
                                }
                            }
                            #endregion Update LastTicketGivenDate
