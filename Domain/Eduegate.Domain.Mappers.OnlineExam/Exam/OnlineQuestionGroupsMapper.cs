    ...
    public class OnlineQuestionGroupsMapper : DTOEntityDynamicMapper
    {
        ...
        public OnlineQuestionGroupsDTO GetQnGroupDetailsByID(int qnGroupID)
        {
            using (var dbContext = new dbEduegateOnlineExamContext())
            {
                ...
                var totalQuestions = dbContext.Questions.Where(a => a.PassageQuestionID == null && a.QuestionGroupID == qnGroupID).ToList();

                var questionsWithPassage = dbContext.Questions.Where(a => a.PassageQuestionID != null && a.QuestionGroupID == qnGroupID).ToList();
                ...
                if (groupData != null)
                {
                    QnGroupDTO = new OnlineQuestionGroupsDTO()
                    {
                        ...
                        TotalPassageQuestions = groupedExamQuestions.Count()
                    };
                }
