    ...
    public class OnlineQuestionsMapper : DTOEntityDynamicMapper
    {
        ...
        private OnlineQuestionsDTO ToDTO(Question entity)
        {
            ...
            var question = new OnlineQuestionsDTO()
            {
                ...
                PassageShortDescription = entity.PassageQuestionID.HasValue ? entity.PassageQuestion?.ShortDescription : null,
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            if (((toDto.AnswerTypeID == toDto.MultipleChoiceTypeID) || (toDto.AnswerTypeID == toDto.MultiSelectTypeID)) && !toDto.QuestionOptionMaps.Any(a => a.IsCorrectAnswer == true))
            {
                throw new Exception("Please select at least one option as an answer!");
            }
        ...
        {
            var counts = new List<KeyValueDTO>();

            var submittedExamID = new Domain.Setting.SettingBL(null).GetSettingValue<byte>("ONLINE_EXAM_STATUSID_SUBMITTED", 2);
            var startedExamID = new Domain.Setting.SettingBL(null).GetSettingValue<byte>("ONLINE_EXAM_STATUSID_START", 3);
            var completedExamID = new Domain.Setting.SettingBL(null).GetSettingValue<byte>("ONLINE_EXAM_STATUSID_COMPLETE", 7);


            using (var dbContext = new dbEduegateOnlineExamContext())
            {
                var data = dbContext.CandidateOnlineExamMaps.Where(x => x.CandidateID == candidateID)
                    .AsNoTracking()
                    .ToList();

                var scheduledExam = data.Where(a => a.OnlineExamStatusID == submittedExamID || a.OnlineExamStatusID == startedExamID).ToList();
                counts.Add(new KeyValueDTO()
                {
                    Key = "Scheduled",
                    Value = scheduledExam.Count().ToString()
                });

                var completedExam = data.Where(a => a.OnlineExamStatusID == completedExamID).ToList();
                counts.Add(new KeyValueDTO()
                {
                    Key = "Completed",
                    Value = completedExam.Count().ToString()
                });

                var publishedExams = dbContext.OnlineExamResults.Where(x => x.CandidateID == candidateID).ToList();
                counts.Add(new KeyValueDTO()
                {
                    Key = "Published",
                    Value = publishedExams.Count().ToString()
                });
        ...
        {
            var exams = new List<CandidateOnlineExamMapDTO>();
            using (var dbContext = new dbEduegateOnlineExamContext())
            {
                var examData = dbContext.CandidateOnlineExamMaps.Where(x => x.CandidateID == candidateID)
                ...
                {
                    exams.Add(new CandidateOnlineExamMapDTO()
                    {
                        OnlineExamID = data.OnlineExamID,
                        OnlineExamDescription = data.OnlineExam.Name,
                        Duration = data.Duration,
                        OnlineExamStatusName = data.OnlineExamStatus.ExamStatusName
                    });
                }
