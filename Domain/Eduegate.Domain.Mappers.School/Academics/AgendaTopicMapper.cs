...
namespace Eduegate.Domain.Mappers.School.Academics
{
    public class AgendaTopicMapper : DTOEntityDynamicMapper
    {
        ...
        private AgendaDTO ToDTO(Agenda entity)
        {
            ...
            if (entity != null)
            {
                ...
                topic = new AgendaDTO()
                {
                    ...
                    Title = entity.Title,
                    Date1 = entity.Date1,
                    Date1String = entity.Date1.HasValue ? entity.Date1.Value.ToString(dateFormat, CultureInfo.InvariantCulture) : null,
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            if (toDto.Date1.Value < DateTime.Now.Date)
            {
                throw new Exception("Please select a future date, as past dates are not supported");
            }

            var taskTypeCheck = toDto.AgendaTaskMap.Any(x => x.AttachmentReferenceID != null && x.TaskTypeID == null);
            if (taskTypeCheck)
            {
                throw new Exception("Task type is missing for one of the attachments.!");
            }

            using (var dbContext = new dbEduegateSchoolContext())
            {
                if (toDto.AgendaIID != 0)
                {
                    var AgendaData = dbContext.Agendas.Where(X => X.AgendaIID == toDto.AgendaIID).AsNoTracking().FirstOrDefault();
                    if (AgendaData != null)
                    {
                        if (AgendaData.AgendaStatusID == 2)
                        {
                            throw new Exception("The note can't be edited; it has already been published!");
        ...
        public List<AgendaDTO> GetAgendaList(long studentID)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var agendas = dbContext.Agendas
                    ...
                    .Where(X => X.AcademicYearID == studentDet.AcademicYearID && X.AgendaStatusID == publishStatusID &&
                    X.ClassID == studentDet.ClassID && X.AgendaSectionMaps.Any(s => s.SectionID == studentDet.SectionID))
                    .AsNoTracking().ToList().OrderByDescending(i => (i.UpdatedDate ?? i.CreatedDate))
;
        ...
        public List<AgendaDTO> GetStudentSubjectWiseAgendas(long studentID, long subjectID, string date)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                DateTime? date1 = !string.IsNullOrWhiteSpace(date) ? DateTime.ParseExact(date, dateFormat, CultureInfo.InvariantCulture) : (DateTime?)null;
                ...
                var agendas = dbContext.Agendas
                    ...
                    .Where(x => x.AcademicYearID == studentDet.AcademicYearID && x.AgendaStatusID == publishStatusID &&
                    x.ClassID == studentDet.ClassID && x.AgendaSectionMaps.Any(s => s.SectionID == studentDet.SectionID)
                    && (subjectID == 0 || x.SubjectID == subjectID) && (!date1.HasValue || x.Date1 == date1.Value))
                    .AsNoTracking().ToList().OrderByDescending(i => (i.UpdatedDate ?? i.CreatedDate));
        ...
        public List<AgendaDTO> GetAgendaStaffwise(long employeeID, long subjectID, string date)
        {
            var agendaList = new List<AgendaDTO>();
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var publishStatusID = new Domain.Setting.SettingBL(null)
                    .GetSettingValue<byte>("AGENDA_PUBLISH_STATUS_ID", 2);
                ...
                // Get class-section mappings for the teacher
                var classSectionMap = dbContext.ClassTeacherMaps
                    .Where(x => x.TeacherID == employeeID && x.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID)
                    .Select(x => new { x.ClassID, x.SectionID })
                    .Distinct()
                    .ToList();

                // Get AgendaIDs linked to those sections
                var agendaIDs = dbContext.AgendaSectionMaps
                    .Where(a => classSectionMap.Select(m => m.ClassID).Contains(a.Agenda.ClassID)
                             && classSectionMap.Select(m => m.SectionID).Contains(a.SectionID)
                             && a.Agenda.AgendaStatusID == publishStatusID
                             && a.Agenda.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID
                             && (subjectID == 0 || a.Agenda.SubjectID == subjectID)
                             && (!filterDate.HasValue || a.Agenda.Date1 == filterDate.Value))
                    .Select(a => a.AgendaID)
                    .Distinct()
                    .ToList();

                var agendas = dbContext.Agendas
                    .Where(a => agendaIDs.Contains(a.AgendaIID))
                    .Include(a => a.Class)
                    .Include(a => a.Section)
                    .Include(a => a.Subject)
                    .Include(a => a.AcademicYear)
                    .Include(a => a.AgendaStatus)
                    .Include(a => a.AgendaTopicMaps)
                    .Include(a => a.AgendaSectionMaps).ThenInclude(s => s.Section)
                    .Include(a => a.AgendaTaskMaps).ThenInclude(t => t.TaskType)
                    .Include(a => a.AgendaTaskMaps).ThenInclude(t => t.AgendaTaskAttachmentMaps)
                    .OrderByDescending(a => a.UpdatedDate ?? a.CreatedDate)
                    .ToList();

                agendaList = agendas.Select(a => ToDTO(a)).ToList();
            }
