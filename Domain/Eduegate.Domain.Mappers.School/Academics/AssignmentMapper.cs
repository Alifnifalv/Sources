    ...
    public class AssignmentMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                int? publishStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<int?>("ASSIGNMENT_PUBLISH_STATUS_ID", 3);

                if (toDto.AssignmentStatusID == publishStatusID)
                {
        ...
        public List<KeyValueDTO> Getstudentsubjectlist(long studentID)
        {
            using (var sContext = new dbEduegateSchoolContext())
            {
                ...
                var langAndOptionalIDs = new Domain.Setting.SettingBL(null).GetSettingValue<string>("LANGUAGE_SUBJECT_TYPE_IDS");
                var splitLangAndOptionalIDs = langAndOptionalIDs.Split(',').ToList();

                List<int> _sSubjectLangAndOptional = splitLangAndOptionalIDs.Select(id => int.Parse(id)).ToList();
                ...
                List<int> additionalSubs = sContext.Students.Where(a => a.ClassID == student.ClassID && a.SectionID == student.SectionID && a.AcademicYearID == student.AcademicYearID && a.SubjectMapID != 0).Select(a => a.SubjectMapID ?? 0).Distinct().ToList();
                ...
                var subject = sContext.ClassSubjectMaps.Where(x => x.ClassID == student.ClassID && x.SectionID == student.SectionID && x.AcademicYearID == student.AcademicYearID)
                    .Include(i => i.Subject)
                    .OrderBy(a => a.Subject.SubjectName)
                    .AsNoTracking()
                    .ToList();

                if (subject.Any())
                    _sStudentSubjcets.AddRange(subject.Where(w => !_sSubjectLangAndOptional.Contains(w.Subject.SubjectTypeID ?? 0) && !additionalSubs.Contains(w.SubjectID ?? 0)).Select(w => w.SubjectID ?? 0));
        ...
        public AssignmentDTO GetAssignmentByAssignmentID(long AssignmentID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var lessonPlan = dbContext.Assignments
                    .Where(lp => lp.AssignmentIID == AssignmentID)
                    .Include(lp => lp.AssignmentStatus)
                    .Include(lp => lp.AssignmentAttachmentMaps)
                    .AsNoTracking()
                    .FirstOrDefault();
        ...
        public long? SaveStudentAssignment(StudentAssignmentDTO requestDTO)
        {
            if (requestDTO == null || requestDTO.StudentAssignmentMaps == null || !requestDTO.StudentAssignmentMaps.Any())
            {
                throw new ArgumentException("StudentAssignmentMaps is empty or null.");
            }

            using (var dbContext = new dbEduegateSchoolContext())
            {
                foreach (var map in requestDTO.StudentAssignmentMaps)
                {
                    var entity = new StudentAssignmentMap()
                    {
                        ...
                        AttachmentReferenceId = map.AttachmentReferenceId,
                        Remarks = map.Remarks,
                        DateOfSubmission = map.DateOfSubmission ?? DateTime.Now,
                        AssignmentStatusID = map.AssignmentStatusID ?? 1, // Default status if missing
                        CreatedBy = map.CreatedBy ?? (int?)_context.LoginID,
                        UpdatedBy = map.UpdatedBy ?? (int?)_context.LoginID,
                        CreatedDate = map.CreatedDate ?? DateTime.Now,
                        UpdatedDate = DateTime.Now
                    };

                    if (entity.StudentAssignmentMapIID == 0)
                    {
                        dbContext.StudentAssignmentMaps.Add(entity);
                ...
                dbContext.SaveChanges();
                return requestDTO.StudentAssignmentMaps.FirstOrDefault()?.StudentAssignmentMapIID;
            }
        }

        private void SendAndSavePushNotification(Assignment entity)
        {
            var loginIDs = new List<long?>();
            Dictionary<string, string> Settings = NotificationSetting.GetParentAppSettings();

            var activeStudStatusID = byte.Parse(new Domain.Setting.SettingBL(null).GetSettingValue<string>("STUDENT_ACTIVE_STATUSID"));

            using (var dbContext = new dbEduegateSchoolContext())
            {
                loginIDs = dbContext.Students.Where(a => a.ClassID == entity.ClassID && entity.AssignmentSectionMaps.Select(b => b.SectionID).Contains(a.SectionID ?? 0) && a.AcademicYearID == entity.AcademicYearID && a.IsActive == true && a.Status == activeStudStatusID)
                    .Include(a => a.Parent)
                    .Select(a => a.Parent.LoginID)
                    .ToList();

                var message = "A new Assignment '" + entity.Title + "' has been uploaded.";
