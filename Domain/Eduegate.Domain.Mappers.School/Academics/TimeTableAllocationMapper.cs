...
namespace Eduegate.Domain.Mappers.School.Academics
{
    public class TimeTableAllocationMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                if (toDto.StaffIDList.Any())
                {
                    ...
                    foreach(var staff in toDto.StaffIDList)
                    {
                        ...
                        .Where(a => a.TimeTableID == entity.TimeTableID && a.ClassId == entity.ClassId && a.SectionID == entity.SectionID && a.WeekDayID == entity.WeekDayID 
                        && a.ClassTimingID == entity.ClassTimingID)
        ...
        public void DeleteTimeTableEntry(long timeTableAllocationID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var entitiesTimeTableLogs = dbContext.TimeTableLogs.Where(a => a.TimeTableAllocationID == timeTableAllocationID).AsNoTracking().ToList();
                var entitiesTimeTableAllocations = dbContext.TimeTableAllocations.Where(a => a.TimeTableAllocationIID == timeTableAllocationID).AsNoTracking().ToList();

                if (entitiesTimeTableAllocations.Count > 0)
                {
                    dbContext.RemoveRange(entitiesTimeTableLogs);
                    dbContext.RemoveRange(entitiesTimeTableAllocations);
                    dbContext.SaveChanges();
                }
            }
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                detailList.AddRange((from timeTableAlloc in entitiesTimeTableAlloc
                                     select new TimeTableAllocInfoDetailDTO
                                     {
                                         ...
                                         StaffIDList = entitiesTimeTableAlloc
                                             .Where(i => i.ClassTimingID == timeTableAlloc.ClassTimingID &&
                                                         i.SubjectID == timeTableAlloc.SubjectID &&
                                                         i.WeekDayID == timeTableAlloc.WeekDayID &&
                                                         i.StaffID.HasValue)
                                             .Select(i => i.StaffID.Value)
                                             .ToList(),

                                         StaffIDs = string.Join(",", entitiesTimeTableAlloc
                                             .Where(i => i.ClassTimingID == timeTableAlloc.ClassTimingID &&
                                                         i.SubjectID == timeTableAlloc.SubjectID &&
                                                         i.WeekDayID == timeTableAlloc.WeekDayID &&
                                                         i.StaffID.HasValue)
                                             .Select(i => i.StaffID.Value.ToString())),

                                         StaffNames = string.Join(",", entitiesTimeTableAlloc
                                             .Where(i => i.ClassTimingID == timeTableAlloc.ClassTimingID &&
                                                         i.SubjectID == timeTableAlloc.SubjectID &&
                                                         i.WeekDayID == timeTableAlloc.WeekDayID &&
                                                         i.Employee != null)
                                             .Select(i => i.Employee.FirstName)),

                                         AllocationIIDList = entitiesTimeTableAlloc
                                             .Where(i => i.ClassTimingID == timeTableAlloc.ClassTimingID &&
                                                         i.SubjectID == timeTableAlloc.SubjectID &&
                                                         i.WeekDayID == timeTableAlloc.WeekDayID)
                                             .Select(i => i.TimeTableAllocationIID)
                                             .ToList(),

                                         AllocationIIDs = string.Join(",", entitiesTimeTableAlloc
                                             .Where(i => i.ClassTimingID == timeTableAlloc.ClassTimingID &&
                                                         i.SubjectID == timeTableAlloc.SubjectID &&
                                                         i.WeekDayID == timeTableAlloc.WeekDayID)
                                             .Select(i => i.TimeTableAllocationIID.ToString())),

                                         SubjectID = timeTableAlloc.SubjectID,

                                         WeekDayID = timeTableAlloc.WeekDayID,

                                         ClassTiming = timeTableAlloc.ClassTiming != null
                                             ? new KeyValueDTO
                                             {
                                                 Key = timeTableAlloc.ClassTiming.ClassTimingID.ToString(),
                                                 Value = timeTableAlloc.ClassTiming.TimingDescription
                                         ...
                                         Subject = timeTableAlloc.Subject != null
                                             ? new KeyValueDTO
                                             {
                                                 Key = timeTableAlloc.Subject.SubjectID.ToString(),
                                                 Value = timeTableAlloc.Subject.SubjectName
                                         ...
                                         WeekDay = timeTableAlloc.WeekDay?.Day != null
                                             ? new KeyValueDTO
                                             {
                                                 Key = timeTableAlloc.WeekDay.WeekDayID.ToString(),
                                                 Value = timeTableAlloc.WeekDay.Day.DayName
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                detailList.AddRange((from timeTableAlloc in entitiesTimeTableAlloc
                                     select new TimeTableAllocInfoDetailDTO
                                     {
                                         ...
                                         Subject = timeTableAlloc.SubjectID.HasValue ? new KeyValueDTO() { Key = timeTableAlloc.Subject.SubjectID.ToString(), Value = timeTableAlloc.Class.ClassDescription  + " " + timeTableAlloc.Section.SectionName + " [" + timeTableAlloc.Subject.SubjectName + "]"  } : new KeyValueDTO() { Key = null, Value = null },
        ...
        {
            var screens = new List<KeyValueDTO>();

            using (var dbContext = new dbEduegateERPContext())
            {
                var menuLinkIDs = new Domain.Setting.SettingBL(null).GetSettingValue<string>("TIMETABLE_SETUP_SCREEN_IDS");
                List<long> menuIDs = menuLinkIDs.Split(',').Select(long.Parse).ToList();


                var menLinks = dbContext.MenuLinks
                        .Where(x => menuIDs.Contains(x.MenuLinkIID))
                        .OrderBy(a => a.SortOrder)
                        .AsNoTracking()
                        .ToList();

                foreach (var menus in menLinks)
                {
                    screens.Add(new KeyValueDTO()
                    {
                        Key = menus.MenuLinkIID.ToString(),
                        Value = menus.ActionLink.ToString(),
                    });
                }
            }
        ...
        public List<KeyValueDTO> GetClassTimes(long studentID)
        {
            ...
            {
                // Get SchoolID and AcademicYearID directly from Students table
                var studentInfo = db.Students
                    .Where(s => s.StudentIID == studentID && s.IsActive == true)
                    .Select(s => new
                    {
                        s.SchoolID,
                        s.AcademicYearID
                ...
                if (studentInfo == null || studentInfo.SchoolID == null || studentInfo.AcademicYearID == null)
                    return result;

                // Fetch class timings based on school and academic year
                result = db.ClassTimings
                    .Where(ct =>
                        ct.SchoolID == studentInfo.SchoolID &&
                        ct.AcademicYearID == studentInfo.AcademicYearID)
                    .Select(ct => new KeyValueDTO
                    {
                        Key = ct.ClassTimingID.ToString(),
                        Value = ct.StartTime + " - " + ct.EndTime
                    })
                    .ToList();
            }
