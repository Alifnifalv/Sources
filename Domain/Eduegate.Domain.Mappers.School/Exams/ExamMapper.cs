    ...
    public class ExamMapper : DTOEntityDynamicMapper
    {
        ...
        public override string GetEntity(long IID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var examdetail = new ExamDTO()
                {
                    ...
                    IncludeInFinalReport = entity.IncludeInFinalReport,
                    IncludeInInterimReport = entity.IncludeInInterimReport,
                };
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                //Check Duplicate Exam in Academic
                var getExams = dbContext.Exams.Where(x => x.AcademicYearID == _context.AcademicYearID && x.ExamDescription == toDto.ExamDescription 
                               && x.ExamTypeID == toDto.ExamTypeID && x.ExamGroupID == toDto.ExamGroupID).AsNoTracking().ToList();

                if(getExams.Count > 0 && toDto.ExamIID == 0)
                {
                    throw new Exception("The exam '" + toDto.ExamDescription + "' already exists; please check!");
                }
                ...
                var entity = new Exam()
                {
                    ...
                    IncludeInFinalReport = toDto.IncludeInFinalReport,
                    IncludeInInterimReport = toDto.IncludeInInterimReport,
                };
        ...
        public List<KeyValueDTO> GetExamsByTermID(MarkEntrySearchArgsDTO argsDTO)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var includeExamTypeIDs = new List<byte>();

                var coScholasticTypeId = new Domain.Setting.SettingBL(null).GetSettingValue<string>("Co-Scholastic_EXAM_TypeIDs");
                var scholasticTypeId = new Domain.Setting.SettingBL(null).GetSettingValue<string>("Scholastic_EXAM_TypeIDs");
                var scholasticInternalTypeId = new Domain.Setting.SettingBL(null).GetSettingValue<string>("Scholastic_Internals_EXAM_TypeIDs");

                if (argsDTO.tabType == "Scholastic")
                {
                    includeExamTypeIDs = scholasticTypeId?.Split(',')
                ...
                {
                    includeExamTypeIDs = coScholasticTypeId?.Split(',')
                                     .Select(id => byte.TryParse(id.Trim(), out var result) ? result : (byte?)null)
                                     .Where(id => id.HasValue)
                                     .Select(id => id.Value)
                                     .ToList() ?? new List<byte>();
                ...
                {
                    includeExamTypeIDs = scholasticInternalTypeId?.Split(',')
                                     .Select(id => byte.TryParse(id.Trim(), out var result) ? result : (byte?)null)
                                     .Where(id => id.HasValue)
                                     .Select(id => id.Value)
                                     .ToList() ?? new List<byte>();
                ...
                examList = (from s in dbContext.Exams
                            ...
                            && includeExamTypeIDs.Contains(ec.Exam.ExamType.ExamTypeID)
        ...
        public List<KeyValueDTO> GetSubjectsBySubjectType(MarkEntrySearchArgsDTO argsDTO)
        {
            ...
            {
                loginIDs = dbContext.ClaimSetLoginMaps.Where(a => a.ClaimSetID == 1).Select(a => a.LoginID).ToList();
            }
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                subjectList = (from s in dbContext.ExamSubjectMaps
                               join c in dbContext.ExamClassMaps on s.ExamID equals c.ExamID
                               join ts in dbContext.ClassTeacherMaps on s.SubjectID equals ts.SubjectID
                               //join cts in dbContext.ClassClassTeacherMaps on ts.TeacherID equals cts.TeacherID
                               join emp in dbContext.Employees on ts.TeacherID equals emp.EmployeeIID
                               where s.Exam.AcademicYearID == argsDTO.AcademicYearID
                               && c.ClassID == argsDTO.ClassID && s.ExamID == argsDTO.ExamID
                               ...
                               && (argsDTO.SectionID == null || c.SectionID == argsDTO.SectionID) && (loginIDs.Contains(_context.LoginID) || emp.LoginID == _context.LoginID) && ts.ClassID == argsDTO.ClassID && ts.SectionID == argsDTO.SectionID
                               select new KeyValueDTO
                               {
                                   Key = s.SubjectID.ToString(),
                                   Value = s.Subject.SubjectName
        ...
        public Exam GetExamData(long examID)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var examExists = dbContext.Exams.Where(a => a.SchoolID == examData.SchoolID && a.AcademicYearID == currentAcademicYearID && a.ExamDescription == examData.ExamDescription && a.ExamGroupID == examGroupID)
                    .FirstOrDefault();
