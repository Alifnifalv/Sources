    ...
    public class MarkPublishMapper : DTOEntityDynamicMapper
    {
                ...
                toDto.MarkRegistersDetails.All(x =>
                {
                    if (x.MarkRegisterSplitDTO.Any())
                    {
                        x.MarkRegisterSplitDTO.All(y =>
                        {
                            ...
                            _markRegisterSubMap.Add(new MarkRegisterSubjectMap()
                            {
                                MarkRegisterSubjectMapIID = y.MarkRegisterSubjectMapIID,
        ...
        private List<long> GetStudentsFeePending(List<long> _sStudents, int _sAcademicYearID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                string searchQuery = "select * from schools.VWS_Fee_Outstanding_Current where " +
                    //"AcadamicYearID in(" + string.Join(",", _sAcademicYearIDs) + ") and  " +
                    "FeeCycleID=1 AND Fee_Bal>0 and StudentIID in(" + string.Join(",", _sStudents) + ") and IsMandatoryToPay=1";
        ...
        //Not using now due to screen changes 
        public List<MarkRegisterDetailsDTO> GetSubjectsAndMarksToPublishOLD(MarkEntrySearchArgsDTO markEntrySearchArgsDTO)
        {
                ...
                if (dFndexam.Any() || dFndSkills.Any())
                {
                    ...
                    if (dFnd.Any())
                    {
                        ...
                        dFnd.All(w =>
                        {
                            ...
                            var reportContentID = GetPreviousPublishedReports(w.StudentID, markEntrySearchArgsDTO.AcademicYearID ?? 0, markEntrySearchArgsDTO.ClassID ?? 0, markEntrySearchArgsDTO.SectionID ?? 0, markEntrySearchArgsDTO.TermID);
        ...
        private long GetPreviousPublishedReports(long studentID, int _sAcademicYearID, int classID, int sectionID, long termID)
        {
            long _rprtData = new long();
            using (dbEduegateSchoolContext dbContext = new dbEduegateSchoolContext())
            {
                var datas = dbContext.ProgressReports.Where(a => a.StudentId == studentID && a.AcademicYearID == _sAcademicYearID && a.ClassID == classID && a.SectionID == sectionID && a.ExamGroupID == termID).AsNoTracking().FirstOrDefault();
        ...
        public List<MarkRegisterDetailsDTO> GetSubjectsAndMarksToPublish(MarkEntrySearchArgsDTO dto)
        {
            var returnDTO = new List<MarkRegisterDetailsDTO>();

            using (var dbContext = new dbEduegateSchoolContext())
            {
                var markRegisters = dbContext.MarkRegisters
                    .Include(x => x.Student)
                    .Where(x => x.AcademicYearID == dto.AcademicYearID
                                 && x.ClassID == dto.ClassID
                                 && x.SectionID == dto.SectionID
                                 && x.ExamGroupID == (dto.TermID != 0 ? dto.TermID : x.ExamGroupID))
                    .AsNoTracking()
                    .ToList();

                var progressReports = dbContext.ProgressReports
                                    .Include(a => a.ProgressReportPublishStatus)
                                    .Include(a => a.Student)
                                    .Where(x => x.AcademicYearID == dto.AcademicYearID
                                                 && x.ClassID == dto.ClassID
                                                 && x.SectionID == dto.SectionID
                                                 && (x.ExamGroupID ?? 0) == dto.TermID)
                                    .AsNoTracking()
                                    .ToList();

                // Assuming distinctStudentIds is a list of long or int representing StudentId  
                var distinctStudentIds = markRegisters.Select(m => m.StudentId).Distinct().ToList();

                // Use Contains to filter StudentFeeDues more efficiently  
                var studentFeeDue = dbContext.StudentFeeDues
                    .Include(x => x.FeeDueFeeTypeMaps)
                        .ThenInclude(f => f.FeePeriod)
                    .Where(x => distinctStudentIds.Contains(x.StudentId) && x.CollectionStatus == false && x.IsCancelled == false
                                 && x.FeeDueFeeTypeMaps.Any(f => f.Status == false && f.FeePeriod.IsMandatoryToPay == true)).ToList();

                // Extract Student IDs from studentFeeDue for later use  
                var feeDueStudentIds = studentFeeDue.Select(x => x.StudentId).ToList();

                // Group progress reports by composite key (StudentId, AcademicYearID, ClassID)  
                var progressReportsDict = progressReports
                    .GroupBy(pr => new
                    {
                        pr.StudentId,
                        pr.AcademicYearID,
                    ...
                    .Select(g => g.OrderByDescending(pr => pr.ProgressReportIID).FirstOrDefault())
                    .ToDictionary(pr => new
                    {
                        pr.StudentId,
                        pr.AcademicYearID,
                ...
                returnDTO = markRegisters
                    .GroupBy(m => m.StudentId) // Grouping to avoid duplicates  
                    .Select(g =>
                    {
                        var markRegister = g.First();
                        var studentId = markRegister.StudentId;
                        ...
                        // Retrieve the progress report using the composite key  
                        var pr = progressReportsDict.TryGetValue(key, out var progressReport)
                            ? progressReport
                            : null;

                        return new MarkRegisterDetailsDTO
                        {
                            ProgressReportIID = pr?.ProgressReportIID,
                            StudentID = studentId,
                            Student = markRegister.Student != null ? new KeyValueDTO
                            {
                                Key = markRegister.StudentId.ToString(),
                                Value = $"{markRegister.Student.FirstName} {markRegister.Student.MiddleName} {markRegister.Student.LastName}".Trim()
                            } : null,
                            AdmissionNumber = markRegister?.Student?.AdmissionNumber,
                            FeeDefaulterStatus = feeDueStudentIds.Contains(studentId) ? "The fee is due for this student" : null,
                            ReportContentID = pr?.ReportContentID,
                            MarkEntryStatusID = markRegister.MarkEntryStatusID,
                            PublishStatusID = pr?.PublishStatusID,
                            PublishStatus = pr?.PublishStatusID.HasValue == true ? new KeyValueDTO
                            {
                                Key = pr.PublishStatusID.ToString(),
                                Value = pr.ProgressReportPublishStatus?.StatusName
                            } : null,
                        };
