    ...
    public class MarkRegisterMapper : DTOEntityDynamicMapper
    {
        ...
        public string SaveCoScholasticEntry(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                {
                    Eduegate.Logger.LogHelper<MarkRegisterMapper>.Fatal(exception.Message.ToString(), exception);
                    //throw exception;
                }
        ...
        public List<MarkRegisterDetailsDTO> GetCoScholasticEntry(MarkEntrySearchArgsDTO markEntrySearchArgsDTO)
        {
            ...
            using (var sContext = new dbEduegateSchoolContext())
            {
                ...
                if (sRetData.Count() > 0)
                {
                    var publishedStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<long?>("PROGRESS_REPORT_PUBLISH_STATUS_PB");
                    var academicID = markEntrySearchArgsDTO.AcademicYearID;
                    var classID = markEntrySearchArgsDTO.ClassID;
                    var sectionID = markEntrySearchArgsDTO.SectionID;
                    var examGroupID = markEntrySearchArgsDTO.TermID;

                    var studentIDs = sRetData.Select(student => student.StudentID).ToList();

                    var publishedReports = sContext.ProgressReports
                        .Where(x => studentIDs.Contains(x.StudentId) &&
                                    x.AcademicYearID == academicID &&
                                    x.ClassID == classID &&
                                    x.SectionID == sectionID &&
                                    x.ExamGroupID == examGroupID && x.PublishStatusID == publishedStatusID)
                        .Select(x => x.StudentId)
                        .ToList();

                    sRetData.ForEach(student =>
                        student.IsReportPublished = publishedReports.Contains(student.StudentID));
                }
            }
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                if (entities.Any())
                {
                    ...
                    // Delete:-SubjectMap
                    {
                        ...
                        {
                            #region Logging_Procedure

                            try
                            {
                                var MarkRegisterSubjectMapIIDs = String.Join(",",removableSmEntities .Select(a => a.MarkRegisterSubjectMapIID).ToList());

                                using (SqlConnection conn = new SqlConnection(Infrastructure.ConfigHelper.GetDefaultConnectionString()))
                                using (SqlCommand cmd = new SqlCommand("[schools].[MarkRegisters_Log]", conn))
                                {
                                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                                    adapter.SelectCommand.CommandType = CommandType.StoredProcedure;

                                    adapter.SelectCommand.Parameters.Add(new SqlParameter("@MarkRegisterSubjectMapIDs", SqlDbType.NVarChar));
                                    adapter.SelectCommand.Parameters["@MarkRegisterSubjectMapIDs"].Value = MarkRegisterSubjectMapIIDs;

                                    adapter.SelectCommand.Parameters.Add(new SqlParameter("@UpdatedDate", SqlDbType.DateTime));
                                    adapter.SelectCommand.Parameters["@UpdatedDate"].Value = DateTime.Now;

                                    adapter.SelectCommand.Parameters.Add(new SqlParameter("@UpdatedBy", SqlDbType.Int));
                                    adapter.SelectCommand.Parameters["@UpdatedBy"].Value = _context.LoginID;
                            ...
                            {
                                Eduegate.Logger.LogHelper<MarkRegisterMapper>.Fatal(ex.Message.ToString(), ex);
                            }
                            #endregion
