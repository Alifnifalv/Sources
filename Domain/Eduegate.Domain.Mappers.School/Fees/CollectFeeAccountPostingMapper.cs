    ...
    public class CollectFeeAccountPostingMapper : DTOEntityDynamicMapper
    {
        ...
        public string SaveFeeAccount(DateTime fromDate, DateTime toDate, long cashierID)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var collectionsQuery = dbContext.FeeCollections
                    .Include(x => x.FeeCollectionPaymentModeMaps)
                        .ThenInclude(y => y.PaymentMode)
                    .Where(feecollect => !feecollect.IsAccountPosted
                        && (feecollect.FeeCollectionStatusID ?? 0) != draftStatusID
                        && feecollect.CashierID == cashierID
                        && feecollect.CollectionDate.HasValue
                        && feecollect.CollectionDate.Value.Date >= fromDate.Date
                        && feecollect.CollectionDate.Value.Date <= toDate.Date);

                var feeCollections = collectionsQuery
                    .AsNoTracking()
                    .ToList();

                // Now perform grouping on the in-memory collection  
                collectionDTO = feeCollections
                    .Select(feecollect => new FeeCollectionDTO()
                    {
                        ...
                        FeeCollectionPaymentModeMapDTO = feecollect.FeeCollectionPaymentModeMaps
                            .GroupBy(split => new
                            {
                                split.FeeCollectionID,
                                split.PaymentModeID,
                                PaymentMode = split.PaymentMode,
                                split.Amount
                            })
                            .Select(ft => new FeeCollectionPaymentModeMapDTO()
                            {
                                FeeCollectionID = ft.Key.FeeCollectionID,
                                PaymentModeID = ft.Key.PaymentModeID,
                                AccountId = ft.Key.PaymentMode?.AccountId ?? default,
                                Amount = ft.Sum(x => x.Amount), // Assuming you want the sum of amounts  
                            }).ToList()
                    })
                    .ToList();
