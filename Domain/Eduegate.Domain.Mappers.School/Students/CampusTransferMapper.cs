...
namespace Eduegate.Domain.Mappers.School.Students
{
    public class CampusTrasnsferMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            if(toDto.OtherFeeTotalFromCampus > 0)
            {
                throw new Exception("Other fees must be cleared before campus transfer can be processed.");
            }

            var monthlyClosingDate = new MonthlyClosingMapper().GetMonthlyClosingDate(toDto.FromSchoolID == null ? (long?)_context.SchoolID : toDto.FromSchoolID);

            if (monthlyClosingDate.HasValue && monthlyClosingDate.Value.Year > 1900 && toDto.TransferDate.Value.Date <= monthlyClosingDate.Value.Date)
            {
                throw new Exception("This details could not be saved due to monthly closing");
            }
            var monthlyClosingDate1 = new MonthlyClosingMapper().GetMonthlyClosingDate(toDto.ToSchoolID == null ? (long?)_context.SchoolID : toDto.ToSchoolID);
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                if (entity.CampusTransferIID != 0)
                {
                    ...
                    //  8.Collected Fee move to campus, if collected in selected academic
                    ...
                    var activeStsID = new Domain.Setting.SettingBL(null).GetSettingValue<string>("STUDENT_ACTIVE_STATUSID");

                    studentData.Status = byte.Parse(activeStsID);
                    ...
                    var oldRemarks = dbContext.RemarksEntryStudentMaps
                        .Where(rm => rm.StudentID == entity.StudentID && rm.RemarksEntry.AcademicYearID == entity.FromAcademicYearID)
                        .Include(rm => rm.RemarksEntry).ThenInclude(rm => rm.ExamGroup)
                        .ToList();

                    if (oldRemarks.Any())
                    {
                        // Fetch all relevant remarks for the new school context  
                        var toSchoolRemarks = dbContext.RemarksEntries
                            .Include(nr => nr.ExamGroup)
                            .Where(nr => nr.AcademicYearID == entity.ToAcademicYearID
                                          && nr.ClassID == entity.ToClassID
                                          && nr.SectionID == entity.ToSectionID)
                            .ToList();

                        foreach (var oldRemark in oldRemarks)
                        {
                            var examGroupNameLower = oldRemark.RemarksEntry.ExamGroup.ExamGroupName.ToLower();

                            // Check for existing remarks to update  
                            var matchingRemark = toSchoolRemarks
                                .FirstOrDefault(newRmk => newRmk.ExamGroup.ExamGroupName.ToLower() == examGroupNameLower);

                            if (matchingRemark != null)
                            {
                                // Update existing record  
                                oldRemark.RemarksEntryID = matchingRemark.RemarksEntryIID; // Update the foreign key reference  
                                dbContext.Entry(oldRemark).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                            }
                            else
                            {
                                // Create a new remark entry if no match is found  
                                var examGroup = dbContext.ExamGroups
                                    .FirstOrDefault(x => x.IsActive == true
                                                         && x.AcademicYearID == entity.ToAcademicYearID
                                                         && x.ExamGroupName.ToLower() == examGroupNameLower);

                                var newRemarkEntity = new RemarksEntry
                                {
                                    SchoolID = entity.ToSchoolID,
                                    AcademicYearID = entity.ToAcademicYearID,
                                    ClassID = entity.ToClassID,
                                    SectionID = entity.ToSectionID,
                                    ExamGroupID = examGroup?.ExamGroupID ?? oldRemark.RemarksEntry.ExamGroupID  // Fallback to old's exam group ID if not found  
                                };

                                // Map student remarks to the new remark entry  
                                newRemarkEntity.RemarksEntryStudentMaps.Add(new RemarksEntryStudentMap
                                {
                                    StudentID = entity.StudentID,
                                    Remarks1 = oldRemark.Remarks1,
                                    Remarks2 = oldRemark.Remarks2,
                                });

                                // Add new entry to the database  
                                dbContext.RemarksEntries.Add(newRemarkEntity);
                                dbContext.RemarksEntryStudentMaps.Remove(oldRemark); // Remove old remark if it’s no longer needed  
                            }
                        }

                        // Save changes outside the loop to optimize database interactions  
                        dbContext.SaveChanges();
                    }
                    ...
                    // Fetch existing health entry records for the student in the specified academic year  
                    var oldHealthEntries = dbContext.HealthEntryStudentMaps
                        .Include(rm => rm.HealthEntry)
                        .ThenInclude(rm => rm.ExamGroup)
                        .Where(rm => rm.StudentID == entity.StudentID && rm.HealthEntry.AcademicYearID == entity.FromAcademicYearID)
                        .ToList();

                    if (oldHealthEntries.Any())
                    {
                        // Fetch health entries for the new school context  
                        var toSchoolHealthEntries = dbContext.HealthEntries
                            .Include(h => h.ExamGroup)
                            .Where(h => h.AcademicYearID == entity.ToAcademicYearID
                                          && h.ClassID == entity.ToClassID
                                          && h.SectionID == entity.ToSectionID)
                            .ToList();

                        foreach (var oldHealth in oldHealthEntries)
                        {
                            var examGroupNameLower = oldHealth.HealthEntry.ExamGroup.ExamGroupName.ToLower();

                            // Attempt to find a matching health entry for updating  
                            var matchingHealthEntry = toSchoolHealthEntries
                                .FirstOrDefault(newHealth => newHealth.ExamGroup.ExamGroupName.ToLower() == examGroupNameLower);

                            if (matchingHealthEntry != null)
                            {
                                // Update fields for the existing record  
                                oldHealth.HealthEntryID = matchingHealthEntry.HealthEntryIID;
                                dbContext.Entry(oldHealth).State = Microsoft.EntityFrameworkCore.EntityState.Modified; // Mark as modified  
                            }
                            else
                            {
                                // Create a new health entry if no match is found  
                                var examGroup = dbContext.ExamGroups.FirstOrDefault(x =>
                                    x.IsActive == true
                                    && x.AcademicYearID == entity.ToAcademicYearID
                                    && x.ExamGroupName.ToLower() == examGroupNameLower);

                                var newHealthEntry = new HealthEntry
                                {
                                    SchoolID = entity.ToSchoolID,
                                    AcademicYearID = entity.ToAcademicYearID,
                                    ClassID = entity.ToClassID,
                                    SectionID = entity.ToSectionID,
                                    ExamGroupID = examGroup?.ExamGroupID ?? oldHealth.HealthEntry.ExamGroupID  // Use old if not found  
                                };

                                newHealthEntry.HealthEntryStudentMaps.Add(new HealthEntryStudentMap
                                {
                                    StudentID = entity.StudentID,
                                    ...
                                    BMI = oldHealth.BMI,
                                    BMS = oldHealth.BMS,
                                    Vision = oldHealth.Vision,
                                    Remarks = oldHealth.Remarks,
                                });

                                // Add the new health entry to the database  
                                dbContext.HealthEntries.Add(newHealthEntry);
                                dbContext.HealthEntryStudentMaps.Remove(oldHealth); // Remove old entry  
                            }
                        }
                    ...
                    var oldMarkEnrty = dbContext.MarkRegisters
                        ...
                        .Include(mr => mr.Exam).ThenInclude(mr => mr.ExamGroup)
                        .ToList();

                    if (oldMarkEnrty.Count > 0)
                    {
                        foreach(var mark in oldMarkEnrty)
                        {
                            var examgroup = mark.ExamGroup.ExamGroupName;

                            var newExamID = dbContext.Exams
                                .Include(x => x.ExamGroup)
                                .Where(x => x.ExamDescription == mark.Exam.ExamDescription && x.AcademicYearID == entity.ToAcademicYearID && x.ExamGroup.ExamGroupName.ToString().ToLower() == examgroup.ToString().ToLower())
                                .FirstOrDefault();
                            ...
                            mark.ExamID = newExamID != null ? newExamID.ExamIID : mark.ExamID;
                        }
                    ...
                    #region Fee Collection Migrattion --- Start

                    var fromAcademic = dbContext.AcademicYears.FirstOrDefault(x => x.AcademicYearID == toDto.FromAcademicYearID);
                    var toAcademic = dbContext.AcademicYears.FirstOrDefault(x => x.AcademicYearID == toDto.ToAcademicYearID);

                    if (fromAcademic.AcademicYearCode == toAcademic.AcademicYearCode)
                    {
                        var collectedStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<int?>("FEECOLLECTIONSTATUSID_COLLECTED");

                        var feeCollectionsToUpdate = dbContext.FeeCollections.Where(x => x.StudentID == toDto.StudentID
                                                     && x.FeeCollectionStatusID == collectedStatusID
                                                     && x.AcadamicYearID == toDto.FromAcademicYearID
                                                     && x.ClassID == toDto.FromClassID
                                                     && x.SectionID == toDto.FromSectionID).ToList();

                        if (feeCollectionsToUpdate != null || feeCollectionsToUpdate.Count > 0)
                        {
                            foreach (var feeCollection in feeCollectionsToUpdate)
                            {
                                feeCollection.ClassID = toDto.ToClassID;
                                feeCollection.SectionID = toDto.ToSectionID;
                                feeCollection.AcadamicYearID = toDto.ToAcademicYearID;
                                feeCollection.SchoolID = toDto.ToSchoolID;
                            }
                            dbContext.SaveChanges();
                        }
                    }

                    #endregion Fee collection --- End
