...
namespace Eduegate.Domain.Mappers.School.Students
{
    public class StudentApplicationMapper : DTOEntityDynamicMapper
    {
        ...
        {
            var documentsMap = entity.StudentApplicationDocumentMaps.Count > 0 ? entity.StudentApplicationDocumentMaps.LastOrDefault() : null;
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var academicYears = dbContext.AcademicYears.Where(a => a.SchoolID == toDto.SchoolID && a.AcademicYearID == toDto.SchoolAcademicyearID).ToList();
                var classes = dbContext.Classes.Where(a => a.SchoolID == toDto.SchoolID).Select(a => a.ClassID).ToList();

                if (academicYears.Count() <= 0 || !classes.Contains(toDto.ClassID ?? 0))
                {
                    throw new Exception("Kindly select the academic year and class for the selected school.");
                }
            }
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                if (entity.ApplicationIID == 0)
                {
                    ...
                    var appliedStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<string>("STUDENT_APPLICATION_APPLIED_STATUSID");

                    if (entity.ApplicationStatusID == byte.Parse(appliedStatusID))
                    {
                        var emailTemplate = EmailTemplateMapper.Mapper(_context).GetEmailTemplateDetails(EmailTemplates.StudentApplicationAppliedMail.ToString());
                        ...
                        if (emailTemplate != null && !string.IsNullOrEmpty(emailTemplate.EmailTemplate))
                        {
                            ...
                            emailBody = emailBody.Replace("{StudentName}", toDto.FirstName + " " + toDto.MiddleName + " " + toDto.LastName);
                            emailBody = emailBody.Replace("{ApplicationNumber}", entity.ApplicationNumber);
                            emailBody = emailBody.Replace("{AppliedDateString}", entity.CreatedDate.Value.ToString(dateFormat, CultureInfo.InvariantCulture));
                            emailBody = emailBody.Replace("{ClassName}", toDto.ClassName);
                            emailBody = emailBody.Replace("{MobileNumber}", toDto.MobileNumber);
                        }
                    }
                }
                else
        ...
        public List<KeyValueDTO> GetAcademicYearBySchool(int schoolID,bool? isActive)
        {
            ...
            var settingCurrentAcademicStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<string>("CURRENT_ACADEMIC_YEAR_STATUSID");
            var settingAdvanceAcademicStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<string>("ADVANCE_ACADEMIC_YEAR_STATUSID");

            using (var dbContext = new dbEduegateSchoolContext())
            {
                var academicyearList = dbContext.AcademicYears
                                       .Where(srcs => (srcs.SchoolID == (byte?)schoolID && (srcs.AcademicYearStatusID == byte.Parse(settingAdvanceAcademicStatusID) || srcs.AcademicYearStatusID == byte.Parse(settingCurrentAcademicStatusID))) 
                                       && (isActive == true ? srcs.IsActive == true : (srcs.IsActive == true || srcs.IsActive == false || srcs.IsActive == null)))
                                       .AsNoTracking()
        ...
        public List<KeyValueDTO> GetClassesByAcademicYearID(int academicYearID)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var classes = dbContext.ParentPortalSchoolAcademicClassMaps.Where(x => x.AcademicYearID == academicYearID).Select(x => x.ClassID).ToList();
                ...
                if (classes.Any())
                {
                    ...
                    {
                        classList = dbContext.Classes.Where(c => classes.Contains(c.ClassID))
                                           .AsNoTracking()
                                           .Select(srcs => new KeyValueDTO
                                           {
        ...
        public List<KeyValueDTO> GetAcademicYearBySchoolForApplications(int schoolID, bool? isActive)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var academicyearList = dbContext.AcademicYears
                                       .Where(srcs => (srcs.SchoolID == (byte?)schoolID && (srcs.AcademicYearStatusID == byte.Parse(settingAdvanceAcademicStatusID) || srcs.AcademicYearStatusID == byte.Parse(settingCurrentAcademicStatusID)))
                                       && (isActive == true ? srcs.IsActive == true : (srcs.IsActive == true || srcs.IsActive == false || srcs.IsActive == null)) && srcs.IsUsedForApplication == true)
                                       .AsNoTracking()
                                       .Select(srcs => new KeyValueDTO
                                       {
                                           Key = srcs.AcademicYearID.ToString(),
