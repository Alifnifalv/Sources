...
namespace Eduegate.Domain.Mappers.School.Students
{
    public class StudentLeaveApplicationMapper : DTOEntityDynamicMapper
    {
        ...
        public string SubmitStudentLeaveApplication(StudentLeaveApplicationDTO leaveData)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var studentData = dbContext.Students
                    .Where(s => s.StudentIID == leaveData.StudentID)
                    .AsNoTracking()
                    .FirstOrDefault();

                var isNew = leaveData.StudentLeaveApplicationIID == 0;
                ...
                var entity = new StudentLeaveApplication()
                {
                    ...
                    CreatedBy = isNew ? (int)_context.LoginID : leaveData.CreatedBy,
                    UpdatedBy = !isNew ? (int)_context.LoginID : leaveData.UpdatedBy,
                    CreatedDate = isNew ? DateTime.Now : leaveData.CreatedDate, // Only set if new
                    UpdatedDate = !isNew ? DateTime.Now : leaveData.UpdatedDate,
                };
        ...
        public List<StudentLeaveApplicationDTO> GetLeaveRequestByClassSectionDate(long classID, long sectionID, DateTime date)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var resultList = new List<StudentLeaveApplicationDTO>();

                //byte?[] statusIDs = { 1, 2 };

                var leaveRequests = dbContext.StudentLeaveApplications
                    .Where(a => a.ClassID == classID
                             && a.Student.SectionID == sectionID
                             && a.FromDate <= date
                             && a.ToDate >= date
                             /*&& statusIDs.Contains(a.LeaveStatusID)*/)
                    .Include(i => i.Student)
                    .Include(i => i.LeaveStatus)
                    .Distinct()
                    .AsNoTracking()
                ...
                {
                    var dto = new StudentLeaveApplicationDTO
                    {
                        StudentID = request.StudentID,
                        AdmissionNumber = request.Student.AdmissionNumber,
                        StudentName = request.Student.FirstName + " " + (request.Student.MiddleName ?? "") + (request.Student.LastName ?? ""),
                        LeaveStatusDescription = request.LeaveStatus?.StatusName,
