    ...
    public class StudentMapper : DTOEntityDynamicMapper
    {
        ...
        private void StudentLoginInfo(Student entity, StudentDTO dto)
        {
            ...
            if (login != null)
            {
                ...
                dto.Login.StatusID = login.StatusID.IsNotNull() ? (Infrastructure.Enums.LoginUserStatus)login.StatusID : 0;
                dto.Login.LastLoginDate = login.LastLoginDate;
        ...
        private void StudenParentLoginInfo(Student entity, StudentDTO dto)
        {
            ...
            if (LogMap != null)
            {
                ...
                dto.ParentLogin.StatusID = LogMap.StatusID.IsNotNull() ? (Infrastructure.Enums.LoginUserStatus)LogMap.StatusID : 0;
                dto.ParentLogin.LastLoginDate = LogMap.LastLoginDate;
            }
            if (rolemap != null)
            {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (dbEduegateSchoolContext dbContext = new dbEduegateSchoolContext())
            {
                if (toDto.AcademicYearID.HasValue)
                {
                    var getData = dbContext.AcademicYears.Where(X => X.AcademicYearID == toDto.AcademicYearID).AsNoTracking().FirstOrDefault();
                    ...
                    if (toDto.StudentIID == 0 && !toDto.SchoolAcademicyearID.HasValue)
                    {
                        toDto.SchoolAcademicyearID = toDto.AcademicYearID;
                    }
                }
                //else
                //{
                ...
                if (toDto.Login.LoginUserID != null)
                {
                    ...
                    entity.Login.StatusID = (byte?)(toDto.StudentIID == 0 ? Infrastructure.Enums.LoginUserStatus.Active : toDto.Login.StatusID);

                    if (toDto.StudentIID == 0)
                    {
                        var roleID = new Domain.Setting.SettingBL(null).GetSettingValue<int?>("Student_App_RoleID");

                        entity.Login.LoginRoleMaps.Add(new LoginRoleMap()
                        {
                            LoginID = entity.Login.LoginIID,
                            RoleID = roleID,
                ...
                if (entity.Parent != null)
                {
                    ...
                    if (entity.StudentIID == 0)
                    {
                        ...
                        if (toDto.StudentIID != 0)
                        {
                            var admissionDateString = (entity.AdmissionDate == null ? "" : entity.AdmissionDate.Value.ToString(dateFormat, CultureInfo.InvariantCulture));
                            var parentPortalLink = new Domain.Setting.SettingBL(null).GetSettingValue<string>("CLIENT_PARENT_PORTAL_LOGIN_LINK");
                            var schoolGuideLinePath = new Domain.Setting.SettingBL(null).GetSettingValue<string>("CLIENT_WELCOME_MAIL_DOC_GUIDELINES");
                            var transportationGuidelinePath = new Domain.Setting.SettingBL(null).GetSettingValue<string>("CLIENT_WELCOME_MAIL_DOC_TRANSPORT");
                            var pdfIconPath = new Domain.Setting.SettingBL(null).GetSettingValue<string>("PDF_FILE_TYPE_PATH");

                            emailTemplate = EmailTemplateMapper.Mapper(_context).GetEmailTemplateDetails(EmailTemplates.AdmissionWelcomeMail.ToString());

                            emailSubject = emailTemplate.Subject;

                            emailBody = emailTemplate.EmailTemplate;

                            var getAcademic = Eduegate.Domain.Mappers.School.Mutual.AcademicYearMapper.Mapper(_context).GetAcademicYear((int)toDto.AcademicYearID);

                            emailBody = emailBody.Replace("{AdmissionNumber}", entity.AdmissionNumber);
                            emailBody = emailBody.Replace("{StudentName}", entity.FirstName + " " + entity.MiddleName + " " + entity.LastName);
                            emailBody = emailBody.Replace("{AcademicYear}", getAcademic.FirstOrDefault().Description);
                            emailBody = emailBody.Replace("{Class}", toDto.ClassName);
                            emailBody = emailBody.Replace("{Section}", toDto.SectionName);
                            emailBody = emailBody.Replace("{AdmissionDate}", admissionDateString);
                            emailBody = emailBody.Replace("{ParentPortalLink}", parentPortalLink);
                            emailBody = emailBody.Replace("{SchoolGeneralGuidelines}", schoolGuideLinePath);
                            emailBody = emailBody.Replace("{DocumentForOwnTransportStudentsOnly}", transportationGuidelinePath);
                            emailBody = emailBody.Replace("{PdfIconPath}", pdfIconPath);
                        }
                    }
                    else
                    {
                    ...
                    var mailParameters = new Dictionary<string, string>()
                {
                    ...
                    string mailMessage = new Eduegate.Domain.Notification.EmailNotificationBL(_context).PopulateBody(toMailID, emailBody);
                    ...
                    if (emailBody != "")
                    {
                        ...
                        {
                            new Eduegate.Domain.Notification.EmailNotificationBL(_context).SendMail(toMailID, emailSubject, mailMessage, EmailTypes.StudentAdmission, mailParameters);
                        }
                        else
                        {
                            new Eduegate.Domain.Notification.EmailNotificationBL(_context).SendMail(defaultMail, emailSubject, mailMessage, EmailTypes.StudentAdmission, mailParameters);
                        }
                    }
        ...
        public List<KeyValueDTO> GetClassStudentsAll(int academicYearID, List<int> classList, List<int> sectionList)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var toHideStatusIDs = new Domain.Setting.SettingBL(null).GetSettingValue<string>("PROMOTION_SCREEN_HIDE_STUDENT_STATUSES");
                var excludeStsIDs = new List<byte>();

                excludeStsIDs = toHideStatusIDs?.Split(',')
                ...
                var entities = dbContext.Students
                    .Where(a => (classList.Contains(0) || classList.Contains(a.ClassID ?? 0)) && (sectionList.Contains(0) || sectionList.Contains(a.SectionID ?? 0)) &&
                        a.IsActive == true && !excludeStsIDs.Contains((byte)a.Status) && a.AcademicYearID == academicYearID)
                    .AsNoTracking()
                    .OrderBy(z => z.AdmissionNumber)
                    .ToList();
        ...
        public List<StudentDTO> GetClasswiseStudentData(int classId, int sectionId)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var activeStatusID = new Domain.Setting.SettingBL(null).GetSettingValue<int>("STUDENT_ACTIVE_STATUSID", 1);

                var academicYearID = _context?.AcademicYearID;

                var studentList = (from std in dbContext.Students
                                   join ay in dbContext.AcademicYears on std.AcademicYearID equals ay.AcademicYearID
                                   where std.ClassID == classId && std.IsActive == true && std.SectionID == sectionId
                                   && std.Status == activeStatusID &&
                                   (academicYearID.HasValue ? std.AcademicYearID == _context.AcademicYearID :
                                   ay.AcademicYearStatusID == currentAcademicYearStatusID)
                                   select new StudentDTO
                                   {
                                       StudentIID = std.StudentIID,
                                       ClassID = std.ClassID,
                                       SectionID = std.SectionID,
                                       SchoolID = std.SchoolID,
                                       AcademicYearID = std.AcademicYearID,
                                       StudentProfile = std.StudentProfile,
                                       AdmissionNumber = std.AdmissionNumber,
                                       FirstName = std.FirstName,
                                       MiddleName = std.MiddleName,
                                       LastName = std.LastName,
                                       StudentFullName = std.FirstName + " " + std.MiddleName + " " + std.LastName,
                                       AdmissionDate = std.AdmissionDate,
                                       FeeStartDate = std.FeeStartDate,
        ...
        public ClassSectionShiftingDTO GetStudentsForShifting(int classID, int sectionID)
        {
            using (dbEduegateSchoolContext dbContext = new dbEduegateSchoolContext())
            {
                ...
                var entityStudent = (from s in dbContext.Students
                                     ...
                                     where (s.ClassID == classID && s.SectionID == sectionID && s.IsActive == true && s.SchoolID == _context.SchoolID && s.Status == activeStatusID
        ...
        public List<KeyValueDTO> GetAcademicYearByProgressReport(int studentID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var academicYears = dbContext.ProgressReports
                                              .Where(x => x.StudentId == studentID && x.PublishStatusID == publishStatusID)
                                              .Include(x => x.AcademicYear)
                                              .AsNoTracking()
                                              ...
                                              .Select(g => new
                                              {
                                                  ...
                                                  Description = g.Select(x => x.AcademicYear.Description).FirstOrDefault()
                                              })
                                              .ToList();

                academicyearLists = academicYears.Select(t => new KeyValueDTO
                {
                    Key = t.AcademicYearID.ToString(),
                    Value = t.Description
                }).ToList();
        ...
        public string GetStudentLoginDetailsByStudentCode(string studentCode)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var student = dbContext.Students.Where(s => s.AdmissionNumber == studentCode && s.IsActive == true)
                    .Include(x => x.Login)
                    .AsNoTracking()
                    .FirstOrDefault();

                var emailID = student?.Login?.LoginEmailID;
        ...
        public StudentDTO FillStudentDTO(Student studentDet, List<Student> siblingList)
        {
            ...
            studentDTO = new StudentDTO()
            {
                ...
                Guardian = new GuardianDTO()
                {
                    ...
                    FatherEmailID = studentDet.Parent.FatherEmailID,
        ...
        // Bring all students keyvalue by parent id without checking active status
        public List<KeyValueDTO> GetStudentsByParent(long parentID)
        {
            var studentList = new List<KeyValueDTO>();
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var entities = dbContext.Students.Where(x => x.ParentID == parentID).AsNoTracking().ToList();
        ...
        // Bring all active students keyvalue by parent id
        ...
        // Bring all students details by parent id without checking active status
        public List<StudentDTO> GetStudentsDetailsByParent(long parentID)
        {
            var studentList = new List<StudentDTO>();
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var entities = dbContext.Students.Where(x => x.ParentID == parentID)
                    .Include(x => x.School)
                    .Include(x => x.AcademicYear)
                    .Include(x => x.Class)
                    .Include(x => x.Section)
        ...
        // Bring all active students details by parent id
        ...
        public StudentDTO GetStudentDetailsByLoginID(long? loginID)
        {
            var studentDTO = new StudentDTO();
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var student = loginID.HasValue ? dbContext.Students.Where(s => s.LoginID == loginID).AsNoTracking().FirstOrDefault() : null;
                if (student != null)
                {
                    studentDTO = new StudentDTO()
                    {
        ...
        public StudentDTO GetStudentDetailsByLoginUserID(string loginUserID)
        {
            var studentDTO = new StudentDTO();
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var student = loginUserID != null ? dbContext.Students.Where(s => s.AdmissionNumber == loginUserID).AsNoTracking().FirstOrDefault() : null;
                if (student != null)
                {
                    studentDTO = new StudentDTO()
                    {
