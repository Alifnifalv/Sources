...
namespace Eduegate.Domain.Mappers.School.Transports
{
    public class AssignVehicleMapper : DTOEntityDynamicMapper
    {
        ...
        public EmployeesDTO GetDriverDetailsByStudent(long studentID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var studentDet = dbContext.Students.Where(y => y.StudentIID == studentID && y.IsActive == true).AsNoTracking().FirstOrDefault();
                ...
                if (studentDet != null)
                {
                    var transportDet = dbContext.StudentRouteStopMaps.Where(a => a.StudentID == studentDet.StudentIID && a.IsActive == true)
                        .Include(i => i.Routes1)
                    ...
                    {
                        var route = transportDet.Routes11 != null ? dbContext.Routes1.AsNoTracking().FirstOrDefault(b => b.RouteID == transportDet.Routes11.RouteID ) : transportDet.Routes1 != null ? dbContext.Routes1.FirstOrDefault(b => b.RouteID == transportDet.Routes1.RouteID) : null;

                        var assign = route != null ? dbContext.AssignVehicleMaps
                                        .Where(d => d.RouteID == route.RouteID  && d.IsActive == true)
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                if (studentDet != null)
                {
                    ...
                    if (transportDet != null)
                    {
                        ...
                        var assign = route != null ? dbContext.AssignVehicleMaps
                            .Where(d => d.RouteID == route.RouteID && d.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusID   && d.IsActive == true)
                        ...
                        if (staffRouteStopMap != null)
                        {
                            ...
                            if (route != null)
                            {
                                ...
                                var assignedVehicleMap = schoolContext.AssignVehicleMaps
                                    .Where(avm => avm.RouteID == route.RouteID && avm.AcademicYear.AcademicYearStatusID == currentAcademicYearStatusId   && avm.IsActive == true)
                                    .Include(v => v.Routes1)
                                    .AsNoTracking()
                                    .FirstOrDefault();
