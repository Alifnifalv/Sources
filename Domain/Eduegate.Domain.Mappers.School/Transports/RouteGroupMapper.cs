    ...
    public class RouteGroupMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var prvsRouteGroup = dbContext.RouteGroups.Where(a => a.RouteGroupID == toDto.RouteGroupID).AsNoTracking().FirstOrDefault();

                if (prvsRouteGroup != null)
                {
                    if (toDto.AcademicYearID != prvsRouteGroup.AcademicYearID)
                    {
                        UpdateAacademicYearDetails(prvsRouteGroup.RouteGroupID, toDto.AcademicYearID);
                    }
                }
        ...
        public void UpdateAacademicYearDetails(int routeGroupID, int? academicYearID)
        {
            var routes = new List<Routes1>();

            using (var dbContext = new dbEduegateSchoolContext())
            {
                var groupData = dbContext.RouteGroups.Where(a => a.RouteGroupID == routeGroupID)
                ...
                var academicYears = dbContext.AcademicYears.Where(a => a.AcademicYearID == academicYearID).FirstOrDefault();

                if (groupData.Routes1 != null && groupData.Routes1.Count > 0)
                {
                    foreach (var route in groupData.Routes1)
                    {
                        // Update AcademicYearID for each route
                        route.AcademicYearID = academicYearID;
                        dbContext.Entry(route).State = EntityState.Modified;

                        foreach (var stopMap in route.RouteStopMaps)
                        {
                            stopMap.AcademicYearID = academicYearID;

                            dbContext.Entry(stopMap).State = EntityState.Modified;
                        }

                        foreach (var vehicleMap in route.RouteVehicleMaps)
                        {
                            vehicleMap.AcademicYearID = academicYearID;
                            vehicleMap.DateFrom = academicYears.StartDate;
                            vehicleMap.DateTo = academicYears.EndDate;

                            dbContext.Entry(vehicleMap).State = EntityState.Modified;
                        }

                        foreach (var assignVehicle in route.AssignVehicleMaps)
                        {
                            assignVehicle.AcademicYearID = academicYearID;
                            assignVehicle.DateFrom = academicYears.StartDate;
                            assignVehicle.DateTo = academicYears.EndDate;

                            dbContext.Entry(assignVehicle).State = EntityState.Modified;
                        }
                    }
