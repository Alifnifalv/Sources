                ...
                if (toDto.StudentLists.Count > 0)
                {
                    foreach (var map in toDto.StudentLists)
                    {
                        ...
                        if (map.StudentID.HasValue)
                        {
                            ...
                            var fromRouteGroupDetails = dbContext.RouteGroups.FirstOrDefault(a => a.RouteGroupID == toDto.RouteGroupID);
                            var toRouteGroupDetails = dbContext.RouteGroups.FirstOrDefault(a => a.RouteGroupID == toDto.ToRouteGroupID);
                            ...
                            var entity = new StudentRouteStopMap()
                            {
                                ...
                                AcademicYearID = toRouteGroupDetails.AcademicYearID,
                            ...
                            var prvsFeePeriods = dbContext.StudentRouteStopMaps.Where(a => a.StudentID == map.StudentID && a.AcademicYearID == fromRouteGroupDetails.AcademicYearID && a.IsActive == true)
                                .Include(a => a.StudentRoutePeriodMaps)
                                .Include(a => a.StudentRouteMonthlySplits)
                                .ToList();
                            ...
                            foreach (var feePrd in prvsFeePeriods)
                            {
                                foreach (var feeMrd in feePrd.StudentRoutePeriodMaps)
                                {
                                    entity.StudentRoutePeriodMaps.Add(new StudentRoutePeriodMap()
                                    {
                                        FeePeriodID = feeMrd.FeePeriodID,
                                ...
                                entity.StudentRouteMonthlySplits = new List<StudentRouteMonthlySplit>();
                                foreach (var monthlySplitDTO in feePrd.StudentRouteMonthlySplits)
                                {
                                    entity.StudentRouteMonthlySplits.Add(new StudentRouteMonthlySplit()
                                    {
                                        ...
                                        DropStopMapID = map.DropStopMapID == null ? null : map.DropStopMapID,
                                        PickupStopMapID = map.PickupStopMapID == null ? null : map.PickupStopMapID,
                                        StudentRouteStopMapID = entity.StudentRouteStopMapIID,
