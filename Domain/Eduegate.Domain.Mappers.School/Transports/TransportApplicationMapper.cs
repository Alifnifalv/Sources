...
namespace Eduegate.Domain.Mappers.School.Transports
{
    public class TransportApplicationMapper : DTOEntityDynamicMapper
    {
                        ...
                        if (fillAppliedDatas == true)
                        {
                            ...
                            if (stud.Parent != null)
                            {
                                ...
                                applicationDTO = new TransportApplicationDTO()
                                {
                                    ...
                                    //DropStopDetails
                                    IsRouteDifferent = locationData?.IsRouteDifferent,
                                    LocationName_Drop = locationData?.LocationName_Drop,
                                    LandMark_Drop = locationData?.LandMark_Drop,
                                    BuildingNo_Drop = locationData?.BuildingNo_Drop,
                                    StreetName_Drop = locationData?.StreetName_Drop,
                                    StreetNo_Drop = locationData?.StreetNo_Drop,
                                    ZoneNo_Drop = locationData?.ZoneNo_Drop,
        ...
        {
          string savedDtoJson = this.SaveEntity(transportApplicationData);
            if (string.IsNullOrEmpty(savedDtoJson))
            {
                 return null;
            }  
            return JsonConvert.DeserializeObject<TransportApplicationDTO>(savedDtoJson);
        }
    }
}