...
namespace Eduegate.Domain.Mappers.SignUp.Meeting
{
    public class MeetingRequestMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            {
                if (!toDto.ApprovedDate.HasValue)
                {
                    throw new Exception("Approved date is required!");
                }
                if (!toDto.ApprovedSignupSlotMapID.HasValue)
                {
            ...
            {
                toDto.ApprovedDate = (DateTime?)null;
                toDto.ApprovedSignupSlotMapID = null;
            }
            ...
            if (entity.MeetingRequestIID > 0)
            {
                ...
                {
                    toDto.MeetingRequestStatusName = GetMeetingRequestStatusNameByID(toDto.MeetingRequestStatusID);

                    SendPushNotificationToParent(toDto, messageType);
                    SendEmailNotificationToParent(toDto, messageType);
                }
            }
        ...
        public void SendPushNotificationToParent(MeetingRequestDTO reqDTO, string messageType)
        {
            ...
            else if (messageType == "applied")
            {
                message = "Received your meeting request";
            }
        ...
        private void FillDetailsForNotification(MeetingRequestDTO reqDTO, string message, List<long?> loginIDs)
        {
            ...
            if (reqDTO.OldMeetingRequestStatusID.HasValue)
            {
                ...
                else if (reqDTO.MeetingRequestStatusID == requestStatusID)
                {
                    message = "We have received your meeting request!";
            ...
            {
                if (reqDTO.MeetingRequestStatusID == requestStatusID)
                {
                    message = "We have received your meeting request!";
        ...
        public void SendEmailNotificationToParent(MeetingRequestDTO reqDTO, string messageType)
        {
            var requestStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("MEETING_REQUEST_STATUS_REQUESTED", 1);
            ...
            string emailSubject = "Meeting Request";
            ...
            {
                receiptBody = receiptBody.Replace("{MEETING_DATE}", reqDTO.RequestedDateString);
                receiptBody = receiptBody.Replace("{MEETING_TIME}", reqDTO.RequestedSlotTime);
            }

            emailBody = receiptBody;
            ...
            {
                EmailProcess(emailID, reqDTO, emailBody, emailSubject);
            }
        }

        public void EmailProcess(string toEmail, MeetingRequestDTO reqDTO, string emailBody, string emailSubject)
        {
            ...
            try
            {
                ...
                if (reqDTO.ClassID.HasValue)
                {
                    var data = new Setting.SettingBL(_context).GetClassDetailByClassID(reqDTO.ClassID.Value);
        ...
        public void SendPushNotificationToFaculty(MeetingRequestDTO reqDTO, string messageType)
        {
            var loginIDs = new List<long?>();
            ...
            {
                message = "The new meeting request has been received.";
            }

            var settings = NotificationSetting.GetEmployeeAppSettings();

            if (reqDTO.FacultyLoginID.HasValue)
            {
                loginIDs.Add(reqDTO.FacultyLoginID);
            }
        ...
        public void SendEmailNotificationToFaculty(MeetingRequestDTO reqDTO, string messageType)
        {
            var requestStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("MEETING_REQUEST_STATUS_REQUESTED", 1);
            string emailBody = string.Empty;
            string emailSubject = "Meeting Request";

            var emailTemplate = EmailTemplateMapper.Mapper(_context).GetEmailTemplateDetails(EmailTemplates.MeetingRequestMailToFaculty.ToString());
            ...
            if (string.IsNullOrEmpty(emailBody))
            {
                emailBody = @"<br />
                        <p align='left'>
                        Dear" + reqDTO.Faculty.Value + @",<br /></p>
                        A new meeting request has been received from the parent side under your name.<br />
                        To view the meeting request,  please login to your portal and check<br /><br />
                        <br/><br/><p style='font-size:0.7rem;'<b>Please Note: </b>do not reply to this email as it is a computer generated email</p>";
            }

            EmailProcess(reqDTO.FacultyEmailID, reqDTO, emailBody, emailSubject);
        }

        public string GetMeetingRequestStatusNameByID(byte? meetingRequestStatusID)
        {
            using (var dbContext = new dbEduegateSignUpContext())
            {
                return dbContext.MeetingRequestStatuses.Where(x => x.MeetingRequestStatusID == meetingRequestStatusID)
                    .AsNoTracking().FirstOrDefault()?.RequestStatusName;
            }
        }
