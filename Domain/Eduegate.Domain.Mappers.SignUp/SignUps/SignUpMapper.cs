...
{
    public class SignUpMapper : DTOEntityDynamicMapper
    {
        public static SignUpMapper Mapper(CallContext context)
        {
            var mapper = new SignUpMapper();
        ...
        {
            return JsonConvert.DeserializeObject<SignUpDTO>(entity);
        }
        ...
        private SignUpDTO ToDTO(long IID)
        {
        ...
        private SignUpDTO ToDTO(Signup entity)
        {
            ...
            FillStudent(entity, signUpDTO);
            FillOrganizerEmployee(entity, signUpDTO);
            FillSignUpAudienceMaps(entity, signUpDTO);
            FillSignUpSlotMaps(entity, signUpDTO);
            GetNotificationSendStatus(signUpDTO);
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSignUpContext())
            {
                ...
                if (entity.SignupIID > 0)
                {
                    ...
                    {
                        toDto.SignupIID = entity.SignupIID;
        ...
        private void SlotAvailabilityCheckForEmployee(Signup entity, SignUpDTO signUpDTO, SignupSlotMapDTO slotMapDTO)
        {
            using (var dbContext = new dbEduegateSignUpContext())
            {
                var signupPublishedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_PUBLISH", 3);
        ...
        private void FillOrganizerEmployee(Signup entity, SignUpDTO signUpDTO)
        {
            if (entity.OrganizerEmployeeID.HasValue)
            {
                ...
                signUpDTO.OrganizerEmployeeName = organizerEmployee;
            }
        }

        private void FillStudent(Signup entity, SignUpDTO signUpDTO)
        {
            if (entity.StudentID.HasValue)
            {
                ...
                signUpDTO.StudentName = studentName;
            }
        }

        private void FillSignUpAudienceMaps(Signup entity, SignUpDTO signUpDTO)
        {
            signUpDTO.SignupAudienceMaps = new List<SignupAudienceMapDTO>();
            foreach (var audMap in entity.SignupAudienceMaps)
            {
                signUpDTO.SignupAudienceMaps.Add(new SignupAudienceMapDTO()
                {
        ...
        private void FillSignUpSlotMaps(Signup entity, SignUpDTO signUpDTO)
        {
            var dateFormat = new Domain.Setting.SettingBL(null).GetSettingValue<string>("DateFormat");
            var timeFormat = new Domain.Setting.SettingBL(null).GetSettingValue<string>("TimeFormatWithoutSecond");
            ...
            {
                var slotAllocMapDTOs = new List<SignupSlotAllocationMapDTO>();
                ...
                foreach (var slotAlloc in slotMap.SignupSlotAllocationMaps)
                {
                    ...
                    {
                        var slotAllocDTO = FillSignUpSlotAllocationMapDTO(slotAlloc);
                ...
                signUpDTO.SignupSlotMaps.Add(new SignupSlotMapDTO()
                {
                    ...
                    SignupSlotAllocationMaps = slotAllocMapDTOs,
        ...
        private void GetNotificationSendStatus(SignUpDTO signUpDTO)
        {
            signUpDTO.NotificationStatus = PushNotificationMapper.Mapper(_context).AlertNotifiedStatusByReferenceID(null, signUpDTO.SignupIID, (long)Screens.Ticket);
        }

        private SignupSlotAllocationMapDTO FillSignUpSlotAllocationMapDTO(SignupSlotAllocationMap slotAlloc)
        {
            var slotAllocMapDTO = new SignupSlotAllocationMapDTO();

            if (slotAlloc != null)
            {
                slotAllocMapDTO = new SignupSlotAllocationMapDTO()
                {
        ...
        public List<SignUpDTO> FillSignUpDetailsByGroupID(int groupID)
        {
            var dtos = new List<SignUpDTO>();
        ...
        public OperationResultDTO SaveSelectedSignUpSlot(SignupSlotAllocationMapDTO allocDTO)
        {
            using (var dbContext = new dbEduegateSignUpContext())
            {
                ...
                else
                {
                    ...
                    SendSlotBookingAlertsToParent(allocDTO);
                    SendSlotBookingAlertsToOrganizerEmployee(allocDTO);
        ...
        public OperationResultDTO CancelSelectedSignUpSlot(SignupSlotMapDTO signUpSlotMap)
        {
            using (var dbContext = new dbEduegateSignUpContext())
            {
                ...
                var entity = dbContext.SignupSlotAllocationMaps
                    .Where(x => x.SignupSlotMapID == signUpSlotMap.SignupSlotMapIID && x.ParentID == signUpSlotMap.ParentID
                    && x.SlotMapStatusID == assignedSlotMapStatsID)
                    .Include(i => i.Parent)
                    .Include(i => i.SignupSlotMap).ThenInclude(i => i.Signup).ThenInclude(i => i.OrganizerEmployee).ThenInclude(i => i.Login)
                    .AsNoTracking().FirstOrDefault();
                ...
                if (entity != null)
                {
                    ...
                    FillDetailsAndSendNotification(entity, signUpSlotMap);
                }
        ...
        private void SendPushNotification(SignUpDTO signDTO)
        {
            ...
            foreach (var login in loginIDs)
            {
                ...
                PushNotificationMapper.SendAndSavePushNotification(toLoginID, fromLoginID, message, title, settings, signDTO.SignupIID, (long)Screens.Signup);
            }
        }

        private void FillDetailsForNotification(SignUpDTO signDTO, string message, List<long?> loginIDs)
        {
            var signupDraftStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_DRAFT", 1);
            var signupCancelledStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_CANCEL", 2);
            var signupPublishedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_PUBLISH", 3);
            var signupClosedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_CLOSED", 4);
            ...
            if (signDTO.SignupOldStatusID.HasValue)
            {
                ...
                else if (signDTO.SignupOldStatusID == signupDraftStatusID && signDTO.SignupStatusID == signupPublishedStatusID)
                {
                    ...
                    else if (signDTO.ClassID.HasValue)
                    {
        ...
        public SignupSlotAllocationMapDTO GetAssignedSlotAllocationDetailBySlotMapID(long signupSlotMapID)
        {
            var assignedSlotMapStatsID = new Domain.Setting.SettingBL(null).GetSettingValue<byte>("SIGNUP_SLOTMAP_STATUSID_ASSIGNED", 2);

            var signupSlotAllocationMapDTO = new SignupSlotAllocationMapDTO();
            ...
            using (var dbContext = new dbEduegateSignUpContext())
            {
                ...
                if (entity != null)
                {
                    signupSlotAllocationMapDTO = FillSignUpSlotAllocationMapDTO(entity);
                }
            }

            return signupSlotAllocationMapDTO;
        }

        private void SendEmailNotification(SignUpDTO signDTO)
        {
            var signupPublishedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_STATUSID_PUBLISH", 3);
            ...
            {
                var data = new Setting.SettingBL(_context).GetClassDetailByClassID(signDTO.ClassID.Value);
            ...
            {
                EmailProcess(emailID, classCode);
            }
        }

        public void EmailProcess(string toEmail, string classCode)
        {
        ...
        private void SendSlotBookingAlertsToParent(SignupSlotAllocationMapDTO allocDTO)
        {
            var slotAssignedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_SLOTMAP_STATUSID_ASSIGNED", 2);
            ...
            if (allocDTO.SlotMapStatusID == slotAssignedStatusID)
            {
                ...
                message = $"Your meeting booking with {allocDTO.OrganizerEmployeeName} on {allocDTO.SlotDateString} at {allocDTO.SlotTimeString} has been completed. Thank you!";
            }
            else if (allocDTO.SlotMapStatusID == slotCancelledStatusID)
            {
                title = "Meeting Slot Has Been Cancelled";
                message = $"Your booked meeting slot with {allocDTO.OrganizerEmployeeName} on {allocDTO.SlotDateString} at {allocDTO.SlotTimeString} has been cancelled. Thank you!";
            }
            else if (allocDTO.SlotMapStatusID == slotClosedStatusID)
            {
                title = "Meeting slot closed";
                message = $"The booked meeting slot with {allocDTO.OrganizerEmployeeName} on {allocDTO.SlotDateString} at {allocDTO.SlotTimeString} has been marked as closed. Thank you!";
            }

            long toLoginID = allocDTO.ParentLoginID ?? 0;
            long fromLoginID = allocDTO.OrganizerEmployeeLoginID ?? 2;
        ...
        public List<SignUpDTO> GetActiveMeetingRequestDetailsByEmployeeID(long employeeID)
        {
            var dtos = new List<SignUpDTO>();
        ...
        public string SaveSignupSlotAllocation(SignupSlotAllocationMapDTO slotAllocMapDTO)
        {
            using (var dbContext = new dbEduegateSignUpContext())
            {
                var slotAssignedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_SLOTMAP_STATUSID_ASSIGNED", 2);
        ...
        private void SendSlotBookingAlertsToOrganizerEmployee(SignupSlotAllocationMapDTO allocDTO)
        {
            var slotAssignedStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_SLOTMAP_STATUSID_ASSIGNED", 2);
            var slotCancelledStatusID = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("SIGNUP_SLOTMAP_STATUSID_CANCEL", 3);

            var title = "A parent has booked the slot.";
            var message = "A parent has successfully booked the slot.";

            if (allocDTO.SlotMapStatusID == slotAssignedStatusID)
            {
                title = "Meeting Slot Booked";
                message = $"A parent has successfully booked a meeting with you on {allocDTO.SlotDateString} at {allocDTO.SlotTimeString}. Thank you!";
            }
            else if (allocDTO.SlotMapStatusID == slotCancelledStatusID)
            {
                title = "Meeting Slot Cancelled";
                message = $"The meeting scheduled with a parent on {allocDTO.SlotDateString} at {allocDTO.SlotTimeString} has been cancelled. Thank you!";
            }

            long toLoginID = allocDTO.OrganizerEmployeeLoginID ?? 0;
            long fromLoginID = 2;
        ...
        public void FillDetailsAndSendNotification(SignupSlotAllocationMap entity, SignupSlotMapDTO signUpSlotMap)
        {
            var dateFormat = new Domain.Setting.SettingBL(null).GetSettingValue<string>("DateFormat");
            var timeFormat = new Domain.Setting.SettingBL(null).GetSettingValue<string>("TimeFormatWithoutSecond");

            var allocDTO = new SignupSlotAllocationMapDTO()
            {
                SlotMapStatusID = entity.SlotMapStatusID,
                ParentID = entity.ParentID,
                ParentLoginID = entity.Parent?.LoginID,
                OrganizerEmployeeID = signUpSlotMap.SignupOrganizerEmployeeID.HasValue ? signUpSlotMap.SignupOrganizerEmployeeID : entity.SignupSlotMap?.Signup?.OrganizerEmployeeID,
                OrganizerEmployeeLoginID = entity.SignupSlotMap?.Signup?.OrganizerEmployee?.Login?.LoginIID,
                OrganizerEmployeeEmailID = entity.SignupSlotMap?.Signup?.OrganizerEmployee?.Login?.LoginEmailID,
            };

            allocDTO.SlotMapStatusID = entity.SlotMapStatusID;

            if (string.IsNullOrEmpty(signUpSlotMap.SignupOrganizerEmployeeName))
            {
                allocDTO.OrganizerEmployeeName = entity.SignupSlotMap?.Signup?.OrganizerEmployee?.EmployeeCode + " - " + entity.SignupSlotMap?.Signup?.OrganizerEmployee?.FirstName + " " +
                    (!string.IsNullOrEmpty(entity.SignupSlotMap?.Signup?.OrganizerEmployee?.MiddleName) ? entity.SignupSlotMap?.Signup?.OrganizerEmployee?.MiddleName + " " : "") +
                    entity.SignupSlotMap?.Signup?.OrganizerEmployee?.LastName;
            }
            else
            {
                allocDTO.OrganizerEmployeeName = signUpSlotMap.SignupOrganizerEmployeeName;
            }

            if (string.IsNullOrEmpty(signUpSlotMap.SlotDateString))
            {
                allocDTO.SlotDateString = entity.SignupSlotMap != null ? entity.SignupSlotMap.SlotDate.HasValue ? entity.SignupSlotMap.SlotDate.Value.ToString(dateFormat, CultureInfo.InvariantCulture) : null : null;
            }
            else
            {
                allocDTO.SlotDateString = signUpSlotMap.SlotDateString;
            }

            if (string.IsNullOrEmpty(signUpSlotMap.StartTimeString))
            {
                allocDTO.StartTimeString = entity.SignupSlotMap != null ? entity.SignupSlotMap.EndTime.HasValue ? DateTime.Today.Add(entity.SignupSlotMap.EndTime.Value).ToString(timeFormat, CultureInfo.InvariantCulture) : null : null;
            }
            else
            {
                allocDTO.StartTimeString = signUpSlotMap.StartTimeString;
            }

            if (string.IsNullOrEmpty(signUpSlotMap.EndTimeString))
            {
                allocDTO.EndTimeString = entity.SignupSlotMap != null ? entity.SignupSlotMap.EndTime.HasValue ? DateTime.Today.Add(entity.SignupSlotMap.EndTime.Value).ToString(timeFormat, CultureInfo.InvariantCulture) : null : null;
            }
            else
            {
                allocDTO.EndTimeString = signUpSlotMap.EndTimeString;
            }

            allocDTO.SlotTimeString = allocDTO.StartTimeString + " - " + allocDTO.EndTimeString;
