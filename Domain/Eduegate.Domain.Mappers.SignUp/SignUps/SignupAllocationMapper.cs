...
{
    public class SignupAllocationMapper : DTOEntityDynamicMapper
    {
        public static SignupAllocationMapper Mapper(CallContext context)
        {
            var mapper = new SignupAllocationMapper();
        ...
        private void FillSlotRemarkMaps(SignupSlotAllocationMap entity, SignupSlotAllocationMapDTO slotAllocationMapDTO)
        {
            ...
            var remarkingHours = new Domain.Setting.SettingBL(_context).GetSettingValue<byte>("MEETING_PARENT_REMARKING_DEADLINE_HOURS", 24);
            ...
            if (slotRemarkMap != null)
            {
                slotAllocationMapDTO.SignupSlotRemarkMap = new SignupSlotRemarkMapDTO()
                {
                    ...
                    ParentRemarkingTimeExpired = false,
                ...
                if (slotRemarkMap.RemarkEnteredDate.HasValue)
                {
                    var maximumTimeForParentRemark = slotRemarkMap.RemarkEnteredDate.Value.AddHours(remarkingHours);
                    if (currentDateTime > maximumTimeForParentRemark)
                    {
                        slotAllocationMapDTO.SignupSlotRemarkMap.ParentRemarkingTimeExpired = true;
                    }
                }
            }
            else
        ...
        {
            var allocationDTOs = new List<SignupSlotAllocationMapDTO>();
