    ...
    public class VisitorMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            if (!toDto.LoginID.HasValue)
            {
                using (var dbContext = new dbEduegateERPContext())
                {
                    var loginDetails = dbContext.Logins.Where(l => l.LoginUserID == toDto.VisitorNumber).AsNoTracking().FirstOrDefault();
        ...
        public VisitorDTO GetVisitorDetails(string QID, string passportNumber)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var entity = !string.IsNullOrEmpty(QID) ? dbContext.Visitors.Where(v => v.QID == QID).Include(i => i.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault() :
                    !string.IsNullOrEmpty(passportNumber) ? dbContext.Visitors.Where(v => v.PassportNumber == passportNumber).Include(i => i.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault() : null;
        ...
        public OperationResultDTO RegisterVisitorDetails(VisitorDTO toDto)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var visitorDetails = !string.IsNullOrEmpty(toDto.QID) ? dbContext.Visitors.Where(v => v.QID == toDto.QID).Include(i => i.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault() :
                    !string.IsNullOrEmpty(toDto.PassportNumber) ? dbContext.Visitors.Where(v => v.PassportNumber == toDto.PassportNumber).Include(i => i.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault() : null;
                ...
                if (visitorDetails != null)
                {
                    return new OperationResultDTO()
                    {
                        ...
                        Message = "User already exists under the same QID or Passport number."
                    };
                }
                // Check if EmailID exists in Login table
                if (!string.IsNullOrEmpty(toDto.EmailID))
                {
                    var emailExists = dbContext.Logins.Any(l => l.LoginEmailID == toDto.EmailID);
                    if (emailExists)
                    {
                        return new OperationResultDTO()
                        {
            ...
            try
            {
                ...
                if (!toDto.LoginID.HasValue)
                {
                    ...
                    {
                        var loginDetails = dbContext1.Logins.Where(l => l.LoginUserID == toDto.VisitorNumber || l.LoginEmailID.ToLower() == toDto.EmailID.ToLower()).AsNoTracking().FirstOrDefault();
        ...
        public VisitorDTO GetVisitorDetailsByLoginID(long loginID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var entity = dbContext.Visitors.Where(v => v.LoginID == loginID).Include(i => i.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault();
