...
{
    public class ReportViewerBL
    {
        private CallContext Context { get; set; }

        public ReportViewerBL(CallContext context)
        {
            Context = context;
        }
        ...
        public byte[] GetReportFile(string reportName, string reportParameter, string format, string rdlcFilePath = null)
        {
            try
            {
                ...
                if (!System.IO.File.Exists(rdlPath))
                {
                    ...
                    {
                        string fileDirPath = Assembly.GetExecutingAssembly().Location.Replace("Eduegate.Domain.Report.dll", string.Empty);
            ...
            catch (Exception ex)
            {
                ...
                Eduegate.Logger.LogHelper<string>.Fatal($"{reportName} generation failed. Error message: {errorMessage}", ex);
        ...
        private void DefaultReportParameters(Microsoft.Reporting.NETCore.LocalReport viewer, List<KeyValueDTO> parameters)
        {
            ...
            //Set default short currency code
            var defaultCurrencyShortCode = new Domain.Setting.SettingBL(null).GetSettingValue<string>("DEFAULT_CURRENCY_SHORT_CODE");
            if (reportParameters.Where(a => a.Name == "DefaultCurrencyShortCode").FirstOrDefault() != null)
            {
                viewer.SetParameters(new Microsoft.Reporting.NETCore.ReportParameter("DefaultCurrencyShortCode", defaultCurrencyShortCode, false));
            }
            var defaultCurrencyShortCodeParam = parameters.FirstOrDefault(p => p.Key == "DefaultCurrencyShortCode");
            if (defaultCurrencyShortCodeParam != null)
            {
                defaultCurrencyShortCodeParam.Value = defaultCurrencyShortCode;
            }
        }
        ...
        #region Bring lookups for report - OLD Method
        public void FillLookups(ScreenMetadataDTO metaData, Eduegate.Utilities.SSRSHelper.Report reportDetails)
        {
            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("AccountName")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("AccountName")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "AccountName",
                    Url = "Mutual/GetLookUpData?lookType=Account"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("GroupName")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("GroupName")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "GroupName",
                    Url = "Mutual/GetLookUpData?lookType=AccountGroup"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("AccountID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("AccountID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
            ...
            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("AllAccountID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("AllAccountID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "AllAccountID",
                    Url = "Mutual/GetLookUpData?lookType=Account"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Month")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Month")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Month",
                    Url = "Mutual/GetLookUpData?lookType=Months"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Brand")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Brand")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Brand",
                    Url = "Mutual/GetDynamicLookUpData?lookType=Brands"
                });
            }
            ...
            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("TStatus")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("TStatus")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "TStatus",
                    Url = "Mutual/GetLookUpData?lookType=TransactionStatus"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Status")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Status")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Status",
                    Url = "Inventories/Customer/GetCustomerStatus"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("SkuID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("SkuID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "SkuID",
                    Url = "Mutual/GetDynamicLookUpData?lookType=Products"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Categoryid")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Categoryid")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Categoryid",
                    Url = "Catalogs/Category/GetCategories"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("StatusID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("StatusID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "StatusID",
                    Url = "Mutual/GetDynamicLookUpData?lookType=ProductStatus"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Branches")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Branches")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Branches",
                    Url = "Mutual/GetLookUpData?lookType=Branch"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("branch")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("branch")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
            ...
            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("DeliveryType")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("DeliveryType")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "DeliveryType",
                    Url = "Mutual/GetDeliveryTypes"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Pmanager")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Pmanager")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Pmanager",
                    Url = "Mutual/GetLookUpData?lookType=Manager"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("sup")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("sup")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "sup",
                    Url = "Mutual/GetSuppliers"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("DepartmentID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("DepartmentID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "DepartmentID",
                    Url = "Mutual/GetDynamicLookUpData?lookType=Department"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Department")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Department")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
            ...
            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("DesignationID")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("DesignationID")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "DesignationID",
                    Url = "Mutual/GetDynamicLookUpData?lookType=Designation"
                });
            }

            if (reportDetails.ReportParameters.Exists(x => x.Name.Equals("Designation")) && !metaData.Urls.Exists(x => x.LookUpName.Equals("Designation")))
            {
                metaData.Urls.Add(new ScreenLookupDTO()
                {
                    IsOnInit = true,
                    LookUpName = "Designation",
                    Url = "Mutual/GetDynamicLookUpData?lookType=Designation"
                });
            }
        }
        #endregion Report lookups
