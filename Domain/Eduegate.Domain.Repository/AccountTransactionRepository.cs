...
namespace Eduegate.Domain.Repository
{
    public class AccountTransactionRepository
    {
        ...
        public List<AccountTransactionDetail> GetAccountTransactionsDetailByHeadID(long headID)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return dbContext.AccountTransactionDetails.Where(a => a.AccountTransactionHeadID == headID)
                          .Include(x => x.Account)
                        .AsNoTracking().ToList();
                }
            }
        ...
        public AccountTransactionHead GetAccountTransactionHeadByCostCenterID(int CostCenterID)
        {
            AccountTransactionHead accountTransactionHead = null;
            try
            {
        ...
        public List<Receivable> GetReceivables(List<long> receivableIds, bool loadReferenceData = true, bool IsAccountReceivableMap = false)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    else
                    {
                        ...
                        {
                            return dbContext.Receivables
                               .Include(x => x.AccountTransactionReceivablesMaps)
                               .Where(a => receivableIds.Contains(a.ReceivableIID))
                               .AsNoTracking()
                               .ToList();
                        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    if (receivable != null)
                    {
                        ...
                        receivable.PaidAmount = (receivable.PaidAmount ?? 0) + amount;
                    }
                    dbContext.SaveChanges();
                    return true;
                }
        ...
        public bool UpdateAccountReceivablesPaidAmount(long? ReceivableId, decimal amount)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var receivableAmount = dbContext.AccountTransactionReceivablesMaps.Where(x => x.ReceivableID == ReceivableId).Sum(x => x.Amount);
                        Receivable receivable = dbContext.Receivables.Where(x => x.ReceivableIID == ReceivableId).FirstOrDefault();
                    if (receivable != null)
                    {
                        // receivable.Amount = receivable.Amount - amount;
                        receivable.PaidAmount = receivableAmount;
                    }
                    dbContext.SaveChanges();
                    return true;
                }
        ...
        public List<Receivable> AddReceivable(List<Receivable> receivablesList)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var documentTypeID = receivablesList.Select(x => x.DocumentTypeID).FirstOrDefault();
                    var transactionNos = receivablesList.Select(x => x.TransactionNumber).ToList();
                    var existingList = dbContext.Receivables.Where(x => documentTypeID == x.DocumentTypeID && transactionNos.Contains(x.TransactionNumber)).ToList();
                    if (existingList != null && existingList.Count() > 0)
                    {
                        foreach (Receivable receivableEntity in existingList)
                        {
                            foreach (Receivable receivable in receivablesList)
                            {
                                receivableEntity.TransactionDate = receivable.TransactionDate;
                                receivableEntity.DueDate = receivable.DueDate;
                                receivableEntity.DocumentStatusID = receivable.DocumentStatusID;
                                receivableEntity.AccountID = receivable.AccountID;
                                receivableEntity.PaidAmount = receivable.PaidAmount;
                                receivableEntity.Amount = receivable.Amount;
                                receivableEntity.DocumentTypeID = receivable.DocumentTypeID;
                                receivableEntity.TransactionNumber = receivable.TransactionNumber;
                                receivableEntity.DiscountAmount = receivable.DiscountAmount;
                                receivableEntity.DebitOrCredit = receivable.DebitOrCredit;
                            }
                        }
                    }
                    else
        ...
        public List<AccountTransactionReceivablesMap> AddAccountTransactionReceivablesMaps(List<AccountTransactionReceivablesMap> mapList)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var receivableIDs = mapList.Select(x => x.ReceivableID).ToList();
                    var headIds = mapList.Select(x => x.AccountTransactionHeadID).ToList();
                    var existingList = dbContext.AccountTransactionReceivablesMaps.Where(x => headIds.Contains(x.AccountTransactionHeadID) && receivableIDs.Contains(x.ReceivableID)).ToList();
                    if (existingList != null && existingList.Count() > 0)
                        dbContext.AccountTransactionReceivablesMaps.RemoveRange(existingList);
                    dbContext.AccountTransactionReceivablesMaps.AddRange(mapList);
                    dbContext.SaveChanges();
                }
                return mapList;
            }
