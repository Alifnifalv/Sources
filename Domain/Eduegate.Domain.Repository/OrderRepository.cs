...
namespace Eduegate.Domain.Repository
{
    public class OrderRepository
    {
        ...
        public bool UpdateOrderIdPaymentDetailsKnet(long trackID, long transactionHeadId, long loginId)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var payments = dbContext.PaymentDetailsKnets.Where(x => x.TrackID == trackID && x.CustomerID == loginId).AsNoTracking().FirstOrDefault();
        ...
        public bool UpdateOrderIdPaymentMasterVisa(long trackID, long transactionHeadId, long loginId, decimal cartAmount)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var payments = dbContext.PaymentMasterVisas.Where(x => x.LoginID == loginId && x.PaymentAmount == cartAmount).OrderByDescending((x => x.TrackIID)).AsNoTracking().FirstOrDefault();
                if (payments != null)
                {
                    payments.OrderID = transactionHeadId;
                    dbContext.SaveChanges();
        ...
        public bool UpdateOrderIdPaymentPaypal(long trackID, long transactionHeadId, long loginId)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var payments = dbContext.PaymentDetailsPayPals.Where(x => x.TrackID == trackID && x.RefCustomerID == loginId).AsNoTracking().FirstOrDefault();
        ...
        public bool UpdateOrderIdPaymentTheFort(long trackKey, long transactionHeadId, long loginId)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var payments = dbContext.PaymentDetailsTheForts.Where(x => x.TrackKey == trackKey && x.CustomerID == loginId).AsNoTracking().FirstOrDefault();
        ...
        public bool UpdateOrderTrackingStatus(OrderTracking track)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                var trans = db.TransactionHeads.Where(x => x.HeadIID == track.OrderID)
                    .AsNoTracking()
                    .FirstOrDefault();
        ...
        public List<OrderTracking> GetOrderTrack(long orderID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.OrderTrackings.Where(x => x.OrderID == orderID)
                    .AsNoTracking()
                    .ToList();
            }
        }
        ...
        public List<TransactionStatus> GetTransactionList()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.TransactionStatuses.OrderBy(a => a.Description)
                    .AsNoTracking()
                    .ToList();
            }
        }
        ...
        public TransactionHead SaveWebsiteOrder(CheckoutPaymentDTO dto, string DocumentID, string transactionNo, long loginID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                var shoppingCart = db.ShoppingCarts.Where(a => a.ShoppingCartIID == shoppingcartID)
                ...
                {
                    var contact = db.Contacts.Where(a => a.LoginID == loginID && a.IsBillingAddress == true).AsNoTracking().FirstOrDefault();
        ...
        private long GetProductIDfromSKU(long skuID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return (long)db.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skuID)
                    .AsNoTracking()
        ...
        public TransactionHead GetTransactionDetails(long headID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var transactionHead = db.TransactionHeads.Where(a => a.HeadIID == headID)
        ...
        public DateTime? GetLastUsedVoucherOrder(long voucherID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var shoppingcartVoucherMap = db.ShoppingCartVoucherMaps.Where(a => a.VoucherID == voucherID)
                    .OrderByDescending(a => a.CreatedBy)
                    .AsNoTracking()
                    .FirstOrDefault();
                if (shoppingcartVoucherMap != null)
                {
                    var transheadShoppingCartMap = db.TransactionHeadShoppingCartMaps.Where(a => a.ShoppingCartID == shoppingcartVoucherMap.ShoppingCartID)
                        .AsNoTracking()
                        .FirstOrDefault();
                    if (transheadShoppingCartMap != null)
                    {
                        var transactionHead = db.TransactionHeads.Where(a => a.HeadIID == transheadShoppingCartMap.TransactionHeadID)
                            .AsNoTracking()
                            .FirstOrDefault();
        ...
        public bool UpdateCustomerLoyaltyPoints(decimal cartAmount, decimal loyaltyPointsValue, decimal categorizationPointsValue, long headID, long customerID)
        {
            try
            {
                using (dbEduegateERPContext db = new dbEduegateERPContext())
                {
                    ...
                    var customerEntity = db.CustomerSettings.Where(a => a.CustomerID == customerID)
                        .AsNoTracking()
                        .FirstOrDefault();
                    ...
                    db.Entry(customerEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    db.SaveChanges();
        ...
        public bool SetCustomerGroup(long customerID)
        {
            try
            {
                ...
                using (dbEduegateERPContext db = new dbEduegateERPContext())
                {
                    var totalLoyaltyPoints = db.CustomerSettings.Where(a => a.CustomerID == customerID)
                        .AsNoTracking().FirstOrDefault().TotalLoyaltyPoints;
                    if (totalLoyaltyPoints > 0)
                    {
                        ...
                        customerGroup = db.CustomerGroups.Where(a => (totalLoyaltyPoints - a.PointLimit) >= 0)
                            .OrderByDescending(a => a.PointLimit)
                            .AsNoTracking()
                            .First();
                    }
                    var customerEntity = db.Customers.Where(a => a.CustomerIID == customerID)
                        .AsNoTracking()
                        .FirstOrDefault();
        ...
        public long GetOrderIDFromOrderNo(string transactionNo)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var headID = db.TransactionHeads.Where(a => a.TransactionNo == transactionNo)
                    .AsNoTracking()
                    .FirstOrDefault();
                if (headID != null)
                {
        ...
        public Eduegate.Framework.Helper.Enums.DeliveryTypes GetDeliveryType(long headID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var deliveryType = db.TransactionHeads.Where(a => a.HeadIID == headID)
                    .AsNoTracking()
                    .Select(a => a.DeliveryTypeID)
                    .FirstOrDefault();
        ...
        public bool isOrderOfUser(long headID, long customerID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.TransactionHeads.Where(a => a.CustomerID == customerID && a.HeadIID == headID)
                    .AsNoTracking()
                    .Any();
            }
        }
