...
namespace Eduegate.Domain.Repository
{
    public class PriceSettingsRepository
    {
        public List<ProductPriceListProductMap> GetProductPriceSettings(long productIID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ProductPriceListProductMaps.Where(x => x.ProductID == productIID)
                    .Include(x => x.ProductPriceListProductQuantityMaps).AsNoTracking().ToList();
            }
        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                    {
                        ...
                        {
                            var pplpm = dbContext.ProductPriceListProductMaps.Where(x => x.ProductPriceListProductMapIID == delete.ProductPriceListProductMapIID).AsNoTracking().FirstOrDefault();
                            ...
                            if (entity.ProductPriceListProductQuantityMaps.IsNotNull() && entity.ProductPriceListProductQuantityMaps.Count > 0)
                            {
                                ...
                                foreach (var quantityMapEntity in entity.ProductPriceListProductQuantityMaps)
                                {
                                    if (quantityMapEntity.ProductPriceListProductQuantityMapIID <= 0)
                                    {
                                        ...
                                        dbContext.Entry(quantityMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                                    }
                                    else
                                    {
                                        dbContext.Entry(quantityMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                                    }
                                }
        ...
        public ProductPriceListProductMap GetProductPrice(long id)
        {
            ...
            {
                return db.ProductPriceListProductMaps.Where(x => x.ProductPriceListProductMapIID == id).AsNoTracking().FirstOrDefault();
            }
        }
        ...
        public ProductPriceListBrandMap GetBrandPrice(long id)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductPriceListBrandMaps.Where(x => x.ProductPriceListBrandMapIID == id).AsNoTracking().FirstOrDefault();
            }
        }
        public ProductPriceListBranchMap GetBranchMaps(long id)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductPriceListBranchMaps
                    ...
                    .Where(x => x.ProductPriceListBranchMapIID == id).AsNoTracking().FirstOrDefault();
            }
        }
        ...
        public IEnumerable<ProductPriceListBranchMap> SaveBranchMaps(List<ProductPriceListBranchMap> entities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                var dbBranchMaps =
                    db.ProductPriceListBranchMaps.Where(x => x.ProductPriceListID == productPriceListID).AsNoTracking().ToList();
                ...
                foreach (var branchMap in dbBranchMaps)
                {
                    ...
                    {
                        // Update
                        db.Entry(branchMap).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    }
                    else
                    {
                        // Delete
                        //db.ProductPriceListBranchMaps.Remove(branchMap);
                        db.Entry(branchMap).State = EntityState.Deleted;
                    }
                }
        ...
        public ProductPriceListCategoryMap GetCategoryPrice(long id)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductPriceListCategoryMaps
                    .Where(x => x.ProductPriceListCategoryMapIID == id)
                    .AsNoTracking()
                    .FirstOrDefault();
            }
        }
        ...
        public List<ProductPriceListCustomerGroupMap> SaveCustomerGroupPrice(List<ProductPriceListCustomerGroupMap> customerGroupPriceSettings, long IID, List<ProductPriceListCustomerGroupMap> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    ...
                    {
                        var pplcgm = db.ProductPriceListCustomerGroupMaps.Where(x => x.ProductPriceListCustomerGroupMapIID == del.ProductPriceListCustomerGroupMapIID)
                            .AsNoTracking()
                            .FirstOrDefault();
                ...
                if (customerGroupPriceSettings.IsNotNull() && customerGroupPriceSettings.Count > 0)
                {
                    foreach (var cgpMap in customerGroupPriceSettings)
                    {
                        if (cgpMap.ProductPriceListCustomerGroupMapIID <= 0)
                        {
                            ...
                            db.Entry(cgpMap).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                        }
                        else
                        {
                            db.ProductPriceListCustomerGroupMaps.Add(cgpMap);
                            db.Entry(cgpMap).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        }
                    }

                    long? skuMapID = customerGroupPriceSettings[0].ProductSKUMapID;
                    var dbData = db.ProductPriceListCustomerGroupMaps.Where(x => x.ProductSKUMapID == skuMapID)
                        .AsNoTracking()
                        .ToList();
        ...
        public ProductPriceListCustomerGroupMap GetCustomerGroupPrice(long id)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.ProductPriceListCustomerGroupMaps.Where(x => x.ProductPriceListCustomerGroupMapIID == id)
                    .AsNoTracking()
                    .FirstOrDefault();
            }
        }
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (skuID > 0)
                    return dbContext.ProductPriceListSKUMaps.Where(x => x.ProductSKUID == skuID && x.CompanyID == companyID)
                        .Include(x => x.ProductPriceListSKUQuantityMaps)
                        .AsNoTracking()
                        .ToList();
                else
        ...
        public List<ProductBundle> GetProductbundles(long skuID, int companyID = 0)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (skuID > 0)
                {
                    var entities = dbContext.ProductBundles.Where(x => x.ToProductSKUMapID == skuID /*&& x.FromProductID == companyID*/)
        ...
        public List<ProductPriceListCategoryMap> GetCategoryPriceSettings(long categoryID)
        {
            ...
            {
                if (categoryID > 0)
                    return dbContext.ProductPriceListCategoryMaps.Where(x => x.CategoryID == categoryID)
                        .AsNoTracking()
                        .ToList();
                else
        ...
        public List<ProductPriceListSKUQuantityMap> GetProductPriceListSkuQuantityMaps(long productPriceSkuMapID)
        {
            ...
            {
                return dbContext.ProductPriceListSKUQuantityMaps.Where(x => x.ProductPriceListSKUMapID == productPriceSkuMapID)
                    .AsNoTracking()
                    .ToList();
            }
        }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (deleteEntites.IsNotNull() && deleteEntites.Count > 0)
                    {
                        ...
                        {
                            var pplsm = dbContext.ProductPriceListSKUMaps.Where(x => x.ProductPriceListItemMapIID == delen.ProductPriceListItemMapIID)
                                .AsNoTracking()
                                .FirstOrDefault();
                    ...
                    if (entities.IsNotNull() && entities.Count > 0)
                    {
                        foreach (var skuMapEntity in entities)
                        {
                            if (skuMapEntity.ProductPriceListItemMapIID <= 0)
                            {
                                ...
                                dbContext.Entry(skuMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                            }
                            else
                            {
                                dbContext.Entry(skuMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                            }
                            ...
                            if (skuMapEntity.ProductPriceListSKUQuantityMaps.IsNotNull() && skuMapEntity.ProductPriceListSKUQuantityMaps.Count > 0)
                            {
                                ...
                                foreach (var skuQuantityMapEntity in skuMapEntity.ProductPriceListSKUQuantityMaps)
                                {
                                    if (skuQuantityMapEntity.ProductPriceListSKUQuantityMapIID <= 0)
                                    {
                                        ...
                                        dbContext.Entry(skuQuantityMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                                    }
                                    else
                                    {
                                        dbContext.Entry(skuQuantityMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                                    }
                                }
                                ...
                                var dbData = dbContext.ProductPriceListSKUQuantityMaps.Where(x => x.ProductPriceListSKUMapID == skuMapEntity.ProductPriceListItemMapIID)
                                    .AsNoTracking()
                                    .ToList();

                                var entitiesToDelete = dbData.Where(x => x.ProductPriceListSKUMapID == skuMapEntity.ProductPriceListItemMapIID).Except(currentData)
                                    .ToList();
                            ...
                            else
                            {
                                dbContext.ProductPriceListSKUQuantityMaps
                                    .RemoveRange(dbContext.ProductPriceListSKUQuantityMaps.Where(x => x.ProductPriceListSKUMapID == skuMapEntity.ProductPriceListItemMapIID)
                                    .AsNoTracking().ToList());
                            }
                        }
        ...
        public string GetCustomerGroupName(long customerGroupID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var customerGroup = dbContext.CustomerGroups.Where(x => x.CustomerGroupIID == customerGroupID)
                    .AsNoTracking()
                    .FirstOrDefault();
        ...
        public List<ProductPriceList> GetProductPriceLists(int? companyID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    return dbContext.ProductPriceLists.OrderBy(a => a.PriceDescription)
                        .AsNoTracking()
                        .ToList();
                }
                else
                {
                    return dbContext.ProductPriceLists.Where(x => x.CompanyID == companyID)
                        .OrderBy(a => a.PriceDescription)
                        .AsNoTracking()
                        .ToList();
                }
            }
        ...
        public List<Branch> GetProductBranchLists(int CompanyID = 0)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Branches.Where(a=> a.CompanyID == CompanyID && a.StatusID == 1)
                    .AsNoTracking()
                    .ToList();
            }
        }
        ...
        public List<ProductPriceListCategoryMap> SaveCategoryPriceSettings(List<ProductPriceListCategoryMap> entities, long categoryIID, List<ProductPriceListCategoryMap> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    foreach (var del in deleteEntities)
                    {
                        var pplcm = db.ProductPriceListCategoryMaps.Where(x => x.ProductPriceListCategoryMapIID == del.ProductPriceListCategoryMapIID)
                            .AsNoTracking()
                            .FirstOrDefault();
                ...
                if (entities.IsNotNull() && entities.Count > 0)
                {
                    foreach (var categoryMapEntity in entities)
                    {
                        if (categoryMapEntity.ProductPriceListCategoryMapIID <= 0)
                        {
                            ...
                            db.Entry(categoryMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                        }
                        else
                        {
                            db.Entry(categoryMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        }
                    }
        ...
        public List<ProductPriceListBrandMap> GetBrandPriceSettings(long brandID)
        {
            ...
            {
                if (brandID > 0)
                    return dbContext.ProductPriceListBrandMaps.Where(x => x.BrandID == brandID)
                        .AsNoTracking()
                        .ToList();
                else
        ...
        public List<ProductPriceListBrandMap> SaveBrandPriceSettings(List<ProductPriceListBrandMap> entities, long brandIID, List<ProductPriceListBrandMap> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    foreach (var del in deleteEntities)
                    {
                        var pplbm = db.ProductPriceListBrandMaps.Where(x => x.ProductPriceListBrandMapIID == del.ProductPriceListBrandMapIID)
                            .AsNoTracking()
                            .FirstOrDefault();
                ...
                if (entities.IsNotNull() && entities.Count > 0)
                {
                    foreach (var brandMapEntity in entities)
                    {
                        if (brandMapEntity.ProductPriceListBrandMapIID <= 0)
                        {
                            ...
                            db.Entry(brandMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                        }
                        else
                        {
                            db.Entry(brandMapEntity).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        }
                    }
        ...
        public List<ProductBundle> Saveproductbundlesettings(List<ProductBundle> entities, long BundleIID, List<ProductBundle> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    foreach (var del in deleteEntities)
                    {
                        var pplbm = db.ProductBundles.Where(x => x.BundleIID == del.BundleIID)
                            .AsNoTracking()
                            .FirstOrDefault();
        ...
        public List<ProductCategoryMap> Getcategories(long productID)
        {
            ...
            {
                if (productID > 0)
                    return dbContext.ProductCategoryMaps.Where(x => x.ProductID == productID)
                        .AsNoTracking()
                        .ToList();
                else
                    return new List<ProductCategoryMap>();
        ...
        public List<ProductAllergyMap> SaveAllergies(List<ProductAllergyMap> entities, long productID, List<ProductAllergyMap> existEntities = null)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                if (existEntities.IsNotNull() && existEntities.Count > 0)
                {
                    foreach (var del in existEntities)
                    {
                        var allergies = db.ProductAllergyMaps.Where(x => x.ProductAllergyMapIID == del.ProductAllergyMapIID)
                            .AsNoTracking()
                            .FirstOrDefault();

                        db.ProductAllergyMaps.Remove(allergies);
                    }
                    db.SaveChanges();
                }
        ...
        public List<ProductAllergyMap> GetAllergies(long productID)
        {
            ...
            {
                if (productID > 0)
                    return dbContext.ProductAllergyMaps.Where(x => x.ProductID == productID)
                        .AsNoTracking()
                        .ToList();
                else
        ...
        public List<ProductSKURackMap> Savecrackentities(List<ProductSKURackMap> entities, long productID, List<ProductSKURackMap> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    foreach (var del in deleteEntities)
                    {
                        var pplbm = db.ProductSKURackMaps.Where(x => x.ProductID == del.ProductID)
                            .AsNoTracking()
                            .FirstOrDefault();
        ...
        public List<ProductSKURackMap> Savecrackentities(long productID)
        {
            ...
            {
                if (productID > 0)
                    return dbContext.ProductSKURackMaps.Where(x => x.ProductID == productID)
                        .AsNoTracking()
                        .ToList();
                else
        ...
        public List<ProductBundle> GetProductbundleSettings(long BundleIID)
        {
            ...
            {
                if (BundleIID > 0)
                    return dbContext.ProductBundles.Where(x => x.BundleIID == BundleIID)
                        .AsNoTracking()
                        .ToList();
                else
                    return new List<ProductBundle>();
        ...
        public List<ProductPriceListCustomerGroupMap> SaveProductCustomerGroupPrice(List<ProductPriceListCustomerGroupMap> customerGroupPriceSettings, long productID, List<ProductPriceListCustomerGroupMap> deleteEntities)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (deleteEntities.IsNotNull() && deleteEntities.Count > 0)
                {
                    ...
                    {
                        var pplcgm = db.ProductPriceListCustomerGroupMaps.Where(x => x.ProductPriceListCustomerGroupMapIID == dele.ProductPriceListCustomerGroupMapIID).AsNoTracking().FirstOrDefault();
                ...
                if (customerGroupPriceSettings.IsNotNull() && customerGroupPriceSettings.Count > 0)
                {
                    ...
                    var dbData = db.ProductPriceListCustomerGroupMaps.Where(x => x.ProductID == productID).AsNoTracking().ToList();
        ...
        public bool UpdateSupplierSKUInventory(SKUDTO sku, long productPriceListID, long branchID, int companyID)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                var pricelistMap = db.ProductPriceListSKUMaps.Where(x => x.ProductPriceListID == productPriceListID && x.ProductSKUID == sku.ProductSKUMapID).AsNoTracking().FirstOrDefault();
                ...
                else
                {
                    ...
                    //db.Entry(productInventory).State = Microsoft.EntityFrameworkCore.EntityState.Added;
        ...
        public ProductPriceListSKUMap GetProductPriceListSKUMap(long productPriceListID, long skuID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                return db.ProductPriceListSKUMaps.Where(x => x.ProductPriceListID == productPriceListID && x.ProductSKUID == skuID).AsNoTracking().FirstOrDefault();
            }
        }
    }
}
