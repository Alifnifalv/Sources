    ...
    public class ProductDetailRepository
    {
            ...
            Eduegate.Services.Contracts.Enums.ProductStatuses productStatus)
        {
            ...
            var propTypeValue = (from qry in db.GetSKUByProperties
                                 ...
                                 select qry).AsNoTracking().FirstOrDefault();
            ...
            var qryProdImage = (from p in db.Products
                                ...
                                select new
                                {
                                    ...
                                    pim.ImageFile,
                                    pim.Sequence
                                }).OrderBy(x => x.ProductImageMapIID).ThenBy(x => x.Sequence).AsNoTracking().ToList();
        ...
        public List<TransactionDetailDTO> GetTransactionByTransactionDate(DateTime transactionDate)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var transaction = (from transactionDet in dbContext.TransactionHeads
                                   ...
                                   select transactionDet).AsNoTracking().FirstOrDefault();
        ...
        public ProductPriceList CreatePriceInformationDetail(ProductPriceList productPrice)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                if (productPrice.ProductPriceListIID == default(long))
                    dbContext.Entry(productPrice).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                else
                {
                    dbContext.Entry(productPrice).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    productPriceListSKUMaps = (from productPriceSKUMap in dbContext.ProductPriceListSKUMaps
                                               select productPriceSKUMap).AsNoTracking().ToList();
                }
            }
            catch (Exception exception)
            {
        ...
        public List<ProductPriceListSKUMap> UpdateSKUProductPrice(List<ProductPriceListSKUMap> productPriceSKUMapList)
        {
            try
            {
                ...
                {
                    // remove the data 
                    List<ProductPriceListSKUMap> productPriceListSKUMaps = dbContext.ProductPriceListSKUMaps.AsNoTracking().ToList().Where(x => x.ProductPriceListID == productPriceSKUMapList[0].ProductPriceListID).ToList();
        ...
        public void UpdateSKUPrice(List<ProductPriceListSKUMap> productPriceSKUMapList)
        {
            ...
            var productPriceListSKUMaps = dbContext.ProductPriceListSKUMaps.Where(x => x.ProductPriceListID == productPriceSKUMapList[0].ProductPriceListID).AsNoTracking().ToList();
                ...
                if (productPriceSKUMapList.Contains(productPriceListSKUMap))
                {
                    ...
                    if (productPriceSKUMapList != null && productPriceSKUMapList.Count > 0)
                    {
                        foreach (ProductPriceListSKUMap productPriceSKUMap in productPriceSKUMapList)
                        {
                            ...
                            {
                                ProductPriceListSKUMap skuMap = dbContext.ProductPriceListSKUMaps.Where(x => x.ProductPriceListItemMapIID == productPriceSKUMap.ProductPriceListItemMapIID).AsNoTracking().FirstOrDefault();
        ...
        public long UpdateProduct(Product productDetails, MultimediaDTO dtoMultimedia, int companyID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (productDetails.ProductIID <= 0)
                    {
                        ...
                        foreach (var productskuMap in productDetails.ProductSKUMaps)
                        {
                            foreach (var skuTag in productskuMap.ProductSKUTagMaps)
                            {
                                ...
                                {
                                    dbContext.Entry(skuTag.ProductSKUTag).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                                }
                            }
                        }
                    }
                    ...
                    {
                        var dbproduct = dbContext.Products.Where(x => x.ProductIID == productDetails.ProductIID)
                            .Include(i => i.ProductSKUMaps)
                            .Include(i => i.ProductCategoryMaps).ThenInclude(i => i.Category)
                            .Include(i => i.ProductCategoryMaps).ThenInclude(i => i.FeeDueInventoryMaps)
                        ...
                        //        //};
                        //        dbContext.Entry(images).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                        //        dbContext.SaveChanges();
                        //    }
                        //}
                                ...
                                if (dbproduct.ProductSKUMaps != null && dbproduct.ProductSKUMaps.Count > 0)
                                {
                                    foreach (var sku in dbproduct.ProductSKUMaps)
                                    {
                                        ...
                                        if (isSKUExist.Count() > 0)
                                        {
                                            ...
                                            #region saving product image map - old code
                                            ...
                                            #region saving product image map - new code

                                            var existingSkuMap = isSKUExist.FirstOrDefault();

                                            ReplaceProductImageMaps(sku.ProductSKUMapIID, existingSkuMap, productDetails.ProductImageMaps);
                                            ...
                                            dbContext.Entry(productskuMap).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        ...
                        if (!dbproduct.SeoMetadataIID.HasValue)
                        {
                            if (productDetails.SeoMetadata != null)
                            {
                                ...
                                dbContext.Entry(dbproduct.SeoMetadata).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                            }
                        }
                        else
                        {
                            //dbContext.Entry(dbproduct.SeoMetadata).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        ...
                        if (dbproduct.SeoMetadata != null)
                        {
                            foreach (var cultureData in dbproduct.SeoMetadata.SeoMetadataCultureDatas)
                            {
                                if (cultureData.SEOMetadataID == 0)
                                    dbContext.Entry(cultureData).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                                else
                        ...
                        if (dbproduct.ProductIID == 0)
                        {
                            dbContext.Entry(dbproduct).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                        }
                        else
                        {
                            dbContext.Entry(dbproduct).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        }
                        //dbContext.SaveChanges();
                    }
        ...
        public List<Brand> GetBrandList()
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                brands = (from brand in dbContext.Brands
                          select brand).OrderBy(a => a.BrandName)
                          .AsNoTracking()
                          .ToList();
            }
        ...
        public List<Brand> GetBrandList(string searchText, int pageSize)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                brands = dbContext.Brands.Where(x => x.BrandName != null && x.BrandName.Contains(searchText)).Take(pageSize)
                    .AsNoTracking()
                    .ToList();
            }
        ...
        public List<ProductFamily> GetProductFamilies(string searchText, int pageSize)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                productFamilies = dbContext.ProductFamilys.Where(c => c.FamilyName != null && c.FamilyName.Contains(searchText)).Take(pageSize)
                    .AsNoTracking()
                    .ToList();
            }
            return productFamilies;
        ...
        public ProductFamily GetProductFamily(long familyID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var family = dbContext.ProductFamilys.Where(x => x.ProductFamilyIID == familyID)
                            .Include(i => i.ProductFamilyPropertyMaps)
                            .Include(i => i.ProductFamilyPropertyMaps).ThenInclude(i => i.Property)
                            .Include(i => i.ProductFamilyPropertyTypeMaps).ThenInclude(i => i.PropertyType)
                            .AsNoTracking()
                            .FirstOrDefault();
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    updatedEntity = dbContext.ProductFamilys
                         .Include(i => i.ProductFamilyPropertyMaps)
                         .Include(i => i.ProductFamilyPropertyMaps).ThenInclude(i => i.Property).Where(x => x.ProductFamilyIID == entity.ProductFamilyIID)
                         .Include(i => i.ProductFamilyPropertyTypeMaps).ThenInclude(i => i.PropertyType).Where(x => x.ProductFamilyIID == entity.ProductFamilyIID)
                         .AsNoTracking()
                         .FirstOrDefault();
                }
            }
        ...
        public List<Entity.Models.PropertyType> GetPropertyTypes()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.PropertyTypes.OrderBy(a => a.PropertyTypeName)
                    .AsNoTracking()
                    .ToList();
            }
        }
        ...
        public List<Property> GetProperties(string searchText, int pageSize)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (searchText.IsNotNullOrEmpty())
                    properties = dbContext.Properties.Where(p => p.PropertyName != null && p.PropertyName.Contains(searchText)).OrderBy(a => a.PropertyName).Take(pageSize)
                        .AsNoTracking()
                        .ToList();
                else
                    properties = dbContext.Properties.Where(p => p.PropertyName != null).OrderBy(p => p.PropertyName).Take(pageSize)
                        .AsNoTracking()
                        .ToList();
            }
        ...
        public Property GetProperty(long? propertyID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Properties.Where(a => a.PropertyIID == propertyID)
                    .AsNoTracking()
                    .FirstOrDefault();
            }
        }
        ...
        public Property SaveProperty(Property entity)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    dbContext.SaveChanges();
                    updatedEntity = dbContext.Properties.Where(x => x.PropertyIID == entity.PropertyIID)
                        .AsNoTracking()
                        .FirstOrDefault();
                }
            }
        ...
        public List<ProductFamilyType> GetProductFamilyTypes()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ProductFamilyTypes.OrderBy(a => a.FamilyTypeName).AsNoTracking().ToList();
            }
        }
        ...
        public List<ProductStatu> GetProductStatus()
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                prductStatusList = (from status in dbContext.ProductStatus
                                    select status).AsNoTracking().ToList();
            }
            return prductStatusList;
        }

        public List<Entity.Models.PropertyType> GetProductPropertyTypes(decimal productFamilyIID)
        {
            var propertyTypes = new List<Entity.Models.PropertyType>();
        ...
        public List<Property> GetPropertiesByPropertyTypeID(decimal propertyTypeIID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                properties = (from ptm in dbContext.ProductPropertyMaps
        ...
        public Entity.Models.PropertyType GetPropertyTypeByPropertyTypeID(decimal propertyTypeID)
        {
            var propertytypes = new Entity.Models.PropertyType();
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
        ...
        public Product GetProduct(long productIID)
        {
            ...
            try
            {
                productDetails = (from product in dbContext.Products
                                  .Include(i => i.ProductSKUMaps).ThenInclude(i => i.ProductImageMaps)
                                  .Include(i => i.Brand)
                                  .Include(i => i.Unit)
                                  .Include(i => i.Unit1)
                                  .Include(i => i.Unit2)
                                  .Include(i => i.UnitGroup)
                                  .Include(i => i.UnitGroup1)
                                  .Include(i => i.UnitGroup2)
                                  .Include(i => i.GLAccount)
                                  .Include(i => i.ProductFamily)
                                  .Include(i => i.ProductCategoryMaps).ThenInclude(i => i.Category)
                                  .Include(i => i.ProductAllergyMaps).ThenInclude(i => i.Allergy)
                                  .Include(i => i.ProductSKURackMaps).ThenInclude(i => i.Rack)
        ...
        public List<Property> GetProperitesByProductFamilyID(long productFamilyID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                properties = (from property in dbContext.Properties.AsEnumerable()
        ...
        public bool ReLocateUploadImages(string sourcePath, string designationPath, long productID, string mode)
        {
            ...
            if (imageNames != null && imageNames.Count() > 0)
            {
                ...
                {
                    largeImagePath = string.Format("{0}\\{1}\\{2}", designationPath, productID, Eduegate.Framework.Helper.Enums.ImageType.LargeImage.ToString());
                }
                else // For Video
                {
        ...
        public List<Product> GetProductListBySearchText(string searchText, long excludeProductFamilyID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    productList = (from product in (dbContext.Products.Select(x => new { x.ProductIID, x.ProductName, x.ProductFamilyID }).Where(a => a.ProductName.Contains(searchText) && a.ProductFamilyID != excludeProductFamilyID).Take(50).AsNoTracking().ToList())
                                   select new Product { ProductIID = product.ProductIID, ProductName = product.ProductName }).ToList();
                }

                else
                {
                    productList = (from product in (dbContext.Products.Select(x => new { x.ProductIID, x.ProductName }).Where(a => a.ProductName.Contains(searchText)).Take(50).AsNoTracking().ToList())
                                   select new Product { ProductIID = product.ProductIID, ProductName = product.ProductName }).ToList();
                }
                    ...
                    foreach (var productMap in productMaps)
                    {
                        if (productMap.ProductIDTo.IsNotNull())
                        {
                            ...
                            else
                            {
                                ...
                                && x.ProductIDTo == productMap.ProductIDTo && x.SalesRelationTypeID == productMap.SalesRelationTypeID).AsNoTracking().FirstOrDefault();
        ...
        public string GetProductNameByID(long productID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                string productName = dbContext.Products.Where(x => x.ProductIID == productID).AsNoTracking().Select(y => y.ProductName).FirstOrDefault();
                return productName;
            }
        }
        ...
        public string GetProductAndSKUNameByID(long productSKUMapIID)
        {
            ...
            if (productSKUMapIID > 0)
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    ProductSKUDetail skuDetail = dbContext.ProductSKUDetails.FromSqlRaw($@"{searchQuery}").AsNoTracking().FirstOrDefault();
        ...
        public List<ProductToProductMap> GetProductMaps(long productID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                List<ProductToProductMap> dbProductMaps = dbContext.ProductToProductMaps.Where(x => x.ProductID == productID).AsNoTracking().ToList();
        ...
        public TransactionDTO GetTransaction(long headIID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                TransactionHead transactionModel = (from transaction in dbContext.TransactionHeads
                                                    ...
                                                    select transaction).AsNoTracking().FirstOrDefault();
                ...
                if (transactionModel != null)
                {
                    ...
                    if (transactionModel.TransactionDetails != null && transactionModel.TransactionDetails.Count > 0)
                    {
                        foreach (var transactionDetail in transactionModel.TransactionDetails)
                        {
                            ...
                            ProductSKUDetail product = dbContext.ProductSKUDetails.FromSqlRaw($@"{searchQuery}").FirstOrDefault();
        ...
        public bool RemoveProductPriceListSKUMaps(long id)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ProductPriceListSKUMap item = db.ProductPriceListSKUMaps.Where(x => x.ProductPriceListItemMapIID == id).AsNoTracking().FirstOrDefault();
        ...
        public ProductPriceList GetProductPriceListDetail(long id, int companyID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ProductPriceList list = db.ProductPriceLists.Where(x => x.ProductPriceListIID == id && x.CompanyID == companyID)
                    ...
                    .Include(x => x.ProductPriceListBranchMaps).ThenInclude(x => x.Branch)
                    //.Include(y => y.ProductPriceListBranchMaps
                    //.Select(y => y.Branch))
                    .AsNoTracking()
                    .FirstOrDefault();
                return list;
        ...
        public List<ProductPriceSKU> GetProductPriceSKU(long id)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                entityList = db.ProductPriceSKUs.FromSqlRaw($@"{searchQuery}").AsNoTracking().ToList();
            }
            return entityList;
        }
        ...
        public List<SalesRelationshipType> GetSalesRelationshipType(Eduegate.Framework.Enums.SalesRelationshipType salesRelationshipType)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                {
                    return db.SalesRelationshipTypes.AsNoTracking().ToList();
                }
                else
                {
                    return db.SalesRelationshipTypes.Where(x => x.SalesRelationTypeID == (int)salesRelationshipType).AsNoTracking().ToList();
                }
            }
        }
        ...
        public ProductInventoryConfig SaveProductInventoryConfig(ProductInventoryConfig entity)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                entityDB = db.ProductInventoryConfigs.Where(x => x.ProductInventoryConfigIID == entity.ProductInventoryConfigIID).AsNoTracking().FirstOrDefault();
                if (entityDB.IsNotNull())
                {
                    ...
                    foreach (var cultureData in entity.ProductInventoryConfigCultureDatas)
                    {
                        ...
                        db.Entry(configUpdated).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    }
        ...
        public ProductInventoryProductConfigMap SaveProductInventoryProductConfigMap(ProductInventoryProductConfigMap entity)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                && x.ProductID == entity.ProductID).AsNoTracking().FirstOrDefault();
                if (entityDB.IsNotNull())
                {
                    // Update
        ...
        public ProductInventorySKUConfigMap SaveProductInventorySKUConfigMap(ProductInventorySKUConfigMap entity)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                && x.ProductSKUMapID == entity.ProductSKUMapID).AsNoTracking().FirstOrDefault();
                if (entityDB.IsNotNull())
                {
                    // Update
        ...
        private static void UpdateProductBundles(Product productDetails, Product product, dbEduegateERPContext dbContext, ICollection<ProductSKUMap> ProductSKUMaps)
        {
            ...
            //Add new bundles
            if (productDetails.ProductBundleFromProducts != null)
            {
                if (productDetails.ProductBundleFromProducts.Count > 0)
                {
                    productDetails.ProductBundleFromProducts = product.ProductBundleFromProducts;
                    foreach (ProductBundle bundle in productDetails.ProductBundleFromProducts)
                    {
                        product.ProductBundleFromProducts.Add(bundle);
                        dbContext.SaveChanges();
                    }
                }
            }
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    if (entityList.IsNotNull() && entityList.Count > 0)
                    {
                        ...
                        dbContext.SaveChanges();
                        dbEntityList = dbContext.Properties.Where(x => x.PropertyTypeID == (byte)PropertyTypes.ProductTag).AsNoTracking().ToList();
                    }
                }
            }
            catch (Exception exception)
        ...
        public ProductSKUMap GetProductSKUMap(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skuID).AsNoTracking().FirstOrDefault();

                //var entity = db.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skuID)
                //           .Include(i => i.ProductInventorySKUConfigMaps")
                //           .Include(i => i.ProductInventorySKUConfigMaps.ProductInventoryConfig")
                //           .FirstOrDefault();
                //return entity;
        ...
        public List<ProductSKUMap> GetProductSKUByCategory(long category, int takeCount, string tag)
        {
            using (var db = new dbEduegateERPContext())
            {
                return (from sku in db.ProductSKUMaps
                        ...
                        select sku).Take(takeCount).AsNoTracking().ToList();
            }
        }
            ...
            using (var db = new dbEduegateERPContext())
            {
                ...
                else
                {
                    var skus = (from sku in db.ProductSKUMaps
                                ...
                                && EF.Functions.Like(skuTagLJ.TagName, tag)
        ...
        public ProductDeliveryCountrySetting RemoveProductDeliveryCountrySetting(ProductDeliveryCountrySetting productDeliveryCountrySetting)
        {
            if (productDeliveryCountrySetting.IsNotNull())
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ProductDeliveryCountrySetting dbEntity = dbContext.ProductDeliveryCountrySettings.Where(x => x.ProductDeliveryCountrySettingsIID == productDeliveryCountrySetting.ProductDeliveryCountrySettingsIID).AsNoTracking().FirstOrDefault();
        ...
        public Entity.Models.PropertyType SavePropertyType(Entity.Models.PropertyType entity)
        {
            Entity.Models.PropertyType updatedEntity = null;
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    var ProductPropertyMaps = dbContext.ProductPropertyMaps.Where(p => p.PropertyTypeID == entity.PropertyTypeID);
                    dbContext.ProductPropertyMaps.RemoveRange(ProductPropertyMaps);
                ...
                foreach (ProductPropertyMap ptpm in entity.ProductPropertyMaps)
                    dbContext.Entry(ptpm).State = Microsoft.EntityFrameworkCore.EntityState.Added;


                dbContext.SaveChanges();
                updatedEntity = dbContext.PropertyTypes.Where(x => x.PropertyTypeID == entity.PropertyTypeID)
                                .Include(i => i.ProductPropertyMaps)
                                .AsNoTracking()
                                .FirstOrDefault();
                updatedEntity.Properties = (from prop in dbContext.Properties
                                            join ptpm in dbContext.ProductPropertyMaps on prop.PropertyIID equals ptpm.PropertyID
                                            where ptpm.PropertyTypeID.Equals(updatedEntity.PropertyTypeID)
                                            select prop).AsNoTracking().ToList();
                return updatedEntity;
            }
        }

        public Entity.Models.PropertyType GetPropertyType(byte propertyTypeID)
        {
            Entity.Models.PropertyType entity = null;

            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                entity = dbContext.PropertyTypes.Where(x => x.PropertyTypeID == propertyTypeID)
                    .Include(i => i.ProductPropertyMaps)
                    .AsNoTracking()
                    .FirstOrDefault();
                //dbContext.Entry(entity).Collection(a => a.ProductPropertyMaps).Load();
                return entity;
            }
        }
        ...
        public List<Property> GetPropertiesByPropertyTypeID(byte PropertyTypeID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                Properties = (from ptpm in dbContext.ProductPropertyMaps
        ...
        public decimal GetProductInventoryByBranch(long skuID, long BranchID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var quantity = db.ProductInventories.Where(a => a.ProductSKUMapID == skuID && a.BranchID == BranchID).AsNoTracking().Sum(a => a.Quantity);
                if (quantity != null)
        ...
        public List<ProductImageMap> GetProductImages(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var productImages = db.ProductImageMaps.Where(a => a.ProductSKUMapID == skuID && a.Sequence == 1 && (a.ProductImageTypeID == 1 || a.ProductImageTypeID == 8)).AsNoTracking().ToList();
        ...
        public ProductImageMap GetProductImage(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var productImages = db.ProductImageMaps.Where(a => a.ProductSKUMapID == skuID && a.Sequence == 1 && (a.ProductImageTypeID == 1)).AsNoTracking().FirstOrDefault();
                return productImages;
            }
        }
        ...
        public List<ProductImageMap> GetAllProductImages(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var productImages = db.ProductImageMaps.Where(a => a.ProductSKUMapID == skuID && (a.ProductImageTypeID == 1 || a.ProductImageTypeID == 8)).OrderBy(x => x.Sequence).AsNoTracking().ToList();
                return productImages;
            }
        }
        ...
        public List<ProductSerialMap> GetProductSerialMaps(long transactionDetailID, long productSKUMapID, int recordLimit)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ProductSerialMaps.Where(p => p.ProductSKUMapID == productSKUMapID && p.DetailID == transactionDetailID).Take(recordLimit).AsNoTracking().ToList();
            }
        }
        ...
        public List<ProductSerialMap> GetProductSerialMaps(string serialNumber, long productSerialIID = 0)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (productSerialIID > 0)
                    serials = db.ProductSerialMaps.Where(p => p.SerialNo == serialNumber && p.ProductSerialIID == productSerialIID).AsNoTracking().ToList();
                else
                    serials = db.ProductSerialMaps.Where(p => p.SerialNo == serialNumber).AsNoTracking().ToList();
        ...
        public bool UpdateProductInventorySerialMaps(List<ProductInventorySerialMap> entities, bool isUpdate = false)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (isUpdate)
                {
                    entities.ForEach(p => dbContext.Entry(p).State = Microsoft.EntityFrameworkCore.EntityState.Modified);
                }
                else
                {
                    dbContext.ProductInventorySerialMaps.AddRange(entities);
                    //dbContext.Entry(entities).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                }
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (!isProductinDeal(InventoryBranch, ProductSKUMapID))
                {
                    ...
                    if (priceListID.IsNotDefault() && priceListID.IsNotNull())
                    {
                        ...
                        var skuDiscountPriceList = db.ProductPriceListSKUMaps.Where(a => a.ProductSKUID == ProductSKUMapID && a.ProductPriceListID == priceListID).Select(a => new { a.Discount, a.Price }).AsNoTracking().FirstOrDefault();
                        ...
                        {
                            if (db.ProductPriceListCustomerGroupMaps.Where(a => a.ProductSKUMapID == ProductSKUMapID && a.ProductPriceListID == priceListID && a.DiscountPercentage != null && a.DiscountPercentage > 0).AsNoTracking().Any())
                            {
                                ...
                                {
                                    productDiscountPrice = db.ProductPriceListCustomerGroupMaps.Where(a => a.ProductSKUMapID == ProductSKUMapID && a.ProductPriceListID == priceListID && a.CustomerGroupID == groupID).AsNoTracking().Select(a => productDiscountPrice - (a.DiscountPercentage * productDiscountPrice / 100)).FirstOrDefault();
                                }
        ...
        public List<Eduegate.Domain.Entity.Models.ValueObjects.ProductQuantityDiscount> GetProductQtyDiscountDetails(long InventoryBranch, long ProductSKUMapID, long CustomerID = 0)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (!isProductinDeal(InventoryBranch, ProductSKUMapID))
                {
                    var priceListID = db.ProductPriceListBranchMaps.Where(a => a.BranchID == InventoryBranch).AsNoTracking().Select(a => a.ProductPriceListID).FirstOrDefault();
                    if (priceListID.IsNotDefault() && priceListID.IsNotNull())
                    {
                        decimal? productDiscountPrice = 0;
                        var skuDiscountPriceList = db.ProductPriceListSKUMaps.Where(a => a.ProductSKUID == ProductSKUMapID && a.ProductPriceListID == priceListID).AsNoTracking().Select(a => new { a.Discount, a.Price }).FirstOrDefault();
                        if (skuDiscountPriceList != null)
                        {
                            productDiscountPrice = skuDiscountPriceList.Discount.HasValue ? skuDiscountPriceList.Discount : skuDiscountPriceList.Price;
                        }
                        if (productDiscountPrice.IsNotDefault() && productDiscountPrice.IsNotNull())
                        {
                            ...
                            {
                                var groupID = db.Customers.Where(a => a.CustomerIID == CustomerID).AsNoTracking().Select(a => a.GroupID).FirstOrDefault();
                                if (groupID.IsNotDefault() && groupID.IsNotNull())
                                {
                                    var customerGrpproductDiscountPrice = db.ProductPriceListCustomerGroupMaps.Where(a => a.ProductSKUMapID == ProductSKUMapID && a.ProductPriceListID == priceListID && a.CustomerGroupID == groupID).AsNoTracking().Select(a => productDiscountPrice - (a.DiscountPercentage * productDiscountPrice / 100)).FirstOrDefault();
        ...
        public string SaveProductSkuBarCode(long skuId, string barCode)
        {
            using (var db = new dbEduegateERPContext())
            {
                var dbEntity = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID == skuId).AsNoTracking().FirstOrDefault();
                if (dbEntity != null)
                {
        ...
        public List<string> GetProductSKUTags(long skuId, int companyID)
        {
            ...
            using (var db = new dbEduegateERPContext())
            {
                var skuTagsList = (from m in db.ProductSKUTagMaps
                                   ...
                                   select t.TagName).AsNoTracking().ToList();
        ...
        public bool SaveProductSKUTags(List<long> selectedTagIds, List<long> skuIDs, List<string> selectedTagNames, long loginID = 0, int companyID = 0)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                {
                    var allProductSkuTags = db.ProductSKUTagMaps.Where(x => x.ProductSKuMapID == skuIDs.FirstOrDefault()).AsNoTracking().ToList();
                    ...
                    var SkuMapsToUpdate = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID == skuIDs.FirstOrDefault()).AsNoTracking().FirstOrDefault();
                    SkuMapsToUpdate.UpdatedDate = DateTime.Now;
                    db.Entry(SkuMapsToUpdate).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
        ...
        public ProductSKUMap GetProductSkuDetails(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var entity = db.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skuID)
                           .Include(i => i.ProductInventorySKUConfigMaps)
                           .Include(i => i.ProductInventorySKUConfigMaps).ThenInclude(i => i.ProductInventoryConfig)
                           .Include(i => i.ProductInventorySKUConfigMaps).ThenInclude(i => i.ProductInventoryConfig).ThenInclude(i => i.ProductInventoryConfigCultureDatas)
                           .AsNoTracking()
                           .FirstOrDefault();
                return entity;
            }
        ...
        public string GetProductSkuCode(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skuID).AsNoTracking().Select(x => x.ProductSKUCode).FirstOrDefault();
            }
        }
        ...
        public ProductSKUMap SaveProductSkuDetails(ProductSKUMap skuMap)
        {
            try
            {
                using (var db = new dbEduegateERPContext())
                {
                    ...
                    {
                        db.Entry(skuConfig.ProductInventoryConfig).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                        db.Entry(skuConfig).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    }
                    db.Entry(skuMap).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    db.SaveChanges();
                    return skuMap;
                }
            }
        ...
        public bool UpdateProductSKUStatus(byte statusID, long brandID, CallContext context)
        {
            ...
            {
                bool result = false;
                var pdt = dbContext.Products.Where(x => x.BrandID == brandID).AsNoTracking().ToList();
                if (pdt.IsNotNull())
                {
        ...
        public ProductSKUMap SaveSKUProductMap(ProductSKUMap skudetails)
        {
            ...
            {
                bool result = false;
                ProductSKUMap SKU = dbContext.ProductSKUMaps.Where(x => x.ProductSKUMapIID == skudetails.ProductSKUMapIID).AsNoTracking().FirstOrDefault();
                if (SKU.IsNotNull())
                {
                ...
                if (result)
                    return dbContext.ProductSKUMaps.Where(x => x.ProductSKUMapIID == skudetails.ProductSKUMapIID).AsNoTracking().FirstOrDefault();
                else return null;
            }
        }
        ...
        public List<ProductImageMap> GetProductImagesByProductID(long productID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.ProductImageMaps.Where(a => a.ProductID == productID).AsNoTracking().ToList();
            }
        }
        ...
        public bool SaveSKUManagers(long SelectedKeyValueOwnerId, long ID, CallContext _callContext)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                var allProductSkuTags = db.ProductInventorySKUConfigMaps.Where(x => x.ProductSKUMapID == ID).AsNoTracking().FirstOrDefault();
                var skuTag = db.ProductInventoryConfigs.Where(x => x.ProductInventoryConfigIID == allProductSkuTags.ProductInventoryConfigID).AsNoTracking().FirstOrDefault();
        ...
        public ProductSKUTag GetSKUTag(long tagId)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.ProductSKUTags.Where(x => x.ProductSKUTagIID == tagId).AsNoTracking().FirstOrDefault();
            }
        }
        ...
        public List<ProductSKUTagMap> GetSKUTagMaps(long tagId)
        {
            ...
            using (var db = new dbEduegateERPContext())
            {
                var tagsList = (from m in db.ProductSKUTagMaps
                                ...
                                select m).Include(x => x.ProductSKUTag).AsNoTracking().ToList();
        ...
        public bool IsPartNumberDuplicated(string partNumber, long productSKUMapID)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                {
                    product = db.ProductSKUMaps.Where(x => x.PartNo == partNumber).AsNoTracking().ToList();
                }
                else
                {
                    product = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID != productSKUMapID && x.PartNo == partNumber).AsNoTracking().ToList();
                }
        ...
        public bool IsBarCodeDuplicated(string barCode, long productSKUMapID)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                if (productSKUMapID == 0)
                {
                    product = db.ProductSKUMaps.Where(x => x.BarCode == barCode).AsNoTracking().ToList();
                }
                else
                {
                    product = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID != productSKUMapID && x.BarCode == barCode).AsNoTracking().ToList();
                }
        ...
        public bool IsProductSKUNameDuplicated(string SKUName, long productSKUMapID)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                if (productSKUMapID == 0)
                {
                    product = db.ProductSKUMaps.Where(x => x.SKUName == SKUName).AsNoTracking().ToList();
                }
                else
                {
                    product = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID != productSKUMapID && x.SKUName == SKUName).AsNoTracking().ToList();
                }
        ...
        public bool IsProductCodeDuplicated(string SKU, long productSKUMapID)
        {
            using (var db = new dbEduegateERPContext())
            {
                ...
                if (productSKUMapID == 0)
                {
                    product = db.ProductSKUMaps.Where(x => x.ProductSKUCode == SKU).AsNoTracking().ToList();
                }
                else
                {
                    product = db.ProductSKUMaps.Where(x => x.ProductSKUMapIID != productSKUMapID && x.ProductSKUCode == SKU).AsNoTracking().ToList();
                }
        ...
        public decimal? GetProductPriceBySKUID(long productSKUMapID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return (from p in db.Products
                        join sku in db.ProductSKUMaps on p.ProductIID equals sku.ProductID
                        join price in db.ProductPriceListSKUMaps on sku.ProductSKUMapIID equals price.ProductSKUID
                        where (sku.ProductSKUMapIID == productSKUMapID && price.ProductPriceListID == 2)
                        select price.Amount).SingleOrDefault();
            }
        }

        public long GetProductIDfromSKU(long skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                return (long)db.ProductSKUMaps
                    .Where(a => a.ProductSKUMapIID == skuID)
                    .Select(b => b.ProductID).FirstOrDefault();
            }
        }

        private void ReplaceProductImageMaps(long skuMapId, ProductSKUMap source, ICollection<ProductImageMap> imageMaps)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var oldImageMaps = dbContext.ProductImageMaps
                .Where(x => x.ProductSKUMapID == skuMapId)
                .ToList();

                dbContext.ProductImageMaps.RemoveRange(oldImageMaps);

                if (imageMaps != null)
                {
                    foreach (var image in imageMaps)
                    {
                        dbContext.ProductImageMaps.Add(new ProductImageMap
                        {
                            ProductID = image.ProductID,
                            ProductSKUMapID = image.ProductSKUMapID,
                            ProductImageTypeID = image.ProductImageTypeID,
                            ImageFile = image.ImageFile,
                            Sequence = image.Sequence,
                            ImageFileReference = image.ImageFileReference,
