    ...
    public class SecurityRepository
    {
        public List<string> GetUserClaimKey(long userID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                foreach (var claim in claims.AsNoTracking().ToList())
                {
        ...
        public List<Eduegate.Domain.Entity.Models.Claim> GetUserClaims(long userID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                return claims.AsNoTracking().ToList();
            }
        }
        ...
        public List<Eduegate.Domain.Entity.Models.Claim> GetUserClaims(long userID, int claimTypeID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (userID == 0)
                {
                    return dbContext.Claims.Where(a => a.ClaimTypeID == claimTypeID).AsNoTracking().ToList();
                }
                else
                {
                    var claims = ((from map in dbContext.ClaimSetLoginMaps
        ...
        public List<Eduegate.Domain.Entity.Models.Claim> GetDashBoardByUserID(long userID, int claimTypeID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                userClaims = dbContext.Claims.Where(x => x.ClaimTypeID == claimTypeID && x.ClaimLoginMaps.Any(l => l.LoginID == userID)).AsNoTracking().ToList();

                if (userClaims.Count <= 0)
                {
                    var claimData = dbContext.ClaimSetLoginMaps.Where(x => x.LoginID == userID)
                        .Include(x => x.ClaimSet)
                        .Include(x => x.ClaimSet.ClaimSetClaimMaps)
                        .ThenInclude(y => y.Claim)
                        .AsNoTracking()
                        .ToList()
                        .LastOrDefault();

                    var claimID = claimData?.ClaimSet?.ClaimSetClaimMaps?.Where(y => y.Claim.ClaimTypeID == claimTypeID)?.Select(z => z.ClaimID)?.FirstOrDefault();

                    userClaims = dbContext.Claims.Where(c => c.ClaimTypeID == claimTypeID && c.ClaimIID == claimID && c.ClaimTypeID == claimTypeID)
                        .AsNoTracking()
                        .ToList();
        ...
        public List<Eduegate.Domain.Entity.Models.ValueObjects.ClaimDetails> GetClaimsByClaimType(int claimTypeID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var claims = (from claim in dbContext.Claims
                              ...
                              }).Where(a=> a.ClaimTypeID== claimTypeID).AsNoTracking().ToList();
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var claims = (from claim in dbContext.Claims
                              ...
                              select new ClaimDetails()
                              {
                                  ...
                                  ClaimTypeName = clType.ClaimTypeName,
                                  ResourceName = claim.ResourceName
                              }).OrderBy(a => a.ClaimName).ThenBy(a=> a.ClaimTypeName).AsNoTracking().ToList();
        ...
        public Eduegate.Domain.Entity.Models.Claim GetClaim(long claimID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return (from claim in dbContext.Claims
                              join clType in dbContext.ClaimTypes
                              on claim.ClaimTypeID equals clType.ClaimTypeID
                              select claim).Where(a=> a.ClaimIID == claimID)
                              .AsNoTracking()
                              .FirstOrDefault();
            }
        }
        ...
        public ClaimSet GetClaimSet(long claimSetID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var claimSet = dbContext.ClaimSets.Where(a=> a.ClaimSetIID == claimSetID)
                    .Include(i => i.ClaimSetClaimMaps).ThenInclude(i => i.Claim).ThenInclude(i => i.ClaimType)
                    .Include(i => i.ClaimSetClaimSetMapClaimSets)
                    .AsNoTracking()
                    .FirstOrDefault();
                ...
                //foreach (var claim in claimSet.ClaimSetClaimSetMapClaimSets)
                //{
                //    dbContext.Entry(claim).Reference(a => a.ClaimSet).Load();
                //}
        ...
        public List<ClaimSet> GetClaimSets()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ClaimSets.OrderBy(a=> a.ClaimSetName).AsNoTracking().ToList();    
            }
        }
        ...
        public List<ClaimType> GetClaimTypes()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.ClaimTypes.OrderBy(a=> a.ClaimTypeName).AsNoTracking().ToList();
            }
        }
            ...
            try
            {
                if (entity.IsNotNull())
                {
                    using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                    {
                        ...
                        updatedEntity = dbContext.Claims.Where(x => x.ClaimIID == entity.ClaimIID).AsNoTracking().FirstOrDefault();                        
                    }
                }
            }
        ...
        public void CreateOrUpdateClaimsByResource(string resource, string name, int type)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    {
                        long claimID;
                        var claim = dbContext.Claims.Where(a => a.ResourceName == resource && a.ClaimTypeID == type).AsNoTracking().FirstOrDefault();
                ...
                if (entity.IsNotNull())
                {
                    using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                    {
                        ...
                        else
                        {
                            ...
                            var deletedClaimMaps = dbContext.ClaimSetClaimMaps.Where(x => x.ClaimSetID == entity.ClaimSetIID && !claimIIDs.Contains(x.ClaimID))
                                .AsNoTracking()
                                .ToList();
                            ...
                            foreach (var claimMap in entity.ClaimSetClaimMaps)
                            {
                                if (claimMap.ClaimSetClaimMapIID == 0)
                                    dbContext.Entry(claimMap).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                                else
                                    dbContext.Entry(claimMap).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                            }

                            var claimSetIDs = entity.ClaimSetClaimSetMapClaimSets.Select(a => a.ClaimSetID).ToList();
                            ...
                            foreach (var calimSetMap in entity.ClaimSetClaimSetMapClaimSets)
                            {
                                if (calimSetMap.ClaimSetClaimSetMapIID == 0)
                                    dbContext.Entry(calimSetMap).State = Microsoft.EntityFrameworkCore.EntityState.Added;
                            }
        ...
        public long GetClaimSetClaimMapIID(long claimSetID, long claimID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var claimSetMaps = dbContext.ClaimSetClaimMaps.Where(x => x.ClaimSetID == claimSetID && x.ClaimID == claimID)
                        .AsNoTracking()
                        .FirstOrDefault();
        ...
        public string GetClaimName(long claimIID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    claimName = dbContext.Claims.Where(x => x.ClaimIID == claimIID).Select(x => x.ClaimName)
                        .AsNoTracking()
                        .FirstOrDefault()
                        .ToString();
                }
        ...
        public long GetClaimSetClaimSetMapIID(long claimSetID, long linkedClaimSetID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var claimSetMaps = dbContext.ClaimSetClaimSetMaps.Where(x => x.ClaimSetID == claimSetID && x.LinkedClaimSetID == linkedClaimSetID)
                        .AsNoTracking()
                        .FirstOrDefault();
                    if (claimSetMaps != null)
                    {
        ...
        public string GetClaimSetName(long claimSetIID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    claimSetName = dbContext.ClaimSets.Where(x => x.ClaimSetIID == claimSetIID).Select(x => x.ClaimSetName)
                        .AsNoTracking()
                        .FirstOrDefault()
                        .ToString();
                }
        ...
        public bool HasClaimAccess(long claimID, long userID)
        {
            ...
            {
                if (db.ClaimLoginMaps.Where(a => a.LoginID == userID && a.ClaimID == claimID)
                    .AsNoTracking()
                    .Any())
                {
                    return true;
        ...
        public bool HasClaimSetAccessByResource(string resourceID, long userID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (db.ClaimLoginMaps.Where(a => a.LoginID == userID && a.Claim.ResourceName == resourceID)
                    .AsNoTracking()
                    .Any())
                {
                    return true;
        ...
        public bool HasClaimSetAccess(long claimSetID,long userID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                if (db.ClaimSetLoginMaps.Where(a => a.LoginID == userID && a.ClaimSetID == claimSetID).AsNoTracking().Any())
                {
                    return true;
                }
                else
        ...
        public async Task<List<LoginRoleMap>> GetUserRoles(long loginID)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return await dbContext.LoginRoleMaps
                     .AsNoTracking()
                    .Include(x => x.Login)
                     .AsNoTracking()
                    .Include(x => x.Role)
                     .AsNoTracking()
                    .Where(x => x.LoginID == loginID).ToListAsync();
                }
            }
            catch (Exception exception)
            {
        ...
        public Login GetLogin(long loginID)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    var login = dbContext.Logins.Where(x => x.LoginIID == loginID)
                        .Include(i => i.LoginRoleMaps)
                        .Include(i => i.ClaimLoginMaps)
                        .Include(i => i.ClaimSetLoginMaps)
                        .AsNoTracking()
                        .FirstOrDefault();
        ...
        public void ResetForceLogout(long loginID)
        {
            ...
            {
                var login = dbContext.Logins.Where(x => x.LoginIID == loginID).AsNoTracking().FirstOrDefault();
                login.EnableForceLogout = false;
                dbContext.SaveChanges();
            }
        }
