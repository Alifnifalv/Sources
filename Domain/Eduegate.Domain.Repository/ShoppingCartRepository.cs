...
namespace Eduegate.Domain.Repository
{
    public class ShoppingCartRepository
    {
        ...
        public Category GetCartDetailsWithItems(string customerID, int shoppingCartStatusID, int? siteID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var category = (from sci in db.ShoppingCartItems
                ...
                return GetOneLevelBelowTopCategory(category, db);
            }
        }


        public List<Category> GetAllCategoriesDetailsWithItems(string customerID, int shoppingCartStatusID, int? siteID)
        {
            using (var db = new dbEduegateERPContext())
            {
        ...
        public Category GetAddedCartDetailsWithItems(long? skuID)
        {
            using (var db = new dbEduegateERPContext())
            {
                var category = (from pskm in db.ProductSKUMaps
                                join pcm in db.ProductCategoryMaps on pskm.ProductID equals pcm.ProductID
                                join cat in db.Categories on pcm.CategoryID equals cat.CategoryIID
        ...
        private Category GetOneLevelBelowTopCategory(Category category, dbEduegateERPContext db)
        {
            // Track the previous (child) category as we move up the chain
            Category child = null;
            ...
            while (category?.ParentCategoryID != null)
            {
                ...
                category = db.Categories
                             .AsNoTracking()
                             .FirstOrDefault(c => c.CategoryIID == category.ParentCategoryID);
            }

            // Return the child that was right below the root
            return child ?? category; // if already at top, return itself
        }
