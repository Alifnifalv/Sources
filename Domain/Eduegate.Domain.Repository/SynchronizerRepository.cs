...
namespace Eduegate.Domain.Repository
{
    public class SynchronizerRepository
    {
        public List<EntityChangeTracker> GetNextChangeFromQueue(Eduegate.Services.Contracts.Enums.Synchronizer.Entities entity, int numberOfChanges)
        {
            ...
            {
                db.Database.SetCommandTimeout(TimeSpan.Zero);
                List<EntityChangeTrackersQueue> queues = null;
                List<EntityChangeTracker> changeTracker = new List<EntityChangeTracker>();

                queues = db.EntityChangeTrackersQueues.Take(numberOfChanges).AsNoTracking().ToList();
        ...
        public bool UpdateEntityChangeTrackerStatus(long changeTrackerID, Eduegate.Services.Contracts.Enums.Synchronizer.TrackerStatuses statusID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                try
                {
                    var entity = dbContext.EntityChangeTrackersInProcesses.Where(a => a.EntityChangeTrackerID == changeTrackerID).AsNoTracking().FirstOrDefault();
                    dbContext.EntityChangeTrackersInProcesses.Remove(entity);

                    //insert into log
                    var entityTracker = dbContext.EntityChangeTrackers.Where(a => a.EntityChangeTrackerIID == changeTrackerID).AsNoTracking().FirstOrDefault();
        ...
        public EntityChangeTracker SaveEntityChangeTracker(EntityChangeTracker tracker)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                dbContext.SaveChanges();
                return dbContext.EntityChangeTrackers.Where(a => a.EntityChangeTrackerIID == tracker.EntityChangeTrackerIID).AsNoTracking().FirstOrDefault();
            }
        }
        ...
        public EntityChangeTrackersQueue AddEntityChangeTrackersQueue(EntityChangeTrackersQueue queueItem)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                dbContext.SaveChanges();
                return dbContext.EntityChangeTrackersQueues.Where(a => a.EntityChangeTrackerQueueIID == queueItem.EntityChangeTrackerQueueIID).AsNoTracking().FirstOrDefault();
            }
        }
        ...
        public List<SyncFieldMap> GetFieldMaps(FieldMapTypes mapType)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.SyncFieldMaps.Where(a => a.SynchFieldMapTypeID.Value == (int)mapType).AsNoTracking().ToList();
            }
        }
