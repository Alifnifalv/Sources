...
namespace Eduegate.Domain
{
    public class AccountBL
    {
        ...
        public UserDTO GetUserData(UserDTO userDTO)
        {
            if (!string.IsNullOrEmpty(userDTO.LoginUserID))
            {
                ...
                if (isUserExist != null)
                {
                    ...
                    userDTO.LoginUserID = isUserExist.LoginUserID;
                }
                else
                {
                    userDTO.LoginEmailID = string.Empty;
                    userDTO.LoginUserID = null;
                }
        ...
        public Login ToEntity(UserDTO userDTO)
        {
            ...
            var isJustCMS = new Domain.Setting.SettingBL().GetSettingValue<bool>("IsJustCMS");

            if (!isJustCMS)
            {
                var blockedDTO = Context.CompanyID.HasValue ?
                    new Domain.Setting.SettingBL().GetSettingDetail(Eduegate.Framework.Helper.Constants.TransactionSettings.ISNEWUSERBLOCKED,
                        long.Parse(Context.CompanyID.ToString())) : new SettingDTO() { SettingValue = "0" };
        ...
        public bool Login(string userName, string password, byte AppId)
        {
            ...
            if (new Domain.Setting.SettingBL().GetSettingValue<bool>("IsMigratedUserCheck"))
            {
        ...
        public UserDTO FromEntity(Login login, bool isSecurityInfoRequired = true)
        {
            ...
            var parentDetails = GetParentDetailsByLoginID(login.LoginIID);
            var studentDetails = GetStudentDetailsByLoginUserID(login.LoginUserID);
            ...
            var user = new UserDTO()
            {
                ...
                StudentID = studentDetails.StudentIID > 0  ? studentDetails.StudentIID : (long?)null,
            };
        ...
        {
            return ParentMapper.Mapper(Context).GetParentDetailsByLoginID(loginID);
        }
        ...
        public long OnlineStoreAddContactContactID(ContactDTO contactDTO)
        {
            ...
            //var isGeolocationEnabled = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("DELIVERYRESTRICTEDTOGEOLOCATION",
            ...
            //}
            //if (new Domain.Setting.SettingBL(null).GetSettingValue<bool>("AUTOSETBRANCHBYAREA") && contactDTO.AreaID.HasValue)
            //{
        ...
        {
            var hasAccess = Eduegate.Framework.CacheManager.MemCacheManager<bool?>
            ...
            if (!hasAccess.HasValue)
            {
                ...
                Eduegate.Framework.CacheManager.MemCacheManager<bool?>
        ...
        public List<SettingDTO> GetUserSettings(long loginID)
        {
            var entities = accountRepo.GetLoginSettings(loginID);
        ...
        public UserDTO EditUserDetails(string loginEmailID, UserDTO updatedDetails, string mobileNumber = null)
        {
            // Retrieve the user record based on provided loginEmailID or mobileNumber
            Login login = !string.IsNullOrEmpty(loginEmailID) ? accountRepo.GetUserByLoginEmail(loginEmailID) : !string.IsNullOrEmpty(mobileNumber) ? accountRepo.GetUserByMobileNumber(mobileNumber) : null;
            ...
            // Map the updated details to the user entity
            login.LoginEmailID = updatedDetails.LoginEmailID;
            login.UserName = updatedDetails.UserName;
            login.ProfileFile = updatedDetails.ProfileFile;


            // Add other fields to update, such as customer details, branch, etc.
            if (updatedDetails.Customer != null)
            {
                var customer = accountRepo.GetCustomerDetails(login.LoginIID);
                if (customer != null)
                {
                    customer.FirstName = updatedDetails.Customer.FirstName;
                    customer.MiddleName = updatedDetails.Customer.MiddleName;
                    customer.LastName = updatedDetails.Customer.LastName;
                    customer.Telephone = updatedDetails.Customer.TelephoneNumber;
                    // Update other customer fields as needed
                    //accountRepo.SaveLoginDetails(customer); // Save customer updates
                }
            }


            // Save the updated login details back to the repository
            //accountRepo.SaveLoginDetails(login);
        ...
        {
            return StudentMapper.Mapper(Context).GetStudentDetailsByLoginID(loginID);
        }

        public StudentDTO GetStudentDetailsByLoginUserID(string loginUserID)
        {
            return StudentMapper.Mapper(Context).GetStudentDetailsByLoginUserID(loginUserID);
        }
