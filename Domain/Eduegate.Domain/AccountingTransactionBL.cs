...
namespace Eduegate.Domain
{
    public class AccountingTransactionBL
    {
        ...
        public List<InvetoryTransactionDTO> GetInvetoryTransactionsByTransactionHeadID(long TransactionHeadID)
        {
            ...
            {
                var transDTOList = new List<InvetoryTransactionDTO>();

                foreach (var trans in InvetoryTransactionList)
                {
                    transDTOList.Add(new InvetoryTransactionDTO()
                    {
                        InventoryTransactionIID = trans.InventoryTransactionIID,
                        SerialNo = trans.SerialNo,
                        DocumentTypeID = trans.DocumentTypeID,
                        TransactionNo = trans.TransactionNo,
                        TransactionDate = trans.TransactionDate.Value.ToLongDateString(),
                        ...
                        LinkDocumentID = trans.LinkDocumentID,
                        BranchID = trans.BranchID,
                        BatchID = trans.BatchID,
                        CompanyID = trans.CompanyID,
                        HeadID = trans.HeadID,
                        LandingCost = trans.LandingCost,
                        LastCostPrice = trans.LastCostPrice,
                        Fraction = trans.Fraction,
                        OriginalQty = trans.OriginalQty,
                    });
                }
        ...
        public List<AccountDTO> GetAutoGeneratedAccounts(long AccountID, int noOfChildAccounts, string Entity, string Entitlment)
        {
            ...
            rootAccount.AccountBehavior = (Eduegate.Infrastructure.Enums.AccountBehavior)defaultAccount.AccountBehavoir.AccountBehavoirID;
            rootAccount.AccountGroup = new AccountGroupDTO() { AccountGroupID = defaultAccount.Group.GroupID };
            rootAccount.ParentAccount = new AccountDTO() { AccountID = defaultAccount.ParentAccount != null ? defaultAccount.ParentAccount.AccountID : 0 };
        ...
        public AccountTransactionHeadDTO SaveAccountTransactionHead(AccountTransactionHeadDTO headDto)
        {
            ...
            var monthlyClosingDate = new MonthlyClosingRepository().GetMonthlyClosingDate(headDto.BranchID);
            if (monthlyClosingDate.HasValue && monthlyClosingDate.Value.Year > 1900 && headDto.TransactionDate.Value.Date <= monthlyClosingDate.Value.Date)
            {            
        ...
        public long AddMissionJobReceivable(JobEntryHead jobEntryHead)
        {
            ...
            if (MissionJobEntryHead != null)
            {
                ...
                {
                    driverGlAccountId = driverAccount.AccountID;
                }
        ...
        public bool AddReceivables(List<ReceivableDTO> dtos)
        {
            ...
            foreach (ReceivableDTO dto in dtos)
            {
                ...
                receivableEntity.TransactionHeadReceivablesMaps = dto.TransactionHeadReceivablesMaps!=null && dto.TransactionHeadReceivablesMaps.Count()>0 ?new List<TransactionHeadReceivablesMap>() { new TransactionHeadReceivablesMap { HeadID = dto.TransactionHeadReceivablesMaps.FirstOrDefault().HeadID } }: new List<TransactionHeadReceivablesMap>() { new TransactionHeadReceivablesMap() { HeadID = null } } ;
                receivableEntity.AccountTransactionReceivablesMaps = dto.AccountTransactionReceivablesMaps != null && dto.AccountTransactionReceivablesMaps.Count()>0 ?new List<AccountTransactionReceivablesMap>() { new AccountTransactionReceivablesMap { AccountTransactionHeadID = dto.AccountTransactionReceivablesMaps.FirstOrDefault().HeadID } }: new List<AccountTransactionReceivablesMap>() { new AccountTransactionReceivablesMap() };
                receivableEntityList.Add(receivableEntity);
            }
        ...
        public bool AddAccountTransactionReceivablesMaps(List<AccountTransactionReceivablesMapDTO> dtos)
        {
            var accountTransactionReceivableEntityList = new List<AccountTransactionReceivablesMap>();

            foreach (AccountTransactionReceivablesMapDTO dto in dtos)
            {
                var accountTransactionReceivable = new AccountTransactionReceivablesMap();
                accountTransactionReceivable.AccountTransactionHeadID = dto.HeadID;
                accountTransactionReceivable.AccountTransactionReceivablesMapIID = dto.AccountTransactionReceivablesMapIID;
                accountTransactionReceivable.ReceivableID = dto.ReceivableID;
                accountTransactionReceivable.Amount = dto.Amount;
                accountTransactionReceivableEntityList.Add(accountTransactionReceivable);
            }
            accountTransactionReceivableEntityList = new AccountTransactionRepository().AddAccountTransactionReceivablesMaps(accountTransactionReceivableEntityList);

            foreach (AccountTransactionReceivablesMapDTO dto in dtos)
            {
                new AccountTransactionRepository().UpdateAccountReceivablesPaidAmount(dto.ReceivableID, 0);
            }
            return true;
        }
        ...
        {
            return Mappers.Accounts.AccountTransactionHeadMapper.Mapper(_callContext).GetCustomerPendingInvoices(accountID, branchID);
        }
