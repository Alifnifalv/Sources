...
namespace Eduegate.Domain
{
    public class DataFeedBL
    {
        ...
        {
            string WBDocumentsRootPath = new Domain.Setting.SettingBL().GetSettingValue<string>("DocumentsPhysicalPath");
        ...
        public DataFeedLogDTO ProcessFeed(string feedFileLocation, long feedLogID)
        {
            ...
            try
            { 
                var feedColumns = feedTemplateColumns.Select(a => a.PhysicalColumnName).ToList<string>();

                var feedDataSet = Export.ToDataSet(feedColumns, feedFileLocation, feedTemplateColumns.Count);
