...
namespace Eduegate.Domain.Mappers.DynamicViews
{
    public partial class DynamicViewMapper
    {
                    ...
                    DataCells = new Services.Contracts.Commons.DataCellListDTO()
                      {
                        ...
                        customer.CustomerCR,customer.CRExpiryDate,customer.GroupID.HasValue ? GetCustomerGroupNameByID((long)customer.GroupID) : "-",customer.StatusID.HasValue ? GetCustomerStatusNameByID((byte)customer.StatusID) :"-" ,
        ...
        public string GetCustomerGroupNameByID (long CustomerGroupID)
        {
            var customerGroup = new CustomerRepository().GetCustomerGroup(CustomerGroupID);
            return customerGroup.IsNotNull() ? customerGroup.GroupName : string.Empty;
        }

        public string GetCustomerStatusNameByID(byte customerStatusID)
        {
            var customerStatus = new CustomerRepository().GetCustomerStatusByID(customerStatusID);
            return customerStatus.IsNotNull() ? customerStatus.StatusName : string.Empty;
        }
