...
namespace Eduegate.Domain.Mappers.Notifications
{
    public class NotificationAlertMapper : IDTOEntityMapper<NotificationAlertsDTO, NotificationAlert>
    {
        ...
        {
            var alerts = new NotificationRepository().GetNotificationAlert(loginID,  page,  pageSize);
        ...
        public List<long?> GetLoginIDsByClaimSetID(long claimSetID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var loginIDs = dbContext.ClaimSetLoginMaps.Where(a => a.ClaimSetID == claimSetID).Select(a => a.LoginID).ToList();
