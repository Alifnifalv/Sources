    ...
    public class PageMapper : IDTOEntityMapper<PageDTO, Page>
    {
        ...
        public Page ToEntity(PageDTO dto)
        {
            ...
            if (dto.IsNotNull())
            {
                ...
                {
                    var IIDs = dto.PageBoilerPlateMaps
                            .Select(a => a.PageBoilerplateMapIID).ToList();
                    //delete maps
                    var pageBoilerEntities = dbContext.PageBoilerplateMaps.Where(x =>
                        (x.PageID == dto.PageID) &&
                        !IIDs.Contains(x.PageBoilerplateMapIID))
                        .Include(a => a.PageBoilerplateMapParameters)
                        .AsNoTracking().ToList();

                    if (pageBoilerEntities.IsNotNull() && pageBoilerEntities.Any())
                    {
                        foreach (var entity in pageBoilerEntities)
                        {
                            if (entity.PageBoilerplateMapParameters.IsNotNull() && entity.PageBoilerplateMapParameters.Any())
                            {
                                dbContext.PageBoilerplateMapParameters.RemoveRange(entity.PageBoilerplateMapParameters);
                            }
                        }

                        dbContext.PageBoilerplateMaps.RemoveRange(pageBoilerEntities);
