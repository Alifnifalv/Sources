...
{
    public class RFQComparisonMapper : DTOEntityDynamicMapper
    {
        public static RFQComparisonMapper Mapper(CallContext context)
        {
            var mapper = new RFQComparisonMapper();
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                if (toSaveList == null)
                {
                    throw new Exception("There is no compared list to save !");
                }

                //Duplicate validation with Quotation
                foreach (var qt in toDto.TransactionHead.Quotations)
                {
                    var transactions = dbContext.TransactionHeads.Where(x => x.ReferenceHeadID == long.Parse(qt.Key)).ToList();

                    if (transactions.Any())
                    {
                        throw new Exception("Already have purchase order "+transactions.FirstOrDefault().TransactionNo + " against the quotation "+qt.Value.ToString());
                    }
                }
                ...
                try
                {
                    ...
                    var getRFQ = dbContext.TransactionHeads
                        .AsNoTracking()
                        .FirstOrDefault(x => x.HeadIID == toDto.TransactionHead.RFQID);
                    ...
                    foreach (var group in groupedBySupplier)
                    {
                        ...
                        entity = new TransactionHead()
                        {
                            ...
                            CurrencyID = getRFQ.CurrencyID,
                            ExchangeRate = getRFQ.ExchangeRate,
                        };
                ...
                catch (Exception ex)
                {
                    ...
                    Eduegate.Logger.LogHelper<string>.Fatal($"Quotation Comparison to PO Saving failed. Error message: {errorMessage}", ex);
                }
