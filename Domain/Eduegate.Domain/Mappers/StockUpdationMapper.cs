    ...
    public class StockUpdationMapper : DTOEntityDynamicMapper
    {
        ...
        public StockVerificationDTO ToDTO(long IID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var compDetails = dbContext.StockCompareDetails.Where(h => h.TransactionHeadID == IID).AsNoTracking().ToList();
                var stkVerifyDetail = compDetails.FirstOrDefault(d => d.TransactionHeadID == IID);
                var docSts = dbContext.DocumentStatuses.Where(b => b.DocumentStatusID == transHead.DocumentStatusID).AsNoTracking().FirstOrDefault();
                var stckVeriftHead = dbContext.StockCompareHeads.Where(x => x.HeadIID == stkVerifyDetail.HeadID).AsNoTracking().FirstOrDefault();
                ...
                var dto = new StockVerificationDTO()
                {
                    ...
                    DocumentStatus = transHead.DocumentStatusID.HasValue ? new KeyValueDTO() { Key = transHead.DocumentStatusID.ToString(), Value = docSts != null ? docSts.StatusName : null } : new KeyValueDTO(),
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            {
                throw new Exception("There is no corrected quantity against the product, please check !");
            }

            var monthlyClosingDate = new MonthlyClosingMapper().GetMonthlyClosingDate(toDto.BranchID);
            if (monthlyClosingDate.HasValue && monthlyClosingDate.Value.Year > 1900 && toDto.TransactionDate.HasValue && toDto.TransactionDate.Value.Date <= monthlyClosingDate.Value.Date)
            {
            ...
            if (toDto.HeadIID != 0)
            {
                ...
                {
                    var getMapDatas = dbContext1.TransactionDetails.Where(s => s.HeadID == toDto.HeadIID).ToList();

                    if (getMapDatas.Count > 0)
                    {
                        dbContext1.TransactionDetails.RemoveRange(getMapDatas);
                    }
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var documentType = dbContext.DocumentTypes.Where(d => d.TransactionNoPrefix == "STKADJ-").AsNoTracking().FirstOrDefault();
                ...
                if (toDto.HeadIID == 0)
                {
                    ...
                    {
                        var StkCmprDetails = dbContext.StockCompareDetails
                            .FirstOrDefault(x => x.DetailIID == compDetail.StockVerficationDetailIID);
                        StkCmprDetails.TransactionHeadID = entity.HeadIID;

                        dbContext.Entry(StkCmprDetails).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    }
                    dbContext.SaveChanges();
                }
                else
                {
                    ...
                    dbContext.Entry(updateHeadSts).State = Microsoft.EntityFrameworkCore.EntityState.Modified;

                    dbContext.SaveChanges();
                }

                //Workflow create when stock updation status is submit
                if (toDto.DocStatusID == (int)Services.Contracts.Enums.DocumentStatuses.Submitted)
                {
                    //Generate Workflow for Stock updation
                    var settingValue = new Domain.Setting.SettingBL(null).GetSettingValue<string>("STOCK_UPDATION_WORKFLOW_ID");
