...
namespace Eduegate.Domain.MobileAppWrapper
{
    public class AppDataBL
    {
        ...
        private static string FromAddress = new Domain.Setting.SettingBL().GetSettingValue<string>("FromAddress");
        private static string RootUrl = "";
        private static string ProductImageURL = string.Format("{0}/{1}/", new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl"), Eduegate.Framework.Enums.EduegateImageTypes.Products.ToString());
        private static string CategoryURL = string.Format("{0}/{1}/", new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl"), Eduegate.Framework.Enums.EduegateImageTypes.Category.ToString());
        private static string BannerURL = string.Format("{0}/{1}/", new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl"), Eduegate.Framework.Enums.EduegateImageTypes.Banners.ToString());
        ...
        public List<BannerMasterDTO> GetHomeBanners()
        {
            ...
            foreach (var banner in homeBanners)
            {
                ...
                banner.BannerFile = banner.BannerFile;
                if (banner.Link.IsNotNullOrEmpty())
                {
        ...
        public List<ProductSKUDetailDTO> GetProducts(BoilerPlateDTO boilerplateDTO)
        {
            ...
            switch (boilerplateDTO.BoilerPlateID)
            {
                case 49:
                    {
                        var hasSearchServer = new Domain.Setting.SettingBL().GetSettingValue<bool>("HasSearchServer");
        ...
        public OperationResultDTO Register(UserDTO user)
        {
            ...
            var settingDetail = new Domain.Setting.SettingBL().GetSettingDetail(Eduegate.Framework.Helper.Constants.TransactionSettings.DOMAINNAME);
            if (!isUserExists)
            {
                if (new Domain.Setting.SettingBL().GetSettingValue<string>("RegisterUserByEmail").ToLower() == "false")
        ...
        public OperationResultDTO Login(UserDTO user, ApplicationType appType = ApplicationType.ERP)
        {
            ...
            if (result)
            {
                var userDTO = string.IsNullOrEmpty(user.LoginID) ? new AccountBL(this.Context).GetUserDetails(user.LoginEmailID) :
                    new AccountBL(this.Context).GetUserDetailsByID(long.Parse(user.LoginID));
        ...
        public bool CheckVoucherBeforePayment(string voucherNo)
        {
            ...
            if (validateVoucherDTO.Status != Eduegate.Infrastructure.Enums.VoucherStatus.Valid)
            {
                isError = true;
            }
        ...
        public StudentDTO GetStudentDetailsByHeadID(string headID)
        {
            ...
            studentDetails.ParentEmailID = emailID;
                ...
                else
                {
                    ...
                    switch (paymentGateWay)
                    {
                        ...
                        case Framework.Payment.PaymentGatewayType.KNET:
                            ...
                            checkpaymentMobileDTO.RedirectUrl = new Domain.Setting.SettingBL().GetSettingValue<string>("KnetMobilePage");
                            checkpaymentMobileDTO.operationResult.operationResult = OperationResult.Success;
                        ...
                        case Framework.Payment.PaymentGatewayType.PAYPAL:
                            ...
                            checkpaymentMobileDTO.PostObject = Newtonsoft.Json.JsonConvert.SerializeObject(checkpaymentMobileDTO.PostObject);
                            break;
                        case Framework.Payment.PaymentGatewayType.QPAY:
                            payment.PaymentGateway = Eduegate.Services.Contracts.Enums.PaymentGatewayType.QPAY;
                            //SetPayPalGateway(ref checkpaymentMobileDTO, ref payment, cartDetails);
                            checkpaymentMobileDTO.PostObject = Newtonsoft.Json.JsonConvert.SerializeObject(checkpaymentMobileDTO.PostObject);
                            break;
                        default:
        ...
        public void GenerateOrder(string paymentGateway, long trackKey, ref CheckoutPaymentMobileDTO checkpaymentMobileDTO)
        {
            ...
            var settingDetail = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINESALESDOCTTYPEID, (long)this.Context.CompanyID);
        ...
        //    checkpaymentMobileDTO.RedirectUrl = string.Concat(new Domain.Setting.SettingBL().GetSettingValue<string>("MIGSMobilePage"), "?redirect=", migspaymentDTO.PaymentGatewayUrl);
        //    checkpaymentMobileDTO.operationResult.operationResult = OperationResult.Success;
        ...
        //    checkpaymentMobileDTO.RedirectUrl = string.Concat(new Domain.Setting.SettingBL().GetSettingValue<string>("PayPalMobilePage"), "?redirect=", paypalDTO.PaymentGatewayUrl);
        ...
        public void SetFortGateway(ref CheckoutPaymentMobileDTO checkpaymentMobileDTO, ref Eduegate.Services.Contracts.Payments.PaymentDTO paymentDTO, CartDTO cartDetail)
        {
            ...
            if (cartDetail.IsNotNull() && cartDetail.Products.IsNotNull() && cartDetail.Products.Count > 0)
            {
                ...
                // From fortDTO
                fortDTO.PShaRequestPhrase = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortShaRequestPhrase");
                fortDTO.PAccessCode = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortAccessCode");
                fortDTO.PMerchantIdentifier = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortMerchantIdentifier");
                fortDTO.PCommand = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortPCommand");
                fortDTO.PCurrency = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortCurrency");
                ...
                fortDTO.RefCountryID = Convert.ToInt16(new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortCountryID"));
                ...
                if (isSuccess)
                {
                    ...
                    if (new Domain.Setting.SettingBL().GetSettingValue<string>("UseTheFortSandbox").Trim().ToLower() == "true")
                        checkpaymentMobileDTO.RedirectUrl = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortPurchaseTestUrl");
                    else
                        checkpaymentMobileDTO.RedirectUrl = new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortPurchaseLiveUrl");
                    ...
                    checkpaymentMobileDTO.RedirectUrl = string.Concat(new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortMobilePage"), "?redirect=", checkpaymentMobileDTO.RedirectUrl);
                    checkpaymentMobileDTO.operationResult.operationResult = OperationResult.Success;
        ...
        public Eduegate.Services.Contracts.Payments.PaymentDTO PaymentFailure(CheckoutPaymentDTO checkoutPaymentDTO)
        {
            ...
            var trackKey = checkoutPaymentDTO.PostObject;
            var cartIID = new ShoppingCartBL(this.Context).GetLoggedInUserCartIID(ShoppingCartStatus.InProcess);

            if (cartIID > 0)
            {
                new OrderBL(this.Context).SetTemporaryBranchTransfer(cartIID, true);

                new ShoppingCartBL(this.Context).UpdateCartStatus(cartIID, ShoppingCartStatus.InProcess, null, ShoppingCartStatus.InProcess);
            }
            ...
            var masterVisaData = PaymentMasterVisaMapper.Mapper(Context).GetPaymentMasterVisaDataByCartID(cartIID);
            var paymentLog = new PaymentLogDTO
            {
                RequestType = "Payment Procees",
                TransNo = masterVisaData.TransID,
                CartID = masterVisaData.CartID,
                Amount = masterVisaData.PaymentAmount,
                CustomerID = masterVisaData.CustomerID,
                ResponseLog = "Payment Cancelled/Failure"
            };
            if (paymentDTO == null)
            {
                paymentDTO = new Eduegate.Services.Contracts.Payments.PaymentDTO();
            }
            paymentDTO.PaymentID = Convert.ToString(masterVisaData?.PaymentID);
            paymentDTO.Amount = Convert.ToString(masterVisaData?.PaymentAmount);
            paymentDTO.TrackID = Convert.ToString(masterVisaData?.TrackIID);
            paymentDTO.InitiatedOn = Convert.ToString(masterVisaData?.InitOn);
            paymentDTO.TrackKey = Convert.ToString(masterVisaData?.TrackKey);
            paymentDTO.ErrorMessage = "Payment Cancelled/Failure";

            Eduegate.Logger.LogHelper<string>.Fatal("PaymentFailure", new Exception("PaymentFailure"));
        ...
        public List<Services.Contracts.OrderHistory.OrderHistoryDTO> GetOrderHistoryDetails(int pageSize, int pageNo)
        {
            ...
            var settingDetail = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.SYSBLOCKEDBRANCH, (long)this.Context.CompanyID);
            ...
            //    {
            //        product.ProductImageUrl = new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl") + product.ProductImageUrl;
            //    }
            //    //var formatstringdate = string.Format(new Domain.Setting.SettingBL().GetSettingValue<string>("DateFormat"), order.TransactionDate).ToString();
        ...
        {
            //var isFallBack = Convert.ToBoolean(new Domain.Setting.SettingBL().GetSettingValue<string>("UseFallBackSearchKeywords"));
        ...
        {
            var settingDetail = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINEBRANCHID, (long)this.Context.CompanyID);
        ...
        public List<PaymentMethodDTO> GetPaymentMethods()
        {
            ...
            {
                payment.ImageName = string.Format("{0}/{1}", new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl"), payment.ImageName);
            }
        ...
        private string GetRequestSignatureText(Eduegate.Services.Contracts.Payments.PaymentDTO payment)
        {
            ...
            return new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortShaRequestPhrase") + "access_code=" + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortAccessCode") + "amount=" + paymentAmount + "command=" + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortPCommand") + "currency=" + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortCurrency") + "customer_email=" + payment.EmailId + "customer_ip=" + payment.InitiatedFromIP + "language=" + fortLanguage + "merchant_identifier=" + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortMerchantIdentifier") + "merchant_reference=" + payment.TrackKey + "return_url=" + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortRedirectionUrl") + new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortShaRequestPhrase");
        }
        ...
        private Dictionary<string, string> GetTheFortParameters(TheFortPaymentDTO fort)
        {
            ...
            parameters.Add("return_url", new Domain.Setting.SettingBL().GetSettingValue<string>("TheFortRedirectionUrl"));
        ...
        private EmailNotificationDTO GetAdditionalNotificationParameters(EmailNotificationDTO notificationDTO, CallContext context, string RootUrl)
        {
            ...
            {
                ParameterName = "Logo",
                ParameterValue = RootUrl + "/Images/" + new Domain.Setting.SettingBL().GetSettingValue<string>("Logo")
            });
        ...
        {
            var minimumValue = new Domain.Setting.SettingBL(Context)
            ...
            if (minimumValue != null && !string.IsNullOrEmpty(minimumValue.SettingValue))
            {
                ...
                var enableAreaCap = new Domain.Setting.SettingBL(Context)
        ...
        public List<DeliveryTimeSlotDTO> GetDeliveryTimeSlots(long deliveryTypeId, DateTime dateTime)
        {
            try
            {
                ...
                var slotCapacityEnabled = new Domain.Setting.SettingBL(Context)
                             .GetSettingValue<bool>("ENABLESLOTCAPACITY", (long)this.Context.CompanyID, true);
                var slotCapacityByBranchEnabled = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("ENABLESLOTCAPACITYBYBRANCH", (long)this.Context.CompanyID, false);
        ...
        public List<PaymentMethodDTO> GetPaymentMethodsforOnlineStore()
        {
            ...
            if (paymentMethodList != null)
            {
                ...
                var testUsers = new Domain.Setting.SettingBL(Context).GetSettingValue<string>("TestUsers");
        ...
        public CheckoutPaymentMobileDTO OnlineStoreGenerateOrder(CheckoutPaymentDTO checkoutPaymentDTO)
        {
            ...
            var paymentMasterVisaDTO = new PaymentMasterVisaDTO();
                ...
                else
                {
                    ...
                    switch (paymentGateWay)
                    {
                        ...
                        case Framework.Payment.PaymentGatewayType.MIGS:
                            ...
                            paymentMasterVisaDTO = new PaymentGatewayBL(Context).GetPaymentMasterVisaDataByCartID(long.Parse(checkoutPaymentDTO.ShoppingCartID));
                            checkpaymentMobileDTO.TrackID = paymentMasterVisaDTO.TrackIID;
                            checkpaymentMobileDTO.Transactiondate = paymentMasterVisaDTO.InitOn.Value.ToString("dd-MM-yyyy HH:mm");
                            checkpaymentMobileDTO.Amount = paymentMasterVisaDTO.PaymentAmount;

                            checkoutPaymentDTO.PaymentTrackID = paymentMasterVisaDTO.TrackIID;
                            checkoutPaymentDTO.PaymentTransactionNumber = paymentMasterVisaDTO.TransID;
                        ...
                        case Framework.Payment.PaymentGatewayType.QPAY:
                            payment.PaymentGateway =
                                    Eduegate.Services.Contracts.Enums.PaymentGatewayType.QPAY;

                            paymentMasterVisaDTO = new PaymentGatewayBL(Context).GetPaymentMasterVisaDataByCartID(long.Parse(checkoutPaymentDTO.ShoppingCartID));
                            checkpaymentMobileDTO.TrackID = paymentMasterVisaDTO.TrackIID;
                            checkpaymentMobileDTO.PaymentTransNumber = paymentMasterVisaDTO.TransID;
                            checkpaymentMobileDTO.Transactiondate = paymentMasterVisaDTO.InitOn.Value.ToString("dd-MM-yyyy HH:mm");
                            checkpaymentMobileDTO.Amount = paymentMasterVisaDTO.PaymentAmount;
        ...
        //    var setting = new Domain.Setting.SettingBL(Context).GetSettingDetail("PUSHNOTIFICATIONTITLE");
        ...
        {
            var deviceInfo = string.Concat(checkpaymentMobileDTO.DevicePlatorm, "|", checkpaymentMobileDTO.DeviceVersion);
            var onlineDocumentType = new Domain.Setting.SettingBL(Context)
            ...
            var setting = new Domain.Setting.SettingBL(Context)
            ...
            foreach (var transactionId in transactionIDs.Split(','))
            {
                ...
                //var enableHub = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("EnableNotificationHub", false);
                ...
                //var enableOrderConfirmation = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("EnableOrderConfirmationEmail", false);
                ...
                //var enableOrderNotification = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("EnableOrderNotification", false);
                ...
                ////Invoice email
                //var invoiceEmail = new Domain.Setting.SettingBL(Context).GetSettingValue<bool>("EnableInvoiceEmailOnOrderGenerates", false);
        ...
        {
            //var userDTO = GetUserDetails();
            var settingDetail = new Domain.Setting.SettingBL(Context).GetSettingValue<string>("ONLINEBRANCHID");
        ...
        public OperationResultDTO CheckLoginforCustomer(UserDTO userDTO)
        {
            ...
            try
            {
                var data = new ShoppingCartRepository().CheckCustomerExistAndInsert(userDTO, Context);
        ...
        public OperationResultDTO ReOrder(long headID)
        {
            ...
            try
            {
                ...
                var settingDetail = new Domain.Setting.SettingBL(Context).GetSettingDetail(Constants.TransactionSettings.SYSBLOCKEDBRANCH, (long)this.Context.CompanyID);
                var customerID = new CustomerBL(Context).GetCustomerIDByContext();
                var orderhistoryDTOList = new UserServiceBL(this.Context).GetOrderHistoryDetails(settingDetail.SettingValue, 1, 10, customerID.ToString(), orderID: headID, withCurrencyConversion: false, context: this.Context);
        ...
        {
            var ParentPortalUrl = new Domain.Setting.SettingBL().GetSettingValue<string>("CLIENT_PARENT_PORTAL");
        ...
        public OperationResultDTO SaveSchedule(ShoppingCartDTO schedule)
        {
            ...
            DateTime startDate = DateTime.ParseExact(schedule.StartDate, dateFormat, CultureInfo.InvariantCulture);
            DateTime endDate = DateTime.ParseExact(schedule.EndDate, dateFormat, CultureInfo.InvariantCulture);

            var isAdded = new ShoppingCartRepository().SaveSchedule(cartDTO.ShoppingCartID, startDate != null ? startDate : (DateTime?)null,
                endDate != null ? endDate : (DateTime?)null,
        ...
        public List<KeyValueDTO> GetOrderTypes()
        {
            try
            {
                ...
                var types = new ShoppingCartRepository().GetOrderTypes();
        ...
        {
            var students = new ShoppingCartRepository().GetStudentsBySchool(this.Context.LoginID.Value, this.Context.UserId);
        ...
        public List<KeyValueDTO> GetVehicles(long LoginID)
        {
            try
            {
                ...
                return VehicleMapper.Mapper(this.Context).GetVehicles(LoginID);
