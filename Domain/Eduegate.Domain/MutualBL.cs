...
namespace Eduegate.Domain
{
    public class MutualBL
    {
        ...
        public Eduegate.Services.Contracts.Search.SearchResultDTO ExecuteView(DynamicViews view, string IID1 = "", string IID2 = "")
        {
            ...
            switch (view)
            {
                ...
                case DynamicViews.DocumentType:
                    var DocumentType = new MetadataRepository().GetDocumentType(int.Parse(IID1));
        ...
        public string GetNextTransactionNumber(long documentTypeID, List<KeyValueParameterDTO> parameters = null)
        {
            ...
            DocumentType entity = new MetadataRepository().GetDocumentType(typeID);
            if (entity.IsNotNull())
            {
        ...
        public CommentDTO SaveComment(CommentDTO dto)
        {
            ...
            var datas = CommentMapper.Mapper(_callContext).ToDTO(comment);

            if (_callContext.EmployeeID == null)
            {
            ...
            if (datas != null)
            {
                string decodedComment = WebUtility.HtmlDecode(datas.CommentText);
                string cleanedCommentText = Regex.Replace(decodedComment, @"<\s*br\s*/?>", "\n", RegexOptions.IgnoreCase);

                // Option B: Replace <br> with a newline character (for a line break in the notification).
                // string cleanedCommentText = Regex.Replace(datas.CommentText, @"<br\s*/?>", "\n", RegexOptions.IgnoreCase);

                // 2. Build the message with the cleaned text.
                string fullMessage = $"{dto.StudentName} {dto.AdmissionNumber} {cleanedCommentText.Trim()}";

                PushNotificationMapper.SendAndSavePushNotification(comment.ToLoginID ?? 0, comment.FromLoginID ?? 0, fullMessage, datas.Username, Settings);
            }
        ...
        {
            var commentsDTO = new List<CommentDTO>();
            var comments = new MutualRepository().GetChats((int)entityType, fromLoginID, toLoginID,  page,  pageSize,  studentID);
        ...
        {
            var result = ServiceHelper.HttpGetRequest<GeoLocationDTO>(new Domain.Setting.SettingBL().GetSettingValue<string>("GeoLocationUrl"), _callContext);
            return result;
        }
        ...
        {
            var sequence = repository.GetNextSequence(sequenceType, null);
        ...
        public List<DocumentTypeDTO> GetDocumentTypesByReferenceType(int referenceTypeID)
        {
            var resultDTOs = new List<DocumentTypeDTO>();
            var entities = new MetadataRepository().GetDocumentTypesByReferenceType(referenceTypeID);

            foreach (var entity in entities)
            {
                var typeDTO = Mappers.DocumentTypeMapper.Mapper(_callContext).ToDTO(entity);

                resultDTOs.Add(typeDTO);
            }
        ...
        public List<CommentDTO> SaveCommentsWithBroadcast(CommentDTO comment, List<BroadcastRecipientDTO> broadcastRecipients)
        {
            ...
            {
                Dictionary<string, string> Settings = NotificationSetting.GetParentAppSettings();

                var user = dbContext.Employees
                    .Where(a => a.LoginID == comment.FromLoginID)
                    .AsNoTracking()
                    .FirstOrDefault();

                var userName = user?.FirstName + " " + (user?.MiddleName ?? "") + " " + (user?.LastName ?? "");

                foreach (var recipient in broadcastRecipients)
                {
                    var newComment = new Comment
                    {
                        Comment1 = comment.CommentText,
                        FromLoginID = comment.FromLoginID,
                        ToLoginID = recipient.LoginID,
                        BroadcastID = comment.BroadcastID,
                        EntityTypeID = 170,
                        ReferenceID = recipient.StudentID ?? 0,
                        PhotoContentID = comment.PhotoContentID,
                        CreatedBy = comment.FromLoginID,
                        CreatedDate = DateTime.Now
                    };

                    dbContext.Comments.Add(newComment);

                    savedComments.Add(new CommentDTO
                    {
                        CommentIID = newComment.CommentIID,
                        CommentText = newComment.Comment1,
                        FromLoginID = newComment.FromLoginID,
                        ToLoginID = newComment.ToLoginID,
                        BroadcastID = newComment.BroadcastID,
                        ReferenceID = newComment.ReferenceID,
                        CreatedDate = newComment.CreatedDate,
                        PhotoContentID = newComment.PhotoContentID,
                    });

                    PushNotificationMapper.SendAndSavePushNotification(
                        recipient.LoginID,
                        comment.FromLoginID ?? 0,
                        comment.CommentText,
                        userName,
                        Settings
                    );
        ...
        public List<BroadCastDTO> SaveBroadcastList(List<BroadCastDTO> broadcastList)
        {
            if (broadcastList == null || !broadcastList.Any())
            {
                throw new ArgumentException("Broadcast list cannot be null or empty.");
            }

            using (var dbContext = new dbEduegateERPContext())
            {
                // Create parent broadcast
                var parentBroadcast = new BroadCast
                {
                    BroadcastName = broadcastList.First().BroadcastName,
                    FromLoginID = (long)broadcastList.First().FromLoginID,
                    CreatedDate = DateTime.Now 
                    // Set FromLoginID if required
                    // CreatedDate is already set to DateTime.UtcNow
                };

                dbContext.BroadCasts.Add(parentBroadcast);
                dbContext.SaveChanges(); // Save parent broadcast first to get the ID

                // Create and save recipient entries with parent ID
                var recipientEntries = broadcastList.Select(item =>
                {
                    return new BroadcastRecipient
                    {
                        BroadcastID = parentBroadcast.BroadcastIID,
                        ToLoginID = item.ToLoginID,
                        StudentID = item.StudentID,
                        CreatedDate = DateTime.Now
                    };
                });

                dbContext.BroadcastRecipients.AddRange(recipientEntries);
                dbContext.SaveChanges(); // Save recipient entries

                // Map to DTO (including BroadcastID from parent)
                return recipientEntries.Select(b => new BroadCastDTO // b is a BroadcastRecipient entity
                {
                    BroadcastIID = parentBroadcast.BroadcastIID,
                    BroadcastName = parentBroadcast.BroadcastName,
                    FromLoginID = parentBroadcast.FromLoginID, // Populate FromLoginID
                    ToLoginID = b.ToLoginID,
                    StudentID = b.StudentID, // ✅ Populate StudentID in the response DTO
      ...
      public BroadCastDTO GetBroadcastDetailsById(long broadcastId)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                // Retrieve the broadcast details
                var broadcastDetails = dbContext.BroadCasts
                    .Where(b => b.BroadcastIID == broadcastId)
                    .Select(b => new
                    {
                        b.BroadcastIID,
                        b.BroadcastName,
                ...
                // Retrieve messages associated with the BroadcastID
                var messages = dbContext.Comments
                    .Where(c => c.BroadcastID == broadcastId)
                    .Select(c => new CommentDTO
                    {
                        ...
                        IsRead = c.IsRead,
                        PhotoContentID = c.PhotoContentID
                    })
                    .ToList();

                // Return the details along with the associated messages
                return new BroadCastDTO
                {
                    BroadcastIID = broadcastDetails.BroadcastIID,
                    BroadcastName = broadcastDetails.BroadcastName,
                    FromLoginID = broadcastDetails.FromLoginID,
                    CreatedDate = broadcastDetails.CreatedDate,
                    Messages = messages
                };
            }
        }
        ...
        public List<BroadCastDetailsDTO> GetBroadcastDetailsByUserId(long userId)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var broadcastIds = dbContext.BroadCasts
                    .Where(b => b.FromLoginID == userId)
                    .Select(b => b.BroadcastIID)
                    .ToList();
                ...
                var broadcasts = dbContext.BroadCasts
                    .Where(b => broadcastIds.Contains(b.BroadcastIID)) // Use Contains for efficiency
                    .Include(i => i.BroadcastRecipients)
                    .AsNoTracking()
                    .ToList();

                return broadcasts.Select(broadcast => new BroadCastDetailsDTO
                {
                    ...
                    Participants = broadcast.BroadcastRecipients.Select(recipient => new BroadcastRecipientsDTO
                    {
                        ToLoginID = recipient.ToLoginID,
                        CreatedDate = recipient.CreatedDate,
                        StudentID = recipient.StudentID // ✅ Populate StudentID
                    }).ToList()
                }).ToList();
            }
        }
        ...
        {
            var parentDTO = new List<GuardianDTO>();
            var classIds = new HashSet<int>();
            var sectionIDs = new HashSet<int>();

            using (var dbContext = new Entity.School.Models.School.dbEduegateSchoolContext())
            {
                ...
                var teacher = dbContext.Employees
                    .Where(e => e.LoginID == _callContext.LoginID)
                    .AsNoTracking()
                    .FirstOrDefault();
                ...
                classIds.UnionWith(teacherClassMaps.Select(a => a.ClassID ?? 0));
                sectionIDs.UnionWith(teacherClassMaps.Select(a => a.SectionID ?? 0));

                classIds.UnionWith(subjectTeacherClassMaps.Select(a => a.ClassID ?? 0));
                sectionIDs.UnionWith(subjectTeacherClassMaps.Select(a => a.SectionID ?? 0));
            }

            using (var dbContext = new dbEduegateERPContext())
            {
                var brodCastLoginIDs = dbContext.BroadcastRecipients
                    .Where(a => a.BroadcastID == broadcastId)
                    .AsNoTracking()
                    .Select(a => a.ToLoginID)
                    .ToList();

                if (brodCastLoginIDs.Any())
                {
                    var parentIds = dbContext.Parents
                        .Where(p => brodCastLoginIDs.Contains(p.LoginID ?? 0))
                        .Select(p => p.ParentIID)
                        .ToList();

                    var students = dbContext.Students
                        .Where(s => parentIds.Contains(s.ParentID ?? 0)
                                    && classIds.Contains(s.ClassID ?? 0)
                                    && sectionIDs.Contains(s.SectionID ?? 0))
                        .ToList();

                    foreach (var student in students)
                    {
                        var parentDetails = ParentMapper.Mapper(_callContext).ToDTO(student.ParentID ?? 0);
                        parentDetails.StudentFullName = $"{student.AdmissionNumber} - {student.FirstName} {(student.MiddleName ?? "")} {student.LastName}".Trim();

                        // Optionally add StudentID if needed
                        parentDetails.StudentID = student.StudentIID;

                        parentDTO.Add(parentDetails);
                    }
                }
        ...
        public List<BroadCastDTO> UpdateBroadcastList(List<BroadCastDTO> broadcastList)
        {
            if (broadcastList == null || !broadcastList.Any())
            {
                throw new ArgumentException("Broadcast list cannot be null or empty.");
            ...
            var broadcastID = broadcastList.First().BroadcastIID; // Assuming BroadcastIID is present and same for all items

            using (var dbContext = new dbEduegateERPContext())
            {
                var existingBroadcast = dbContext.BroadCasts
                    .FirstOrDefault(b => b.BroadcastIID == broadcastID);
                ...
                var existingRecipients = dbContext.BroadcastRecipients
                    .Where(r => r.BroadcastID == broadcastID)
                    .ToList();

                dbContext.BroadcastRecipients.RemoveRange(existingRecipients);
                // dbContext.SaveChanges(); // Optional: batch save at the end

                var newRecipients = broadcastList.Select(item => new BroadcastRecipient
                {
                    BroadcastID = broadcastID,
                    ToLoginID = item.ToLoginID,
                    StudentID = item.StudentID, // ✅ Use StudentID from input DTO
                    CreatedDate = DateTime.Now
                }).ToList();

                dbContext.BroadcastRecipients.AddRange(newRecipients);
                dbContext.SaveChanges(); // Save removals and additions

                return newRecipients.Select(r => new BroadCastDTO
                {
                    BroadcastIID = broadcastID,
                    BroadcastName = existingBroadcast.BroadcastName,
                    FromLoginID = existingBroadcast.FromLoginID,
                    ToLoginID = r.ToLoginID,
                    StudentID = r.StudentID, // ✅ Include StudentID in response DTO
