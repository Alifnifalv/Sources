...
namespace Eduegate.Domain
{
    public class OrderBL
    {
        ...
        {
            var settingOnlineBranch = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.ONLINEBRANCHID, _context.CompanyID.HasValue ? (long)_context.CompanyID.Value : default(long));
            var settingSysBlock = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.SYSBLOCKEDBRANCH, _context.CompanyID.HasValue ? (long)_context.CompanyID.Value : default(long));

            // get marketplace branch Id
            var marketplaceSettingDto = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.MARKETPLACESALES, _context.CompanyID.HasValue ? (long)_context.CompanyID.Value : default(long));
        ...
        public long OrderGeneration(string paymentMethod, long trackID, CallContext callContext, CartDTO cart, long branchID, int deliveryTypeID, int documentTypeID, bool isSetDeliveryCharge, ref decimal voucherAmount, long sysBlockedBranchID = 0, int deliveryDays = 0, UserRole? userRole = null, string deviceInfo = "", long CustomerID = 0, decimal erpDeliveryCharge = -1)
        {
            ...
            if (cart.IsNotNull())
            {
                ...
                //transaction.TransactionHead.CurrencyID = Convert.ToInt32(new UtilityBL().GetCurrencyDetail(callContext.CurrencyCode.IsNotNullOrEmpty() ? (callContext.CurrencyCode == "KD" ? "KWD" : callContext.CurrencyCode) : callContext.CompanyID.HasValue && callContext.CompanyID > 0 ? new ReferenceDataRepository().GetCurrency(Convert.ToInt32(new SettingRepository().GetSettingDetail(Constants.TransactionSettings.DEFAUTCURRENCY, (int)callContext.CompanyID).SettingValue)).AnsiCode : "KWD").CurrencyID);//DEFAULT_CURRENCY_ID
                ...
                if (transaction.TransactionHead.HeadIID != default(long))
                {
                    ...
                    var loyaltyPointsValue = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.LOYALTYPOINTSVALUE, (long)_context.CompanyID).SettingValue;
                    var categorizaionPointsValue = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.CATEGORIZATIONPOINTSVALUE, (long)_context.CompanyID).SettingValue;
        ...
        public bool SetTemporaryBranchTransfer(long shoppingCartID, bool isReverse = false)
        {
            ...
            try
            {
                ...
                {
                    var settingDTO = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.SYSBLOCKEDBRANCH, _context.CompanyID.Value);
                    ...
                    var settingBranchTransferDOC = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.BRANCHTRANSFERSYSBLOCKEDDOC, _context.CompanyID.Value);
                    //var settingBranchBlocked = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.SYSBLOCKEDBRANCH, _context.CompanyID.Value);
                    ...
                    var marketplaceSettingDto = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.MARKETPLACESYSTEMBLOCKEDORDER, _context.CompanyID.Value);
                    var marketplaceBranchTransferDOC = new Domain.Setting.SettingBL().GetSettingDetail(Constants.TransactionSettings.MARKETPLACEBRANCHTRANSFERSYSBLOCKEDDOC, _context.CompanyID.Value);
        ...
        public KeyValueDTO SaveCancelReplaceReturnRequest(long headID, List<Eduegate.Services.Contracts.OrderHistory.OrderDetailDTO> orderDetails)
        {
            ...
            {
                var allowedCancelStatus = new Domain.Setting.SettingBL().GetSettingDetail(Constants.OrderCancelStatusSettings.ALLOWCANCELLATIONJOBSTATUS, this._context.CompanyID.Value);
                var allowedReturnStatus = new Domain.Setting.SettingBL().GetSettingDetail(Constants.OrderCancelStatusSettings.ALLOWRETURNJOBSTATUS, this._context.CompanyID.Value);
            ...
            if (vContinue)
            {
                ...
                var documentTypeID = new Domain.Setting.SettingBL().GetSettingDetail(Eduegate.Framework.Helper.Constants.OrderCancelStatusSettings.WEBSITEORDERCHANGEREQUEST, this._context.CompanyID.Value);
