...
namespace Eduegate.Domain
{
    public class ProductBL
    {
        ...
        public List<ProductDTO> GetProductByCategory(long CategoryId, long MenuId, int pageNumber, int pageSize, CallContext context, string sortBy, ProductStatuses productStatus)
        {
            ...
            //            LogoFile = string.Format("{0}\\{1}\\{2}\\{3}", new Domain.Setting.SettingBL().GetSettingValue<string>("EduegatesHostUrl").ToString(), "WBImages", WBImageTypes.Brands.ToString(), x.LogoFile)
            //        });
            //    });
        ...
        public List<POSProductDTO> ProductSKUSearch(string searchText, int dataSize, string documentTypeID)
        {
            ...
            if (documentTypeID != "" && documentTypeID != null)
            {
                ...
                var settingValue = new Domain.Setting.SettingBL(null).GetSettingValue<string>("SERVICE_DOCUMENT_TYPE_IDs");

                if (settingValue != null)
                {
                    if (settingValue == documentTypeID)
                    {
                        productTypeID = 4;
                    }
                }
        ...
        public bool GetProductWishList(long skuID, long? customerID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var wishList = dbContext.WishLists.Where(w => w.SKUID == skuID && w.CustomerID == customerID)
                    .AsNoTracking()
                    .Select(w => w.IsWishList)
                    .FirstOrDefault();
        ...
        public long SOSearch(string searchText)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var IID = dbContext.TransactionHeads.Where(i => i.TransactionNo == searchText).AsNoTracking()
                    .Select(w => w.HeadIID)
                    .FirstOrDefault();
