    ...
    public class ProductDetailBL
    {
            ...
            if (transaction.IsNotNull())
            {
                if (transaction.TransactionHead.IsNotNull())
                {
                    transactionHead = new TransactionHead()
                    {
                        ...
                        DocumentStatusID = transaction.TransactionHead.DocumentStatusID
        ...
        void SynchToFTPLocation(Product productDetails)
        {
            try
            {
                ...
                var imagePath = new Domain.Setting.SettingBL().GetSettingValue<string>("ImagesPhysicalPath") + "Products\\";
                ...
                if (productDetails.ProductIID > 0)
                {
                    foreach (var skuMaps in productDetails.ProductSKUMaps)
                    {
                        foreach (var item in skuMaps.ProductImageMaps)
                        {
                            ...
                            {
                                if (ftpClient == null)
                                    ftpClient = new FtpClient(new Domain.Setting.SettingBL().GetSettingValue<string>("ImageSynchFtpServer"),
                                     new Domain.Setting.SettingBL().GetSettingValue<string>("ImageSynchFtpServerUserID"), new Domain.Setting.SettingBL().GetSettingValue<string>("ImageSynchFtpServerPassword"));
        ...
        private static void MapProductBundlesToEntity(AddProductDTO productDetails, Product product)
        {
            ...
            {
                product.ProductBundleFromProducts = new List<ProductBundle>();
                ...
                foreach (var sku in productDetails.SKUMappings)
                {
                    ...
                    foreach (var productMap in bundleProductMapped)
                    {
                        ...
                        product.ProductBundleFromProducts.Add(new ProductBundle()
                        {
        ...
        {
            //Bundles
            if (product.ProductBundleFromProducts != null)
            {
                //Product Bundles
                if (product.ProductBundleFromProducts.Where(x => x.ToProductID != null).Any() && product.ProductBundleFromProducts.Where(x => x.ToProductSKUMapID == null).Any())
                {
                    productDetails.ProductBundles = new ProductToProductMapDTO();
                    productDetails.ProductBundles.ToProduct = new List<ProductMapDTO>();

                    foreach (ProductBundle bundle in product.ProductBundleFromProducts.Where(x => x.ToProductID != null))
                    {
                ...
                //SKU Bundles
                if (product.ProductBundleFromProducts.Where(x => x.ToProductSKUMapID != null).Any())
                {
                    productDetails.SKUBundles = new ProductToProductMapDTO();
                    productDetails.SKUBundles.ToProduct = new List<ProductMapDTO>();
                    var distinctBundleList = product.ProductBundleFromProducts.Where(x => x.ToProductSKUMapID != null).GroupBy(y => y.ToProductSKUMapID).Select(grp => grp.FirstOrDefault()).ToList();
        ...
        private void MapSKUNamesFromProductBundle(AddProductDTO productDetails, Product product)
        {
            ...
            if (product.ProductBundleFromProducts.Where(x => x.ToProductSKUMapID != null).Any())
            {
                foreach (var productSKU in productDetails.SKUMappings)
                {
                    ...
                    var mappedSKUBundleList = product.ProductBundleFromProducts.Where(x => x.FromProductSKUMapID == productSKU.ProductSKUMapID).ToList();
        ...
        {
            return new ProductDetailRepository().GetProductPriceBySKUID(productSKUMapID);
        }
        ...
        {
            return new ProductDetailRepository().GetProductAndSKUNameByID(productSKUMapIID);
        }

        public long GetProductIDfromSKU(long skuID)
        {
            return new ProductDetailRepository().GetProductIDfromSKU(skuID);
        }
