...
namespace Eduegate.Domain
{
    public class SupplierBL
    {
        ...
        {
            var dto = Mappers.Inventory.SupplierMapper.Mapper(_callContext).ToDTO(supplierID);
        ...
        public SupplierDTO SaveSupplier(SupplierDTO dtoSupplier)
        {
            ...
            if (supplier.SupplierIID.IsNotNull() && supplier.SupplierIID > 0)
            {
                ...
                {
                    var marketPlaceBranchGroupSetting = new Domain.Setting.SettingBL().GetSettingDetail("MARKETPLACEBRANCHGROUPID");
                    var marketPlaceWareHouseSetting = new Domain.Setting.SettingBL().GetSettingDetail("MARKETPLACEWAREHOUSEID");
                    ...
                    productPriceDTO.ProductPriceListTypeID = Convert.ToInt16(new Domain.Setting.SettingBL().GetSettingDetail("MARKETPLACEPRODUCTPRICELISTTYPEID").SettingValue);
                    productPriceDTO.ProductPriceListLevelID = Convert.ToInt16(new Domain.Setting.SettingBL().GetSettingDetail("MARKETPLACEPRODUCTPRICELISTLEVELID").SettingValue);
            ...
            var dto = mapper.ToDTO(supplier.SupplierIID);
            dto = GetSupplierAccountMaps(dto);
            return dto;
        }
        ...
        private void SaveEntityTypePaymentMethodMap(SupplierDTO dtoSupplier)
        {
            ...
            //        entity.ReferenceID = dtoSupplier.SupplierIID;
            //        entity.AccountID = item.AccountNumber;
        ...
        public static List<SupplierAccountMap> FromSupplierAccountMapsDTOtoEntity(List<SupplierAccountEntitlmentMapsDTO> SupplierAccountMapsDTOs)
        {
            List<SupplierAccountMap> entityList = new List<SupplierAccountMap>();

            SupplierAccountEntitlmentMapsDTO baseEntitlementDTO = SupplierAccountMapsDTOs.Where(e => e.EntitlementID == null).FirstOrDefault();
            ...
            foreach (SupplierAccountEntitlmentMapsDTO dto in SupplierAccountMapsDTOs)
            {
        ...
        public static List<SupplierAccountEntitlmentMapsDTO> FromEntityToSupplierAccountMapsDTO(List<SupplierAccountMap> entityList)
        {
            ...
            foreach (SupplierAccountMap entity in entityList)
            {
                ...
                var dto = new SupplierAccountEntitlmentMapsDTO();
                ...
                if (entity.Account != null)
                {
                    ...
                    entitelmentAccount.AccountBehavior = entity.Account.AccountBehavoir != null ? (Eduegate.Infrastructure.Enums.AccountBehavior)entity.Account.AccountBehavoir.AccountBehavoirID : 0;
                    entitelmentAccount.AccountGroup = new AccountGroupDTO() { AccountGroupID = entity.Account.Group.GroupID };
                    entitelmentAccount.ParentAccount = entity.Account != null && entity.Account.ParentAccount != null ? new AccountDTO() { AccountID = entity.Account.ParentAccount.AccountID } : new AccountDTO();
        ...
        #region //For VendorPortal

        //New Vendor or Supplier Register using Vendor portal
        public VendorRegisterDTO RegisterVendor(VendorRegisterDTO registrationDTO)
        {
            var returnData = new SupplierRepository().NewVendorRegistration(registrationDTO);

            if(returnData.IsNotNull() && returnData.IsError == false)
            {
                SendWelcomeMail(returnData);
            }
        ...
        {
            var returnData = new SupplierRepository().GetQuotationListByLoginID(loginID);

            return returnData;
        }

        public TransactionDTO GetRFQItemListByIID(long iid)
        {
            var returnData = new SupplierRepository().GetRFQItemListByIID(iid,_callContext.SupplierID); 

            return returnData;
        }

        public string RegisterContactPerson(ContactDTO contactDto)
        {
            return ContactMapper.Mapper(_callContext).SubmitContactPerson(contactDto);
        }

        public List<ContactDTO> GetVendorContactListByLoginID(long loginID)
        {
            var returnData = new ContactMapper().GetVendorContactByLoginID(loginID);
        ...
        #region SaveSupplier
        public SupplierDTO SupplierToEntity(SupplierDTO dtoSupplier) 
        {
            var resultDto = new SupplierDTO();
            var mapper = Mappers.Inventory.SupplierMapper.Mapper(_callContext);
            var supplier = mapper.ToEntity(dtoSupplier);//Save using mapper

            resultDto = GetSupplier(supplier.SupplierIID);
        ...
        {
            var returnData = new SupplierRepository().BidLoginValidate(bidLoginDTO);
        ...
        {
            var returnData = new SupplierRepository().GetBidUserDetails(id);
        ...
        {
            var returnData = new SupplierRepository().GetBidUserListByTenderID(tenderID);
        ...
        {
            var returnData = new SupplierRepository().GetTenderList(loginID);
        ...
        {
            var returnData = new SupplierRepository().OpenAndUpdateTenderLog(iid);
        ...
        public string SendWelcomeMail(VendorRegisterDTO dto)
        {
            ...
            var emailTemplate = EmailTemplateMapper.Mapper(_callContext).GetEmailTemplateDetails(EmailTemplates.SupplierWelcomeMail.ToString());
            ...
            if (emailTemplate != null && !string.IsNullOrEmpty(emailTemplate.EmailTemplate))
            {
                ...
                emailBody = emailBody.Replace("{FirstName}", dto.FirstName);
                emailBody = emailBody.Replace("{SupplierCode}", dto.SupplierCode);
                emailBody = emailBody.Replace("{VendorCR}", dto.VendorCr);
                emailBody = emailBody.Replace("{EmailID}", dto.Email);
                emailBody = emailBody.Replace("{PhoneNumber}", dto.TelephoneNo);
                emailBody = emailBody.Replace("{Password}", dto.Password);
                emailBody = emailBody.Replace("{LoginLink}", vendorUrl+ "Account/Login");
            }

            string mailMessage = new Eduegate.Domain.Notification.EmailNotificationBL(_callContext).PopulateBody(dto.Email, emailBody);

            if (emailBody != "")
            {
                if (hostDet.ToLower() == "live")
                {
                    new Eduegate.Domain.Notification.EmailNotificationBL(_callContext).SendMail(dto.Email, emailSubject, mailMessage, EmailTypes.VendorPortal, mailParameters);
                }
                else
                {
                    new Eduegate.Domain.Notification.EmailNotificationBL(_callContext).SendMail(defaultMail, emailSubject, mailMessage, EmailTypes.VendorPortal, mailParameters);
                }
            }
