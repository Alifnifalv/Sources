    ...
    public class TransactionBL
    {
        ...
        public TransactionDTO SaveTransactions(TransactionDTO transaction)
        {
            ...
            if (transaction.TransactionHead.DocumentReferenceTypeID != null)
            {
                switch (transaction.TransactionHead.DocumentReferenceTypeID.Value)
                {
                    ...
                    case Services.Contracts.Enums.DocumentReferenceTypes.SalesDeliveryNotes:
                
                        var monthlyClosingDate = new MonthlyClosingRepository().GetMonthlyClosingDate(transaction.TransactionHead.BranchID);
                        var transactionDate = transaction?.TransactionHead?.TransactionDate;
                        if (monthlyClosingDate.HasValue && monthlyClosingDate.Value.Year > 1900 && transactionDate.Value.Date <= monthlyClosingDate.Value.Date)
                        {
        ...
        public StockVerificationDTO GetStockVerificationDatas(long headIID)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                returnData.StockVerificationHeadID = getStockData.HeadIID;
                ...
                foreach (var listDat in getMapDatas)
                {
                    ...
                    returnData.StockVerificationMap.Add(new StockVerificationMapDTO()
                    {
                        ...
                        ProductCode = prdctItem != null ? prdctItem.Product.ProductCode : null,
                    });
                }
                return returnData;
            }
        ...
        public List<StockVerificationMapDTO> GetProductListsByBranchID(long branchID, DateTime date)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var completedTransactionStatusId = dbContext.Settings
                    .Where(s => s.SettingCode == "COMPLETED_TRANSACTION_STATUS_ID")
                    .Select(s => int.Parse(s.SettingValue))
                    .FirstOrDefault();

                // Query to match the SQL logic
                var query = from p in dbContext.ProductInventories
                            join h in dbContext.TransactionHeads on p.HeadID equals h.HeadIID
                            join sk in dbContext.ProductSKUMaps on p.ProductSKUMapID equals sk.ProductSKUMapIID
                            join pt in dbContext.Products on sk.ProductID equals pt.ProductIID
                            where h.BranchID == branchID
                                  && h.TransactionStatusID == completedTransactionStatusId
                                  && h.TransactionDate <= date.Date
                            group new { p, h, sk, pt } by new
                            {
                                p.BranchID,
                                h.TransactionStatusID,
                                sk.ProductID,
                                pt.ProductName,
                                sk.ProductSKUCode,
                                sk.ProductSKUMapIID,
                            ...
                            select new
                            {
                                ProductSKUMapIID = grouped.Key.ProductSKUMapIID,
                                BranchID = grouped.Key.BranchID,
                                Quantity = grouped.Sum(x => x.p.Quantity),
                                TransactionStatusID = grouped.Key.TransactionStatusID,
                                ProductID = grouped.Key.ProductID,
                                ProductSKUValue = grouped.Key.ProductName + " - " + grouped.Key.ProductSKUCode,
                                ProductSKUKey = grouped.Key.ProductSKUMapIID,
                                Description = grouped.Key.ProductDescription ?? grouped.Key.SKUName,
                                ProductCode = grouped.Key.ProductCode,
                            };

                listData = query.ToList().Select(row => new StockVerificationMapDTO
                {
        ...
        public TransactionSummaryDetailDTO GetCountByDocumentTypeID(int docTypeID)
        {
            ...
            var result = new TransactionRepository().GetTransactionCountsForDashboard(docTypeID, (byte?)_callContext.SchoolID);
        ...
        public string SendQTSubmissionPushNotification(TransactionDTO dto, List<Eduegate.Services.Contracts.Payroll.EmployeeDTO> toEmpList)
        {
            foreach (var emp in toEmpList)
            {
                ...
                var settings = Eduegate.Domain.Mappers.Notification.Notifications.Helpers.NotificationSetting.GetEmployeeAppSettings();
