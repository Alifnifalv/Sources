    ...
    public class UserServiceBL
    {
        ...
        public List<Services.Contracts.OrderHistory.OrderHistoryDTO> GetOrderHistoryDetails(string documentTypeID, int pageNumber, int pageSize, string customerID = null, CallContext context = null, long orderID = default(long), bool withCurrencyConversion = true, bool showSerialKey = false)
        {
            ...
            if (context.IsNull())
            {
                ...
                context.CurrencyCode = new Domain.Setting.SettingBL().GetSettingValue<string>("CurrencyCode");
            }
            ...
            if (_callContext == null)
            {
                ...
                lang = new Domain.Setting.SettingBL().GetSettingValue<string>("DefaultLanguageCode");
            }
            else
            {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                if (userHistoryList.IsNotNull() && userHistoryList.Count > 0)
                {
                    foreach (HistoryHeader historyHeader in userHistoryList)
                    {
                        ...
                        orderHistory.TransactionDateString = historyHeader.TransactionDate.HasValue ? historyHeader.TransactionDate.Value.ToString(dateFormat, CultureInfo.InvariantCulture) : null;
                        orderHistory.TransactionNo = historyHeader.TransactionNo;
                        orderHistory.TransactionOrderIID = historyHeader.TransactionOrderIID;
                        ...
                        orderHistory.DocumentStatusID = historyHeader.DocumentStatusID;
                        var DocumentStat = dbContext.DocumentStatuses.Where(a => a.DocumentStatusID == historyHeader.DocumentStatusID).AsNoTracking().FirstOrDefault();
                        ...
                        var classDet = dbContext.Classes.Where(a => a.ClassID == StudentDetails.ClassID).AsNoTracking().FirstOrDefault();
                        var secDet = dbContext.Sections.Where(a => a.SectionID == StudentDetails.SectionID).AsNoTracking().FirstOrDefault();
            ...
            if (SaveForLaterList.Count > 0)
            {
                ...
                foreach (SearchResult product in SaveForLaterList)
                {
                    ...
                    SaveForLaterListDTO.Add(new WishListDTO()
                    {
                        ProductIID = product.ProductIID,
                        ProductName = product.ProductName,
                        ProductImageUrl = string.Format("{0}/{1}", ProductImageURL, product.ImageFile),
        ...
        public List<KeyValueDTO> GetReplacementActions(long headID)
        {
            ...
            if (jobstatusID != default(int?) && jobstatusID.HasValue)
            {
                ...
                var allowedCancelStatus = new Domain.Setting.SettingBL().GetSettingDetail(Constants.OrderCancelStatusSettings.ALLOWCANCELLATIONJOBSTATUS, this._callContext.CompanyID.Value);
                var allowedReturnStatus = new Domain.Setting.SettingBL().GetSettingDetail(Constants.OrderCancelStatusSettings.ALLOWRETURNJOBSTATUS, this._callContext.CompanyID.Value);
        ...
        public long? RegisterUserDevice(string token, string userType)
        {
            ...
            long? excludedUserID = new Domain.Setting.SettingBL(_callContext).GetSettingValue<long?>("USEREXCLUDED", null);
