...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    public class AssetMasterController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.AssetMaster);
        ...
        public ActionResult Get(long ID = 0)
        {
            ...
            return Json(assetViewModel, JsonRequestBehavior.AllowGet);
        }        
        ...
        public JsonResult Save(AssetViewModel assetViewModel)
        {
             try
            {
                var fixedAssetServiceClient = ClientFactory.FixedAssetServiceClient();
                if (assetViewModel != null )
                {
                    if (assetViewModel.DetailViewModel.Where(x => x.AssetCode == null ).Count()>0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset code should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.Description == null).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset Description should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.AssetCategory == null).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset Category should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.AssetGlAccount == null).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset GL Account should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.DepreciationExpGLAccount == null).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset Depreciation GL Account should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.AccumulatedDepGLAccount == null).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "Asset Accumlated Depreciation GL Account should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    if (assetViewModel.DetailViewModel.Where(x => x.DepreciationYears == 0).Count() > 0)
                    {
                        return Json(new { IsError = true, UserMessage = "DepreciationYears should be entered" }, JsonRequestBehavior.AllowGet);
                    }
                    ...
                    foreach (var detailVM in assetViewModel.DetailViewModel)
                    {
                        ...
                        {
                            return Json(new { IsError = true, UserMessage = "Asset code should be unique. Asset code "+ detailVM.AssetCode+ " repeated in the screen list." }, JsonRequestBehavior.AllowGet);
                        }
                        ...
                        {
                            return Json(new { IsError = true, UserMessage = "Asset code should be unique. Asset code " + detailVM.AssetCode + " already available in Database." }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    ...
                    {
                        return Json(new { IsError = true, UserMessage = "Failed to save assets" }, JsonRequestBehavior.AllowGet);
                    }
        ...
        public JsonResult AssetCodesSearch(string lookupName, string searchText)
        {
           ...
           try
            ...
            return Json(new { LookUpName = lookupName, Data = assets }, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult AccountCodesSearch(string lookupName, string searchText)
        {
            ...
            return Json(new { LookUpName = lookupName, Data = accounts }, JsonRequestBehavior.AllowGet);
            //return Json(assets, JsonRequestBehavior.AllowGet);
        }
