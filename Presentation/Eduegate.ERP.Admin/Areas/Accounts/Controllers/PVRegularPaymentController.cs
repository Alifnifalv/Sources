...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    public class PVRegularPaymentController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.PVRegularPayment);
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.PVRegularPayment);
        ...
        [HttpPost]
        public JsonResult Save(PVRegularPaymentViewModel pvRegularReceiptViewModel)
        {
            try
            {
                if (pvRegularReceiptViewModel != null)
                {
                    if (pvRegularReceiptViewModel.DetailViewModel != null)
                    {
                        ...
                        if (pvRegularReceiptViewModel.DetailViewModel.Any(x => (x.Credit == 0 || x.Credit == null) && (x.Debit == 0 || x.Debit == null)))
                        {
                    ...
                    {
                        return Json(new { IsError = true, UserMessage = "Cannot update the trasaction, it's already processed.", data = pvRegularReceiptViewModel }, JsonRequestBehavior.AllowGet);
                    }
            ...
            return Json(pvRegularReceiptViewModel, JsonRequestBehavior.AllowGet);
        }
