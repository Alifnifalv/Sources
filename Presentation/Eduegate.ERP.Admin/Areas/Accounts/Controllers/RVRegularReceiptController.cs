...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    public class RVRegularReceiptController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.RVRegularReceipt);
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.RVRegularReceiptSummary);
        ...
        [HttpPost]
        public JsonResult Save(RVRegularReceiptViewModel rvRegularReceiptViewModel)
        {
            try
            {
                if (rvRegularReceiptViewModel != null)
                {
                    ...
                    {
                        return Json(new { IsError = true, UserMessage = "Cannot update the trasaction, it's already processed.", data = rvRegularReceiptViewModel }, JsonRequestBehavior.AllowGet);
                    }
                }
            }
            catch (Exception exception)
            {
                ...
                return Json(new { IsError = true, UserMessage = exception.Message.ToString() }, JsonRequestBehavior.AllowGet);
            }

            return Json(rvRegularReceiptViewModel, JsonRequestBehavior.AllowGet);
        }
