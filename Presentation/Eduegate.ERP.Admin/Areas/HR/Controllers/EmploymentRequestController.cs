...
namespace Eduegate.ERP.Admin.Areas.HR.Controllers
{
    public class EmploymentRequestController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.EmploymentRequest);
        ...
        public JsonResult ValidateField(EmploymentRequestViewModel vm, string fieldName)
        {
            ...
            return Json(new { IsError = keyvalueDTO.Key.ToLower() == "false" ? true : false, UserMessage = keyvalueDTO.Value }, JsonRequestBehavior.AllowGet);
        }


        [HttpPost]
        public JsonResult Save(EmploymentRequestViewModel vm)
        {
            ...
            {
                return Json(new { IsError = true, UserMessage = keyvalueDTO.Value }, JsonRequestBehavior.AllowGet);
            }
