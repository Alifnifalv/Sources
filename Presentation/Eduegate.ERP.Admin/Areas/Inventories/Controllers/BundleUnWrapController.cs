...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    public class BundleUnWrapController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.BundleUnWrap);

            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.BundleUnWrap,
                ViewName = Infrastructure.Enums.SearchView.BundleUnWrap,
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            return PartialView("_ChildList", new SearchListViewModel
            {
                ...
                IsChild = true,
                ParentViewName = Infrastructure.Enums.SearchView.BundleUnWrap,
        ...
        {
            var view = Infrastructure.Enums.SearchView.BundleUnWrap;
        ...
        //{
        //    return base.SearchSummaryData(Infrastructure.Enums.SearchView.BundleUnWrapSummary);
        //}
        ...
        {
            var vm = new BundleUnWrapViewModel()
            {
        ...
        {
            var vm = TransactionViewModel.FromBundleUnWrapDTO(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, false, false));
            if (vm != null)
            {
                vm.DetailViewModel.Add(new BundleUnWrapDetailViewModel());
            }
        ...
        [HttpPost]
        public JsonResult Save(BundleUnWrapViewModel vm)
        {
            try
            {
                var updatedVM = new BundleUnWrapViewModel();
                var isSuccess = true;
                if (vm != null)
                {
                    var result = new TransactionDTO();
                    var transactionDTO = TransactionViewModel.ToBundleUnWrapDTO(vm);

                    isSuccess = ClientFactory.TransactionServiceClient(CallContext).CheckAvailibility(transactionDTO.TransactionHead.BranchID, transactionDTO.TransactionDetails);

                    if (!isSuccess)
                    {

                        return Json(new { IsError = true, UserMessage = "There is not enough quantity!" }, JsonRequestBehavior.AllowGet);
                    }


                    result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(transactionDTO);

                    updatedVM = TransactionViewModel.FromBundleUnWrapDTO(result);

                    if (updatedVM != null)
                    {
                        updatedVM.DetailViewModel.Add(new BundleUnWrapDetailViewModel());
                    }
            ...
            {
                Eduegate.Logger.LogHelper<BundleUnWrapController>.Fatal(ex.Message.ToString(), ex);
        ...
        public JsonResult GetCountries()
        {
            ...
            var message = "";
            var siteID = ConfigurationExtensions.GetAppConfigValue<int>("SiteID");
            ...
            return Json(new { CountryList = countries, Message = message }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetAreaByCountryID(int countryID = 0)
        {
            ...
            return Json(new { AreaList = areaVMList, Message = message }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult SaveDeliveryDetails(OrderDeliveryDetailViewModel order)
        {
            ...
            {
                Eduegate.Logger.LogHelper<BundleUnWrapController>.Info("Exception : " + ex.Message.ToString());
        ...
        public ActionResult GetDeliveryOptions()
        {
            ...
            return Json(kVMs, JsonRequestBehavior.AllowGet);
        }
    }
}