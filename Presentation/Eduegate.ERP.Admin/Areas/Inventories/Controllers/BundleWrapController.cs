...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    public class BundleWrapController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.BundleWrap);
        ...
        [HttpPost]
        public JsonResult Save(BundleWrapViewModel vm)
        {
            try
            {
                ...
                if (vm != null)
                {
                    ...
                    isSuccess = ClientFactory.TransactionServiceClient(CallContext).CheckAvailibilityBundleItem(transactionDTO.TransactionHead.BranchID.Value, transactionDTO.TransactionDetails);
