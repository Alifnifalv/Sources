...
{
    public class FOCSalesController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.FOCSales);

            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.FOCSales,
                ViewName = Infrastructure.Enums.SearchView.FOCSales,
        ...
        {
            var view = Infrastructure.Enums.SearchView.FOCSales;
        ...
        public ActionResult Create(long ID = 0)
        {
            ...
            //FOCSalesMasterViewModel FOCSalesViewModel = new FOCSalesMasterViewModel();
            //FOCSalesViewModel.DocumentReferenceTypeID = Services.Contracts.Enums.DocumentReferenceTypes.SalesInvoice;
            //vm.MasterViewModel = FOCSalesViewModel;
        ...
        {
            var vm = TransactionViewModel.FromFOCSalesDTO(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, true, false));
            if (vm != null)
            {
                vm.DetailViewModel.Add(new FOCSalesDetailViewModel());
            }
            return Json(vm, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult Save(FOCSalesViewModel vm)
        {
            try
            {
                var updatedVM = new FOCSalesViewModel();
                var userMessage = string.Empty;
                if (vm != null)
                {
                    vm.MasterViewModel.DocumentReferenceTypeID = Services.Contracts.Enums.DocumentReferenceTypes.SalesInvoice;
                    var result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(TransactionViewModel.ToFOCSalesDTO(vm));

                    updatedVM = TransactionViewModel.FromFOCSalesDTO(result);
                }

                if (updatedVM != null)
                {
                    updatedVM.DetailViewModel.Add(new FOCSalesDetailViewModel());
                }
        ...
        public ActionResult GenerateInvoiceScreenForJob(long ID = 0)
        {
            try
            {
                ...
                viewModel.Name = "FOCSales";
                viewModel.ListActionName = "FOCSales";
                viewModel.ListButtonDisplayName = "Lists";
                viewModel.DisplayName = "FOC Sales";
                viewModel.PrintPreviewReportName = "FOCSales";
                viewModel.MasterViewModel = new FOCSalesMasterViewModel();
                viewModel.DetailViewModel = new FOCSalesDetailViewModel();
                ...
                {
                    viewModel.Model = new FOCSalesViewModel()
                    {
                ...
                {
                    List<FOCSalesDetailViewModel> salesInvoiceDetails = new List<FOCSalesDetailViewModel>();
                    ...
                    if (jobDetail.IsNotNull())
                    {
                        ...
                        {
                            FOCSalesDetailViewModel sidVM = null;

                            foreach (JobOperationDetailDTO jodDTO in jobDetail.Detail)
                            {
                                sidVM = new FOCSalesDetailViewModel();
                                ...
                                sidVM.CostPrice = jodDTO.Price;
                                //sidVM.Amount = Convert.ToDouble(sidVM.Quantity) * sidVM.UnitPrice;
                                sidVM.Amount = sidVM.Quantity * sidVM.CostPrice;
                    ...
                    var FOCSalesMasterViewModel = new FOCSalesMasterViewModel()
                    {
                        JobEntryHeadID = ID,
                    ...
                    //FOCSalesMasterViewModel.DeliveryDetails.OrderContactMapID = 0;
                    //if (transactionDetail.OrderContactMaps.IsNotNull() && transactionDetail.OrderContactMaps.Count > 0)
                    //{
                    //    FOCSalesMasterViewModel.DeliveryDetails = DeliveryAddressViewModel.FromOrderContactDTOToVM(transactionDetail.OrderContactMaps.Where(a => a.IsShippingAddress == true).FirstOrDefault());
                    //}

                    viewModel.Model = new FOCSalesViewModel()
                    {
                        DetailViewModel = salesInvoiceDetails,
                        MasterViewModel = FOCSalesMasterViewModel,
                    };
                }
            ...
            {
                Eduegate.Logger.LogHelper<FOCSalesController>.Fatal(ex.Message.ToString(), ex);
                return Json(new { IsError = true, UserMessage = ex.Message }, JsonRequestBehavior.AllowGet);
            }
        }

        public FOCSalesViewModel GetTransactionByJobEntryHeadId(long jobEntryHeadId)
        {
            var vm = TransactionViewModel.FromFOCSalesDTO(ClientFactory.TransactionServiceClient(CallContext).GetTransactionByJobEntryHeadId(jobEntryHeadId));
            if (vm != null)
            {
                vm.DetailViewModel.Add(new FOCSalesDetailViewModel());
            }
        ...
        public JsonResult SaveTransactions(TransactionViewModel transactionModel)
        {
            ...
            {
                Eduegate.Logger.LogHelper<FOCSalesController>.Fatal(exception.Message.ToString(), exception);
                return Json(new { IsError = !result, Transaction = transactionModel, UserMessage = exception.Message.ToString() }, JsonRequestBehavior.AllowGet);
            }

            return Json(new { IsError = !result, Transaction = transactionModel, Message = "Transaction Done Successfully", Data = @"" }, JsonRequestBehavior.AllowGet);
        }
