...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    public class OrderChangeRequestController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.OrderChangeRequest);
        ...
        [HttpPost]
        public JsonResult Save(OrderChangeRequestViewModel vm)
        {
            //return Json(vm, JsonRequestBehavior.AllowGet);
            try
            {
                    ...
                    foreach (var transactionDetail in vm.DetailViewModel)
                    {
                        if (transactionDetail.ChildGrid != null && transactionDetail.ChildGrid.Count > 0)
                        {
                            foreach (var child in transactionDetail.ChildGrid)
                            {
                                ...
                                {
                                    return Json(new { IsError = true, UserMessage = "Quantity mismatch!", data = vm }, JsonRequestBehavior.AllowGet);
                                }
                            }
                        }
                    }
        ...
        public ActionResult Get(long ID = 0)
        {
            try
            {
                ...
                {
                    return Json(new { IsError = true, UserMessage = PortalWebHelper.GetErrorMessage(vm.MasterViewModel.ErrorCode), data = vm }, JsonRequestBehavior.AllowGet);
                }
                return Json(vm, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
