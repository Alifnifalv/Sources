...
{
    public class SubscriptionSalesOrderController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.SalesOrder);

            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.SalesOrder,
                ViewName = Infrastructure.Enums.SearchView.SalesOrder,
        ...
        {
            var vm = TransactionViewModel.FromSalesOrderDTO(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, false, false));
            if (vm != null)
            {
                vm.DetailViewModel.Add(new SalesOrderDetailViewModel());
            }
        ...
        public JsonResult Save(SalesOrderViewModel vm)
        {
            try
            {
                var updatedVM = new SalesOrderViewModel();

                if (vm != null)
                {
                    var result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(TransactionViewModel.ToSalesOrderDTO(vm));
                    updatedVM = TransactionViewModel.FromSalesOrderDTO(result);

                    if (updatedVM != null)
                    {
                        updatedVM.DetailViewModel.Add(new SalesOrderDetailViewModel());
                    }
            ...
            {
                Eduegate.Logger.LogHelper<SubscriptionSalesOrderController>.Fatal(ex.Message.ToString(), ex);
        ...
        public ActionResult SaveDeliveryDetails(OrderDeliveryDetailViewModel order)
        {
            ...
            {
                Eduegate.Logger.LogHelper<SubscriptionSalesOrderController>.Info("Exception : " + ex.Message.ToString());
