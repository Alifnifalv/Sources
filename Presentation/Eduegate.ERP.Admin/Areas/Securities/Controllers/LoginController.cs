...
{
    public class LoginController : BaseSearchController
    {
        // GET: Login
        ...
        public ActionResult List()
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.Login);

            return View(new SearchListViewModel
            {
                ViewName = Eduegate.Infrastructure.Enums.SearchView.Login,
        ...
        {
            return base.SearchData(Eduegate.Infrastructure.Enums.SearchView.Login, currentPage, orderBy);
        }

        [HttpGet]
        public JsonResult SearchSummaryData()
        {
            return base.SearchSummaryData(Eduegate.Infrastructure.Enums.SearchView.LoginSummary);
        }
        ...
        public ActionResult Create(long ID = 0)
        {
            ...
            viewModel.ListActionName = "Login";
            ...
            {
                viewModel.Urls.Add(new UrlViewModel() { LookUpName = "Claims_" + claim.Key.ToString(), Url = "Securities/Login/GetClaimsByType?lookupName=Claims&claimType=" + claim.Key.ToString(), IsOnInit = false });
            }
            ...
            return RedirectToAction("Create", "CRUD", new { area = "Frameworks", screen = Screens.Login});
        }
        ...
        public ActionResult Get(long ID = 0)
        {
            ...
            var claimTypes = ClientFactory.ReferenceDataServiceClient(CallContext).GetLookUpData(Eduegate.Services.Contracts.Enums.LookUpTypes.ClaimType, string.Empty, 0, 0);       
            return Json(UserMasterViewModel.FromDTO(userDTO, claimTypes), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult Save(UserMasterViewModel vm)
        {
            ...
            string tempFolderPath = string.Format("{0}\\{1}\\{2}\\{3}\\{4}",
                    ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.UserProfile.ToString(), Constants.TEMPFOLDER, userID, fileName);
            string orignalFolderPath = string.Format("{0}//{1}//{2}",
                     ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.UserProfile.ToString(), fileName);
        ...
        {
            bool isAvailibilty = false;
            if (ClientFactory.AccountServiceClient(CallContext).CheckStudentEmailIDAvailability(loginID, EmailID))
            {
                isAvailibilty = true;
            }
        ...
        public ActionResult Delete(long ID = 0)
        {
            ...
            return Json(UserMasterViewModel.FromDTO(dto), JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetClaimsByType(ClaimType claimType, string lookupName, string searchText)
        {
            return Json(new { LookUpName = lookupName, 
                Data = Eduegate.Web.Library.ViewModels.Security.ClaimViewModel.ToKeyValueVM(ClientFactory.SecurityServiceClient(CallContext).GetClaimsByType(0, claimType)) }, JsonRequestBehavior.AllowGet); ;
        }

        [HttpGet]
        public JsonResult GetClaimsByLoginID(long? employeeID)
        {
            var result = ClientFactory.SecurityServiceClient(CallContext).GetClaimsByLoginID(employeeID);
