app.controller("CRUDController", ["$scope", "$compile", "$http", "$timeout", "productService", "purchaseorderService", "accountService", "$rootScope", function ($scope, $compile, $http, $timeout, productService, purchaseorderService, accountService, $rootScope) {
        ...
        $scope.$apply(function () {
            ...
            $http({ method: 'Get', url: url })
                .then(function (result) {
                    ...
                    $scope.LookUps.AcademicYear = result.data;
                    model.Academicyear = $scope.LookUps.AcademicYear.find(x => x.Value == academicYear).Key;
                ...
                if (result.Success === true && result.FileInfo.length > 0) {
                    $scope.$apply(function () {
                        ...
                        if (dataRow !== undefined) {
                            ...
                            if (result.FileInfo[0].ContentFileIID) {
                                if (model.SignatureUrl !== undefined && imageType == "Social") {
                                    {
                                        model.SignatureUrl = result.FileInfo[0].ContentFileIID.toString();
                                    }
                                }
                                else {
    ...
    $scope.AdvanceSearchCallBack = function (view, data, viewFullPath) {
        ...
        switch (view) {
            ...
            case 'Login':
                $('#Overlay').show();
                $http({ method: 'Get', url: 'Login/Get/' + data.LoginIID })
                    .then(function (result) {
            ...
            case 'ParentLogin':
                $('#Overlay').show();
                $http({ method: 'Get', url: 'ParentLogin/Get/' + data.LoginIID })
                    .then(function (result) {
                        ...
                        $scope.CRUDModel.ViewModel.ParentLogin.Status.Key = result.data.UserStatus;
            ...
            case 'PhysicalStockEntryPick':
                $('#Overlay').show();
                var url = utility.myHost + "StockVerification/FillPhysicalStockDataINStockUpdate?ID=" + data.HeadIID;
                $http({
                    method: 'Get', url: url
                })
                    .then(function (result) {
            ...
            success: function (productItem) {
                if (productItem != null) {
                    ...
                    if (currentRow.hasOwnProperty("Amount") == true)
                        currentRow.Amount = (productItem.UnitPrice ?? 0) * (currentRow.Quantity ?? 0);
                    ...
                    if (currentRow.IsScreen == "PurchaseInvoice" || currentRow.IsScreen == "PurchaseOrder" || currentRow.IsScreen == "GRN" || currentRow.IsScreen == "PurchaseReturn" || currentRow.IsScreen == "ServiceEntry" || currentRow.IsScreen == "BranchTransfer") {
                    ...
                    if (currentRow.IsScreen == "SalesOrder" || currentRow.IsScreen == "SalesInvoice" || currentRow.IsScreen == "SalesReturn" || currentRow.IsScreen == "FOCSales") {
        ...
        switch (field) {
            ...
            case "CostPrice":
                if (!isNaN(parseFloat(currentRow.UnitPrice)) && !isNaN(parseInt(currentRow.Quantity)) || !isNaN(parseFloat(currentRow.ItemCostPrice)) && !isNaN(parseInt(currentRow.ItemSellingPrice))) {
                    if (currentRow.IsScreen == "FOCSales") {
                        currentRow.Amount = 0;
                    }
                    else {
                    ...
                    if (currentRow.hasOwnProperty("ForeignRate")) {
                        if (parseFloat(currentRow.ForeignRate ?? 0) == 0 && parseFloat(currentRow.UnitPrice ?? 0) != 0) {
    ...
    $scope.BringDynamicPopup = function (detail) {
        detail.ShowDynamicPopup = true;
    }

    $scope.DyanmicPoupClose = function (detail) {
        detail.ShowDynamicPopup = false;
    }
    ...
    $scope.BringPopup = function (detail) {
        ...
        if (Entity == "SalesInvoice" && detail.hasOwnProperty("IsSerialNumber") == true) { if (detail.IsSerialNumber != true) return; }
        else if (Entity == "PurchaseInvoice" && detail.hasOwnProperty("IsSerialNumberOnPurchase") == true) { if (detail.IsSerialNumberOnPurchase != true) return; }
        else return;
    ...
    $scope.ProductExists = function (event) {
        ...
        if (event.keyCode == 13) {
            $.ajax({
                url: "Catalogs/ProductSKU/ProductSKUSearch?searchText=" + $scope.CRUDModel.Model.MasterViewModel.SearchText,
                type: 'GET',
                    ...
                    if (productsList.length > 0) {
                        $.each(productsList, function (index, item) {
                            ...
                            if (productExists != null && productExists != undefined) {
                                ...
                                $scope.$apply(function () {
                                    var newItem = angular.copy($scope.ModelStructure.DetailViewModel);
    ...
    $scope.PrintTransaction = function (event, reportName, reportHeader, reportFullName) {
        ...
        //$('#' + windowName).append('<center><span class="bounce-wrapper" id="Load"><span class="three-bounce-inner"><span class="bounce-item bi-item1"></span><span class="bounce-item bi-item2"></span><span class="bounce-item bi-item3"></span></span></span></center><iframe reportname="' + reportName + '" src=' + reportUrl + ' width=100% height=1200px onload="onLoadComplete(this);"></></iframe>');
    ...
    $scope.ReLoadPage = function () {
        ...
        if ($scope.CRUDModel.IsGenericCRUDSave == true) {
            ...
            if ($scope.CRUDModel.ReferenceIIDs != null)
                getUrl = Entity + '/Get/' + $scope.CRUDModel.ReferenceIIDs;
            else
                getUrl = Entity + '/Get/' + IID.toString();
    ...
    $scope.SearchStudent = function ($event, $element) {
        ...
        if ($scope.CRUDModel.Model.ClassID == undefined || $scope.CRUDModel.Model.ClassID == null) {
            if ($scope.CRUDModel.Model.Class.Key != undefined && $scope.CRUDModel.Model.Class.Key != null) {
                classID = $scope.CRUDModel.Model.Class.Key;
            }
            else
                classID = 0;
        }
    ...
    $scope.PrintMasterReport = function (event, reportName, reportHeader, reportFullName) {
        ...
        if ($scope.CRUDModel.Name == 'CollectFeeAccountPosting') {
            ...
            if ($scope.CRUDModel.ViewModel.CollectionDateFromString == null || $scope.CRUDModel.ViewModel.CollectionDateToString == null || CashierEmployee == null) {
                ...
                showOverlay();
                var sDate = new Date(moment($scope.CRUDModel.ViewModel.CollectionDateFromString, 'DD/MM/YYYY'));
                //format that date into a different format
                var formattedSDate = moment(sDate).format("MM/DD/YYYY");

                var eDate = new Date(moment($scope.CRUDModel.ViewModel.CollectionDateToString, 'DD/MM/YYYY'));
                //format that date into a different format
                var formattedEDate = moment(eDate).format("MM/DD/YYYY");

                $http({ method: 'GET', url: "Home/ViewReports?returnFileBytes=true&SDATE=" + formattedSDate + "&EDATE=" + formattedEDate + "&CASHIER_ID=" + $scope.CRUDModel.ViewModel.CashierEmployee.Key + "&reportName=" + reportFullName })
                    .then(function (filename) {
                        hideOverlay();
                        var w = window.open();
                        w.document.write('<iframe onload="isLoaded()" id="pdf" name="pdf" src="' + filename.data + '"></iframe><script>function isLoaded(){window.frames[\"pdf\"].print();}</script>');
        ...
        //var reportUrl = "Home/ViewReports?reportName=" + reportFullName + "&HeadID=" + headIID;
        ///*$('#' + windowName).append('<center><span class="bounce-wrapper" id="Load"><span class="three-bounce-inner"><span class="bounce-item bi-item1"></span><span class="bounce-item bi-item2"></span><span class="bounce-item bi-item3"></span></span></span></center><iframe reportname="' + reportName + '" src=' + reportUrl + ' width=100% height=1200px onload="onLoadComplete(this);"></></iframe>');*/
        //$('#' + windowName).append('<center><span class="bounce-wrapper" id="Load"><span class="three-bounce-inner"><span class="bounce-item bi-item1"></span><span class="bounce-item bi-item2"></span><span class="bounce-item bi-item3"></span></span></span></center><iframe reportname="' + reportName + '" src=' + reportUrl + ' width=100% height=1200px;"></></iframe>');
        //var $iFrame = $('iframe[reportname=' + reportName + ']');
        //$iFrame.load(function () {
        //$("#Load", $('#' + windowName)).hide();
        //});
