app.controller("RouteShiftingController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.GetStudentDetails = function ($event, $element, crudModel) {
        ...
        if (crudModel.Academic == null || crudModel.Academic.Key == "" || crudModel.Academic.Key == null || crudModel.Academic.Key == 0) {
            $().showMessage($scope, $timeout, true, "Please select academic year!");
            return false;
        }
        showOverlay();
        var model = crudModel;
        var url = "Schools/School/GetStudentRouteDetailsList?routeId=" + model.Routes.Key + "&academicId=" + model.Academic.Key;
    ...
    $scope.GetRouteStudStaffData = function ($event, $element, crudModel) {
        $scope.CRUDModel.ViewModel.StaffList = [];
        $scope.CRUDModel.ViewModel.StudentList = [];
        var model = crudModel;
        var academicYearId = model.Academic?.Key;
        var routeID = model.Routes?.Key;

        if (routeID == undefined || routeID == null || routeID == "") {
            $().showMessage($scope, $timeout, true, "Please select route!");
            hideOverlay();
            $scope.CRUDModel.ViewModel.Academic = null;
            return false
        }
        ...
        showOverlay();
        $scope.LookUps.TransportStudent = [];
        $scope.LookUps.TransportStaff = [];
        var url = "Schools/School/GetStudentRouteDetailsList?routeId=" + model.Routes.Key + "&academicId=" + academicYearId;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                result.data.forEach(x => {
                    $scope.LookUps.TransportStudent.push({
                            "Key": x.Student.Key,
                            "Value": x.Student.Value,
                        });
                });
                hideOverlay();
        ...
        var url = "Schools/School/GetStaffRouteDetailsList?routeId=" + model.Routes.Key + "&academicId=" + academicYearId;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                result.data.forEach(x => {
                    $scope.LookUps.TransportStaff.push({
                        "Key": x.Staff.Key,
                        "Value": x.Staff.Value,
                    });
                });
    ...
    $scope.GetStaffDetails = function ($event, $element, crudModel) {
        ...
        showOverlay();
        var model = crudModel;
        var url = "Schools/School/GetStaffRouteDetailsList?routeId=" + model.Routes.Key + "&academicId=" + model.Academic.Key;
        $http({ method: 'Get', url: url })
            .then(function (result) {
    ...
    $scope.ToRouteGroupChanges = function ($event, $element, viewModel) {
        ...
        $http({
            method: 'Get',
            url: "Schools/School/GetPickupStopMapsByRouteGroupID?routeGroupID=" + model.ToRouteGroup,
        }).then(function (result) {
        ...
        $http({
            method: 'Get',
            url: "Schools/School/GetDropStopMapsByRouteGroupID?routeGroupID=" + model.ToRouteGroup,
        }).then(function (result) {
