...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    ...
    public class BudgetingController : BaseController
    {
        ...
        [HttpGet]
        public JsonResult FillBudgetEntriesByID(int budgetID,int? screenType)
        {
            var dateFormat = new Domain.Setting.SettingBL(null).GetSettingValue<string>("DateFormat");
            ...
            foreach (var budgetEntry in budgetEntries)
            {
                ...
                foreach (var allocation in budgetEntry.BudgetEntryAllocationDtos)
                {
                    ...
                    allocations.Add(new AllocationAmountViewModel()
                    {
                        ...
                        EstimateValue = screenType ==2? !allocation.EstimateValue.HasValue|| allocation.EstimateValue ==0? allocation.Amount : allocation.EstimateValue: allocation.EstimateValue,
                ...
                budgetList.Add(new BudgetingViewModel()
                {
                    ...
                    SuggestedValue = budgetEntry.SuggestedValue,
                    Percentage = budgetEntry.Percentage,
                    EstimateValue = screenType == 2 ? !budgetEntry.EstimateValue.HasValue || budgetEntry.EstimateValue == 0 ? budgetEntry.Amount : budgetEntry.EstimateValue : budgetEntry.EstimateValue,
                    Amount = budgetEntry.Amount,
                    HeaderTitles = headerTitles,
