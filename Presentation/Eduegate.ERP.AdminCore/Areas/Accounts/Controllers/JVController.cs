...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    ...
    public class JVController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.JV);
        ...
        public async Task<IActionResult> ChildList(long ID)
        {
            ...
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.JVDetail);
        ...
        public JsonResult Save(string model)
        {
            ...
            var jvViewModel = JsonConvert.DeserializeObject<JVViewModel>(jsonData.SelectToken("data").ToString());
            ...
            try
            {
                if (jvViewModel != null)
                {
                    ...
                    {
                        jvViewModel.DetailViewModel = jvViewModel.DetailViewModel.Where(x => x.Account != null && !string.IsNullOrEmpty(x.Account.Key)).ToList();
                    ...
                    if (jvViewModel.MasterViewModel.IsError && !string.IsNullOrEmpty(jvViewModel.MasterViewModel.ErrorCode))
                    {
                        return Json(new { IsError = true, UserMessage = PortalWebHelper.GetErrorMessage(jvViewModel.MasterViewModel.ErrorCode), data = jvViewModel });
                    }
