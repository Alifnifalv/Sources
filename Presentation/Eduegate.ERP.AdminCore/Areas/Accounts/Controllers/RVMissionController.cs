...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    ...
    public class RVMissionController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.RVMission);
        ...
        public async Task<IActionResult> ChildList(long ID)
        {
            ...
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.RVMission);
        ...
        public JsonResult Get_AllDrivers_Accounts(string lookupName, string searchText)
        {
            ...
            return Json(new { LookUpName = lookupName, Data = driverAccount });
        }
        ...
        public JsonResult Get_AllSuppliers_Accounts(string lookupName, string searchText)
        {
            ...
            return Json(new { LookUpName = lookupName, Data = supplierAccount });
        }
        ...
        public JsonResult Get_AllCustomers_Accounts(string lookupName, string searchText)
        {
            ...
            return Json(new { LookUpName = lookupName, Data = customerAccount });
        }
        ...
        public JsonResult GetChildAccounts_ByParentAccountId(string lookupName, string searchText)
        {
            ...
            return Json(new { LookUpName = lookupName, Data = debitAccount });
        }
        ...
        [HttpPost]
        public JsonResult GetNextTransactionNumberByMonthYear([FromBody]RVMissionViewModel vm)
        {
        ...
        public JsonResult Save(string model)
        {
            ...
            var rvMissionViewModel = JsonConvert.DeserializeObject<RVMissionViewModel>(jsonData.SelectToken("data").ToString());
            ...
            try
            {
                if (rvMissionViewModel != null)
                {
                    ...
                    {
                        rvMissionViewModel.DetailViewModel = rvMissionViewModel.DetailViewModel.Where(x => !string.IsNullOrEmpty(x.InvoiceNumber)).ToList();
                    ...
                    if (rvMissionViewModel.MasterViewModel.IsError && !string.IsNullOrEmpty(rvMissionViewModel.MasterViewModel.ErrorCode))
                    {
                        return Json(new { IsError = true, UserMessage = PortalWebHelper.GetErrorMessage(rvMissionViewModel.MasterViewModel.ErrorCode), data = rvMissionViewModel });
                    }
                }
            }
            catch (Exception exception)
            ...
            return Json(rvMissionViewModel);
        }
