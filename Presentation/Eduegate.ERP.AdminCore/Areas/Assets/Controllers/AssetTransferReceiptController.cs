...
{
    [Area("Assets")]
    public class AssetTransferReceiptController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.AssetTransferReceipt);
            return View(new SearchListViewModel
            {
                ControllerName = "Assets/" + Infrastructure.Enums.SearchView.AssetTransferReceipt.ToString(),
                ViewName = Infrastructure.Enums.SearchView.AssetTransferReceipt,
        ...
        public Task<IActionResult> SearchData(int currentPage = 1, string orderBy = "", string runtimeFilter = "")
        {
            ...
            var view = Infrastructure.Enums.SearchView.AssetTransferReceipt;
        ...
        {
            return base.SearchSummaryData(Infrastructure.Enums.SearchView.AssetTransferReceiptSummary);
        }
        ...
        {
            var vm = new AssetTransferReceiptViewModel()
            {
                DetailViewModel = new List<AssetTransferReceiptDetailViewModel>() { new AssetTransferReceiptDetailViewModel() { } },
                MasterViewModel = new AssetTransferReceiptMasterViewModel()
                {
        ...
        {
            var vm = AssetTransferReceiptViewModel.ToVM(ClientFactory.FixedAssetServiceClient(CallContext).GetAssetTransaction(ID, false, false));

            if (vm != null)
                vm.DetailViewModel.Add(new AssetTransferReceiptDetailViewModel());
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<AssetTransferReceiptViewModel>(jsonData.SelectToken("data").ToString());

                var updatedVM = new AssetTransferReceiptViewModel();

                if (vm != null)
                {
                    var result = ClientFactory.FixedAssetServiceClient(CallContext).SaveAssetTransactions(AssetTransferReceiptViewModel.ToDTO(vm));
                    updatedVM = AssetTransferReceiptViewModel.ToVM(result);
                    updatedVM.DetailViewModel.Add(new AssetTransferReceiptDetailViewModel());
                }
            ...
            {
                Eduegate.Logger.LogHelper<AssetTransferReceiptController>.Fatal(ex.Message.ToString(), ex);
