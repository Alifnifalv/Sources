...
namespace Eduegate.ERP.Admin.Areas.CMS
{
    ...
    public class BoilerPlateController : BaseSearchController
    {
        ...
        public async Task<IActionResult> Template(BoilerPlateInfoViewModel boilerPlateInfo)
        {
            ...
            var boilerPlate = new BoilerPlateDTO();

            if(boilerPlateInfo.BoilerPlateID != 0)
            {
                //CallContext.EnableCache = true;
                boilerPlate = ClientFactory.BoilerPlateServiceClient(CallContext).GetBoilerPlate(((int)boilerPlateInfo.BoilerPlateID).ToString());
            }
            else
            {
                boilerPlate.Template = boilerPlateInfo.Template;
            }
