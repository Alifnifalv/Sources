...
namespace Eduegate.ERP.Admin.Areas.CMS
{
    ...
    public class StaticContentController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.StaticContent);
...
//                        </li>"
        ...
        public ActionResult Get(long ID = 0)
        {
            ...
            return Json(StaticContentViewModel.FromDTO(dto));
        }

        [HttpPost]
        public JsonResult Save([FromBody] StaticContentViewModel vm)
        {
            ...
            string tempFolderPath = string.Format("{0}\\{1}\\{2}\\{3}\\{4}",
                    new Domain.Setting.SettingBL().GetSettingValue<string>("ImagesPhysicalPath").ToString(), EduegateImageTypes.Styled.ToString(), Constants.TEMPFOLDER, userID, fileName);

            if (System.IO.File.Exists(tempFolderPath))
            {
                string orignalFolderPath = string.Format("{0}//{1}//{2}",
                       new Domain.Setting.SettingBL().GetSettingValue<string>("ImagesPhysicalPath").ToString(), EduegateImageTypes.Styled.ToString(), fileName);
            ...
            updatedVM.ImageFile = string.Format("{0}\\{1}\\{2}",
                  new Domain.Setting.SettingBL().GetSettingValue<string>("ImageHostUrl").ToString(), EduegateImageTypes.Styled.ToString(), vm.ImageUrl);
