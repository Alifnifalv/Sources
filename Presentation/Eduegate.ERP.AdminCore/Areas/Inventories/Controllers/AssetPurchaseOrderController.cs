...
{
    [Area("Inventories")]
    public class AssetPurchaseOrderController : BaseSearchController
    {
        private string dateTimeFormat = new Domain.Setting.SettingBL().GetSettingValue<string>("DateTimeFormat");

        // GET: Inventories/AssetPurchaseOrder
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.AssetPurchaseOrder);
            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.AssetPurchaseOrder,
                ViewName = Infrastructure.Enums.SearchView.AssetPurchaseOrder,
        ...
        public async Task<IActionResult> ChildList(long ID)
        {
            ...
            return PartialView("_ChildList", new SearchListViewModel
            {
                ...
                IsChild = true,
                ParentViewName = Infrastructure.Enums.SearchView.AssetPurchaseOrder,
        ...
        {
            var view = Infrastructure.Enums.SearchView.AssetPurchaseOrder;
        ...
        {
            return base.SearchSummaryData(Infrastructure.Enums.SearchView.AssetPurchaseOrderSummary);
        }
        ...
        {
            var vm = new AssetPurchaseOrderViewModel()
            {
            ...
            //vm.MasterViewModel.DocumentReferenceTypeID = DocumentReferenceTypes.AssetPurchaseOrder;
            return Json(vm);
        }
        ...
        public ActionResult Get(long ID = 0, bool partialCalculation = false)
        {
            try
            {
                var vm = AssetPurchaseOrderViewModel.ToVM(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, partialCalculation, false));
                if (vm != null)
                {
                    if ((vm.DetailViewModel.IsNull() || vm.DetailViewModel.Count == 0) && partialCalculation)
                    {
                        return Json(new { IsError = true, UserMessage = "No items available to create purchase invoice." });
                    }

                    vm.DetailViewModel.Add(new AssetPurchaseOrderDetailViewModel());
                }
                return Json(vm);
            }
            catch (Exception ex)
            {
                Eduegate.Logger.LogHelper<AssetPurchaseOrderController>.Fatal(ex.Message.ToString(), ex);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<AssetPurchaseOrderViewModel>(jsonData.SelectToken("data").ToString());

                var updatedVM = new AssetPurchaseOrderViewModel();

                if (vm != null)
                {
                    var result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(AssetPurchaseOrderViewModel.ToDTO(vm));
                    updatedVM = AssetPurchaseOrderViewModel.ToVM(result);
        ...
        private string AddOrderNotification(long orderID, string supplierID, EmailNotificationTypeDTO emailNotificationTypeDetail, string transactionNumber, string deliveryDate)
        {
            ...
            var supplierDetail = ClientFactory.SupplierServiceClient(CallContext, null).GetSupplier(supplierID);
