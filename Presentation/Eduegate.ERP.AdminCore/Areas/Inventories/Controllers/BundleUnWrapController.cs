...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class BundleUnWrapController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.BundleUnWrap);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<BundleUnWrapViewModel>(jsonData.SelectToken("data").ToString());
                ...
                if (vm != null)
                {
                    ...
                    if (!isSuccess)
                    {
                        ...
                        return Json(new { IsError = true, UserMessage = "There is not enough quantity!" });
                    }
