...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class CustomerController : BaseSearchController
    {
        ...
        public ActionResult Create(long ID = 0)
        {
            ...
            //{
                customerVm.Contacts.IsCityMandatory = isCityMandatory; // CallContext.CompanyID == 1 ? false : true; // city mandatory only for KSA
                customerVm.Contacts.IsAreaMandatory = isAreaMandatory; // CallContext.CompanyID == 1 ? false : true; // city mandatory only for KSA
            //}
        ...
        public ActionResult Get(long ID = 0)
        {
            ...
            //{
                vm.Contacts.IsCityMandatory = isCityMandatory; // CallContext.CompanyID == 1 ? false : true; // city mandatory only for KSA
            //}
            ...
            SupplierAccountMapDTO dto = new SupplierAccountMapDTO();
        ...
        public JsonResult Save([FromBody] CustomerMasterViewModel vm)
        {
            ...
            vm.Login.StatusID = string.IsNullOrEmpty(vm.Login.Status.Value) ? (Infrastructure.Enums.LoginUserStatus?)null : (Infrastructure.Enums.LoginUserStatus)Enum.Parse(typeof(Infrastructure.Enums.LoginUserStatus), vm.Login.Status.Value);
            vm.TitleID = vm.Title.Key;
            List<ContactsViewModel> contactsViewModels = new List<ContactsViewModel>();
            contactsViewModels.Add(vm.Contacts);
            ...
            {
                // Save Contacts
                if (vm.Contacts.IsNotNull())
                {
                    List<ContactDTO> contactDTOList = ContactsToDTO(contactsViewModels, updatedDTO.CustomerIID);
