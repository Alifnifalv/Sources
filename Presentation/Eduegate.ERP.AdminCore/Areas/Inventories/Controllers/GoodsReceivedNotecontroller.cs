...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class GoodsReceivedNotecontroller : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.GoodsReceivedNote);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<GoodsReceivedNoteViewModel>(jsonData.SelectToken("data").ToString());
