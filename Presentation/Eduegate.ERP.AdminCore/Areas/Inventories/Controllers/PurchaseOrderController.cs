...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class PurchaseOrderController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.PurchaseOrder);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<PurchaseOrderViewModel>(jsonData.SelectToken("data").ToString());
