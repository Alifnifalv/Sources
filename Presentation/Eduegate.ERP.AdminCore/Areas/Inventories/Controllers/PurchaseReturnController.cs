...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class PurchaseReturnController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.PurchaseReturn);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<PurchaseReturnViewModel>(jsonData.SelectToken("data").ToString());
