...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class SalesOrderController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.SalesOrder);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<SalesOrderViewModel>(jsonData.SelectToken("data").ToString());
