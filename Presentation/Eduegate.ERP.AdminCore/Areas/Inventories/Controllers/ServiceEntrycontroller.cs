...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class ServiceEntrycontroller : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.ServiceEntry);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<ServiceEntryViewModel>(jsonData.SelectToken("data").ToString());
