    ...
    public class OnlineExamController : BaseSearchController
    {
        ...
        public JsonResult GetOnlineExamsByCandidateAndAcademicYear(long candidateID, int academicYearID)
        {
            try
            {
                ...
                {
                    return Json(new { IsError = true, Response = examList });
                }
                else
                {
            ...
            if (result != null)
            {
                ...
                foreach (var map in result.OnlineExamResultQuestionMaps)
                {
                    ...
                    examResultQuestionMaps.Add(new OnlineExamResultQuestionViewModel()
                    {
                        ...
                        IsMarkEditable = map.IsMarkEditable,
                    });
                }
                ...
                if (examResult != null)
                {
                    return Json(new { IsError = false, Response = examResult });
                }
                else
                {
        ...
        public ActionResult UpdateOnlineExamResult([FromBody] OnlineExamResultViewModel examResult)
        {
        ...
        [HttpPost]
        public ActionResult SaveOnlineExamResultEntries([FromBody] OnlineExamResultViewModel model)
        {
        ...
        public JsonResult GetQnGroupDetailsByID(int qnGroupID)
        {
            ...
            if (groupData != null)
            {
                return Json(new { IsError = false, Response = groupData });
            }
            else
            {
        ...
        [HttpGet]
        public JsonResult GetQuestionGroupByExamIDs(string subjectIDs)
        {
            var subjectIds = subjectIDs?.Split(',').Select(int.Parse).ToList();
            var result = ClientFactory.OnlineExamServiceClient(CallContext).GetQuestionGroupByExamIDs(subjectIds);
            return Json(result);
        }

        [HttpGet]
        public JsonResult GetQuestionsByMarks(int marks, int subjectID, int classID, bool isPassage)
        {
            var result = ClientFactory.OnlineExamServiceClient(CallContext).GetQuestionsByMarks(marks, subjectID, classID, isPassage);
            return Json(result);
        }

        [HttpGet]
        public JsonResult GetQuestionDetails(int subjectID, int classID)
        {
            var result = ClientFactory.OnlineExamServiceClient(CallContext).GetQuestionDetails(subjectID, classID);
            return Json(result);
        }
    }
}