...
namespace Eduegate.ERP.Admin.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public JsonResult GetContextDetails()
        {
            var contextDetail = new UserDTO();

            contextDetail.AcademicYearID = CallContext.AcademicYearID;
            if (CallContext.AcademicYearID.HasValue)
            {
                contextDetail.AcademicYear = ClientFactory.SchoolServiceClient(CallContext).GetAcademicYearNameByID(CallContext.AcademicYearID.Value);
            }
            contextDetail.SchoolID = (byte?)CallContext.SchoolID;
            if (CallContext.SchoolID.HasValue)
            {
                contextDetail.School = ClientFactory.SchoolServiceClient(CallContext).GetSchoolNameByID(CallContext.SchoolID.Value);
            }
            contextDetail.CompanyID = contextDetail.SchoolID == 30 ? 2 : 1;
            return Json(new { IsError = false, Response = contextDetail });
        }
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult GetTranslation(string key)
        {
            string translatedText = Globalization.ResourceHelper.GetValue(key);
