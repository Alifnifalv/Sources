...
namespace Eduegate.ERP.Admin.Controllers
{
    public class HomeController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            UpdateCookiesByContext(CallContext);
        ...
        public JsonResult GetNotificationAlerts(int page , int pageSize )
        {
            var notifications = ClientFactory.SchoolServiceClient(CallContext).GetNotificationAlerts(CallContext.LoginID.Value,  page ,  pageSize );
        ...
        public JsonResult CheckExcelUploadButtonRequired(Screens view)
        {
            bool isButtonRequired = false;

            var settingValue = new Domain.Setting.SettingBL(CallContext).GetSettingValue<string>("REQUIRED_EXCEL_UPLOAD_SCREEN_IDS", null);

            if (!string.IsNullOrEmpty(settingValue))
            {
                // Convert comma-separated string to a list of integers
                var screenIDs = settingValue.Split(",").Select(int.Parse).ToList();

                if (screenIDs.Any(ID => ID == (int)view))
                {
                    isButtonRequired = true;
                }
            }

            return Json(isButtonRequired);
        }

        public JsonResult CheckLastDayOfMonth(string stringDate)
        {
            var dateFormat = new Domain.Setting.SettingBL(CallContext).GetSettingValue<string>("DateFormat");

            if (DateTime.TryParseExact(stringDate, dateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime inputDate))
            {
                ...
                bool isLastDay = inputDate.Date == lastDayOfMonth.Date;

                // Format the last day of the month dynamically
                string formattedLastDay = lastDayOfMonth.ToString(dateFormat, CultureInfo.InvariantCulture);

                return Json(new
                {
                    IsError = !isLastDay,
                    Response = isLastDay ? "Valid Date" : $"Only {formattedLastDay} is allowed for this month."
                });
            }

            return Json(new { IsError = true, Response = "Error: The provided date format is incorrect." });
        }

        public JsonResult CheckNotPastDate(string stringDate)
        {
            var dateFormat = new Domain.Setting.SettingBL(CallContext).GetSettingValue<string>("DateFormat");
            ...
            if (DateTime.TryParseExact(stringDate, dateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime inputDate))
            {
                ...
                // Check if input date is today or future
                bool isValidDate = inputDate.Date >= currentDate;

                return Json(new
                {
                    IsError = !isValidDate,
                    Response = isValidDate ? "Valid Date" : "Date cannot be in the past."
                });
            }
