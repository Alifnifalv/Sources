app.controller("BudgetingController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller, $root) {
    ...
    $scope.AccountChanges = function (budget) {
        ...
        if (isDuplicate) {
            ...
            $().showGlobalMessage($root, $timeout, true, "This account has already been selected in another entryt!");
        ...
        if ($scope.BudgetEntries.length > 0) {
            ...
            if ($scope.ScreenType == 2) {
                $scope.BudgetEntries.forEach(entry => {
                    messageTimeSec = 1000;
                    var budgetAllocations = entry.Allocations.filter(x => x.EstimateValue != null);
                    if (budgetAllocations.length != 0) {
                        ...
                        entry.ScreenType = $scope.ScreenType;
                        var totalMonthAmount = entry.Allocations.reduce((total, alloc) => {
                            const amount = alloc.EstimateValue !== null && !isNaN(alloc.EstimateValue) ? parseFloat(alloc.EstimateValue) : 0;
                            return total + amount;
                        }, 0);
                        ...
                        else if (totalMonthAmount != entry.EstimateValue) {
                            isError = true;
    ...
    $scope.FillBudgetEntriesByID = function (budgetID) {
        ...
        if (budgetID) {
            showOverlay();
            var url = utility.myHost + "Accounts/Budgeting/FillBudgetEntriesByID?budgetID=" + budgetID + "&screenType=" + $scope.ScreenType+"";
            $http({ method: 'Get', url: url })
                .then(function (result) {
    ...
    $scope.RowPercentageChanges = function (row) {
        ...
        if ($scope.ScreenType == 2) {
            row.EstimateValue = entrySugVal + (entrySugVal * (percentage / 100));
        }
        else {
            ...
            if (row.EstimateValue > 0) {
                ...
                if (row.SuggestedValue) {
                    row.Allocations.slice(0, monthCounts - 1).forEach(alloc => {
                        ...
                        var columnAmount = (alloc.SuggestedValue / row.SuggestedValue) * row.EstimateValue;

                        // Round down to the nearest integer
                        alloc.EstimateValue = Math.floor(columnAmount);

                        // Calculate the remainder and store it
                        remainder += alloc.EstimateValue - Math.floor(alloc.EstimateValue);
                    });

                    var lastRow = row.Allocations[row.Allocations.length - 1];
                    lastRow.EstimateValue += remainder;

                    var difference = row.EstimateValue - row.Allocations.reduce((total, alloc) => total + parseFloat(alloc.EstimateValue), 0);

                    lastRow.EstimateValue += difference;

                    roundedNumber = lastRow.EstimateValue.toFixed(2)

                    lastRow.EstimateValue = parseFloat(roundedNumber);
                }
                else {
                    // Calculate the integer amount for the first 11 months
                    const integerAmountPerMonth = Math.floor(row.EstimateValue / monthCounts);

                    // Calculate the remainder
                    const remainder = row.EstimateValue % monthCounts;

                    // Set the integer amount for the first 11 months
                    row.Allocations.slice(0, monthCounts - 1).forEach(alloc => {
                        alloc.EstimateValue = integerAmountPerMonth;
                    });

                    // Add the remainder to the last month
                    row.Allocations[monthCounts - 1].EstimateValue = integerAmountPerMonth + remainder;
                }
            }
        }
        else {
    ...
    $scope.MonthAmountChanges = function (row) {
        ...
        if ($scope.ScreenType == 2) {
            ...
            var sumofAmount = row.Allocations.reduce((total, alloc) => {
                ...
                var amount = parseFloat(alloc.EstimateValue);
    ...
    $scope.ViewForecastingReport = function (row) {
        ...
        var reportName = "ForecastingReport";
        var reportHeader = "Forecasting Report";
