app.controller("C3ChartBoilerPlateController", ["$scope", "$compile", "$http", "$timeout", "$controller", "$document",
    function ($scope, $compile, $http, $timeout, $controller, $document) {
        ...
        vm.childInit = function (boilerPlateInfo, windowName,y_axisLabel) {
            ...
            $timeout(function () {
                if (vm.boilerPlate && vm.boilerPlate.RuntimeParameters) {
                    var lookUp = new JSLINQ(vm.boilerPlate.RuntimeParameters)
        ...
        vm.intializeCallBack = function (datas) {
            ...
            // **Generate Color Array (or reuse it)**
            //var predefinedColors = ["#006ae6" ]; // Example colors
            // **Detect Dark Mode**
            var isDarkMode = document.documentElement.getAttribute('data-bs-theme') === 'dark';

            // **Predefined Colors Based on Mode**
            var predefinedColors = isDarkMode ? [] : [];

            //var predefinedColors = []; // Example colors
            // Ensure enough colors for your data
            if (vm.chartSeriesData.length > predefinedColors.length) {
                // Optionally: Extend the color array or handle the overflow
                console.warn("Not enough colors defined for all data points. Colors will repeat.");
            }
            ...
            if (vm.chart != null && vm.chart.chartobject != null && vm.chartSeriesData.length > 0) {
                vm.chartBuilder.reloaddata(vm.chart, vm.chartSeriesData, vm.tableHeaders, predefinedColors);
            }
        }
