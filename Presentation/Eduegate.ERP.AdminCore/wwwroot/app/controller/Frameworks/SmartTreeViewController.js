app.controller('SmartTreeViewController', ['$scope', '$timeout', '$window', '$http', '$compile', '$sce', '$q',
    function ($scope, $timeout, $window, $http, $compile, $sce, $q) {
        ...
        $scope.HandleEditCategory = function ($event, data) {
            $('.smartviewTab').removeClass('showQuickViewPanel');
            // (view, event, title, windowName, menuParameters, isEdit) 
            //$scope.Create('Frameworks/CRUD/Create?screen=Category&ID=' + data.NodeID, $event, 'Edit Category', 'Category', null, true, true);

            var windowName = "Category";
            var viewName = "Edit Category";
            var IID = data.NodeID;
