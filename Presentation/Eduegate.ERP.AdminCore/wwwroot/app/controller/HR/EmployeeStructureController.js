app.controller("EmployeeStructureController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller, $root) {
    ...
    $scope.SalaryComponentChanges = function ($event, $element, salaryComponent, gridModel) {

        if (salaryComponent != null && salaryComponent.Key == 18) {
            var url = "Payroll/Employee/GetComponentVariable?SalaryComponentID=" + salaryComponent.Key;
            $http({ method: 'Get', url: url })
                .then(function (result) {
                    gridModel.EmployeeSalaryStructureVariableMap = result.data;
        ...
        if ($scope.CRUDModel.ViewModel.Department == null || $scope.CRUDModel.ViewModel.SlipDateString == null) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    "IsGenerateLeaveOrVacationsalary": $scope.CRUDModel.ViewModel.IsGenerateLeaveOrVacationsalary
                },
                success: function (result) {
                    if (result != null && result.IsFailed != null) {
        ...
        if ($scope.CRUDModel.ViewModel.SlipDateString == null) {
            ...
            if (model.SlipComponents.length > 0) {
                ...
                model.SlipComponents.forEach(comp => {
                    ...
                    componentList.push({
                        ...
                        "SalaryComponentID": comp.SalaryComponentID != null ? comp.SalaryComponentID : comp.SalaryComponent.Key,
                        "Deduction": comp.Deduction,
                        "Earnings": comp.Earnings,
                        "Description": comp.Description != null ? comp.Description : comp.SalaryComponent.Value,
            ...
            $.ajax({
                ...
                data: {
                    ...
                    "DepartmentID": $scope.CRUDModel.ViewModel.DepartmentID,
                    "IsRegenerate": true,
                    "SalaryComponent": componentList
                },
                success: function (result) {
    ...
    //For Package negotiation screen 
    $scope.SendOfferLetter = function (crudModel) {

        showOverlay();

        if (crudModel.EmployeeSalaryStructureIID == 0) {
            $().showMessage($scope, $timeout, true, "Please structure the salary and save it first.");
            hideOverlay();
            return false;
        }

        if (crudModel.JobInterviewMapID) {
            $.ajax({
                url: utility.myHost + "Schools/School/MailJobOfferLetter",
                type: "POST",
                data: {
                    "JobInterviewMapID": crudModel.JobInterviewMapID,
                    "ApplicantName": crudModel.ApplicantName,
                    "Designation": crudModel.Designation,
                    "CompanyName": crudModel.CompanyName,
                    "CandidateMailID": crudModel.CandidateMailID,
                    "SchoolID": crudModel.SchoolID,
                },
                success: function (result) {
    ...
    $scope.LeaveSalaryStructureChanges = function ($event, $element, gridModel) {
        showOverlay();
        var model = gridModel;
        var url = "Payroll/Employee/GetEmployeeComponents?SalaryStructureID=" + model.SalaryStructure.Key;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                model.LeaveSalaryComponents = result.data;
                hideOverlay();
            }, function () {
                hideOverlay();
            });
