app.controller("SalarySlipPublishController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $root) {
    ...
    $scope.EmployeeChanges = function (employee) {
    ...
    $scope.GetEmployeesSalarySlip = function () {
        ...
        var employeeID = $scope.SelectedEmployee?.Key;
        ...
        showOverlay();
        var url = "Payroll/Employee/GetSalarySlipEmployeeData?departmentID=" + departmentID + "&Month=" + Month + "&Year=" + Year + "&employeeID=" + employeeID;
        $http({
            method: 'Get',
            url: url
        }).then(function (result) {
    ...
    $scope.AllIsVerifiedStatusChanges = function () {

        var selectedSalarySlipIIDs = $scope.SalarySlips
            .filter(slip => slip.IsSelected) // Filter for checked items
            .map(slip => slip.SalarySlipIID);

        if (selectedSalarySlipIIDs.length <= 0) {
            $().showGlobalMessage($root, $timeout, true, "Please select atleast one salary slip to be verified");
        }
        else {
            showOverlay();
            var url = "Payroll/Employee/VerifyAllSalarySlips?salarySlipIDString=" + encodeURIComponent(selectedSalarySlipIIDs.join(","));
            $http({
                method: 'Post',
                url: url
            }).then(function (result) {

                $().showGlobalMessage($root, $timeout, false, result.data.Message);
    ...
    $scope.getTodayYear = function () {
        var today = new Date();
        var yyyy = today.getFullYear();
        var todayYear = $scope.Years.find(year => year.Value == yyyy);

        $scope.SelectedYear = {
            "Key": todayYear.Key,
            "Value": todayYear.Value
        }
    };
