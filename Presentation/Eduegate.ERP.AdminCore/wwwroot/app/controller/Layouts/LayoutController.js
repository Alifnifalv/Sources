app.controller('LayoutController', ['$scope', '$http', '$compile', "$rootScope", "subscriptionService", "toaster", "$timeout",
    function ($scope, $http, $compile, $root, $subscription, $toaster, $timeout) {
        ...
        $scope.layout = null;
        $scope.WindowCount = 0;
        $scope.DashboardType = 'Default';
        $root.ReportingService = null;
        $root.DateFormat = null;
        $root.CultureLanguage = "";
        ...
        function FillSettingValues() {
            $http({
                method: 'Get', url: "Settings/Setting/GetSettingValueByKey?settingKey=" + "REPORTING_SERVICE",
            }).then(function (result) {
                $root.ReportingService = result.data;
                $root.ReportingService = !$root.ReportingService || $root.ReportingService.trim() === "" ? $root.ReportingService : $root.ReportingService.trim().toLowerCase();
            });

            $http({
                method: 'Get', url: "Settings/Setting/GetSettingValueByKey?settingKey=" + "DateFormat",
            }).then(function (result) {
                $root.DateFormat = result.data;
            });
        };
        ...
        $scope.ChangePassword = function (event) {
            ...
            $.ajax({
                ...
                success: function (content) {
                    $('#' + targetElement.attr('aria-describedby')).find('.popover-body').html($compile(content)($scope));
        ...
        $root.FireEvent = function ($event, parameters, menuParameters) {
            ...
            switch (params[0]) {
                case 'SmartTree':
                    $root.ShowTreeView(params[1].trim(), $event, params[3].trim(), params[4].trim(), menuParameters)
                    break
                case 'Create':
        ...
        $root.LoadReport = function (event, reportName, reportFullName, reportTitle) {
            ...
            if (reportingService == "ssrs") {
                ...
                var reportUrl = ""
                $.ajax({
                    url: utility.myHost + "Reports/Report/GetReportUrlandParameters?reportName=" + reportFullName,
                    type: 'GET',
                    success: function (result) {
                        if (result.Response) {
                            reportUrl = result.Response;
                ...
                var reportUrl = utility.myHost + 'Reports/ReportView/ViewReports?reportName=' + reportFullName;
                $.ajax({
                    url: reportUrl,
                    type: 'GET',
                    success: function (result) {
        ...
        $scope.GlobalSearch = function (searchText) {
            if (globalSearchAjaxHandler != null) {
                globalSearchAjaxHandler.abort()
            }

            clearTimeout(searchTimer);

            searchTimer = setTimeout(() => {
                globalSearchAjaxHandler = $.ajax({
                    url: 'Mutual/ExecuteSearchView?view=QuickSearchView&searchText=' + searchText,
                    type: 'GET',
                    success: function (result) {
                        if (!result != null) {
                            $scope.$apply(function () {
                                $scope.GlobalSearchData = [];
                                $scope.GlobalSearchData = JSON.parse(result).Datas
                            })
                        }
                    },
                    complete: function () {
                        globalSearchAjaxHandler = null
                    }
                }, 1000)
            });
        }
        $scope.HandleGlobalSearch = function ($event, data) {

            var windowName = data.SearchType;
            var viewName = 'Edit ' + data.SearchType;
            var IID = data.ID;

            switch (data.SearchType) {
                case "Customer":
                    // (view, event, title, windowName, menuParameters, isEdit) 
                    $scope.Create('Frameworks/CRUD/Create?screen=Customer&ID=' + data.ID, $event, 'Edit Customer', 'Customer', null, true, true)
                    break;
                case "Transactions":
                    // (view, event, title, windowName, menuParameters, isEdit) 
                    $scope.Create('Frameworks/CRUD/Create?screen=' + data.SearchType2 + '&ID=' + data.ID, $event, 'Edit ' + data.SearchType2, data.SearchType2, null, true, true);
                    break;
                default:
                    //$scope.Create('Frameworks/CRUD/Create?screen=' + data.SearchType + '&ID=' + data.ID, $event, 'Edit ' + data.SearchType, data.SearchType, null, true, true);

                    $scope.AddWindow("Edit" + windowName, viewName, "Edit" + data.SearchType);
                    editUrl = 'Frameworks/CRUD/Create?screen=' + windowName + "&ID=" + IID;
        ...
        $root.ShowViewJSON = function ($event, data) {
            $event.preventDefault();
            ...
            $($event.currentTarget).popover({
                ...
                html: true,
                content: function () {
                    return '<center><span id="Load" class="fa fa-circle-o-notch fa-pulse waypoint" style="font-size:20px;color:black;"></span></center>';
                }
            });

            $($event.currentTarget).on('shown.bs.popover', function () {
                $root.ContentData = data;
                var content = $compile('<i style="float: right;" class="fa fa-copy" ng-click="CopyToContent()"></i><pre id="jsonviewer" class="overflow-auto"></pre>')($root);
                var popover = $('#' + $($event.currentTarget).attr('aria-describedby'));
                popover.find('.popover-body').html(content);
                $('#jsonviewer').jsonViewer(JSON.parse(data));
        ...
        $root.GetKeys = function (object) {
            if (!object) return [];

            return Object.keys(object).filter(x => x !== '$$hashKey');
        };
