app.controller('LeftMenuController', ['$scope', '$http', "$compile", "$window", '$location', "$timeout", "$rootScope", function ($scope, $http, $compile, $window, $location, $timeout, $root) {
    ...
    $scope.getNameWithoutExtension = function (fileName) {
        return fileName.split('.').slice(0, -1).join('.');
    };

    function ExpandMenu($target) {
        const $submenu = $($target.closest('li').find('ul')[0]);
        $target.toggleClass('open');

        if ($target.hasClass('open')) {
            $submenu
                .css({ display: 'flex', height: 0, overflow: 'hidden' })
                .animate({ height: $submenu.get(0).scrollHeight }, 200, function () {
            ...
            $submenu
                ...
                .animate({ height: 0 }, 200, function () {
                    $submenu.css({ display: 'none', height: '', overflow: '' });
                });
        }
    }
