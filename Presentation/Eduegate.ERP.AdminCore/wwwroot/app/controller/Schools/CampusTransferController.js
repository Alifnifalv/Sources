app.controller("CampusTransferController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.StudentChanges = function ($event, $element, crudModel) {
        ...
        $scope.LookUps.ToClasses = null;
        $scope.LookUps.AcademicYear = null;
        model.ToClass = null;
        model.ToSection = null;
        ...
        $http({ method: 'Get', url: url })
            .then(function (result) {
                ...
                $scope.CRUDModel.ViewModel.ClassOrder = result.data.ClassOrder;
                ...
                $scope.LookUps.School = $scope.LookUps.School.filter(x => x.Key != result.data.SchoolID);  
                hideOverlay();
            }, function () {
                hideOverlay();
            });
    ...
    $scope.SchoolChanges = function ($event, $element, crudModel) {
        ...
        const classOrderStart = $scope.CRUDModel.ViewModel.ClassOrder;
        const classOrderEnd = classOrderStart + 1;

        var url = "Schools/School/GetClassListBySchool?SchoolID=" + model.ToSchool;
        $http({ method: 'Get', url: url })
            .then(function (result) {

                var filteredClassList = result.data.filter(x => x.ORDERNO >= classOrderStart && x.ORDERNO <= classOrderEnd); 

                if (filteredClassList == null || filteredClassList.length == 0) {
                    $().showMessage($scope, $timeout, true, "There is no classes available for campus transfer");
                    hideOverlay();
                    return false;
                }
                else {
                    $scope.LookUps.ToClasses = filteredClassList.map(item => {
                        return {
                            Key: item.ClassID,
