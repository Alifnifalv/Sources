app.controller("MarkController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $root) {
    ...
    $scope.GetCoScholasticEntry = function () {
        ...
        var className = $scope.SearchArgs.ClassID?.Value;
        ...
        if (!className.contains("KG") && (skillID == undefined || skillID == null)) {
            hideOverlay();
            $().showGlobalMessage($root, $timeout, true, "Please select skills!");
            return false;
        }
    ...
    $scope.SaveCoScholasticEntry = function () {
        ...
        $scope.CoScholasticEntry.forEach(x => {
            coScholasticDetail.push({
                StudentID: x.StudentIID,
                MarkStatusID: x.MarkStatusID,

                SkillGroupMasterID: skillSetID,
                SkillMasterID: skillID,
                MarksGradeMapID: x.SkillGroups[0]?.Skills[0]?.GradeID,
                MarkRegisterSkillGroupID: x.SkillGroups[0]?.Skills[0]?.MarkRegisterSkillGroupID,
                MarkRegisterSkillIID: x.SkillGroups[0]?.Skills[0]?.MarkRegisterSkillIID,
                MarkRegisterID: x.SkillGroups[0]?.Skills[0]?.MarkRegisterID,
            });
        });
        ...
        $.ajax({
            ...
            data: {
                ...
                CoScholasticEntryDetailDTOs: coScholasticDetail,
            },
    ...
    $scope.checkEntervalidateMark = function (event, student, index) {
        if (event.which === 13) { 
            event.preventDefault(); 
            $timeout(function () {
                const nextIndex = index + 1;

                if (nextIndex < $scope.MarkEntry.length) {
                    const nextInputId = 'studentGirdMark' + nextIndex;   
    ...
    $scope.validateMark = function (student) {
        ...
        if (student.MarksObtained == null) {
            student.TotalMark = null;
            student.GradeID = null;
            return null;
        }

        let mark = student.MarksObtained;

        if (mark < 0) {
           alert("Please enter the correct mark, the entered mark is negative.");
        ...
        $.each($scope.SubjectGrades, function (index, item) {
            ...
            if (item.GradeFrom < student.TotalMark && item.GradeTo >= student.TotalMark) {
    ...
    $scope.checkEnterScholasticInternal = function (event, skill, parentindex) {
        if (event.which === 13) {
            event.preventDefault();
            $timeout(function () {
                const nextIndex = parentindex + 1;

                if (nextIndex < $scope.ScholasticInternalEntry.length) {
                    const nextInputId = 'studentSkillGirdMark' + nextIndex;
    ...
    $scope.GetScholasticInternalEntry = function () {
        ...
        var className = $scope.SearchArgs.Internal.ClassID?.Value;
    ...
    $scope.SaveScholasticInternalEntry = function () {
        ...
        $scope.ScholasticInternalEntry.forEach(x => {
            scholasticInternals.push({
                StudentID: x.StudentIID,
                MarkStatusID: x.MarkStatusID,
                ...
                MarkRegisterSkillIID: x.SkillGroups[0]?.Skills[0]?.MarkRegisterSkillIID,
                MarkRegisterID: x.SkillGroups[0]?.Skills[0]?.MarkRegisterID,
                MarksObtained: x.SkillGroups[0]?.Skills[0]?.MarksObtained,
            });
        });
        ...
        $.ajax({
            ...
            data: {
                ...
                CoScholasticEntryDetailDTOs: scholasticInternals,
            },
    ...
    $scope.ResetGrid = function (data) {
        data.MarksObtained = null;
        data.GradeID = null;
        data.MarkStatusID = null;
        data.TotalMark = null;
    }

    $scope.ResetCoScholasticGrid = function (data) {
        data.MarkStatusID = null;

        if (Array.isArray(data.SkillGroups)) {
            for (var i = 0; i < data.SkillGroups.length; i++) {
                var skillGroup = data.SkillGroups[i];
    ...
    $scope.ResetScholasticInternalGrid = function (data) {
        data.MarkStatusID = null;
