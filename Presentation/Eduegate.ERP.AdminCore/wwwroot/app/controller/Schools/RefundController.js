app.controller("RefundController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.AcademicYearChanges = function (model) {
        showOverlay();
        if (model.Student != undefined && model.Student.Key != undefined) {
            var url = "Schools/School/FillCollectedFeesDetails?studentId=" + model.Student.Key + "&AcademicId=" + model.AcademicYear.Key;
    ...
    $scope.StudentChanges = function (gridModel) {
        ...
        $http({ method: 'Get', url: url })
            .then(function (result) {
                ...
                $scope.CRUDModel.ViewModel.AcademicYear.Value = result.data.AcademicYear;
                $scope.CRUDModel.ViewModel.SchoolID = result.data.SchoolID;
                var url = "Schools/School/FillCollectedFeesDetails?studentId=" + model.Student.Key + "&AcademicId=" + $scope.CRUDModel.ViewModel.AcademicYear.Key;
                $http({ method: 'Get', url: url })
                    .then(function (result) {
                        if (result == null || result.data.length == 0) {
                            $().showMessage($scope, $timeout, true, "There is no fee details available for this student");
    ...
    $scope.ClearSelection = function (gridModel) {
        ...
        if (model.IsRowSelected == false) {
            ...
            for (var i = 0, l = gridModel.MonthlySplits.length; i < l; i++) {
                gridModel.MonthlySplits[i].IsRowSelected = false;
            }
            ...
            for (var i = 0, l = gridModel.MonthlySplits.length; i < l; i++) {
                gridModel.MonthlySplits[i].IsRowSelected = true;
            }
        }
        hideOverlay();
    ...
    $scope.SplitFeeAmount = function (gridModel, type) {
        ...
        for (var i = 0, l = gridModel.MonthlySplits.length; i < l; i++) {
            ...
            var amt = gridModel.MonthlySplits[i].Amount - (gridModel.MonthlySplits[i].CreditNote ?? 0) - (gridModel.MonthlySplits[i].PrvCollect ?? 0) - (gridModel.MonthlySplits[i].PayableAmount ?? 0);
            if (amt == 0)
                amt = gridModel.MonthlySplits[i].PrvCollect ?? 0;
            gridModel.MonthlySplits[i].Refund = 0;
            if (amt >= sumRefundAmount) {
                gridModel.MonthlySplits[i].Refund = parseFloat(sumRefundAmount);
                gridModel.MonthlySplits[i].Balance = (gridModel.MonthlySplits[i].PayableAmount ?? 0) - (gridModel.MonthlySplits[i].Refund ?? 0);
                gridModel.MonthlySplits[i].IsRowSelected = true;
                ...
                gridModel.MonthlySplits[i].Refund = amt;
                gridModel.MonthlySplits[i].Balance = (gridModel.MonthlySplits[i].PayableAmount ?? 0) - (gridModel.MonthlySplits[i].Refund ?? 0);
                gridModel.MonthlySplits[i].IsRowSelected = true;
    ...
    $scope.UpdateParentFee = function (parentrow, index, e) {
        ...
        var refund = 0;
        var balance = 0;
        e.MonthlySplits.Balance = e.MonthlySplits.Refund;
        parentrow.MonthlySplits.forEach(element => {
