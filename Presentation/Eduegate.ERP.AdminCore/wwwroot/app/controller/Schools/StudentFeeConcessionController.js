app.controller("StudentFeeConcessionController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.ParentChanges = function ($event, $element, viewvModel) {
        ...
        model.Student = null;
        $scope.LookUps.Students = [];
        var url = "Schools/School/GetStudentsByParent?parentID=" + model.Parent.Key;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                $scope.LookUps.Students = result.data;
                //if (result.data.length == 1) {
    ...
    $scope.ConcessionPercentageChanges = function (gridModel) {
        var model = gridModel;
        var sum = 0;
        if (!isNaN(model.ConcessionPercentage && model.ConcessionPercentage != undefined)) {
            if (model.ConcessionPercentage > 100) {
                $().showMessage($scope, $timeout, true, "Percentage greater than 100 % are not allowed.");
                model.ConcessionPercentage = 0.00;
                model.ConcessionAmount = 0.000
                model.NetToPay = 0.000;
                return false;
            }
