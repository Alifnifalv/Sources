app.controller("MeetingRequestController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.SameDateBoxClick = function ($event, $element, model) {

        model.ApprovedDateString = model.RequestedDateString;

        $scope.FillTimeSlots(model);
    };

    $scope.SameTimeBoxClick = function ($event, $element, model) {
        model.ApprovedSlotMap = {
            "Key": null,
            "Value": null
        };

        if (model.IsTimeSame) {
            var availableSlot = $scope.LookUps.AvailableMeetingSlots.find(x => x.Key == model.RequestedSignupSlotMapID);
            //model.ApprovedSlotMap = model.RequestedSignupSlotMapID.toString();
            model.ApprovedSlotMap = availableSlot;
        }
    };

    $scope.FillTimeSlots = function (viewModel) {

        var approvedDateString = viewModel.ApprovedDateString;
        $scope.LookUps.AvailableMeetingSlots = [];

        if (!approvedDateString) {
            return false;
        }
        else {
            var url = "Signups/Signup/GetAvailableSlotsByDate?stringDate=" + approvedDateString;
            $http({ method: 'Get', url: url })
                .then(function (result) {

                    if (!result.data.IsError) {

                        $scope.LookUps.AvailableMeetingSlots = result.data.Response;
                    }
