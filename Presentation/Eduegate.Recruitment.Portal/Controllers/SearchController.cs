...
namespace Eduegate.Recruitment.Portal.Controllers
{
    public class SearchController : BaseSearchController
    {
        ...
        public async Task<IActionResult> List(SearchView view, bool isSortableList = false, string parameters = null)
        {
            // call the service and get the meta data
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)view);
        ...
        [HttpGet]
        public new async Task<IActionResult> SearchData(SearchView view, int currentPage = 1, string orderBy = "", string runtimeFilter = "", int pageSize = 0)
        {
            return await base.SearchData(view, currentPage, orderBy, runtimeFilter.Split(';')[0], pageSize);
        }

        [HttpGet]
        public new async Task<IActionResult> SearchSummaryData(SearchView view)
        {
            return await base.SearchSummaryData((SearchView)((int)view) + 1);
        }

        [HttpGet]
        //[OutputCache(CacheProfile = "CacheEduegate")]
        public async Task<IActionResult> ChildList(SearchView view, long ID)
        {
            // call the service and get the meta data
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)view);
