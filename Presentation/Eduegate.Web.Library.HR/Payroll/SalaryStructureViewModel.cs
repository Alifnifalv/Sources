...
namespace Eduegate.Web.Library.HR.Payroll
{
    ...
    public class SalaryStructureViewModel : BaseMasterViewModel
    {
        public SalaryStructureViewModel()
        {
            ...
            SalaryScale = new List<SalaryScaleViewModel>() { new SalaryScaleViewModel() };
        ...
        [CustomDisplay("")]
        public List<SalaryScaleViewModel> SalaryScale { get; set; }
        ...
        [ContainerType(Framework.Enums.ContainerTypes.Grid, "SalaryScale", "CRUDModel.ViewModel.SalaryScale")]
        [DisplayName("Components")]
        public class SalaryScaleViewModel : BaseMasterViewModel
        {
            ...
            [CustomDisplay("ScaleMinAmount")]
            public decimal? MinAmount { get; set; }

            [Required]
            [ControlType(Framework.Enums.ControlTypes.TextBox)]
            [MaxLength(10, ErrorMessage = "Maximum Length should be within 10!")]
            [CustomDisplay("ScaleMaxAmount")]
            public decimal? MaxAmount { get; set; }

            [ControlType(Framework.Enums.ControlTypes.DropDown)]
            [LookUp("LookUps.AccomodationType")]
            [CustomDisplay("School Accomodation Type")]
            public string AccomodationType { get; set; }
            public int? AccomodationTypeID { get; set; }

            [ControlType(Framework.Enums.ControlTypes.TextBox)]
            [CustomDisplay("IncrementNote")]
            public string IncrementNote { get; set; }

            [ControlType(Framework.Enums.ControlTypes.DropDown)]
            [LookUp("LookUps.MaritalStatus")]
            [CustomDisplay("Marital Status")]
            public string MaritalStatus { get; set; }
            public int? MaritalStatusID { get; set; }

            [ControlType(Framework.Enums.ControlTypes.TextBox)]
            [CustomDisplay("Leave Ticket")]
            public string LeaveTicket { get; set; }


            [ControlType(Framework.Enums.ControlTypes.Button, "small-col-width", "ng-click='InsertGridRow($index, ModelStructure.SalaryScale[0], CRUDModel.ViewModel.SalaryScale)'")]
            [DisplayName("+")]
            public string Add { get; set; }

            [ControlType(Framework.Enums.ControlTypes.Button, "small-col-width", "ng-click='RemoveGridRow($index, ModelStructure.SalaryScale[0],CRUDModel.ViewModel.SalaryScale)'")]
            [DisplayName("-")]
        ...
        public override BaseMasterViewModel ToVM(BaseMasterDTO dto)
        {
            ...
            vm.SalaryScale = new List<SalaryScaleViewModel>();
            vm.SalaryScale = ssDto.SalaryStructureScale.Select(x => new SalaryScaleViewModel()
            {
                StructureScaleID = x.StructureScaleID,
                IsSponsored = x.IsSponsored,
                MinAmount = x.MinAmount,
                MaxAmount = x.MaxAmount,
                AccomodationType = x.AccomodationTypeID.HasValue ? x.AccomodationTypeID.ToString() :null,
                IncrementNote = x.IncrementNote,
                MaritalStatus = x.MaritalStatusID.HasValue ? x.MaritalStatusID.ToString() : null,
            ...
            {
                List<SalaryComponentVariableMapViewModel> salaryComponentVariableMapViewModel = new List<SalaryComponentVariableMapViewModel>();
                foreach (var componentVariableMap in Component.SalaryComponentVariableMap)
                {
                    if (!string.IsNullOrEmpty(componentVariableMap.VariableKey) && !string.IsNullOrEmpty(componentVariableMap.VariableValue))
                    {
                        var componentVariableMapDTO = new SalaryComponentVariableMapViewModel()
                        {
                            SalaryComponentVariableMapIID = componentVariableMap.SalaryComponentVariableMapIID,
                            SalaryStructureComponentMapID = componentVariableMap.SalaryStructureComponentMapID,
                            VariableValue = componentVariableMap.VariableValue,
                            VariableKey = componentVariableMap.VariableKey,

                        };

                        salaryComponentVariableMapViewModel.Add(componentVariableMapDTO);
                    }
                }
                ...
                vm.SalaryComponents.Add(new SalaryStructureComponentViewModel()
                {
                    ...
                    SalaryComponentVariableMap = salaryComponentVariableMapViewModel
        ...
        public override BaseMasterDTO ToDTO()
        {
            ...
            dto.SalaryStructureScale = new List<SalaryStructureScaleDTO>();

            dto.SalaryStructureScale = this.SalaryScale.Where(x => x.MinAmount != null).Select(x => new SalaryStructureScaleDTO()
            {
                StructureScaleID = x.StructureScaleID,
                IsSponsored = x.IsSponsored,
                MinAmount = x.MinAmount,
                MaxAmount = x.MaxAmount,
                AccomodationTypeID = string.IsNullOrEmpty(x.AccomodationType) ? (int?)null : int.Parse(x.AccomodationType),
                IncrementNote = x.IncrementNote,
                MaritalStatusID = string.IsNullOrEmpty(x.MaritalStatus) ? (int?)null : int.Parse(x.MaritalStatus),
                LeaveTicket = x.LeaveTicket,
                SalaryStructureID = x.SalaryStructureID,
            }).ToList();
            ...
            foreach (var salaryComp in this.SalaryComponents)
            {
                if (!string.IsNullOrEmpty(salaryComp.SalaryComponent.Key))
                {
                    ...
                    List<SalaryComponentVariableMapDTO> salaryComponentVariableMapDTO = new List<SalaryComponentVariableMapDTO>();
                    foreach (var componentVariableMap in salaryComp.SalaryComponentVariableMap)
                    {
                        if (!string.IsNullOrEmpty(componentVariableMap.VariableKey) && !string.IsNullOrEmpty(componentVariableMap.VariableValue))
                        {
                    ...
                    dto.SalaryComponents.Add(new SalaryStructureComponentDTO()
                    {
                        ...
                        SalaryComponentVariableMap = salaryComponentVariableMapDTO
                    }); ;
                }                
            }
