...
{
    public class RegularCreditViewModel : BaseMasterViewModel
    {
        public RegularCreditViewModel()
        {
        ...
        public List<RegularCreditDetailViewModel> DetailViewModel { get; set; }
        public RegularCreditMasterViewModel MasterViewModel { get; set; }



        public AccountTransactionHeadDTO ToAccountTransactionHeadDTO(RegularCreditViewModel viewModel)
        {
            RegularCreditMasterViewModel masterViewModel = viewModel.MasterViewModel;
            List<RegularCreditDetailViewModel> DeatilViewModelList = viewModel.DetailViewModel;
            ...
            headDTO.DetailAccountID = masterViewModel.ReferenceHeadID != null ? masterViewModel.ReferenceHeadID.HasValue && masterViewModel.ReferenceHeadID!=0 ? Convert.ToInt32(masterViewModel.ReferenceHeadID) : (int?)null : (int?)null;
            ...
            if (headDTO.PaymentModeID == null)
            {
                throw new Exception("Select Payment mode!");
            }
            ...
            headDTO.Remarks = masterViewModel.InvoiceID != null && masterViewModel.InvoiceID.Key != null ? masterViewModel.Remarks + "#" + masterViewModel.InvoiceID.Key +"#"+ masterViewModel.InvoiceID.Value: masterViewModel.Remarks;           
            ...
            foreach (var detailVMItem in DeatilViewModelList)
            {
               ...
               // detailDTO.CostCenterID = detailVMItem.CostCenterID;
                ...
                detailDTO.ReceivableID = masterViewModel.ReceivableID;
        ...
        public RegularCreditViewModel ToRegularCreditViewModel(AccountTransactionHeadDTO headDTO)
        {
            var dateFormat = new Domain.Setting.SettingBL().GetSettingValue<string>("DateFormat");
            var RegularCreditViewModel = new RegularCreditViewModel();
            RegularCreditViewModel.MasterViewModel = new RegularCreditMasterViewModel();
            RegularCreditViewModel.DetailViewModel = new List<RegularCreditDetailViewModel>();
            RegularCreditViewModel.MasterViewModel.DocumentStatusID = headDTO.DocumentStatusID;
            RegularCreditViewModel.MasterViewModel.DocumentTypeID = headDTO.DocumentTypeID;
            RegularCreditViewModel.MasterViewModel.TransactionDate = headDTO.TransactionDate.Value.ToString(dateFormat, CultureInfo.InvariantCulture);
            RegularCreditViewModel.MasterViewModel.CostCenterID = headDTO.CostCenterID;
            RegularCreditViewModel.MasterViewModel.AccountID = headDTO.AccountID;
            RegularCreditViewModel.MasterViewModel.AccountTransactionHeadIID = headDTO.AccountTransactionHeadIID;
            RegularCreditViewModel.MasterViewModel.AdvanceAmount = Convert.ToDouble(headDTO.AdvanceAmount);
            RegularCreditViewModel.MasterViewModel.Amount = headDTO.AdvanceAmount != null ? Convert.ToDecimal(headDTO.AdvanceAmount) : 0;
            RegularCreditViewModel.MasterViewModel.AmountPaid = Convert.ToDouble(headDTO.AmountPaid);
            RegularCreditViewModel.MasterViewModel.CurrencyID = headDTO.CurrencyID;
            RegularCreditViewModel.MasterViewModel.Remarks = headDTO.Remarks;
            RegularCreditViewModel.MasterViewModel.UpdatedBy = headDTO.UpdatedBy;
            RegularCreditViewModel.MasterViewModel.UpdatedDate = headDTO.UpdatedDate;
            RegularCreditViewModel.MasterViewModel.DetailAccountID = headDTO.DetailAccountID;           
            RegularCreditViewModel.MasterViewModel.PaymentModeID = headDTO.PaymentModeID;
            RegularCreditViewModel.MasterViewModel.TimeStamps = headDTO.TimeStamps;
            RegularCreditViewModel.MasterViewModel.TransactionNumber = headDTO.TransactionNumber;
            RegularCreditViewModel.MasterViewModel.TransactionStatusID = headDTO.TransactionStatusID;
            RegularCreditViewModel.MasterViewModel.IsTransactionCompleted = headDTO.IsTransactionCompleted;
            RegularCreditViewModel.MasterViewModel.Branch = headDTO.BranchID.ToString();
            RegularCreditViewModel.MasterViewModel.ErrorCode = headDTO.ErrorCode;
            RegularCreditViewModel.MasterViewModel.IsError = headDTO.IsError;
            RegularCreditViewModel.ErrorCode = headDTO.ErrorCode;
            RegularCreditViewModel.IsError = headDTO.IsError;

           var receivableID= headDTO.AccountTransactionDetails.Select(x => x.ReceivableID).FirstOrDefault();
            if (headDTO.DocumentStatus != null)
                RegularCreditViewModel.MasterViewModel.DocumentStatus = new KeyValueViewModel() { Key = headDTO.DocumentStatus.Key, Value = headDTO.DocumentStatus.Value };
            if (headDTO.DocumentType != null)
                RegularCreditViewModel.MasterViewModel.DocumentType = new KeyValueViewModel() { Key = headDTO.DocumentType.Key, Value = headDTO.DocumentType.Value };
            if (headDTO.TransactionStatus != null)
                RegularCreditViewModel.MasterViewModel.TransactionStatus = new KeyValueViewModel() { Key = headDTO.TransactionStatus.Key, Value = headDTO.TransactionStatus.Value };

            if (headDTO.Account != null)
                RegularCreditViewModel.MasterViewModel.Account = new KeyValueViewModel() { Key = headDTO.Account.Key, Value = headDTO.Account.Value };
            if (headDTO.CostCenter != null)
                RegularCreditViewModel.MasterViewModel.CostCenter = new KeyValueViewModel() { Key = headDTO.CostCenter.Key, Value = headDTO.CostCenter.Value };
            if (headDTO.Currency != null)
                RegularCreditViewModel.MasterViewModel.Currency = new KeyValueViewModel() { Key = headDTO.Currency.Key, Value = headDTO.Currency.Value };
            if (headDTO.DetailAccount != null)
                RegularCreditViewModel.MasterViewModel.DetailAccount = new KeyValueViewModel() { Key = headDTO.DetailAccount.Key, Value = headDTO.DetailAccount.Value };
            if (headDTO.PaymentModes != null)
                RegularCreditViewModel.MasterViewModel.PaymentModes = new KeyValueViewModel() { Key = headDTO.PaymentModes.Key, Value = headDTO.PaymentModes.Value };
            ...
            foreach (var detailDTOItem in headDTO.AccountTransactionDetails)
            {
                ...
                RegularCreditViewModel.DetailViewModel.Add(detailViewModel);
            }
            return RegularCreditViewModel;
        }
