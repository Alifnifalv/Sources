...
namespace Eduegate.Web.Library.ViewModels
{
    ...
    public class TenderMasterViewModel : BaseMasterViewModel
    {
        ...
        public override BaseMasterViewModel ToVM(BaseMasterDTO dto)
        {
            ...
            foreach (var dat in tDto.TenderAuthenticationList)
            {
                vm.AuthendicationList.Add(new TenderAuthenticationViewModel()
                {
                    ...
                    EmailID = dat.EmailID,
                    Employee = dat.EmployeeID.HasValue ? new KeyValueViewModel() { Key = dat.Employee.Key, Value = dat.Employee.Value } : new KeyValueViewModel(),
                });
            }
        ...
        public override BaseMasterDTO ToDTO()
        {
            ...
            {
                if (list.UserName != null && !dto.TenderAuthenticationList.Any(d => d.EmailID == list.EmailID))
                {
                    dto.TenderAuthenticationList.Add(new TenderAuthenticationDTO()
                    {
                        TenderAuthMapIID = list.TenderAuthMapIID,
                        TenderID = list.TenderID,
                        ...
                        EmployeeID = string.IsNullOrEmpty(list.Employee.Key) ? (long?)null : long.Parse(list.Employee.Key),
                    });
                }
            }
