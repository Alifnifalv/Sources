    ...
    public class ContentController : ApiControllerBase
    {
        ...
        public long UploadStudentDocuments()
        {
            ...
            catch (Exception ex)
            {
                ...
                Eduegate.Logger.LogHelper<string>.Fatal($"UploadStudentDocuments failed. Error message: {errorMessage}", ex);
        ...
        public long UploadContentAsString(ContentFileDTO content)
        {
            var imageInfoList = new ContentFileDTO();
            var fileInfo = new ContentFileDTO();

            try
            {
                var contentData = content.ContentDataString.IsNotNull() ? Convert.FromBase64String(content.ContentDataString) : content.ContentData;
                byte[] decodedBytes = contentData;
                byte[] compressedBytes = Compress(decodedBytes);

                fileInfo.ContentFileName = content.ContentFileName;
                fileInfo.ContentData = compressedBytes;
                fileInfo.FilePath = "Content/ReadContentsByID";
                fileInfo.IsCompressed = true;

                imageInfoList = ClientFactory.ContentServicesClient(CallContext).SaveFile(fileInfo);
            }
            catch (Exception ex)
            {
                Eduegate.Logger.LogHelper<ContentController>.Fatal(ex.Message.ToString(), ex);
            ...
            return imageInfoList.ContentFileIID;
        }
        ...
        [HttpGet]
        [Route("DirectReadContentsByID")]
        public async Task DirectReadContentsByID(long? contentID)
        {
            var imageData = contentID.HasValue
                ? ClientFactory.ContentServicesClient(CallContext).ReadContentsById(contentID.Value)
                : null;

            if (imageData != null)
            {
                string fileName = imageData.ContentFileName ?? "UnknownFile";
                string contentType = GetContentType(fileName);
                Response.ContentType = contentType;

                Response.Headers.Append("Content-Disposition", $"attachment; filename=\"{fileName}\"");

                byte[] dataToWrite = IsCompressed(imageData.ContentData)
                    ? Decompress(imageData.ContentData)
                    : imageData.ContentData;

                await Response.Body.WriteAsync(dataToWrite, 0, dataToWrite.Length);
            }
            else
            {
        ...
        private string GetContentType(string fileName)
        {
            ...
            if (fileName.EndsWith(".doc") || fileName.EndsWith(".docx") || fileName.EndsWith(".rtf")) return "application/msword";
            if (fileName.EndsWith(".gif")) return "image/gif";
            if (fileName.EndsWith(".htm") || fileName.EndsWith(".html")) return "text/html";
            if (fileName.EndsWith(".txt")) return "text/plain";
            if (fileName.EndsWith(".png")) return "image/png";
            if (fileName.EndsWith(".jpeg") || fileName.EndsWith(".jpg") || fileName.EndsWith(".jfif") || fileName.EndsWith(".webp")) return "image/jpeg";
            return "image/jpg";
        }
