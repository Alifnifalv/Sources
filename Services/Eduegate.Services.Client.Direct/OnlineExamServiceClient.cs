...
namespace Eduegate.Services.Client.Direct
{
    public class OnlineExamServiceClient : IOnlineExamService
    {
        ...
        {
            return service.GetQuestionGroupByExamIDs(subjectIDs);
        }

        public string GetQuestionsByMarks(int marks, int subjectID, int classID, bool isPassage)
        {
            return service.GetQuestionsByMarks(marks, subjectID, classID, isPassage);
        }

        public List<string> GetQuestionDetails(int subjectID, int classID)
        {
            return service.GetQuestionDetails(subjectID, classID);
        }
    }
}