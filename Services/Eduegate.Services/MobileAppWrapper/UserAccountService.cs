...
namespace Eduegate.Services.MobileAppWrapper
{
    public class UserAccountService : BaseService, IUserAccountService
    {
        ...
        public OperationResultDTO RegisterUserDevice(string token, string userType)
        {
            ...
            var excludedUserID = new Domain.Setting.SettingBL(CallContext).GetSettingValue<long?>("USEREXCLUDED", null);
            var isExcludeDeviceChecking = new Domain.Setting.SettingBL(CallContext).GetSettingValue<bool>("IS_EXCULDE_DEVICE_CHECKING", false);

            var UserDeviceMapID = new UserServiceBL(CallContext).RegisterUserDevice(token, userType);

            // Check if the user is active based on userType
            if (userType == "Staff" && !isExcludeDeviceChecking && CallContext.LoginID != excludedUserID && UserDeviceMapID.HasValue)
            {
