...
namespace Eduegate.Services
{
    public class OnlineExamService : BaseService, IOnlineExamService
    {
        ...
        public List<KeyValueDTO> GetQuestionGroupByExamIDs(List<int> subjectIDs)
        {
            return OnlineExamsMapper.Mapper(CallContext).GetQnGroupDetailsByID(subjectIDs);
        }

        public string GetQuestionsByMarks(int marks, int subjectID, int classID, bool isPassage)
        {
            return OnlineExamsMapper.Mapper(CallContext).GetQuestionsByMarks(marks, subjectID, classID, isPassage);
        }

        public List<string> GetQuestionDetails(int subjectID, int classID)
        {
            return OnlineExamsMapper.Mapper(CallContext).GetQuestionDetails(subjectID, classID);
        }
