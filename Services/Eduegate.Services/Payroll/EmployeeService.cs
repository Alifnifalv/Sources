...
namespace Eduegate.Services.Payroll
{
    public class EmployeeService : BaseService, IEmployeeService
    {
        ...
        {
            return new EmployeeBL(CallContext).GetEmployeeByBranch(branchID);
        }
        ...
        {
            return EmployeeRoleMapper.Mapper(CallContext).GetEmployeesByDepartmentBranch(departmentID, branchID);
        }
        ...
        {
            return SalarySlipMapper.Mapper(CallContext).GetSalarySlipByID(salarySlipID);
        }
        public SectorTicketAirfareDetailDTO GetSectorTicketAirfare(SectorTicketAirfareDTO sectorTicketAirfareDTO)
        {
            return SectorTicketAirfareMapper.Mapper(CallContext).GetSectorTicketAirfare(sectorTicketAirfareDTO);
        }
        public TicketEntitilementEntryDTO GetAirfareEntry(TicketEntitilementEntryDTO ticketEntitilementEntryDTO)
        {
            return EmployeeTicketEntitilementEntryMapper.Mapper(CallContext).GetAirfareEntry(ticketEntitilementEntryDTO);
        }
        public OperationResultWithIDsDTO GenerateLeaveSalaryProvision(EmployeeLSProvisionHeadDTO lsProvisionDTO)
        {
            return LSProvisionMapper.Mapper(CallContext).GenerateLeaveSalaryProvision(lsProvisionDTO);
        }
        public OperationResultWithIDsDTO GenerateESBProvision(EmployeeESBProvisionHeadDTO esbProvisionDTO)
        {
            return ESBProvisionMapper.Mapper(CallContext).GenerateESBProvision(esbProvisionDTO);
        }
        public string UpdateESBAccountPostStatus(EmployeeESBProvisionHeadDTO dto)
        {
            return ESBProvisionMapper.Mapper(CallContext).UpdateESBAccountPostStatus(dto);
        }
        public string UpdateLSAccountPostStatus(EmployeeLSProvisionHeadDTO dto)
        {
            return LSProvisionMapper.Mapper(CallContext).UpdateLSAccountPostStatus(dto);
        }
    }
}