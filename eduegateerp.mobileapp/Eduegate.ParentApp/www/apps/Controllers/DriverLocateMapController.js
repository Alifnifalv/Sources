app.controller("DriverLocationController", ["$scope", "$http", "rootUrl", "$location", "GetContext", "$state", "$stateParams", "$rootScope", "loggedIn", "$sce", "$timeout", "$translate", function ($scope, $http, rootUrl, $location, GetContext, $state, $stateParams, $rootScope, loggedIn, $sce, $timeout, $translate) {
    ...
    $http({
      ...
      if (!result.data) {
        ...
        var mapOptions = {
          ...
          streetViewControl: false,   // Keep fullscreen control (optional)
        };
        var noPoi = [
          {
            featureType: "poi",
  ...
  $scope.GetStudentInOutVehicleStatus = function () {
    $http({
      url:
        dataService +
        '/GetStudentInOutVehicleStatus?studentID=' + $scope.StudentID,
      method: "GET",
      headers: {
        Accept: "application/json;charset=UTF-8",
        "Content-type": "application/json; charset=utf-8",
  ...
  $scope.GetStudentInOutVehicleStatus();
