  ...
  function (
    ...
    $scope.LoadAttendenceData = function () {
      ...
      $.ajax({
        ...
        url:
          dataService +
          "/GetAcademicCalenderByMonthYear?month=" +
          ($scope.SelectedMonth + 1) +
          "&year=" +
          $scope.SelectedYear,
        ...
        complete: function (result) {
          $.ajax({
            ...
            url:
              dataService +
              "/GetStudentAttendenceByDayClassSection?month=" +
              $scope.SelectedMonth +
              "&year=" +
              $scope.SelectedYear +
              "&day=" +
    ...
    $scope.LoadAttendence = function () {
      ...
      $.ajax({
        url:
          dataService +
          "/GetClasswiseStudentData?classId=" +
          classId +
          "&sectionId=" +
          sectionId,
        type: "GET",
            ...
            $scope.$apply(function () {
              angular.forEach(result3, function (student) {
                ...
                if ($scope.AttendenceData != null) {
                  var existingData = $scope.AttendenceData.find(
                    (a) =>
                      a.StudentID == student.StudentIID &&
                      i == moment(a.AttendenceDate).format("DD") &&
                      $scope.SelectedMonth + 1 ==
                        moment(a.AttendenceDate).format("M") &&
                      $scope.SelectedYear ==
                        moment(a.AttendenceDate).format("YYYY")
                  );
                }
                if ($scope.HoliDayData != null) {
                  var holiDay = $scope.HoliDayData.find(
                    (h) =>
                      i == h.Day &&
                ...
                var actualDate = moment(
                  i.toString() +
                    "-" +
                    $scope.Months[$scope.SelectedMonth] +
                    "-" +
                    $scope.SelectedYear.toString(),
                  "D-MMMM-YYYY"
                ).toDate();
                ...
                var dateValue = parseInt(
                  student.AdmissionDate.replace(/\/Date\((\d+)\)\//g, "$1")
                );
                var admissionDate = new Date(dateValue);

                var feeStartDateString = moment(student.FeeStartDate).format(
                  "MM/DD/YYYY"
                );
                var feeStartDate = moment(student.FeeStartDate).toDate(); // Use Moment's parsing

                var dataStatus = false;
                var PresentStatusID = 9;
                ...
                student.attendances.push({
                  date:
                    i.toString() +
                    "/" +
                    ($scope.SelectedMonth + 1).toString() +
                    "/" +
                    $scope.SelectedYear.toString(),
                  ...
                  studentName:
                    ...
                    " " +
                    student.LastName,
                  StatusDescription: $scope.PresentStatuses.find(
                    (a) => a.PresentStatusID == PresentStatusID
                  ).StatusDescription,
                  statusTitle: $scope.PresentStatuses.find(
                    (a) => a.PresentStatusID == PresentStatusID
                  ).StatusTitle,
                  reason:
                  ...
                  feeStartDate: moment(feeStartDate).toDate(), // Use Moment's parsing
                });
                //}
                if (
                  !$scope.Students.find(
                    (s) => s.StudentIID === student.StudentIID
                  )
                ) {
    ...
    $scope.SendAttendanceNotificationsToParents = function () {
      ...
      $.ajax({
        url:
          dataService +
          "/SendAttendanceNotificationsToParents?classId=" +
          classId +
          "&sectionId=" +
          sectionId,
        type: "POST",
