...
function ($scope, $http, loggedIn, rootUrl, $location, GetContext, $state, $stateParams, $sce, $rootScope,
    ...
    $scope.GetProducts = function (attachwaypoints) {
        ...
        ajaxHandler = $.ajax({
            ...
            type: 'GET',
            data: {
                pageIndex: $scope.lazyLoadPageNumber, pageSize: $scope.pageSize, searchText: $scope.CategorySearchText,
                searchVal: $scope.filterVal, searchBy: $scope.filterBy, sortBy: $scope.SelectedSortValue,
            ...
            success(result) {
                if (result.WarningMessage && result.PageIndex === 1) {
                    $timeout(function () {
                        $rootScope.showModalWindow("notification",
                ...
                $timeout(function () {
                    $scope.$apply(function () {
                        ...
                        if (result.CatalogGroups != null && result.CatalogGroups != undefined && result.CatalogGroups.length > 0) {
                            $.each(result.CatalogGroups, function (index, item) {
                            ...
                            if ($scope.IsFacetLoad) {
                                $scope.ProductListViewModel = result;
                                $.each($scope.ProductListViewModel.FacetItems, function (index, itemFacet) {
