app.controller('StudentAttendanceController', ['$scope', '$http', '$state', 'rootUrl', '$location', '$rootScope', '$stateParams', 'GetContext', '$sce', "$timeout", function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
    ...
    $scope.SelectedDate = { SelectedYear: new Date().getFullYear().toString(), SelectedMonth: new Date().getMonth().toString(), SelectedDay: new Date().getDate().toString() }
    ...
    $scope.GetListData = function () {
        ...
        $http({
            ...
            $scope.PresentStatuses.forEach(x => {
                if (x.StatusTitle !== "H" && x.StatusTitle !== "W" && x.StatusTitle !== "LA" && x.StatusTitle !== "UM" && x.StatusTitle !== "P" && x.StatusTitle !== "A") attnStatuses.push({
            ...
            $scope.PresentStatuses.forEach(x => {
                if ( x.StatusTitle === "P" || x.StatusTitle === "A") attnStatusesFiltered.push({
                    'PresentStatusID': x.PresentStatusID,
                    'StatusDescription': x.StatusDescription,
                    'StatusTitle': x.StatusTitle,
                })
    ...
    $scope.LoadDropdownList = function () {
        ...
        $http({
            method: 'GET',
            url: dataService + '/GetDynamicLookUpDataForMobileApp?lookType=' + 'PresentStatus' + '&defaultBlank=' + 'false',
            headers: {
                "Accept": "application/json;charset=UTF-8",
                "Content-type": "application/json; charset=utf-8",
                "CallContext": JSON.stringify(context)
        ...
        $http({
            method: 'GET',
            url: dataService + '/GetDynamicLookUpDataForMobileApp?lookType=' + 'AttendenceReason' + '&defaultBlank=' + 'false',
            headers: {
                "Accept": "application/json;charset=UTF-8",
                "Content-type": "application/json; charset=utf-8",
                "CallContext": JSON.stringify(context)
    ...
    $scope.ClassSectionChange = function () {
        ...
        var url = dataService + "/GetClassStudents?classId=" + classId + "&sectionId=" + sectionId;
    ...
    $scope.getStudentAttendance = function () {
        ...
        $scope.SelectedDay = parseInt($("#attendanceDay").val());
    ...
    $scope.LoadAttendenceData = function () {
        ...
        $.ajax({
            ...
            //     $.ajax({
            //         type: 'GET',
            //         url: dataService + "/GetStudentAttendenceByYearMonthClassSection?month=" + $scope.SelectedMonth + '&year=' + $scope.SelectedYear + '&classId=' + $scope.selectedClass + '&sectionId=' + $scope.selectedSection,
            //         success: function (result) {
            ...
            complete: function (result) {
                $.ajax({
                    type: 'GET',
                    url: dataService + "/GetStudentAttendenceByDayClassSection?month=" + $scope.SelectedMonth + '&year=' + $scope.SelectedYear + '&day=' + $scope.SelectedDay + '&classId=' + $scope.selectedClass + '&sectionId=' + $scope.selectedSection,
                    success: function (result) {
                        $scope.AttendenceData = result;
                    },
                    complete: function (result) {
    ...
    $scope.LoadAttendence = function () {
        ...
        $scope.Students = [];
        $.ajax({
            url: dataService + "/GetClasswiseStudentData?classId="
                ...
                if (!result3.IsError && result3 != null) {
                    $scope.$apply(function () {
                        angular.forEach(result3, function (student) {
                            student.attendances = [];
                            //for (var i = 1; i <= $scope.SelectedMonthDate; i++) {
                                ...
                                var obj = { admissionDate: student.AdmissionDate };

                                var dateValue = parseInt(student.AdmissionDate.replace(/\/Date\((\d+)\)\//g, "$1"));
                                var admissionDate = new Date(dateValue);

                                var feeStartDateString = moment(student.FeeStartDate).format("MM/DD/YYYY");
                                ...
                                student.attendances.push({
                                    ...
                                    StatusDescription: $scope.PresentStatuses.find(a => a.PresentStatusID == PresentStatusID).StatusDescription,
                                    ...
                                    // ColorCode: (typeof (holiDay) == 'undefined' || holiDay == null) ? '' : holiDay.ColorCode,
                    ...
                    $rootScope.ShowLoader = false;
                    // $rootScope.ToastMessage = "No Class found!";
    ...
    function saveAttendence() {
        ...
        if ($scope.PresentStatus == null) {
            ...
            var attendenceDate = moment($scope.CurrentAttendence.actualDate).format("DD/MM/YYYY");
            ...
            $http({
                ...
                data: {
                    ...
                    'AttendanceReasonID': $scope.PStatus == 0 ? $scope.AttendenceReason : null,
                    'Reason': $scope.Reason,
