...
namespace Eduegate.Domain.Repository
{
    public class AccountRepository : IAccountRepository
    {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return dbContext.Logins
                        ...
                        .Include(b => b.UserSettings)
                        .Where(a => a.LoginIID == loginID).FirstOrDefault();
                }
            }
            catch (Exception)
            {
        ...
        public Login GetEmailDetails(string LoginEmailID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var repository = new EntiyRepository<Login, dbEduegateERPContext>(dbContext);
                var loginDetail = repository.Get(a => a.LoginEmailID == LoginEmailID).LastOrDefault();
                return loginDetail;
            }
        }
        ...
        public Login GetEmail(string UserID)
        {
            ...
            {
                var repository = new EntiyRepository<Login, dbEduegateERPContext>(dbContext);
                var loginDetail = repository.Get(a => a.LoginUserID == UserID && a.StatusID == activeStatus).LastOrDefault();
                return loginDetail;
            }
        }
        ...
        public Login GetEmployeeDetails(string LoginEmailID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var loginrepository = new EntiyRepository<Login, dbEduegateERPContext>(dbContext);
                var loginDetail = loginrepository.Get(a => a.LoginEmailID == LoginEmailID).LastOrDefault();

                var employeerepository = new EntiyRepository<Employee, dbEduegateERPContext>(dbContext);
                var employeeDetail = loginDetail != null ? employeerepository.Get(a => a.LoginID == loginDetail.LoginIID).LastOrDefault() : null;
        ...
        public Login RegisterUser(Login loginDetails)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    {
                        dbContext.Entry(loginDetails).State = System.Data.Entity.EntityState.Modified;
                    }
        ...
        public Login SaveLogin(Login login)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    dbContext.LoginRoleMaps.RemoveRange(dbContext.LoginRoleMaps.Where(a => a.LoginID == login.LoginIID).ToList());
                    ...
                    foreach (var customer in login.Customers.ToList())
                    {
                        ...
                        {
                            dbContext.Entry(customer).State = System.Data.Entity.EntityState.Detached;
                            var customerEntity = dbContext.Customers.Where(a => a.CustomerIID == customer.CustomerIID).FirstOrDefault();
                            dbContext.Entry(customerEntity).State = System.Data.Entity.EntityState.Modified;
                    ...
                    foreach (var supplier in login.Suppliers.ToList())
                    {
                        ...
                        {
                            dbContext.Entry(supplier).State = System.Data.Entity.EntityState.Added;
                        }
                        else
                        {
                            dbContext.Entry(supplier).State = System.Data.Entity.EntityState.Detached;
                            var supplierEntity = dbContext.Suppliers.Where(a => a.SupplierIID == supplier.SupplierIID).FirstOrDefault();
                            dbContext.Entry(supplierEntity).State = System.Data.Entity.EntityState.Modified;
                    ...
                    {
                        dbContext.ClaimSetLoginMaps.RemoveRange(dbContext.ClaimSetLoginMaps.Where(a => a.LoginID == login.LoginIID).ToList());
                    }

                    if (login.ClaimLoginMaps != null)
                    {
                        dbContext.ClaimLoginMaps.RemoveRange(dbContext.ClaimLoginMaps.Where(a => a.LoginID == login.LoginIID).ToList());
                    }
        ...
        public Common UpdatePassword(Login loginDetails)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    var loginDet = dbContext.Logins.FirstOrDefault(x => x.LoginIID == loginDetails.LoginIID);
        ...
        public Login GetLoginDetailByLoginID(long loginId)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Logins.Where(x => x.LoginIID == loginId).FirstOrDefault();
            }
        }
        ...
        public Login GetUserByLoginEmail(string loginEmailId)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var user = dbContext.Logins.Where(u => u.LoginEmailID == loginEmailId).FirstOrDefault();
                if (user.IsNotNull())
                {
        ...
        public Login GetUserDetailsByID(long loginIID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var user = dbContext.Logins
                    .Where(a => a.LoginIID == loginIID).FirstOrDefault();
        ...
        public bool Login(string loginEmailId, string password, byte AppId)
        {
            ...
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                else if (AppId == (int)ApplicationType.ERP || AppId == (int)ApplicationType.StudentMobileApp)
                {
                    bool isSuccess = (from u in db.Logins
                                      join rm in db.LoginRoleMaps on u.LoginIID equals rm.LoginID
                                      where u.LoginEmailID == loginEmailId && u.Password == password && u.StatusID == activeStatus
        ...
        public bool isValideAccount(string loginEmailId)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var loginid = (from u in dbContext.Logins
                               where u.LoginEmailID == loginEmailId
                               select u.LoginIID
                           ).FirstOrDefault();

                var valideStudent = (from s in dbContext.Students
        ...
        public Customer GetCustomerDetails(long loginID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Customers
                    ...
                    .Where(x => x.LoginID == loginID).FirstOrDefault();
        ...
        public List<Contact> GetContacts(long loginID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Contacts.Where(x => x.LoginID == loginID).ToList();
            }
        }
        ...
        public bool UpdateContact(ContactDTO contact)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                Contact contactEntity = dbContext.Contacts.Where(x => x.ContactIID == contact.ContactID).FirstOrDefault();
        ...
        public bool RemoveContact(long contactID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                Contact contact = dbContext.Contacts.Where(x => x.ContactIID == contactID).FirstOrDefault();
        ...
        public List<Contact> GetContactSummaryList(long loginID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var summary = from c in dbContext.Contacts
        ...
        public Contact GetContactDetail(long contactID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Contacts.Where(x => x.ContactIID == contactID).FirstOrDefault();
            }
        }
        ...
        public bool IsPasswordResetRequired(string emailID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var loginAccount = dbContext.Logins.Where(x => x.LoginEmailID.ToLower() == emailID.Trim().ToLower()).ToList();
                if (loginAccount.Any())
                {
                    loginAccount.All(a =>
                    {
        ...
        public List<Role> GetRoles()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Roles.OrderBy(a => a.RoleName).ToList();
            }
        }
        ...
        public List<KnowHowOption> GetKnowHowOptions()
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.KnowHowOptions.ToList();
            }
        }
        ...
        public bool isCustomerConfirmed(long customerID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.CustomerSettings.Where(a => a.CustomerID == customerID && a.IsConfirmed == true).Any();
            }
        }
        ...
        public long GetCustomerIDbyLoginID(long loginID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.Customers.Where(a => a.LoginID == loginID).Select(b => b.CustomerIID).FirstOrDefault();
            }
        }
        ...
        public long GetLoginIDbyCustomerID(long customerID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var loginID = db.Customers.Where(a => a.CustomerIID == customerID).Select(b => b.LoginID).FirstOrDefault();
        ...
        public bool SaveWebsiteOrderAddress(long contactID, bool sameBillingAddress, long loginID, long headID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                var contact = db.Contacts.Where(a => a.ContactIID == contactID).FirstOrDefault();
                if (contact != null)
                {
                    ...
                    if (!sameBillingAddress)
                    {
                        ...
                        var billingcontact = db.Contacts.Where(a => a.LoginID == loginID && a.IsBillingAddress == true).FirstOrDefault();
        ...
        public CustomerSetting GetCustomerSettings(long customerID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.CustomerSettings.Where(c => c.CustomerID == customerID).SingleOrDefault();
            }
        }
        ...
        public bool CheckCustomerEmailIDAvailability(long loginID, string loginEmailID)
        {
            ...
            using (var db = new dbEduegateERPContext())
            {
                ...
                if (isAvailibility && loginID > 0)
                {
                    ...
                    var login = db.Logins.Where(x => x.LoginIID == loginID).FirstOrDefault();
        ...
        public bool CheckStudentEmailIDAvailability(long loginID, string EmailID)
        {
            ...
            using (var db = new dbEduegateERPContext())
            {
                isAvailibility = db.Logins.Any(x => x.LoginEmailID == EmailID);
        ...
        public Account GetAccountDetails(long accountID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.Accounts.Where(x => x.AccountID == accountID).FirstOrDefault();
            }
        }
        ...
        public short NotifyMe(Notify entity)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (dbContext.Notifies.Where(x => x.EmailID == entity.EmailID && x.ProductSKUMapID == entity.ProductSKUMapID && x.SiteID == entity.SiteID).Any() == false)
                {
        ...
        public Login GetUserByLoginUserID(string userID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var user = dbContext.Logins.Where(u => u.LoginUserID == userID).FirstOrDefault();
                if (user.IsNotNull())
                {
                    dbContext.Entry(user).Collection(a => a.ClaimSetLoginMaps).Load();
                    dbContext.Entry(user).Collection(a => a.ClaimLoginMaps).Load();
        ...
        public Login GetUserByMobileNumber(string mobileNumber)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var user = dbContext.Customers
                    ...
                    .Select(c => c.Login).FirstOrDefault();
        ...
        public bool IsRoleExists(long loginID, int roleID)
        {
            try
            {
                using (var db = new dbEduegateERPContext())
                {
                    return db.LoginRoleMaps.Any(x => x.LoginID == loginID && x.RoleID == roleID);
                }
            }
            catch
            {
        ...
        public Student GetStudentDetails(long studentID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Students.FirstOrDefault(x => x.StudentIID == studentID);
            }
        }
    }
}