    ...
    public class AccountTransactionRepository
    {
        ...
        public CustomerAccountMap GetCustomerAccountMap(long CustomerID, int? EntitlementID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                return dbContext.CustomerAccountMaps.Where(x => x.CustomerID == CustomerID).FirstOrDefault();
            }
        }
        ...
        public SupplierAccountMap GetSupplierAccountMap(long SupplierID, int EntitlementID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                return dbContext.SupplierAccountMaps.Where(x => x.SupplierID == SupplierID).FirstOrDefault();
            }
        }
            ...
            try
            {
                using (var dbContext = new dbEduegateERPContext())
                {
                    foreach (var accountTransactionItem in transactionList)
                    {
                        ...
                        dbContext.Entry(accountTransactionItem).State = System.Data.Entity.EntityState.Added;
                        dbContext.SaveChanges();
        ...
        public bool UpdateAccountTransactionProcessStatus(TransactionHead TransactionHeadDBModel)
        {
            ...
            using (var dbContext = new dbEduegateERPContext())
            {
                var TransactionHeadFromDB = dbContext.AccountTransactionHeads.Where(a => a.AccountTransactionHeadIID == TransactionHeadDBModel.HeadIID).FirstOrDefault();
        ...
        public bool Update_AssetTransaction_ProcessStatus(long HeadID, int TransactionStatus)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    AssetTransactionHead TransactionHeadFromDB = dbContext.AssetTransactionHeads.Where(a => a.HeadIID == HeadID).FirstOrDefault();
                    if (TransactionHeadFromDB != null)
                    {
        ...
        public bool Update_AccountingTransaction_ProcessStatus(long HeadID, int TransactionStatus)
        {
            ...
            try
            {
                using (var dbContext = new dbEduegateERPContext())
                {
                    var TransactionHeadFromDB = dbContext.AccountTransactionHeads.Where(a => a.AccountTransactionHeadIID == HeadID).FirstOrDefault();
        ...
        public bool Update_MissionJob_ProcessStatus(long HeadID, int TransactionStatus)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    JobEntryHead TransactionHeadFromDB = dbContext.JobEntryHeads.Where(a => a.JobEntryHeadIID == HeadID).FirstOrDefault();
                    if (TransactionHeadFromDB != null)
                    {
        ...
        public List<InvetoryTransaction> GetInvetoryTransactionsByTransactionHeadID(long TransactionHeadID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.InvetoryTransactions.Where(x => x.HeadID == TransactionHeadID).ToList();
            }
        }
        ...
        public Account SetAccountChildLastID(long AccountID, int noOfChildAccounts)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    Account baseAccount = dbContext.Accounts.Where(x => x.AccountID == AccountID).FirstOrDefault();
                    dbContext.Entry(baseAccount).Reference(a => a.Group).Load();
                    dbContext.Entry(baseAccount).Reference(a => a.Account1).Load();
        ...
        public List<TransactionHeadEntitlementMap> GetTransactionEntitlementByHeadId(long headId)
        {
            using (var db = new dbEduegateERPContext())
            {
                return db.TransactionHeadEntitlementMaps.Where(x => x.TransactionHeadID == headId)
                     .Include(x => x.EntityTypeEntitlement).ToList();
            }
        }
        ...
        public AccountTransaction GetDayBookData(long IID)
        {
            using (dbEduegateERPContext dbcontext = new dbEduegateERPContext())
            {
                return dbcontext.AccountTransactions.Where(x => x.TransactionIID == IID).FirstOrDefault();
            }
        }
        ...
        public AccountTransactionHead SaveAccountTransactionHead(AccountTransactionHead accountTransactionHead)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    var transactionHeadInDB = dbContext.AccountTransactionHeads.Where(x => x.AccountTransactionHeadIID == HeadIID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        foreach (AccountTransactionDetail detailItem in accountTransactionHead.AccountTransactionDetails)
                        {
                            ...
                            var accountTransactionDetailInDb = transactionHeadInDB.AccountTransactionDetails.Where(x => x.AccountTransactionDetailIID == detailItem.AccountTransactionDetailIID).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                accountTransactionDetailInDb.DepartmentID = detailItem.DepartmentID;
        ...
        public List<AdditionalExpensesTransactionsMapDTO> GetAdditionalExpensesTransactions(List<AdditionalExpensesTransactionsMapDTO> additionalExpenseData, long accountTransactionHeadIID, long referenceID)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    var additionalTransdata = dbContext.AdditionalExpensesTransactionsMaps.Where(x => x.RefInventoryTransactionHeadID == referenceID).ToList();
                    if (additionalTransdata.Count() > 0)
                    {
                        ...
                        var AdditionalExpList = dbContext.AdditionalExpenses.Where(x => lstAdditionalExp.Contains(x.AdditionalExpenseID)).ToList();
                        var ProvisionalAcntList = dbContext.Accounts.Where(x => lstProvisionalAcnt.Contains(x.AccountID)).ToList();
                        var CurrencyList = dbContext.Currencies.Where(x => lstCurrency.Contains(x.CurrencyID)).ToList();
        ...
        public AccountTransactionHead GetAccountTransactionHeadById(long HeadID)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    .Include("AccountTransactionHeadAccountMaps")
                    //.Include("DocumentReferenceStatusMap")
                    //.Include("DocumentReferenceStatusMap.DocumentStatus")
                    //.Include("DocumentReferenceStatusMap1")
                  
                    .Include(x => x.Account)
                    .Include(x => x.DocumentType)
                    .Include(x => x.DocumentStatus)
                    .Include("AccountTransactionDetails.Budget")
                    .Include("TransactionStatus")
                    .Include("CostCenter")
                    .Include("Currency")
                    .Include("AccountTransactionDetails.Account")
                    .Include("AccountTransactionDetails.CostCenter")
                    .Include("AccountTransactionDetails.Department")
                    .Include("AccountTransactionDetails.Accounts_SubLedger")
                    .Include("AccountTransactionDetails.ProductSKUMap")
                    .Include("AccountTransactionDetails.Currency")
                    .Include("Account")
                    .Include("AccountTaxTransactions")
                    .Include("AccountTaxTransactions.TaxTemplate")
                    //.Include("AccountTaxTransactions.TaxTemplateItem")
                    //.Include("AccountTaxTransactions.TaxType")
                    .Where(x => x.AccountTransactionHeadIID == HeadID).FirstOrDefault();
        ...
        public void SavePayables(List<Payable> payables)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    dbContext.Entry(payable).State = System.Data.Entity.EntityState.Modified;
                }
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                foreach (var receivable in receivables)
                {
                    if (receivable.ReceivableIID != 0)
                        dbContext.Entry(receivable).State = System.Data.Entity.EntityState.Modified;
                    else
                        dbContext.Entry(receivable).State = System.Data.Entity.EntityState.Added;
                }
        ...
        public List<Receivable> GetMissionReceivablesByAccountId(long AccountID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Receivables
                                    ...
                                    .Include(x => x.DocumentStatus)
                                    .Include(x => x.DocumentType)
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail")
                                    .Include("TransactionHeadReceivablesMaps")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.JobEntryDetails")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.TransactionHead")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.TransactionHead.TransactionHead1")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.TransactionHead.TransactionHead1.TransactionDetails")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.TransactionHead.TransactionHead1.DocumentType.DocumentReferenceType")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryDetail.JobEntryHead1.TransactionHead.Currency")
                                    .Include("JobsEntryHeadReceivableMaps.JobEntryHead")
                                    .ToList();
            }
        }
        ...
        public List<Receivable> GetInventoryReceivablesByAccountId(long AccountID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Receivables
                                    ...
                                    .Include(x => x.DocumentStatus)
                                    .Include(x => x.DocumentType)
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead")
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead.TransactionDetails")
                                    .Include("JobsEntryHeadReceivableMaps")
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead.TransactionHead1")
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead.TransactionHead1.TransactionDetails")
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead.TransactionHead1.DocumentType.DocumentReferenceType")
                                    .Include("TransactionHeadReceivablesMaps.TransactionHead.Currency")
                                    .ToList();
            }
        }
        ...
        public bool UpdateReceivablesPaidAmount(long? ReceivableId, decimal amount)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    Receivable receivable = dbContext.Receivables.Where(x => x.ReceivableIID == ReceivableId).FirstOrDefault();
                    if (receivable != null)
                    {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    return dbContext.JobEntryHeads
                            .Include("JobEntryDetails")
                            .Include("JobEntryDetails.JobEntryHead1")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead") //--Invoice
                            .Include("JobEntryDetails.JobEntryHead1.JobEntryDetails")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionDetails")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionDetails")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionHead1")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionHead1.TransactionDetails")
                            .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionHead1.DocumentType.DocumentReferenceType")
                            .Include("JobsEntryHeadReceivableMaps")
            ...
            try
            {
                ...
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    JobEntryHeadList = dbContext.JobEntryHeads
                                    ...
                                    .Include("JobEntryDetails")
                                    .Include("JobEntryDetails.JobEntryHead1")
                                    .Include("JobEntryDetails.JobEntryHead1.TransactionHead")
                                    .Include("JobEntryDetails.JobEntryHead1.JobEntryDetails")
                                    .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionDetails")
                                    .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionDetails")
                                    .Include("JobEntryDetails.JobEntryHead1.TransactionHead.TransactionHead1")
        ...
        public Account GetEmployeeAccount(long EmployeeID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var empAccountMap = dbContext.EmployeeAccountMaps.Include("Account").Where(x => x.EmployeeID == EmployeeID).FirstOrDefault();
        ...
        public List<Payable> GetInventoryPayablesByAccountId(long AccountID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Payables
                                    ...
                                    .Include(x => x.TransactionStatus)
                                    .Include(x => x.Currency)
                                    .Include("TransactionHeadPayablesMaps.TransactionHead")
                                    .Include("TransactionHeadPayablesMaps.TransactionHead.TransactionDetails")
                                    .Include("TransactionHeadPayablesMaps")
                                    .Include("TransactionHeadPayablesMaps.TransactionHead.TransactionHead1")
                                    .Include("TransactionHeadPayablesMaps.TransactionHead.TransactionHead1.TransactionDetails")
                                    .Include("TransactionHeadPayablesMaps.TransactionHead.TransactionHead1.DocumentType.DocumentReferenceType")
                                    .Include("TransactionHeadPayablesMaps.TransactionHead.Currency")
                                    .ToList();
            }
        }
        ...
        public List<SupplierAccountMap> GetSuppliersAccount(string searchText)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.SupplierAccountMaps.Include("Account").Where(x => x.EntitlementID == null && (x.Account.AccountName.Contains(searchText) || searchText == null || searchText == "")).ToList();
            }
        }
        ...
        public bool UpdateProductAvgCost(long ProductSKUId, decimal NewAvgCost)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ProductInventory productInventory = dbContext.ProductInventories.Where(x => x.ProductSKUMapID == ProductSKUId).FirstOrDefault();
        ...
        public bool UpdatePayablesPaidAmount(long? PayableId, decimal paidAmount)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    Payable payable = dbContext.Payables.Where(x => x.PayableIID == PayableId).FirstOrDefault();
                    if (payable != null)
                    {
        ...
        public List<EmployeeAccountMap> Get_AllDrivers_Accounts(string searchText)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.EmployeeAccountMaps
                    .Include("Account")
                    .Include("Employee")
                    .Include("Employee.EmployeeRoleMaps")
                    .Where(x => x.Employee.EmployeeRoleMaps.Any(roles => roles.EmployeeRoleID == EmployeeRoleID_DRIVER)
                            && (x.Account.AccountName.Contains(searchText) || searchText == null || searchText == "")).ToList();
            }
        ...
        public List<Account> GetChildAccounts_ByParentAccountId(long ParentAccountID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var childAccounts = dbContext.Accounts.Where(x => x.ParentAccountID == ParentAccountID).ToList();
            ...
            if (code != null)
            {
                ...
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    account = dbContext.Accounts
                        ...
                        .Where(x => x.AccountCode.Equals(code)).FirstOrDefault();
                }
            }
        ...
        public List<PaymentMode> GetPaymentModes()
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                return dbContext.PaymentModes.ToList();
            }
        }
        ...
        public TransactionHead GetParentTransactionHeadByTransHeadID(long headID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                TransactionHead head = dbContext.TransactionHeads.Include("TransactionHead2").Where(x => x.HeadIID == headID).FirstOrDefault();
                if (head != null)
                {
        ...
        public void ClearPostedData(long accountingHeadID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var headDetail = dbContext.AccountTransactionHeads.Where(a => a.AccountTransactionHeadIID == accountingHeadID).FirstOrDefault();
                ...
                {
                    var existingReference = dbContext.Receivables.Where(a => a.ReceivableIID == recievable.ReferenceReceivablesID).LastOrDefault();
