...
namespace Eduegate.Domain.Repository
{
    public class CategoryRepository
    {
        public List<CategoryTag> GetCategoryTags()
        {
            ...
            {
                return dbContext.CategoryTags.ToList<CategoryTag>();
            }
        }
        ...
        public List<Category> GetCategoryByParentID(long? parentID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var categories = dbContext.Categories.Where(a => (long?)a.ParentCategoryID == parentID && a.IsActive == true).ToList<Category>();
        ...
        public List<Category> GetParentCategoryID(long? parentID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var categories = dbContext.Categories.Where(a => (long?)a.ParentCategoryID == null && a.IsActive == true).ToList<Category>();
                //foreach (var category in categories)
                //{
                //    dbContext.Entry(category).Collection(a => a.CategoryImageMaps).Load();
                //}
        ...
        public List<Category> GetCategoryByParentIDWithImage(long? parentID, string cultureCode)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var categories = dbContext.Categories.Where(a => (long?)a.ParentCategoryID == parentID && a.IsActive == true).OrderBy(a => a.CategoryName).ToList<Category>();
                foreach (var category in categories)
                {
                    dbContext.Entry(category).Collection(a => a.CategoryImageMaps).Load();
                }
        ...
        public Category GetCategoryByID(long categoryID, string cultureCode)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var catagory = dbContext.Categories.Where(a => a.CategoryIID == categoryID).FirstOrDefault();
                if (catagory != null)
                {
        ...
        public List<Category> GetSubCategories(long categoryID, string cultureCode)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var categories = dbContext.Categories.Where(a => a.ParentCategoryID == categoryID && a.IsActive == true).ToList();
        ...
        public List<Category> GetSubCategories(long categoryID, List<string> tagName, string cultureCode, int noOfRecords)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                try
                {
                    ...
                    foreach (var category in categories)
                    {
                        category.CategoryImageMaps = dbContext.CategoryImageMaps.Where(x => x.CategoryID == category.CategoryIID).ToList();
                    }
        ...
        public List<Category> GetCategoryList(List<long> categoryIDs)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return (from category in dbContext.Categories
                                  select category).Where(a=> categoryIDs.Contains(a.CategoryIID)).ToList();
            }
        }
        ...
        public List<Category> LoadCategories(string cultureCode, bool isReporting, string searchText)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext()){
                ...
                var maxCount = Eduegate.Framework.Extensions.ConfigurationExtensions.GetAppConfigValueWithDefault<int>("MaxFetchCount", 200);
                ...
                if (searchText.IsNullOrEmpty())
                {
                    if (!isReporting)
                    {
                        categories = (from category in dbContext.Categories
                                      select category).Take(maxCount).ToList();
                    }
                    else
                    {
                        categories = (from category in dbContext.Categories
                ...
                else
                {
                    ...
                    {
                        categories = (from category in dbContext.Categories
                                      select category).Where(a=> a.CategoryName.Contains(searchText)).Take(maxCount).ToList();
                    }
                    else
                    {
                        categories = (from category in dbContext.Categories
        ...
        public List<CategoryImageMap> GetCategoryImage(Eduegate.Services.Contracts.Enums.ImageTypes? imageType, long categoryID, int companyID = 0)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    if (companyID == 0)
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID).OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                    else
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID && a.CompanyID == companyID)
                            .OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                }
                else
                    if (imageType == null)
                {
                    if (categoryID == 0)
                        categoryImageMap = dbContext.CategoryImageMaps.OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                    else
                    if (companyID == 0)
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID).OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                    else
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID && a.CompanyID == companyID).OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                }
                else
                {
                    if (companyID == 0)
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID && a.ImageTypeID == (int)imageType).OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                    else
                        categoryImageMap = dbContext.CategoryImageMaps.Where(a => a.CategoryID == categoryID && a.CompanyID == companyID && a.ImageTypeID == (int)imageType).OrderBy(x => x.SerialNo == null).ThenBy(x => x.SerialNo).ToList();
                }
        ...
        public List<CategorySetting> GetCategorySettings(long categoryID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                ...
                if (categoryID > 0)
                {
                    var categorySettingList = db.CategorySettings.Where(x => x.CategoryID == categoryID).ToList();
        ...
        public CategorySetting GetCategorySettings(long companyID, string categoryCode, string settingCode)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.CategorySettings.Where(x => x.Category.CategoryCode == categoryCode && x.SettingCode == settingCode).FirstOrDefault();
            }
        }
    }
}
