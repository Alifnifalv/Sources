...
namespace Eduegate.Domain.Repository
{
    public class CertificateRepository
    {
        ...
        public CertificateTemplate SaveCertificateTemplate(CertificateTemplate entity)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var existing = dbContext.CertificateTemplates.Where(a => a.ReportName == entity.ReportName && a.CertificateName == entity.CertificateName).FirstOrDefault();
        ...
        public List<CertificateLog> GetCertificateLogDetail(long loginID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var certificateLog = (from CertificateLog in dbContext.CertificateLogs select CertificateLog).ToList();
        ...
        public List<CertificateTemplate> GetCertificateTemplateDetail(string reportName)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var CertificateTemp = (from CertificateTemplate in dbContext.CertificateTemplates
                        ...
                        select CertificateTemplate).ToList();
        ...
        public CertificateTemplate GetCertificateTemplate(string reportName)
        {
            ...
            {
                var certificateTemp = (from certificateTemplate in dbContext.CertificateTemplates where certificateTemplate.ReportName.Equals(reportName) select certificateTemplate).FirstOrDefault();
                if (certificateTemp == null)
        ...
        public CertificateTemplate GetCertificateTemplateByID(long masterId)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var certificateTemp = (from certificateTemplate in dbContext.CertificateTemplates where certificateTemplate.CertificateTemplateIID.Equals(masterId) select certificateTemplate).FirstOrDefault();
                if (certificateTemp == null)
                    return new CertificateTemplate();
                else
                    return certificateTemp;
        ...
        public CertificateLog GetCertificateLogByID(long masterId)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var certificateLog = (from CertificateLog in dbContext.CertificateLogs where CertificateLog.CertificateLogIID.Equals(masterId) select CertificateLog).FirstOrDefault();
                if (certificateLog == null)
                    return new CertificateLog();
                else
                    return certificateLog;
