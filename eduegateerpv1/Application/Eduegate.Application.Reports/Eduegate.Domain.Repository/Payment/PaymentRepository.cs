...
namespace Eduegate.Domain.Repository.Payment
{
    public class PaymentRepository
    {
            ...
            try
            {
                ...
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    ...
                    PaymentDetailsKnet paymentDetailUpdate = dbContext.PaymentDetailsKnets.Where(
                        ...
                        && x.InitStatus.Equals(paymentStatus.ToString())).FirstOrDefault();
        ...
        public PaymentDetailsKnet GetKnetTrackID(long customerID, long paymentID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                if (paymentID > 0)
                {
                    ...
                    return dbContext.PaymentDetailsKnets.Where(x => x.PaymentID == paymentID && x.CustomerID == customerID).SingleOrDefault();
                }
                else
                {
        ...
        public PaymentDetailsKnet GetPaymentDetailKNET(long orderID, long trackID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                if (orderID > 0)
                {
                    ...
                    return dbContext.PaymentDetailsKnets.Where(x => x.OrderID == orderID).SingleOrDefault();
                }
                else
                {
                    Eduegate.Logger.LogHelper<PaymentRepository>.Info("getting payment detail by trackid:" + trackID);
                    return dbContext.PaymentDetailsKnets.Where(x => x.TrackID == trackID).SingleOrDefault();
                }
            }
        }
        ...
        public long GetKnetDetails(long appKey)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.PaymentDetailsKnets.Where(a => a.AppKey == appKey).Select(a => a.TrackID).FirstOrDefault();
            }
        }
        ...
        public PaymentMasterVisa GetPaymentDetailMIGS(long orderID, long trackID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (trackID > 0)
                {
                    return dbContext.PaymentMasterVisas.Where(x => x.TrackIID == trackID).SingleOrDefault();
                }
                else
                {
                    return dbContext.PaymentMasterVisas.Where(x => x.OrderID == orderID).SingleOrDefault();
                }
            }
        }
        #endregion
        ...
        public PaymentDetailsPayPal GetPayPalPaymentDetail(long trackID, long trackKey, long customerID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                return dbContext.PaymentDetailsPayPals.Where(x => x.RefCustomerID == customerID && x.TrackID == trackID && x.TrackKey == trackKey).FirstOrDefault();
            }
        }
        ...
        public PaymentDetailsPayPal GetPaymentDetailPaypal(long orderID, long trackID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (trackID > 0)
                {
                    return dbContext.PaymentDetailsPayPals.Where(x => x.TrackID == trackID).SingleOrDefault();
                }
                else
                {
                    return dbContext.PaymentDetailsPayPals.Where(x => x.OrderID == orderID).SingleOrDefault();
                }
            }
        }
        ...
        public PaymentDetailsKnet GetPaymentKnetDetails(long orderID, long trackID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (trackID > 0)
                {
                    return dbContext.PaymentDetailsKnets.Where(x => x.TrackID == trackID).FirstOrDefault();
                }
                else
                {
                    return dbContext.PaymentDetailsKnets.Where(x => x.OrderID == trackID).FirstOrDefault();
                }
            }
        ...
        public PaymentMasterVisa GetMasterVisaDetails(long orderID, long trackID = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (trackID > 0)
                {
                    return dbContext.PaymentMasterVisas.Where(x => x.TrackIID == trackID).FirstOrDefault();
                }
                else
                {
                    return dbContext.PaymentMasterVisas.Where(x => x.OrderID == trackID).FirstOrDefault();
                }
            }
        ...
        public PaymentDetailsTheFort GetPaymentDetailTheFort(long orderID, long trackKey = default(long))
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (trackKey > 0)
                {
                    return dbContext.PaymentDetailsTheForts.Where(x => x.TrackKey == trackKey).SingleOrDefault();
                }
                else
                {
                    return dbContext.PaymentDetailsTheForts.Where(x => x.OrderID == orderID).SingleOrDefault();
                }
            }
        }
        ...
        public long GetFortCustomerID(string trackKey, string email)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (dbContext.PaymentDetailsTheForts.Where(a => a.PMerchantReference == trackKey && a.PCustomerEmail == email).Any())
                {
                    return dbContext.PaymentDetailsTheForts.Where(a => a.PMerchantReference == trackKey && a.PCustomerEmail == email).Select(a => a.CustomerID).FirstOrDefault();
                }
                else
                {
                    return 0;
        ...
        public PaymentDetailsTheFort GetPaymentDetailsTheFortByMerchantReference(string trackkey)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.PaymentDetailsTheForts.Where(a => a.PMerchantReference == trackkey).FirstOrDefault();
            }
