    ...
    public class StaticContentRepository
    {
        public List<StaticContentType> GetStaticContentTypes()
        {
            ...
            try
            {
                ...
                {
                    entity = dbContext.StaticContentTypes.OrderBy(a => a.ContentTypeName).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public StaticContentData SaveContent(StaticContentData contentData, CallContext callContext)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                {
                    StaticContentData dbContent = dbContext.StaticContentDatas.Where(x => x.ContentDataIID == contentData.ContentDataIID).FirstOrDefault();
        ...
        public StaticContentData GetStaticContent(long contentID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var staticContent = dbContext.StaticContentDatas.Where(x => x.ContentDataIID == contentID).FirstOrDefault();
        ...
        public long GetDefaultSKUID(long productID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var sku = dbContext.ProductSKUMaps.Where(x => x.ProductID == productID && x.Sequence == 1).FirstOrDefault();
                if (sku != null)
                {
        ...
        public StaticContentType GetStaticContentTypes(int ContentTypeID)
        {
            ...
            StaticContentType staticContentType = db.StaticContentTypes.Where(x => x.ContentTypeID == ContentTypeID).FirstOrDefault();
        ...
        public List<StaticContentData> GetStaticContentData(Eduegate.Services.Contracts.Enums.StaticContentTypes staticPageTypes, int pageSize, int pageNumber)
        {
            ...
            if (Eduegate.Services.Contracts.Enums.StaticContentTypes.All == staticPageTypes)
            {
                ...
                .OrderByDescending(x => x.ContentDataIID).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
            }
            else
            {
