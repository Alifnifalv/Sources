...
namespace Eduegate.Domain.Repository
{
    public class UtilityRepository 
    {
        ...
        public static Language GetLanguageCultureId(string languageCode)
        {
            ...
            {
                return dbContext.Languages.Include("Culture")
        ...
        public static List<Language> GetLanguages()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Languages.Include("Culture").Where(x => x.IsEnabled == true).ToList();
            }
        }
        ...
        public static List<KeyValuePair<string, string>> GetCurrencies(int companyID)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var query = from c in dbContext.Currencies
                            ...
                            select c;
                query.ToList().ForEach(x => currencyList.Add(new KeyValuePair<string, string>(x.DisplayCode, x.AnsiCode)));
                return currencyList;
            }
        }
        ...
        public static decimal GetExchangeRate(int? companyID, string forCurrencyCode)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                if (query.Any())
                    return (decimal)query.FirstOrDefault().ExchangeRate;
                else
                    return 1;
            }
        }
        ...
        public static List<Language> GetEnabledMultiLanguages()
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Languages.Include("Culture").Where(l => l.IsEnabled == true).ToList();                
            }
        }
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                if (!string.IsNullOrEmpty(loginEmailID))
                {
                    return (from login in dbContext.Logins
                            ...
                            select cust.CustomerIID).FirstOrDefault();
                }
                else
                {
                    return (from login in dbContext.Logins
        ...
        {
            dbEduegateERPContext db = new dbEduegateERPContext();
            Brand brand = db.Brands.Where(x => x.BrandIID == brandID).FirstOrDefault();
            return brand;
        }
        ...
        public static Currency GetCurrencyDetail(string ansiCode)
        {
            ...
            {
                return db.Currencies.Where(c => c.AnsiCode == ansiCode).SingleOrDefault();
            }
        }
        ...
        public Currency GetCurrencyByID(long currencyID)
        {
            using (dbEduegateERPContext db = new dbEduegateERPContext())
            {
                return db.Currencies.Where(c => c.CurrencyID == currencyID).SingleOrDefault();
            }
        }
    }
}
