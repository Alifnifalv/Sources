    ...
    public class WalletRepository
    {
        ...
        public bool CreateWalletCustomerLog(WalletCustomerLog log)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var userLogs = dbContext.WalletCustomerLogs.Where(w => w.CustomerId == log.CustomerId).ToList();
        ...
        public bool UpdateWalletEntry(WalletTransactionDetail walletTransaction)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    WalletTransactionDetail updateWallet = dbContext.WalletTransactionDetails.Where(x => x.TrackId == walletTransaction.TrackId && x.CustomerId == walletTransaction.CustomerId).FirstOrDefault();
        ...
        public WalletCustomerLog GetWalletCustomer(string walletGUID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.WalletCustomerLogs.Where(x => x.Guid.Equals(walletGUID)).FirstOrDefault();
            }
        }
        ...
        public decimal GetWalletBalance(string walletGUID)
        {
            ...
            using (dbEduegateERPContext dbcontext = new dbEduegateERPContext())
            {
                var walletCustomer = dbcontext.WalletCustomerLogs.Where(x => x.Guid.Equals(walletGUID)).FirstOrDefault();
        ...
        public long CreateVoucherWalletTransaction(VoucherWalletTransaction voucherDetail)
        {
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    voucherDetail =  dbContext.VoucherWalletTransactions.Add(voucherDetail);
                    dbContext.SaveChanges();
                }
            }
            catch (Exception ex)
        ...
        public bool UpdateWalletTransactionStatus(long walletTransactionId, short statusId)
        {
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                    WalletTransactionDetail updateWallet = dbContext.WalletTransactionDetails.Where(x => x.WalletTransactionId == walletTransactionId).FirstOrDefault();
                    if (updateWallet.IsNotNull())
                    {
