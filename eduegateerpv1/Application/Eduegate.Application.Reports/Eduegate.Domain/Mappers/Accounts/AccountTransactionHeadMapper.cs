...
namespace Eduegate.Domain.Mappers.Accounts
{
    public class AccountTransactionHeadMapper : DTOEntityDynamicMapper
    {
        ...
        public AccountTransactionHeadDTO ToDTO(AccountTransactionHead headEntity)
        {
            ...
            headDTO.DocumentReferenceTypeID = (DocumentReferenceTypes?)headEntity.DocumentType.ReferenceTypeID;
            ...
            headDTO.TaxDetails = new List<Services.Contracts.Inventory.TaxDetailsDTO>();

            if (headEntity.AccountTaxTransactions != null)
            {
                foreach (var tax in headEntity.AccountTaxTransactions)
                {
                    headDTO.TaxDetails.Add(new Services.Contracts.Inventory.TaxDetailsDTO()
                    {
            ...
            foreach (var detailEntityItem in headEntity.AccountTransactionDetails)
            {
                ...
                detailDTO.DepartmentID = detailEntityItem.DepartmentID;
                ...
                if (detailEntityItem.CostCenterID.HasValue && detailEntityItem.CostCenterID != 0 && detailEntityItem.CostCenter != null)
                {
                ...
                if (detailEntityItem.DepartmentID.HasValue && detailEntityItem.DepartmentID != 0 && detailEntityItem.Department != null)
                {
                    detailDTO.Department = new Eduegate.Framework.Contracts.Common.KeyValueDTO() { Key = detailEntityItem.DepartmentID.ToString(), Value = detailEntityItem.Department.DepartmentName };
                }
            ...
            if (taxAmount != 0 && headDTO.TaxDetails.Count == 0) // take the tax from detail and put it
            {
                headDTO.TaxDetails.Add(
                    new Services.Contracts.Inventory.TaxDetailsDTO()
                    {
        ...
        public AccountTransactionHead ToEntity(AccountTransactionHeadDTO headDto)
        {
            ...
            foreach (AccountTransactionDetailsDTO detailDTOItem in headDto.AccountTransactionDetails)
            {
                ...
                detailEntity.DepartmentID = detailDTOItem.DepartmentID;
                detailEntity.Amount = detailDTOItem.Amount;
        ...
        private AccountTransactionDetailsDTO AccountGroupDetail(long? accountID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var accountRepository = new EntiyRepository<Account, dbEduegateERPContext>(dbContext);
                var accountDetail = accountRepository.Get(a => a.AccountID == accountID).LastOrDefault();

                if (accountDetail != null)
                {
                    var groupRepository = new EntiyRepository<Group, dbEduegateERPContext>(dbContext);
                    var groupDetail = groupRepository.Get(a => a.GroupID == accountDetail.GroupID).LastOrDefault();
        ...
        public List<KeyValueDTO> GetAccountByPayementModeID(long paymentModeID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var repository = new EntiyRepository<PaymentMode, dbEduegateERPContext>(dbContext);

                var paymentMode = repository.Get(X => X.PaymentModeID == paymentModeID).FirstOrDefault();

                var accountRepository = new EntiyRepository<Account, dbEduegateERPContext>(dbContext);

                var entities = accountRepository.Get(X => X.AccountID == paymentMode.AccountId).ToList();
        ...
        public List<KeyValueDTO> GetAccountGroupByAccountID(long accountID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                ...
                var repository = new EntiyRepository<Account, dbEduegateERPContext>(dbContext);

                var account = repository.Get(X => X.AccountID == accountID).FirstOrDefault();

                var groupRepository = new EntiyRepository<Group, dbEduegateERPContext>(dbContext);

                var entities = groupRepository.Get(X => X.GroupID == account.GroupID).ToList();
