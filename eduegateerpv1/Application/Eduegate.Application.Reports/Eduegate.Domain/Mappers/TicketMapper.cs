    ...
    public class TicketMapper : IDTOEntityMapper<TicketDTO, Ticket>
    {
        ...
        public TicketDTO ToDTO(Ticket entity)
        {
            ...
            if (entity.IsNotNull())
            {
                ...
                ticketDTO.TicketStatus = entity.TicketStatusID.IsNotNull() ? new SupportBL(_context).GetTicketStatusByStatusID(Convert.ToInt32(entity.TicketStatusID)).Value : string.Empty;
                ...
                ticketDTO.ManagerEmployee = entity.ManagerEmployeeID.IsNotNull() ? (employeeRepository.GetEmployee(Convert.ToInt32(entity.ManagerEmployeeID)).IsNotNull() ? employeeRepository.GetEmployee(Convert.ToInt32(entity.ManagerEmployeeID)).EmployeeName : string.Empty) : string.Empty;
                ticketDTO.CustomerID = entity.CustomerID;
                ticketDTO.CustomerName = entity.CustomerID.IsNotNull() ? (customerRepository.GetCustomerDetail(Convert.ToInt32(entity.CustomerID)).IsNotNull() ?
                    string.Concat(entity.CustomerID + "-" + customerRepository.GetCustomerDetail(Convert.ToInt32(entity.CustomerID)).FirstName +
                    customerRepository.GetCustomerDetail(Convert.ToInt32(entity.CustomerID)).MiddleName + "  " +
                    customerRepository.GetCustomerDetail(Convert.ToInt32(entity.CustomerID)).LastName) : string.Empty) : string.Empty;
                ...
                if (entity.TicketProductMaps.IsNotNull() && entity.TicketProductMaps.Count > 0)
                {
                    ...
                    var orderDetail = new UserServiceBL(_context).GetOrderHistoryDetails("0", 0, 0, orderID: Convert.ToInt64(entity.HeadID), withCurrencyConversion: false).FirstOrDefault();
                    ...
                    foreach (TicketProductMap tpMap in entity.TicketProductMaps)
                    {
                        ...
                        tpDTO.SKUName = tpMap.ProductSKUMapID.HasValue ? productDetailRepository.GetProductAndSKUNameByID((long)tpMap.ProductSKUMapID.Value) : string.Empty;
                        ...
                        tpDTO.UnitPrice = orderDetail.IsNotNull() && orderDetail.OrderDetails.Any(d => d.ProductSKUMapID == tpMap.ProductSKUMapID) ? orderDetail.OrderDetails.Where(o => o.ProductSKUMapID == tpMap.ProductSKUMapID).FirstOrDefault().UnitPrice : null; // where to get unit price if not connected to Order
                        tpDTO.ProductImageUrl = orderDetail.IsNotNull() && orderDetail.OrderDetails.Any(d => d.ProductSKUMapID == tpMap.ProductSKUMapID) ? orderDetail.OrderDetails.Where(o => o.ProductSKUMapID == tpMap.ProductSKUMapID).FirstOrDefault().ProductImageUrl : null;
                ...
                if (entity.TicketActionDetailMaps.IsNotDefault() && entity.TicketActionDetailMaps.Count > 0)
                {
                    ...
                    ticketDTO.TicketActionDetail.AssignedEmployee = new Eduegate.Framework.Contracts.Common.KeyValueDTO();
                    ...
                    ticketDTO.TicketActionDetail.SubActionName = subActionId.IsNotNull() ? new SupportBL(_context).GetTicketActionByActionID(Convert.ToInt32(subActionId)).Value : null;
                ...
                if (dto.TicketProductSKUs.IsNotNull() && dto.TicketProductSKUs.Count > 0)
                {
                    ...
                    foreach (TicketProductDTO tpDTO in dto.TicketProductSKUs)
                    {
                        ...
                        if (tpDTO.SKUID > 0)
                        {
                            ...
                            tpMap.ProductID = tpDTO.ProductID > 0 ? tpDTO.ProductID : supportRepository.GetProductID(tpDTO.SKUID);
