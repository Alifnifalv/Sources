...
namespace Eduegate.Domain.Payroll
{
    public class EmployeeBL
    {
        ...
        public List<EmployeeDTO> GetEmployees()
        {
            ...
            foreach (var employee in repository.GetEmployees())
            {
        ...
        {
            return FromEntity(repository.SaveEmployee(ToEntity(employeeDetails, _callContext), _callContext), _callContext);
        }
        ...
        public static EmployeeDTO FromEntity(Employee entity, CallContext context, bool requiredAdditionalInfo = true)
        {
            ...
            {
                foreach (var details in entity.EmployeeRelationsDetails)
                {
                    dto.RelationDetails.Add(new EmployeeRelationsDetailDTO()
                    {
                        ...
                        EmployeeRelationType = details.EmployeeRelationTypeID.HasValue ? new KeyValueDTO()
                        {
                            Key = details.EmployeeRelationTypeID.ToString(),
                            Value = details.EmployeeRelationType.EmployeeRelationType1
                        } : new KeyValueDTO()
                        {
                            Key = null,
                            Value = null
        ...
        public static Employee ToEntity(EmployeeDTO dto, CallContext callContext)
        {
            ...
            var TempEmployeeCode = mutualRepository.GetSettingData("PREFIX_TERMPORARY_EMPLOYEE_CODE").SettingValue;
            var permtEmployeeCode = mutualRepository.GetSettingData("PREFIX_PERMENANT_EMPLOYEE_CODE").SettingValue;
           ...
           if (dto.EmployeeLeaveAllocationInfo.Count() != 0)
            ...
            if (dto.Login != null)
            {
                if (dto.Login.LoginUserID == null)
                {
                    ...
                    dto.Login.LoginEmailID = empEntity.EmployeeCode + "." + empEntity.FirstName.ToLower() + "@pearlschool.org";
            ...
            if (dto.RelationDetails.Count() != 0)
            {
                foreach (var details in dto.RelationDetails)
                {
                    ...
                    {
                        EmployeeID = dto.EmployeeIID,
                        EmployeeRelationTypeID = details.EmployeeRelationTypeID,
                        FirstName = details.FirstName,
                        MiddleName = details.MiddleName,
                        LastName = details.LastName,
                        NationalIDNo = details.NationalIDNo,
