...
namespace Eduegate.Domain
{
    public class SupportBL
    {
        ...
        public TicketDTO SaveTicket(TicketDTO ticketDTO)
        {
            ...
            DocumentType entity = new MutualBL(_callContext).UpdateLastTransactionNo(Convert.ToInt32(ticket.DocumentTypeID), ticket.TicketNo);
        ...
        public bool AddCustomerSupportTicket(CustomerSupportTicketDTO ticketDTO)
        {
            ...
            ticket.CreatedBy = _callContext.LoginID.IsNotNull() ? int.Parse(_callContext.LoginID.ToString()) : (int?)null;
            var documenttype = new SettingRepository().GetSettingDetail("TICKETDCMNTTYPEID", _callContext.CompanyID.Value);
            ...
            {
                var Customers = new AccountRepository().GetCustomerIDbyLoginID(long.Parse(_callContext.LoginID.Value.ToString()));
            ...
            var lastticketnumber = new MutualBL(_callContext).GetNextTransactionNumber(long.Parse(ticket.DocumentTypeID.ToString()));
            DocumentType entity = new MutualBL(_callContext).UpdateLastTransactionNo(Convert.ToInt32(ticket.DocumentTypeID), lastticketnumber);
        ...
        private void AddEmailNotification(Ticket ticket)
        {
            ...
            notificationDTO.FromEmailID = "customer.support@blink.com.kw"; // ConfigurationExtensions.GetAppConfigValue("EmailFrom").ToString(); // can I add setting entry for this
            ...
            var domainsetting = new SettingBL().GetSettingDetail("DOMAINNAME");
