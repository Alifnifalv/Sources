...
{
    public class PurchaseInvoiceAccounting : AccountingBase, ITransactions
    {
        private Action<string> _logError;

        public PurchaseInvoiceAccounting(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        public void Process(ViewModels.TransactionHeadViewModel transaction)
        {
            ...
            try
            {
                ...
                string Description = "Purchase Invoice: Transaction-" + transactionHeadIID.ToString() + " ";
                ...
                foreach (var entitlementMapDTO in TransactionHeadEntitlementMapsList)
                {
                    ...
                    {
                        EntitlementMapValue = entitlementMapDTO.Amount;
                    }
                ...
                //var additionalExpensesTransList = GetAdditionalExpensesTransactionsMap(transactionHeadIID);
                //var affectSupplierList = additionalExpensesTransList.FindAll(x => x.ISAffectSupplier == true);
                //{
                //    foreach (AdditionalExpensesTransactionsMapDTO detail in affectSupplierList)
                //    {
                //        new Eduegate.TransactionEgine.ClientFactory.ClientFactory().AccountTransactionSync(0, transactionHeadIID,  11);
                //    }
