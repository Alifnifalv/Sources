...
{
    public class SalesQuotationAccounting : AccountingBase, ITransactions
    {
        private Action<string> _logError;

        public SalesQuotationAccounting(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        {
            WriteLog("SalesQuotation-AccountingProcess started for TransactionID:" + transaction.HeadIID.ToString());
            try
            {
                long transactionHeadIID = transaction.HeadIID;
                string Description = "Sales Quotation: Transaction-" + transactionHeadIID.ToString()+" ";
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in SalesQuotationAccounting-Process.:" + ex.Message, ex);
            }
        }
