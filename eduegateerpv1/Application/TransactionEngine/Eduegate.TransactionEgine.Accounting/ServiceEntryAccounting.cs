...
{
   public class ServiceEntryAccounting : AccountingBase, ITransactions
    {
        private Action<string> _logError;

        public ServiceEntryAccounting(Action<string> logError)
        {
            _logError = logError;
        }
        ...
        {
            get { return Framework.Enums.DocumentReferenceTypes.ServiceEntry; }
        }
        ...
        {
            WriteLog("ServiceEntryAccounting-AccountingProcess started for TransactionID:" + transaction.HeadIID.ToString());

            try
            {
                long transactionHeadIID = transaction.HeadIID;
                string Description = "ServiceEntryAccounting: Transaction-" + transactionHeadIID.ToString() + " ";
            ...
            catch (Exception ex)
            {
                ...
                WriteLog("Exception occured in ServiceEntryAccounting-Process.:" + ex.Message, ex);
            }
        }
