...
namespace Eduegate.Domain.Mappers.CRM.Leads
{
    public class LeadMapper : DTOEntityDynamicMapper
    {
        ...
        public override KeyValueDTO ValidateField(BaseMasterDTO dto, string field)
        {
            ...
            {
                var hasDuplicated = IsLeadDuplicated(toDto.StudentName.ToString(), toDto.EmailAddress, toDto.MobileNumber, toDto.ClassID, toDto.LeadIID);
                if (hasDuplicated)
                {
                    valueDTO.Key = "true";
        ...
        public bool IsLeadDuplicated(string studentName, string emailAddress, string mobileNumber, int? classID, long leadID)
        {
            using (var db = new dbEduegateCRMContext())
            {
                ...
                if (leadID == 0)
                {
                    stud = db.Leads.Where(x => (x.StudentName).ToUpper().Replace(" ", string.Empty) == studentName.ToUpper().Replace(" ", string.Empty) && (x.MobileNumber == mobileNumber || x.EmailAddress == emailAddress) && x.ClassID == classID).AsNoTracking().ToList();
                }
                else
                {
