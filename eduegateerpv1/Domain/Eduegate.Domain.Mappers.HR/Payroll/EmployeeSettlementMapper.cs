    ...
    public class EmployeeSettlementMapper : DTOEntityDynamicMapper
    {
                ...
                using (dbEduegateHRContext dbContext = new dbEduegateHRContext())
                {
                    ...
                    if (toDto.EmployeeSettlementTypeID == 1)
                    {
                        salarySlipEntities = (from ss in dbContext.SalarySlips
                                              ...
                                              where ss.EmployeeID.Value == toDto.EmployeeID && ess.EmployeeSettlementTypeID == 1 && (ss.SlipDate.Value.Year == toDto.SalaryCalculationDate.Value.Year
                ...
                using (var dbContext = new dbEduegateHRContext())
                {
                    ...
                    if (empData != null)
                    {
                        ...
                        var lopDetails = dbContext.LeaveApplications
                            .Where(X => X.EmployeeID == employeeSettlementDTO.EmployeeID && X.IsLeaveWithoutPay == true && X.FromDate.Value.Date > leaveDueFromDate.Value.Date && X.ToDate.Value.Date <= employeeSettlementDTO.SalaryCalculationDate.Value.Date && X.LeaveStatusID == leaveStatusID)
                            .AsNoTracking()
                            .ToList();
        ...
        public EmployeeSettlementDTO GetSettlementDateDetails(EmployeeSettlementDTO settlementDTO)
        {
            using (dbEduegateHRContext dbContext = new dbEduegateHRContext())
            {
                ...
                {
                    var settingBL = new Domain.Setting.SettingBL(_callContext);
                    settlementDTO.NoofDaysInTheMonth = settingBL.GetSettingValue<byte?>("LEAVE_SALARY_NOOF_MONTH_DAYS");
            ...
            using (dbEduegateHRContext dbContext = new dbEduegateHRContext())
            {
                try
                {
                    ...
                    if (employees.Count > 0)
                    {
                        ...
                        settlementDTO.LossofPay = CalculateNoOfLeaveDays(dbContext, settlementDTO.EmployeeID, prevPayrollCuttofDate.Value.Date, payrollCuttofDate.Value.Date, leaveStatusID);
       ...
       int hrsPerDay, int normalOTTimeTypeID, int specialOTTimeTypeID, byte withCUTOFDate, int defaultDecimalNos)
        {
            ...
            settlementDTO.VacationSalaryComponent = settingBL.GetSettingValue<int?>("VACATION_SALARY_COMPONENT");
            ...
            if (settlementDTO.EmployeeSettlementTypeID == 1 && settlementDTO.EmployeeTypeID == academicTypeID && settlementDTO.IsVacationSalary == true && settlementDTO.VacationDaysInCurrentMonth == settlementDTO.NoofDaysInTheMonth)
            {
                ...
                var vacationSalryCurMonth = (basicLeaveSalaryAmount ?? 0) * ((settlementDTO.VacationDaysInCurrentMonth ?? 0) / (settlementDTO.NoofDaysInTheMonth ?? 0));
            ...
            else
            {
                if (payrollCutoffDate > dateOfJoining)
                {
                    ...
                    var lop = (int)settlementDTO.LossofPay;
                    if (settlementDTO.IsVacationSalary==true)
                    {
                        noofleave = noofleave + (settlementDTO.VacationDaysInCurrentMonth.HasValue ? (int)settlementDTO.VacationDaysInCurrentMonth.Value : 0);
                    }
                    #endregion
                    ...
                    if (employeeSalaryStructComp.Count() > 0)
                    {
                        ...
                        settlementDTO.NoofSalaryDays = settlementDTO.NoofDaysInTheMonth - noofleave;
                    ...
                    foreach (var e in emplLeaveSalaryStrComp)
                    {
                        ...
                        description = description + e.SalaryComponent.Description + "Amount :" + e.Amount;
                    }
                    ...
                    if (sumSalaryComponent != 0)
                    {
                        ...
                        {
                            var leaveSalaryAmountperDays = (sumSalaryComponent / settlementDTO.NoofDaysInTheMonth);
                            ...
                            if (settlementDTO.LeaveStartDate.HasValue)
                            {
                               ...
                               " NoofDaysInTheMonth :" + settlementDTO.NoofDaysInTheMonth + "";
                            }
                            else
                            {
                                description = @"Calculated with components : " + description +
