                ...
                try
                {
                    ...
                    if (salarySlipPDFInfo.Any())
                    {
                        salarySlipPDFInfo.All(x =>
                        {
                            ...
                            slipdate.Contains(xy.SlipDate) && xy.EmployeeID == employeeId).AsNoTracking().ToList();
                            if (allEntities.Any())
                            {
                                allEntities.All(en =>
                                {
                        ...
                        else//Normal Salary generation
                        {
                            ...
                            foreach (var employee in employees)
                            {
                                if (!listOFLeaveEmployees.Any(x => x.Value == employee.EmployeeIID))
                                {
                                    ...
                                    if (empSalarySlip.Any(x => x.SalaryComponentID == leaveSalaryComp))
                                    {
                                    ...
                                    else if (eoSBssalarySlipEntities.Count() > 0 && eoSBssalarySlipEntities.Any(x => x.EmployeeID == employee.EmployeeIID))
                                    {
                                        empSalarySlip.RemoveRange(0, empSalarySlip.Count());
                                        if (employees.Count() == 1)
                                        {
