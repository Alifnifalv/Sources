...
namespace Eduegate.Domain.Mappers.OnlineExam.Exam
{
    public class CandidateMapper : DTOEntityDynamicMapper
    {
            ...
            using (var dbContext = new dbEduegateOnlineExamContext())
            {
                ...
                foreach (var examMap in candidateOnlineExamMaps)
                {
                    ...
                    foreach (var grpMap in examGroupMaps)
                    {
                        ...
                        if (grpMap?.QuestionGroup?.Questions.Count > 0)
                        {
                            groupDTO = GroupWiseQestions(grpMap, shuffleMaxLoopCount, examName);
        ...
        public OnlineQuestionGroupsDTO GroupWiseQestions(ExamQuestionGroupMap grpMap, int shuffleMaxLoopCount, string examName)
        {
            var groupData = new OnlineQuestionGroupsDTO();

            int allTotalNoofQuestions = grpMap.NumberOfQuestions.HasValue ? Convert.ToInt32(grpMap.NumberOfQuestions) : 0;

            var allQuestionList = grpMap.QuestionGroup.Questions.ToList();

            var questionList = allQuestionList.Where(a => a.PassageQuestionID == null).ToList();

            var passageQuestionList = allQuestionList.Where(a => a.PassageQuestionID != null).ToList();

            var totalNoofQuestions = allTotalNoofQuestions - passageQuestionList.Count();

            var maximumMarks = allQuestionList.Select(a => a.Points).Sum() - passageQuestionList.Select(a => a.Points).Sum();

            int shuffleLoopCount = 0;

            if (totalNoofQuestions >= 0)
            {
                groupData = ShuffleAndGetQuestions(questionList, totalNoofQuestions, maximumMarks, shuffleMaxLoopCount, shuffleLoopCount, examName);
            }

            if (passageQuestionList.Count() != 0)
            {
                var questionIDs = passageQuestionList.Select(a => a.QuestionIID).ToList();

                groupData.QuestionIDs.AddRange(questionIDs);
            }

            return groupData;
        }

        public OnlineQuestionGroupsDTO ShuffleAndGetQuestions(List<Question> questionList, int totalNoofQuestions, decimal? groupMaxMark, int shuffleMaxLoopCount, int shuffleLoopCount, string examName)
        {
            var groupData = new OnlineQuestionGroupsDTO();
            ...
            if (shuffledQnsTotalMark != groupMaxMark)
            {
                ...
                {
                    shuffleLoopCount += 1;
                    groupData = ShuffleAndGetQuestions(questionList, totalNoofQuestions, groupMaxMark, shuffleMaxLoopCount, shuffleLoopCount, examName);
                }
                else
                {
                    if (shuffledQnsTotalMark < groupMaxMark)
