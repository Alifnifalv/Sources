...
namespace Eduegate.Domain.Mappers.School.Circulars
{
    public class CircularMapper : DTOEntityDynamicMapper
    {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                if (toDto.CircularIID != 0)
                {
                    ...
                    if (circularData != null)
                    {
                        ...
                        {
                            throw new Exception("The Circular can't edit, its already Published!");
                        }
                    }
                }
            }
        ...
        private long SaveAndSendCircular(CircularDTO toDto)
        {
            ...
            var result = SendCircular(entity);
        ...
        private void SendAndSavePushNotification(Circular entity, CircularDTO dto)
        {
            ...
            if (entity.CircularStatusID.HasValue)
            {
                if (entity.CircularStatusID == 2)
                {
                    ...
                    {
                        departmentIDs = dto.CircularMaps.Where(a => a.DepartmentID.HasValue).Select(a => a.DepartmentID).ToList();
                        var departmentID = departmentIDs?.Intersect(dto.CircularMaps.Select(a => a.DepartmentID)).ToList();

                        foreach (var department in departmentID)
                        {
                            loginIDs.AddRange(GetStaffLoginIDs(entity, department));
                        }
                        //loginIDs = GetStaffLoginIDs(entity);
                    }
        ...
        private async Task SendCircular(Circular entity)
        {
            if (entity.CircularStatusID == 2)
            {
        ...
        private List<StudentDTO> GetDetailsForMail(Circular entity)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                foreach (var classID in classIDs)
                {
                    var gaurdianEmails = dbContext.Parents
                        .Where(p => p.Students.Any(std => std.ClassID == classID && sectionIDs.Any(s => s == std.SectionID)
                        && std.IsActive == true && std.Status == 1 && std.SchoolID == entity.SchoolID && std.AcademicYearID == entity.AcadamicYearID)
                        && p.LoginID.HasValue)
                        .Include(i => i.Students)
                        .AsNoTracking()
                        .Select(l => l.GaurdianEmail)
        ...
        public Circular CircularEmailProcess(Circular entity)
        {
            //List<String> _sEmails = GetParentEmailIDs(entity);

            var mailDetails = GetDetailsForMail(entity);

            //if (_sEmails.Any())
            //{
            //    _sEmails.All(w => { EmailProcess(w, entity); return true; });
            //}
            ...
            if (mailDetails.Any())
            {
                mailDetails.All(x =>
                {
                    EmailProcess(x.ParentEmailID, x.ClassCode);
                    return true;
                });
            }
        ...
        public void EmailProcess(string toEmail, string classCode)
        {
            ...
            string emailBody = @"<br />
                        ...
                        A Circular has been upload to your parent portal.<br />
                        To view the circular,  please login to your parent portal by clicking on the link below<br /><br />
                        <a href='https://parent.pearlschool.org/Home/CircularList'>Click Here</a><br />
                        <br/><br/><p style='font-size:0.7rem;'<b>Please Note: </b>do not reply to this email as it is a computer generated email</p>";

            string emailSubject = "Circular";
        ...
        private List<long?> GetStaffLoginIDs(Circular entity, long? departmentID)
        {
            List<long?> loginIDs = new List<long?>();
            using (var dbContext = new dbEduegateSchoolContext())
            {
                loginIDs = dbContext.Employees
                    .Where(i => i.IsActive == true && i.BranchID == entity.SchoolID && (departmentID == null || i.DepartmentID == departmentID))
                    .AsNoTracking()
                    .Select(l => l.LoginID)
                    .Distinct()
                    .ToList();
