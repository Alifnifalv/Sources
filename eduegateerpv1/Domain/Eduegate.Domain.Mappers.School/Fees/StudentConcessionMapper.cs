...
namespace Eduegate.Domain.Mappers.School.Fees
{
    public class StudentConcessionMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                else
                {
                    ...
                    if (monthlyClosingDate != null && monthlyClosingDate.Value.Year > 1900 && toDto.ConcessionDate.Value >= monthlyClosingDate)
                    {
        ...
        public List<KeyValueDTO> GetStudentDetailsByParent(long parentID)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                var studentDetails = (from ss in dbContext.Students
