    ...
    public class StudentPickupRequestMapper : DTOEntityDynamicMapper
    {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                if (barCodeValue != "null")
                {
                    pickupRequests = dbContext.StudentPickerStudentMaps
                                        .AsNoTracking()
                                        .Where(x => x.PickUpLoginID == loginID && x.IsActive == true && x.StudentPicker.Parent.Students.Any(s => s.AdmissionNumber == barCodeValue))
                                        .Include(x => x.Student)
                                        ...
                                        .OrderByDescending(y => y.IsActive == true)
                                        .ToList();
        ...
        public StudentPickupRequestDTO GePickupRegisteredDetailsByQR(string qrCode)
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var mapList = dbContext.StudentPickerStudentMaps
                    .Where(x => x.QRCODE == qrCode && x.IsActive == true || x.Student.AdmissionNumber == qrCode && x.IsActive == true
                    || x.StudentPicker.Parent.Students.Any(a => a.AdmissionNumber == qrCode))
        ...
        public List<StudentPickupRequestDTO> GetTodayStudentPickLogs()
        {
            ...
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                x.CreatedDate.Value.Day == DateTime.Now.Day && x.CreatedDate.Value.Month == DateTime.Now.Month && x.CreatedDate.Value.Year == DateTime.Now.Year)
                    .Include(i => i.Student).ThenInclude(i => i.Class)
                    .Include(i => i.Student).ThenInclude(i => i.Section)
                    .Include(i => i.Student).Include(i => i.StudentPicker)
                    .OrderByDescending(x => x.StudentPickLogIID).AsNoTracking().ToList();
        ...
        public VisitorDTO GetVisitorDetailsByVisitorCode(string visitorCode)
        {
            using (var dbContext = new dbEduegateSchoolContext())
            {
                ...
                var vistorDetails = dbContext.Visitors.Include(x => x.VisitorAttachmentMaps).AsNoTracking().FirstOrDefault(x => x.VisitorNumber == vsCode);
        ...
        {
            var dateTimeFormat = Eduegate.Framework.Extensions.ConfigurationExtensions.GetAppConfigValue("12HrDateTimeFormat");
