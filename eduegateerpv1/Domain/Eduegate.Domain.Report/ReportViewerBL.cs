...
namespace Eduegate.Domain.Report
{
    public class ReportViewerBL
    {
        ...
        private void DefaultReportParameters(Microsoft.Reporting.NETCore.LocalReport viewer, List<KeyValueDTO> parameters)
        {
            ...
            if (schoolParam != null)
            {
                schoolID = string.IsNullOrEmpty(schoolParam.Value) ? schoolID : short.Parse(schoolParam.Value);
                schoolParam.Value = schoolID.ToString();
            }
            if (reportParameters.Where(a => a.Name == "SchoolID").FirstOrDefault() != null)
            {
                viewer.SetParameters(new Microsoft.Reporting.NETCore.ReportParameter("SchoolID", schoolID.Value.ToString(), false));
            }

            //Set academic year ID
            var academicYearParam = parameters.FirstOrDefault(p => p.Key == "CurrentAcademicYearID");
            if (academicYearParam != null)
            {
                academicYearID = string.IsNullOrEmpty(academicYearParam.Value) ? academicYearID : short.Parse(academicYearParam.Value);
            ...
            {
                viewer.SetParameters(new Microsoft.Reporting.NETCore.ReportParameter("CurrentAcademicYearID", academicYearID.Value.ToString(), false));
            }
