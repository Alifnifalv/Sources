...
namespace Eduegate.Domain.Repository
{
    public class FixedAssetsRepository
    {
        ...
        public Asset GetAssetById(long AssetId)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                return dbContext.Assets
                                    ...
                                    .Include(i => i.AssetTransactionDetails).ThenInclude(i => i.AssetTransactionHead)
                                    .AsNoTracking()
                                    .FirstOrDefault();
            }
        }
        ...
        public decimal GetAccumulatedDepreciation(long AssetId, int documentTypeID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var a = dbContext.AssetTransactionDetails
                    .Include(i => i.AssetTransactionHead)
                    .AsNoTracking()
            ...
            try
            {
                using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                {
                  var assetTransactionHeadInDB=  dbContext.AssetTransactionHeads
                        .Include(x => x.AssetTransactionDetails)
                        .Where(x => x.HeadIID == assetTransactionHead.HeadIID)
                        .AsNoTracking()
                        .FirstOrDefault();
        ...
        public bool UpdateAssetTransactionHead(AssetTransactionHead assetTransactionHead)
        {
            ...
            try
            {
                AssetTransactionHead query = db.AssetTransactionHeads.Where(x => x.HeadIID == assetTransactionHead.HeadIID).AsNoTracking().FirstOrDefault();
