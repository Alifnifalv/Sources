            ...
            else
            {
                switch (workflow.WorkflowID)
                {
                    ...
                    case var value when value == long.Parse(libraryBookReturnWorkFlowID):
                        {
                            status = new WorkflowRepository().UpdateLibraryBookTransactionStatusID((long?)workflow.WorkflowID, workflow.WorkflowRuleIID, workflow.Value1, statusID, referenceID, employeeID);
                        }
                        break;
                }
                if (status == true)
