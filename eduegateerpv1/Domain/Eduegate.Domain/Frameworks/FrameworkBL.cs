...
namespace Eduegate.Domain.Frameworks
{
    public class FrameworkBL
    {
        ...
        public string GetScreenData(long screenID, long IID)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(screenData.EntityMapperAssembly) && !string.IsNullOrEmpty(screenData.EntityMapperViewModel))
                {
                    var mapper = DTOEntityDynamicMapper.GetMapper(screenData.EntityMapperAssembly, screenData.EntityMapperViewModel);
                    mapper.SetContext(_callContext);
                    var entity = mapper.GetEntity(IID);
            ...
            {
                Eduegate.Logger.LogHelper<FrameworkBL>.Fatal(exception.Message, exception);
                throw new Exception("Mapper not able to instantiate, screen metadata is not correctly configured. Please look the logs for the more info.");
            }
        }
        ...
        public ScreenMetadataDTO GetScreenMetadataByName(string screenName)
        {
            var dto = Framework.CacheManager.MemCacheManager<ScreenMetadataDTO>.Get("SCREEN_BY_NAME_" + screenName);

            if (dto == null)
            {
                dto = Mappers.Frameworks.ScreenMetadataMapper
                        .Mapper(_callContext)
                        .ToDTO(new FrameworkRepository().GetScreenMetadataByName(screenName));
                Framework.CacheManager.MemCacheManager<ScreenMetadataDTO>.Add(dto, "SCREEN_BY_NAME_" + screenName);
            }

            if (_callContext.LoginID.HasValue)
            {
                dto.Claims = new SecurityBL(_callContext)
                    .GetUserClaimsByType(_callContext.LoginID.Value,
                    ClaimType.Features);
            }
