...
namespace Eduegate.Domain.Handlers
{
    public class DeliveryHandler
    {
        ...
        {
            var activeDeliveryTypes = new List<DeliverySettingDTO>();
            activeDeliveryTypes = Framework.CacheManager.MemCacheManager<List<DeliverySettingDTO>>
            .Get("GETACTIVEDELIVERYSETTINGS");

            var cartItems = shoppingCartRepository.GetCartDetailsWithItems(Convert.ToInt64(_callContext.UserId).ToString(), (int)ShoppingCartStatus.InProcess, null);


            if (activeDeliveryTypes == null || activeDeliveryTypes.Count == 0)
            {
                activeDeliveryTypes = new List<DeliverySettingDTO>();

                var deliveryTypes = new Repository.ReferenceDataRepository().GetActiveDeliverySettings(cartItems?.CategoryIID);

                foreach (var deliveryType in deliveryTypes)
                {
                    var data = DeliveryTypeMapper.Mapper(this._callContext).ToDTO(deliveryType);
                    activeDeliveryTypes.Add(data);
                }
            }
