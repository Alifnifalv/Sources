...
namespace Eduegate.Domain.Handlers
{
    public class TransactionHandler
    {
        ...
        public string GetEntitlementType(Eduegate.Framework.Payment.PaymentGatewayType paymentGatewayMethod)
        {
            ...
            switch (paymentGatewayMethod)
            {
                case Framework.Payment.PaymentGatewayType.MIGS:
                    entitlementID = new Domain.Setting.SettingBL(_callContext).GetSettingValue<string>("VISAMASTERCARD_ENTITLEMENTID");
                    break;

                case Framework.Payment.PaymentGatewayType.QPAY:
                    entitlementID = new Domain.Setting.SettingBL(_callContext).GetSettingValue<string>("QPAY_ENTITLEMENTID");
                    break;

                case Framework.Payment.PaymentGatewayType.COD:
                default:
                    entitlementID = new Domain.Setting.SettingBL(_callContext).GetSettingValue<string>("COD_ENTITLEMENTID");
                    break;
            }
            ...
            if (transaction.IsNotNull())
            {
                ...
                if (transaction.TransactionHeadEntitlementMap.IsNotNull())
                {
                    ...
                    transactionHead.TransactionHeadEntitlementMaps.Add(new TransactionHeadEntitlementMap()
                    {
                        ...
                        PaymentTrackID = transaction.TransactionHeadEntitlementMap.PaymentTrackID,
                        PaymentTransactionNumber = transaction.TransactionHeadEntitlementMap.PaymentTransactionNumber,
                    });
                }
        ...
        public long? GetSellingUnit(long skumapID)
        {
            ...
            {
                var skumap = dbContext.ProductSKUMaps.Where(a => a.ProductSKUMapIID == skumapID).AsNoTracking().FirstOrDefault();

                var productID = skumap.ProductID;

                var product = dbContext.Products.Where(a => a.ProductIID == productID).AsNoTracking().FirstOrDefault();
