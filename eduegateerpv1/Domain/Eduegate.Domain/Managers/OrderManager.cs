    ...
    public class OrderManager
    {
        ...
        public List<Services.Contracts.OrderHistory.OrderHistoryDTO> GetPaymentPendingOrders()
        {
            try
            {
                ...
                var failedPaymenetPeriod = new Domain.Setting.SettingBL(_callContext).GetSettingValue<int>("FAILEDPAYMENTPERIODINDAYS", 4);
            ...
            foreach (var historyHeader in historyHeaderList)
            {
                ...
                var StudentDetails = new AccountRepository().GetStudentDetails(Convert.ToInt64(historyHeader.StudentID));
                    if (StudentDetails != null) {
                        var classDet = historyHeader.StudentID != null ? dbContext.Classes.Where(a => a.ClassID == StudentDetails.ClassID).AsNoTracking().FirstOrDefault() : null;
                        var secDet = historyHeader.StudentID != null ? dbContext.Sections.Where(a => a.SectionID == StudentDetails.SectionID).AsNoTracking().FirstOrDefault() : null;
                    ...
                    var deliveryType =  dbContext.DeliveryTypes1.Where(a => a.DeliveryTypeID == historyHeader.DeliveryTypeID).AsNoTracking().FirstOrDefault();
        ...
        public List<Services.Contracts.OrderHistory.OrderHistoryDTO> GetScheduledOrderHistoryDetails(string documentTypeID, string customerIID, int pageNumber, int pageSize,CallContext context = null, bool withCurrencyConversion = true)
        {
            ...
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
                ...
                foreach (var historyHeader in historyHeaderList)
                {
                    ...
                    {
                        var days = dbContext.Days.Where(a => a.DayID == day.WeekDayID).AsNoTracking().FirstOrDefault();
