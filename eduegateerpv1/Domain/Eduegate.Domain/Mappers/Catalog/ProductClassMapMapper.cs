...
namespace Eduegate.Domain.Mappers.Catalog
{
    public class ProductClassMapMapper : DTOEntityDynamicMapper
    {
        ...
        public override string SaveEntity(BaseMasterDTO dto)
        {
            ...
            return ToDTOString(ToDTO(Convert.ToInt64(toDto.ClassID)));
        ...
        private ProductClassMapDTO ToDTO(long IID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var entity = dbContext.ProductClassMaps.Where(x => x.ClassID == IID).AsNoTracking().FirstOrDefault();
                var mapEntity = dbContext.ProductClassMaps.Where(x => x.ClassID == IID).AsNoTracking().ToList();

                var classes = dbContext.Classes.Where(x => x.ClassID == IID).AsNoTracking().FirstOrDefault();
