    ...
    public class TransactionDetailsMapper : IDTOEntityMapper<TransactionDetailDTO, TransactionDetail>
    {
        ...
        public TransactionDetail ToEntity(TransactionDetailDTO dto)
        {
            if (dto != null)
            {
                ...
                var entity = new TransactionDetail();
                    ...
                    entity.ActualUnitPrice = dto.ActualUnitPrice;
        ...
        public TransactionDetailDTO ToDTO(TransactionDetail entity, int companyID)
        {
            if (entity != null)
            {
                ...
                transactionDetailDTO.ActualUnitPrice = entity.ActualUnitPrice;
                ...
                var invtrydtls = new ProductCatalogRepository().GetProductSKUInventoryDetail(companyID > 0 ? companyID : _context.CompanyID.Value, skuDetail.ProductSKUMapIID, _context.IsNotNull() ? Convert.ToInt64(_context.SchoolID) : Convert.ToInt64(entity.TransactionHead.SchoolID));
                if (invtrydtls != null)
                transactionDetailDTO.AvailableQuantity = invtrydtls.Quantity;
        ...
        private TransactionDetailDTO CostCenterDetail(long? costCenterID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                ...
                var costCenterDetail = dbContext.CostCenters.Where(x => x.CostCenterID == costCenterID).AsNoTracking().FirstOrDefault();

                if (costCenterDetail != null)
                {
                    dtos.CostCenter = new KeyValueDTO()
                    {
                        Key = costCenterDetail.CostCenterID.ToString(),
                        Value = costCenterDetail.CostCenterName
                    };
                }
        ...
        private string UnitDetail(long? unitID)
        {
            using (dbEduegateERPContext dbContext = new dbEduegateERPContext())
            {
                var unitDetail = dbContext.Units.Where(x => x.UnitID == unitID).AsNoTracking().FirstOrDefault();
        ...
        private List<UnitsDTO> GetUnitDataByUnitGroup(long? groupID)
        {
            ...
            {
                var units = dbContext.Units.Where(unts => unts.UnitGroupID == groupID)
                    .Select(unts => new UnitsDTO
                    {
                        UnitID = unts.UnitID,
        ...
        private string GetProductCode(long? productID)
        {
            using (var dbContext = new dbEduegateERPContext())
            {
                var prdct = dbContext.Products.Where(x=>x.ProductIID== productID).Select(x => x.ProductCode).AsNoTracking().FirstOrDefault();
                return prdct;
