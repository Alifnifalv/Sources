...
namespace Eduegate.Domain
{
    public class SearchBL
    {
        ...
        public async Task<search.SearchResultDTO> SearchData(Eduegate.Services.Contracts.Enums.SearchView view, int currentPage, int pageSize, string orderBy, string runtimeFilter, char viewType, byte? schoolID, int? academicYearID)
        {
            long totalRecords = 0;
            ...
            {
                default:
                    var result = await new SearchRepository().SearchData((SearchView)(long)view, totalRecords, currentPage, pageSize, orderBy, Convert.ToInt64(_callContext.LoginID), runtimeFilter,
                    ...
                    var viewInfo1 = await new MetadataRepository().GetViewInfo((SearchView)(long)view, _callContext.LanguageCode);
            ...
            // Assuming userViewGridDataList is a DataTable and NetSalary is a column in the table
            decimal sumOfNetSalary = 0;
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                ...
                foreach (System.Data.DataRow item in dataTable.Rows)
                {
                    ...
                    if (item.Table.Columns.Contains("NetSalary") && item["NetSalary"] != DBNull.Value)
                    {
                        sumOfNetSalary += Convert.ToDecimal(item["NetSalary"]);
                    }

                }
            }

            dto.TotalSalaries = sumOfNetSalary;

            //convert this table to DTO
            return dto;
        }

        public async Task<search.SearchGridMetadata> SearchList(Eduegate.Services.Contracts.Enums.SearchView view)
        {
            ...
            var viewInfo = await metadataRepository.GetViewInfo((SearchView)(long)view, _callContext.LanguageCode);
