...
namespace Eduegate.Domain
{
    public class SmartTreeViewBL
    {
        ...
        public SmartTreeViewDTO GetSmartTreeView(SmartViewType type, long? parentID, string searchText)
        {
            ...
            switch (type)
            {
                ...
                case SmartViewType.Area:
                    ...
                    break;
                case SmartViewType.Student:
                    smartView = GetProductTree(parentID);
                    break;
                case SmartViewType.PrvFinancialAudit:
                    smartView = GetFinancialAuditTree(parentID,SmartViewType.PrvFinancialAudit,searchText);
                    break;
                case SmartViewType.CrntFinancialAudit:
                    smartView = GetFinancialAuditTree(parentID, SmartViewType.CrntFinancialAudit, searchText);
                    break;
            }
        ...
        //public SmartTreeViewDTO GetStudentTree(long? categoryID)
        //{
        //    var smartView = new SmartTreeViewDTO()
        //    {
        //        Node = new TreeNodeDTO() { Caption = "Student View", ID = -1, SmartTreeNodeType = Services.Contracts.Enums.SmartTreeNodeType.Root },
        //        SmartViewType = Services.Contracts.Enums.SmartViewType.Student
        //    };
        //    FromProductEntity(new StudentMapper().GetStudentTree(categoryID).CategoryTree, smartView.Node);
        //    return smartView;
        //}

        //public SmartTreeViewDTO GetStudents(long? parentID, string searchText)
        //{
        //    var smartView = new SmartTreeViewDTO()
        //    {
        //        Node = new TreeNodeDTO()
        ...
        //        SmartViewType = SmartViewType.Student
        //    };

        //    var students = new StudentMapper().GetStudentTrees(parentID, searchText);
        //    var filterdStudents = parentID.HasValue ? students :
        //        students.Where(x => x.ParentID == null).OrderBy(x => x.StudentIID).ToList();

        //    foreach (var stud in filterdStudents)
        //    {
        //        smartView.Node.Nodes.Add(new TreeNodeDTO()
        //        {
        //            ID = stud.StudentIID,
        //            Caption = stud.AdmissionNumber + "-" + stud.FirstName + " " + stud.MiddleName + " " + stud.LastName,
        //            Nodes = StudentBuildTree(students, stud.StudentIID)
        //        });
        //    }
        ...
        //private List<TreeNodeDTO> StudentBuildTree(List<Entity.Models.Student> areas, long studentID)
        //{
        //    var nodes = new List<TreeNodeDTO>();

        //    foreach (var stud in areas.Where(a => a.StudentIID == studentID))
        //    {
        //        nodes.Add(new TreeNodeDTO()
        //        {
        //            ID = stud.StudentIID,
        //            Caption = stud.AdmissionNumber + "-" + stud.FirstName + " " + stud.MiddleName + " " + stud.LastName,
        //            Nodes = StudentBuildTree(areas, stud.StudentIID)
        //        });
        //    }
        ...
        public SmartTreeViewDTO GetFinancialAuditTree(long? categoryID, SmartViewType smartViewType, string searchText)
        {
            var smartView = new SmartTreeViewDTO()
            {
                Node = new TreeNodeDTO() { Caption = "Product View", ID = -1, SmartTreeNodeType = Services.Contracts.Enums.SmartTreeNodeType.Root },
            ...
            FromProductEntity(new FinancialYearClosingMapper().GetFinancialAuditTree(categoryID,smartView.SmartViewType, searchText).CategoryTree, smartView.Node);
            return smartView;
        }
    }
}