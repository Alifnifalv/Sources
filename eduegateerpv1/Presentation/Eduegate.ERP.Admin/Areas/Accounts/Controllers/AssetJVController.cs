...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    public class AssetJVController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.AssetJV);
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.AssetJVBranch);
        ...
        [HttpPost]
        public JsonResult Save(AssetDepreciationViewModel assetDepreciationViewModel)
        {
            ...
            try
            {
                if (assetDepreciationViewModel != null)
                {
                    if (assetDepreciationViewModel.DetailViewModel != null)
                    {
                        ...
                        {
                            return Json(new { IsError = true, UserMessage = "Please select AssetCode" }, JsonRequestBehavior.AllowGet);
                        }
                        if (assetDepreciationViewModel.DetailViewModel.Any(x => x.AssetCode != null && x.StartDate == null))
                        {
                            return Json(new { IsError = true, UserMessage = "Please select Depreciation Start date" }, JsonRequestBehavior.AllowGet);
                        }
            ...
            return Json(assetDepreciationViewModel, JsonRequestBehavior.AllowGet);
        }
