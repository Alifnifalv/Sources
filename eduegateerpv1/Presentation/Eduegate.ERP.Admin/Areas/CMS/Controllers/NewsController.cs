...
namespace Eduegate.ERP.Admin.Areas.CMS
{
    public class NewsController : BaseSearchController
    {
        ...
        public ActionResult List()
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.News);
        ...
        public ActionResult Get(long ID = 0)
        {
            ...
            vm.NewsImage = string.Format("{0}//{1}//{2}",
               ConfigurationExtensions.GetAppConfigValue("ImageHostUrl").ToString(), EduegateImageTypes.News.ToString(), vm.ImageUrl);
            vm.ImageUrl = vm.NewsImage; // assigning newsimage to ImageUrl to show it in image preview
            return Json(vm, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult Save(NewsViewModel vm)
        {
            ...
            string tempFolderPath = string.Format("{0}\\{1}\\{2}\\{3}\\{4}",
                    ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.News.ToString(), Constants.TEMPFOLDER, userID, fileName);

            if (System.IO.File.Exists(tempFolderPath))
            {
                string orignalFolderPath = string.Format("{0}//{1}//{2}",
                       ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.News.ToString(), fileName);
