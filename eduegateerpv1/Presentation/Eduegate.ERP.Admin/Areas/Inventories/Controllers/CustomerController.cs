...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    public class CustomerController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.Customer);
        ...
        [HttpPost]
        public JsonResult Save(CustomerMasterViewModel vm)
        {
            Eduegate.Services.Contracts.Accounting.SupplierAccountMapDTO dto = new Services.Contracts.Accounting.SupplierAccountMapDTO();
            ...
            if (updatedDTO.IsNotNull())
            {
                ...
                string destinationPath = string.Format(@"{0}{1}\{2}\{3}",
                       ConfigurationExtensions.GetAppConfigValue("ImagesPhysicalPath").ToString(), EduegateImageTypes.Documents.ToString(), Eduegate.Framework.Contracts.Common.Enums.EntityTypes.Customer.ToString(), updatedDTO.CustomerIID);
            ...
            return Json(customerMasterViewModel, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult GetCustomerSummaryDetails(long ID = 0)
        {
            ...
            return Json(CustomerViewModel.FromCustomerDTO(dto), JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult IsCustomerExist(string email, string phone)
        {
            ...
            return Json(CustomerMasterViewModel.FromDTO(dto), JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetEmployees(string searchText, string lookupName)
        {
            try
            {
                var dtos = ClientFactory.EmployeeServiceClient(CallContext).SearchEmployee(searchText, ConfigurationExtensions.GetAppConfigValue<int>("Select2DataSize"));
                var VMs = new List<KeyValueViewModel>();
                foreach (var item in dtos)
                {
                    KeyValueViewModel vm = new KeyValueViewModel();
        ...
        public JsonResult GetContactByCustomerId(long customerId)
        {
            ...
            return Json(new { ShippingContact = contact }, JsonRequestBehavior.AllowGet);
        }
    }
}