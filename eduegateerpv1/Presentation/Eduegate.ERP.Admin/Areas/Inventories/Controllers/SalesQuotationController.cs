...
{
    public class SalesQuotationController : BaseSearchController
    {
        private string dateTimeFormat = ConfigurationExtensions.GetAppConfigValue("DateTimeFormat");

        // GET: Inventories/SalesQuotation
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.SalesQuotation);

            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.SalesQuotation,
                ViewName = Infrastructure.Enums.SearchView.SalesQuotation,
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            return PartialView("_ChildList", new SearchListViewModel
            {
                ...
                IsChild = true,
                ParentViewName = Infrastructure.Enums.SearchView.SalesQuotation,
        ...
        {
            var view = Infrastructure.Enums.SearchView.SalesQuotation;
        ...
        {
            return base.SearchSummaryData(Infrastructure.Enums.SearchView.SalesQuotationSummary);
        }
        ...
        {
            var vm = new SalesQuotationViewModel()
            {
        ...
        {
            var vm = TransactionViewModel.FromSalesQuotationDTO(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, false, false));
            if (vm != null)
            {
                vm.DetailViewModel.Add(new SalesQuotationDetailViewModel());
            }
        ...
        [HttpPost]
        public JsonResult Save(SalesQuotationViewModel vm)
        {
            try
            {
                var updatedVM = new SalesQuotationViewModel();

                if (vm != null)
                {
                    var result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(TransactionViewModel.ToSalesQuotationDTO(vm));
                    updatedVM = TransactionViewModel.FromSalesQuotationDTO(result);

                    if (updatedVM != null)
                    {
                        updatedVM.DetailViewModel.Add(new SalesQuotationDetailViewModel());
                    }
            ...
            {
                Eduegate.Logger.LogHelper<SalesQuotationController>.Fatal(ex.Message.ToString(), ex);
        ...
        public ActionResult SaveDeliveryDetails(OrderDeliveryDetailViewModel order)
        {
            ...
            {
                Eduegate.Logger.LogHelper<SalesQuotationController>.Info("Exception : " + ex.Message.ToString());
