...
//using Eduegate.Services.Contracts.Eduegate;
...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    public class ShoppingCartController : BaseSearchController
    {
        ...
        {
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.ShoppingCart);
        ...
        public ActionResult ChildList(long ID)
        {
            ...
            var metadata = ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.ShoppingCartItems);
        ...
        public JsonResult GetInventoryDetailsSKUID(long skuID, long customerID)
        {
            ...
            return Json(new { ViewModel = vm }, JsonRequestBehavior.AllowGet);
        ...
        {
            vm.Message = "";
            if (string.IsNullOrEmpty(vm.Description)) { vm.Message = "Description cannot be empty"; return Json(vm, JsonRequestBehavior.AllowGet); }
        ...
        public JsonResult CreateCart(long customerID)
        {
            ...
            if (customerID == 0)
            {
                return Json(new { IsError = true, Message = "Please select a customer." }, JsonRequestBehavior.AllowGet);
            }
        ...
        public JsonResult UpdateCartStatus(long cartID, ShoppingCartStatus statusID, string paymentMethod)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { IsError = !IsCartStatusUpdated, Message = ex.Message.ToString() }, JsonRequestBehavior.AllowGet);
            }

            return Json(new { IsError = !IsCartStatusUpdated, data = IsCartStatusUpdated }, JsonRequestBehavior.AllowGet);
        }
        ...
        //    return Json(new { IsError = !Convert.ToBoolean(serviceResult) }, JsonRequestBehavior.AllowGet);
        //}

        [HttpPost]
        public JsonResult Save(CartViewModel vm)
        {
            ...
            {
                return Json(new { IsError = true, UserMessage = "Payment initalization failed" }, JsonRequestBehavior.AllowGet);
            }
            ...
            string orderService = string.Concat(ConfigurationExtensions.GetAppConfigValue("ServiceHost"), Constants.ORDER_SERVICE_NAME);
            ...
            if (!GenerateOrderNotification(transactionID))
            {
                return Json(new { IsError = false, UserMessage = "Successfully saved, email generation failed.", isAutoClose = true }, JsonRequestBehavior.AllowGet);
            }

            return Json(new { IsError = false, UserMessage = "Successfully saved", isAutoClose = true }, JsonRequestBehavior.AllowGet);
        }
        ...
        //    {
        //        Eduegate.Logger.LogHelper<ShoppingCartController>.Fatal(ex.Message.ToString(), ex);
        //        return Json(new { IsError = false, Message = ex.Message.ToString() }, JsonRequestBehavior.AllowGet);
        //    }
        //}
        ...
        //    {
        //        Eduegate.Logger.LogHelper<ShoppingCartController>.Fatal(ex.Message.ToString(), ex);
        //        return Json(new { IsError = true, Message = ex.Message.ToString() }, JsonRequestBehavior.AllowGet);
        //    }
        //    return Json(new { IsError = false, data = cart, CartQtyUpdated = currentUpdatedQty }, JsonRequestBehavior.AllowGet);
        //}
        ...
        //    {
        //        Eduegate.Logger.LogHelper<ShoppingCartController>.Fatal(ex.Message.ToString(), ex);
        //        return Json(new { IsError = !IsItemRemovedFromCart, Message = ex.Message.ToString() }, JsonRequestBehavior.AllowGet);
        //    }
        //    return Json(new { IsError = !IsItemRemovedFromCart }, JsonRequestBehavior.AllowGet);
        //}
