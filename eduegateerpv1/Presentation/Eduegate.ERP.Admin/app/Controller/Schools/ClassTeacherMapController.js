app.controller("ClassTeacherMapController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    console.log("ClassTeacherMapController Loaded");
    ...
    $scope.FillClassWiseDropDowns = function ($event, $element, crudModel) {
        ...
        showOverlay();
        var url = "Schools/School/FillClassSectionWiseCoordinators?classID=" + model.Class.Key + "&sectionID=" + model.Section.Key;
        $http({ method: 'Get', url: url })
            .then(function (result) {
    ...
    $scope.RefreshButtonClicks = function ($event, $element, crudModel) {
        ...
        showOverlay();
        var url = "Schools/School/FillEditDatasAndSubjects?IID=" + model.ClassClassTeacherMapIID + "&classID=" + model.Class.Key + "&sectionID=" + model.Section.Key;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                $scope.CRUDModel.ViewModel.OtherClassTeachers = result.data;
                $scope.fillSubjectLookUp(model.Class.Key, model.Section.Key, model.ClassClassTeacherMapIID);
                hideOverlay();
            }, function () {
                hideOverlay();
            });
