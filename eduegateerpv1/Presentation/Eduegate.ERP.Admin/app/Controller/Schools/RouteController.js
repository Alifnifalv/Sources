app.controller("RouteController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$controller", function ($scope, $http, $compile, $window, $timeout, $location, $route, $controller) {
    ...
    $scope.PeriodChanges = function ($event, $element, gridModel) {
        ...
        if (academicYearId == null || academicYearId == 0) {
            $().showMessage($scope, $timeout, true, "Please Select an Academic year");
            $scope.AllPeriods = null;
            hideOverlay();
            return false;
        }
        if ($scope.AllPeriods == undefined || $scope.AllPeriods.length == 0) {
    ...
    ////// For Event transport Allocation start ////////////////
    ...
    $scope.FillStudentStaffDatasByRouteIDforEvent = function ($event, $element, crudModel) {
        showOverlay();
        var model = crudModel;
        if (model.IsPickUp == false && model.IsDrop == false) {
            $().showMessage($scope, $timeout, true, "Please select pickUp/Drop checkbox !");
            hideOverlay();
            $scope.CRUDModel.ViewModel.Route = null;
            return false
        }
        var routeId = $scope.CRUDModel.ViewModel.Route?.Key;
        ...
        showOverlay();
        var url = "Schools/School/GetStudentandStaffsByRouteIDforEvent?routeID=" + routeId + "&IsRouteType=" + model.IsRouteType;
        $http({ method: 'Get', url: url })
            .then(function (result) {
                $scope.CRUDModel.ViewModel.StudentList = result.data.filter(x => x.StudentID != null);
                $scope.CRUDModel.ViewModel.StaffList = result.data.filter(x => x.StaffID != null);

                if ($scope.CRUDModel.ViewModel.StudentList.length <= 0 && $scope.CRUDModel.ViewModel.StaffList.length <= 0) {
                    $scope.CRUDModel.ViewModel.SelectedDetails = "No datas found for " + $scope.CRUDModel.ViewModel.IsRouteType + " Route : " + model.Route.Value;
                }
                else {
                    $scope.CRUDModel.ViewModel.SelectedDetails = "List of " + $scope.CRUDModel.ViewModel.IsRouteType + " data for Route : " + model.Route.Value;
                }
    ...
    ////// For Event transport Allocation end ////////////////
