...
namespace Eduegate.ERP.Admin.Areas.Accounts.Controllers
{
    ...
    public class AssetRemovalController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Eduegate.Infrastructure.Enums.SearchView.AssetRemoval);
        ...
        public async Task<IActionResult> ChildList(long ID)
        {
            ...
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList(Services.Contracts.Enums.SearchView.AssetRemovalBranch);
        ...
        public JsonResult Save(string model)
        {
            ...
            var assetDisposalViewModel = JsonConvert.DeserializeObject<AssetDisposalViewModel>(jsonData.SelectToken("data").ToString());
            ...
            try
            {
                if (assetDisposalViewModel != null)
                {
                    ...
                    {
                        assetDisposalViewModel.DetailViewModel = assetDisposalViewModel.DetailViewModel.Where(x => x.AssetCode != null).ToList();
                        ...
                        {
                            return Json(new { IsError = true, UserMessage = " Maximum number of asset code should be one" });
                        }
            ...
            return Json(assetDisposalViewModel);
        }
