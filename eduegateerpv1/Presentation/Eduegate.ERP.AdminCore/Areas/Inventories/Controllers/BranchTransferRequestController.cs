...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class BranchTransferRequestController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.BranchTransferRequest);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<BranchTransferRequestViewModel>(jsonData.SelectToken("data").ToString());
