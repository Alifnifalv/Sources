...
namespace Eduegate.ERP.Admin.Areas.Inventories.Controllers
{
    ...
    public class PurchaseInvoiceController : BaseSearchController
    {
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.PurchaseInvoice);
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<PurchaseInvoiceViewModel>(jsonData.SelectToken("data").ToString());
                ...
                if (vm != null)
                {
                    ...
                    {
                        line.IsError = false;
                        if (line.Quantity > 0 && line.SKUID.IsNotNull() && /*line.SKUDetails.Count > 0 &&*/ line.SKUDetails != null)
                        {
                    ...
                    if (isError)
                    {
                        return Json(new { IsError = true, UserMessage = "Error with serial numbers!", data = vm });
                    }
                    ...
                    if (vm.DetailViewModel.Count > 0)
                    {
                        foreach (var x in vm.DetailViewModel)
                            if(x.SKUID != null)
                            {
                                ...
                                {
                                    return Json(new { IsError = true, UserMessage = "Unit or Fraction is not found for " + x.Description, data = vm });
                                }
                            }
                    }
        ...
        public JsonResult SerialKeyDuplicationCheck(PurchaseInvoiceViewModel vm)
        {
            ...
            return Json(new { IsError = isError, ErrorMessage = message });
        }
