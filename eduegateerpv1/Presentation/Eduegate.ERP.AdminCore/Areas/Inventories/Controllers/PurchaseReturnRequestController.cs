...
{
    [Area("Inventories")]
    public class PurchaseReturnRequestController : BaseSearchController
    {
        private string dateTimeFormat = new Domain.Setting.SettingBL().GetSettingValue<string>("DateTimeFormat");

        [HttpGet]
        // GET: Inventories/PurchaseReturnRequest
        ...
        {
            var metadata = await ClientFactory.SearchServiceClient(CallContext).SearchList((Services.Contracts.Enums.SearchView)(int)Infrastructure.Enums.SearchView.PurchaseReturnRequest);

            return View(new SearchListViewModel
            {
                ControllerName = "Inventories/" + Infrastructure.Enums.SearchView.PurchaseReturnRequest,
                ViewName = Infrastructure.Enums.SearchView.PurchaseReturnRequest,
        ...
        public async Task<IActionResult> ChildList(long ID)
        {
            ...
            return PartialView("_ChildList", new SearchListViewModel
            {
                ...
                IsChild = true,
                ParentViewName = Infrastructure.Enums.SearchView.PurchaseReturnRequest,
        ...
        {
            var view = Infrastructure.Enums.SearchView.PurchaseReturnRequest;
        ...
        {
            return base.SearchSummaryData(Infrastructure.Enums.SearchView.PurchaseReturnRequestSummary);
        }
        ...
        {
            var vm = new PurchaseReturnRequestViewModel()
            {
        ...
        public JsonResult Save(string model)
        {
            try
            {
                ...
                var vm = JsonConvert.DeserializeObject<PurchaseReturnRequestViewModel>(jsonData.SelectToken("data").ToString());

                var updatedVM = new PurchaseReturnRequestViewModel();

                if (vm != null)
                {
                    var result = ClientFactory.TransactionServiceClient(CallContext).SaveTransactions(PurchaseReturnRequestViewModel.FromVMToTransactionDTO(vm));
                    updatedVM = PurchaseReturnRequestViewModel.FromTransactionDTOToVM(result);
                    updatedVM.DetailViewModel.Add(new PurchaseReturnRequestDetailViewModel());
        ...
        public ActionResult Get(long ID = 0)
        {
            try
            {
                var vm = PurchaseReturnRequestViewModel.FromTransactionDTOToVM(ClientFactory.TransactionServiceClient(CallContext).GetTransaction(ID, false, false));
                if (vm != null)
                {
                    vm.DetailViewModel.Add(new PurchaseReturnRequestDetailViewModel());
                }
                return Json(vm);
            }
            catch (Exception ex)
