...
namespace Eduegate.ERP.AdminCore.Areas.Payroll.Controllers
{
    ...
    public class EmployeeLeaveSettlementController : BaseSearchController
    {
        ...
        public JsonResult GetEmployeeDetailsToSettlement(string employeeID, string salaryCalculationDateString, decimal? noofDaysInTheMonth, byte? employeeSalarySettlementTypeID)
        {
            try
            {
                var dateFormat = new Domain.Setting.SettingBL().GetSettingValue<string>("DateFormat");
                var salaryCalculationDate = DateTime.ParseExact(salaryCalculationDateString, dateFormat, CultureInfo.InvariantCulture);
                var employeeSettlementDTO = new EmployeeSettlementDTO();
                var operationResultDTOs = ClientFactory.EmployeeServiceClient(CallContext).GetEmployeeDetailsToSettlement(new EmployeeSettlementDTO() { EmployeeID = long.Parse(employeeID), SalaryCalculationDate = salaryCalculationDate, NoofDaysInTheMonth = noofDaysInTheMonth, EmployeeSettlementTypeID = employeeSalarySettlementTypeID , IsFromSalarySlipGeneration =false});
        ...
        public JsonResult GetEmployeeDetailsToFinalSettlement(string employeeID, string salaryCalculationDateString, string dateOfLeavingString, decimal? endofServiceDays, decimal? noofDaysInTheMonthES, decimal? noofDaysInTheMonthEoSB, byte? employeeSalarySettlementTypeID)
        {
            try
            {
                var dateFormat = new Domain.Setting.SettingBL().GetSettingValue<string>("DateFormat");
