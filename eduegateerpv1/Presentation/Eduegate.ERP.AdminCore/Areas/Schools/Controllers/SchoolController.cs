...
namespace Eduegate.ERP.Admin.Areas.Schools.Controllers
{
    ...
    public class SchoolController : BaseController
    {
        ...
        public ActionResult SaveCoScholasticEntry(CoScholasticEntryDTO dto)
        {
            ...
            if (dto.CoScholasticEntryDetailDTOs.Where(x => x.MarkStatusID != null || x.MarksGradeMapID != null).Count() == 0)
            {
                rtnMsg = "0#Please enter mark details for at least a student!";
            }
            else
            {
                foreach (var z in dto.CoScholasticEntryDetailDTOs.Where(x => x.MarkStatusID != null || x.MarksGradeMapID != null))
                {
     ...
     int academicYearID, int? skillID, short? languageTypeID, int? subjectMapID, long subjectID)
        ...
        public JsonResult GetSubjectsAndMarksToPublish(int classID, int sectionID, long termID, long examID, int academicYearID)
        {
                                  ...
                                  select new MarkApprovalViewModel
                                  {
                                      ...
                                      StudentMarkList = (from sg in s.MarkRegisterSplitDTO
                                                         select new MarkApprovalMarkListViewModel
                                                         {
                                                             ...
                                                             TotalMark = sg.TotalMark == null ? sg.Mark : sg.TotalMark,
            ...
            var result = Json(approvalResult);
        ...
        public List<ProgressReportDTO> FillProgressReportDetails(MarkRegisterDTO markRegister, List<SettingDTO> settings, string reportName)
        {
            ...
            var groupedPubllishedStudMarkRegDetails = markRegister.MarkRegistersDetails != null && markRegister.MarkRegistersDetails.Count > 0 ? markRegister.MarkRegistersDetails.Where(r => r.MarkEntryStatusID == markEntryPublishedStatusID || r.MarkEntryStatusID == markEntryGeneratedStatusID).GroupBy(g => g.StudentID).ToList() : null;
                ...
                foreach (var publlishedStudMarkRegDetails in groupedPubllishedStudMarkRegDetails.ToList())
                {
                    ...
                    if (regDet != null)
                    {
                        progressReportDTO = new ProgressReportDTO()
                        {
                            ...
                            PublishStatusID = regDet.MarkEntryStatusID,
                        };
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Eduegate.Logger.LogHelper<string>.Fatal($"Progress report generation failed. Error message: {errorMessage} ,reportName: {reportName} ,studentID: {progressReportDTO.StudentID}", ex);
                        }
        ...
        public long? GetProgressReportContentID(ProgressReportDTO progressReportDTO, string reportName)
        {
            ...
            if (progressReportDTO != null)
            {
                ...
                if (!string.IsNullOrEmpty(contentData.ContentFileName))
                {
        ...
        public async Task<FileStreamResult> DownloadReports(int classID, int sectionID, int examGroupID, int examID, int academicYearID, byte statusID, string customFolder)
        {
            try
            {
                var contentIDs = ClientFactory.SchoolServiceClient(CallContext).GetProgressReportIDsByStudPromStatus(classID, sectionID, academicYearID, statusID, examID, examGroupID);

                var result = await DownloadAllAsZip(contentIDs);
                return result;
            }
            catch (Exception ex)
            {
                ...
                {
                    FileDownloadName = "error.zip" // Provide a default error file name
                };
            }
        }
