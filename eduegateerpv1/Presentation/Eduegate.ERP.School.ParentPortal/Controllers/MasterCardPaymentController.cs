...
namespace Eduegate.ERP.School.ParentPortal.Controllers
{
    ...
    {
        public ActionResult GetMasterCardSessionID()
        {
            var result = ClientFactory.PaymentGatewayServiceClient(CallContext).GenerateCardSessionID();

            if (result != null)
            {
                return Json(new { IsError = false, Response = "Successfully generated" }, JsonRequestBehavior.AllowGet);
            }
            else
            {
                return Json(new { IsError = true, Response = "There are some issues!" }, JsonRequestBehavior.AllowGet);
            }

        }

        public ActionResult GenerateMasterCardSessionIDByTransactionNo(string transactionNo)
        {
            var result = ClientFactory.PaymentGatewayServiceClient(CallContext).GenerateCardSessionIDByTransactionNo(transactionNo);
