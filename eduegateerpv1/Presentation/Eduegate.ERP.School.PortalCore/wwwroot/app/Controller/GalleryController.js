app.controller('GalleryController', ['$scope', '$http', '$compile', "$rootScope", "subscriptionService", "toaster", '$timeout', function ($scope, $http, $compile, $root, $subscription, $toaster, $timeout) {
    ...
    $scope.GetGalleryDataByAcademic = function () {
        ...
        //showOverlay();
        $scope.Gallery = [];
        var academicYearID =  $scope.SchoolAcademicYear != null ? $scope.SchoolAcademicYear.Key : null;
        $.ajax({
            type: "GET",
    ...
    $scope.GetSchoolsList = function () {
        ...
        $.ajax({
            ...
            url: utility.myHost + "Home/GetSchoolsByLoginIDActiveStuds?loginID",
            contentType: "application/json;charset=utf-8",
            success: function (result) {
    ...
    $scope.SchoolChanges = function () {
        ...
        var schoolID = $scope.GallerySchool != null ? $scope.GallerySchool.Key  : null;
        $.ajax({
            type: "GET",
