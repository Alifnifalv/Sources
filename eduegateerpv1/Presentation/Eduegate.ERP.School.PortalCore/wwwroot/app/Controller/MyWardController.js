...
app.controller("MyWardController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $rootScope, $uibModal) {
    ...
    $scope.fillInvoiceOld = function (studentId) {
        //showOverlay();
        $scope.FeeMonthly = [];
        $.ajax({
            type: "GET",
    ...
    $scope.FillInvoice = function (studentId) {
        ...
        $.ajax({
            type: "GET",
            data: { studentId: studentId },
            url: utility.myHost + "Home/GetStudentFeeDetails",
            contentType: "application/json;charset=utf-8",
            success: function (result) {
                if (!result.IsError && result !== null) {
                    $scope.$apply(function () {
    ...
    //#region Leave request related
    $scope.LeaveApplicationListClick = function () {

        $("#leaveApplication .featuresTab ul li, #leaveApplication div.featureContent").removeClass("active");
        $('#leaveList .featuresTab ul li[data-attr="LeaveList"], #leaveList div.featureContent[data-attr="LeaveList"]').addClass('active');

        $('#leaveApplication .featuresTab ul li[data-attr="LeaveApplication"], #leaveApplication div.featureContent[data-attr="LeaveApplication"]').removeClass('active');
    ...
    $scope.NewLeaveApplicationClick = function () {
        ...
        $("#leaveList .featuresTab ul li, #leaveList div.featureContent").removeClass("active");
        $('#leaveApplication .featuresTab ul li[data-attr="LeaveApplication"], #leaveApplication div.featureContent[data-attr="LeaveApplication"]').addClass('active');

        $('#leaveList .featuresTab ul li[data-attr="LeaveApplication"], #leaveList div.featureContent[data-attr="LeaveList"]').removeClass('active');

        $("#SubmitLeaveBtn").html("Submit");
        $("#SubmitLeaveBtn").prop("disabled", false);

        $scope.GetLeaveApplicationModel();
    };

    $scope.GetLeaveApplicationModel = function () {

        $.ajax({
            type: "GET",
            url: utility.myHost + "Home/GetleaveApplication?Id=" + 0,
            contentType: "application/json;charset=utf-8",
            success: function (result) {
                if (!result.IsError && result !== null) {
                    $scope.$apply(function () {
                        ...
                        if (!$scope.LeaveApplication.StudentID) {

                            $scope.LeaveApplication.StudentID = $scope.activeStudentID;
                        }
    ...
    $scope.SubmitLeaveApplication = function () {
        ...
        var leaveApplication = $scope.LeaveApplication;

        if (!leaveApplication.Reason) {
            callToasterPlugin('error', 'Reason is required');
            return false;
        }

        if (!leaveApplication.FromDateString || leaveApplication.FromDateString == "?") {
            callToasterPlugin('error', 'From date is required');
            return false;
        }
        else {
            leaveApplication.FromDateString = moment(leaveApplication.FromDateString).format('DD/MM/YYYY');
        }

        if (!leaveApplication.ToDateString || leaveApplication.ToDateString == "?") {
            callToasterPlugin('error', 'To date is required');
            return false;
        }
        else {
            leaveApplication.ToDateString = moment(leaveApplication.ToDateString).format('DD/MM/YYYY');
        }
                ...
                if (result) {
                    if (result.Response) {
                        if (result.Response.IsError) {
                            ...
                            $scope.GetLeaveApplicationModel();
                            $scope.LeaveApplicationListClick();
                        }
                    }
                    else {
                        callToasterPlugin('error', "Unable to save!");
                    }
                }
                else {
                    callToasterPlugin('error', "Unable to save!");
    ...
    $scope.EditLeaveRequestClick = function (requestId) {
        ...
        $('#Leave .featuresTab ul li[data-attr="LeaveApplication"], #Leave div.featureContent[data-attr="LeaveList"]').removeClass('active');
    ...
    //#endregion Leave request related
    ...
    //#region TC related
    $scope.TCApplication_checkbox1 = function () {
    ...
    $scope.TCApplication_checkbox2 = function () {
    ...
    $scope.TCRequestListClick = function () {
        $("#TCRequest #TCRequest01").addClass("active");
        $('#TCRequest #TCRequest02').removeClass('active');

        $("#TCRequest .featuresTab ul li, #TCRequest div.featureContent").removeClass("active");
        $('#TCRequest .featuresTab ul li[data-attr="LeaveList"], #TCRequest div.featureContent[data-attr="LeaveList"]').addClass('active');
    ...
    $scope.NewTCRequestApplicationClick = function () {
        ...
        $("#TCRequest .featuresTab ul li, #TCRequest div.featureContent").removeClass("active");
        $('#TCRequest .featuresTab ul li[data-attr="transferApplication"], #TCRequest div.featureContent[data-attr="transferApplication"]').addClass('active');

        $("#SubmitTcRequestBtn").html("Submit");
        $("#SubmitTcRequestBtn").prop("disabled", false);

        $scope.GetTCRequestModel();
    };

    $scope.GetTCRequestModel = function () {

        $scope.StudentTransferRequestReasons = {};

        $.ajax({
            type: "GET",
            url: utility.myHost + "Home/StudentTransferApplication?Id=" + 0,
            contentType: "application/json;charset=utf-8",
            success: function (result) {
                if (!result.IsError && result !== null) {
                    $scope.$apply(function () {
                        ...
                        if (!$scope.TransferApplication.StudentID) {
    ...
    $scope.SubmitTransferApplication = function () {
        ...
        var transferApplication = $scope.TransferApplication;

        transferApplication.StudentTransferRequestReasons = transferApplication.TransferRequestReason.Key;
        if (!transferApplication.StudentTransferRequestReasons) {
            callToasterPlugin('error', 'Reason is required');
            return false;
        }
        if (!transferApplication.ExpectingRelivingDateString || transferApplication.ExpectingRelivingDateString == "?") {
            callToasterPlugin('error', 'Date is required');
            return false;
        }
        else {
            transferApplication.ExpectingRelivingDateString = moment(transferApplication.ExpectingRelivingDateString).format('DD/MM/YYYY');
        }
                    ...
                    function format(d) {
                        ...
                        for (let item of array1) {
                            ...
                            allItemsHTML +=  (
                                ...
                                '</dd>' +
                                '<dt>Attachment</dt>' +
                                `<a class="btn btn-icon-white btn-sm btn-primary me-2" href="ReadContentsByID?contentID=${item.ContentFileIID}"><i class="fa fa-2x fa-download fs-3" aria-hidden="true" ng-click="DirectDownload(${item.ContentFileIID})"></i>download</a>` +
                                '</dl>'
    ...
    $scope.getCounselorList = function () {
        showOverlay();
        $.ajax({
            type: "GET",
            data: { parentId: 0 },
    ...
    $scope.GetAcademicYearsByStudentID = function (studentID) {
        ...
        if (!schoolID) {
            ...
            $.ajax({
                ...
                url: utility.myHost + "Home/GetAcademicYearBySchool",
                contentType: "application/json;charset=utf-8",
                success: function (result) {
