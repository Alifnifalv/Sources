app.controller("ExamQuestionController", ["$scope", "$http", "$compile", "$window", "$timeout", "$location", "$route", "$rootScope", function ($scope, $http, $compile, $window, $timeout, $location, $route, $root) {
    ...
    $scope.Init = function (questionList, candidateExamMap) {
        ...
        $scope.PassageQuestionCount = questionList.filter(question => !question.IsPassageQn).length;
    ...
    $scope.FillIndex = function (indexNo, passageQnNo, subIndexNo) {
        if (passageQnNo != null && passageQnNo != undefined) {
            $scope.QuestionIndexNo = passageQnNo + indexNo + 1 + '.' + subIndexNo; 
        } else {
            $scope.QuestionIndexNo = indexNo + 1;
        }
    };
