...
namespace Eduegate.Web.Library.School.Circulars
{
    ...
    public class CircularViewModel : BaseMasterViewModel
    {
        ...
        public override BaseMasterDTO ToDTO()
        {
            ...
            if (this.Departments != null && this.Departments.Count > 0)
            {
                if (this.Departments.Any(x => x.Value.Contains("All Departments")))
                {
                    var cls = this.Departments.FirstOrDefault(x => x.Value == "All Departments");

                    dto.CircularMaps.Add(new CircularMapDTO()
                    {
                        AllDepartment = (cls.Value == null || cls.Value == "") &&
                        cls.Value != "All Departments"
                            ? false : true,
                        Department = new KeyValueDTO() { Key = null, Value = "All Departments" },
                    });
                }
                else
                {
                    foreach (var dep in this.Departments)
                    {
                        if (!string.IsNullOrEmpty(dep.Key))
                        {
                            dto.CircularMaps.Add(new CircularMapDTO()
                            {
                                DepartmentID = int.Parse(dep.Key),
