...
namespace Eduegate.Web.Library.ViewModels.HR
{
    ...
    [DisplayName(" ")]
    public class EmploymentRequestDocumentFileViewModel : BaseMasterViewModel
    {
        ...
        public static List<EmploymentRequestDocumentFileViewModel> FromDTO(List<DocumentFileDTO> dto)
        {
            Mapper<DocumentFileDTO, EmploymentRequestDocumentFileViewModel>.CreateMap();
            Mapper<KeyValueDTO, KeyValueViewModel>.CreateMap();
            var maps = Mapper<List<DocumentFileDTO>, List<EmploymentRequestDocumentFileViewModel>>.Map(dto);

            if (maps.Count == 0)
            {
                maps.Add(new EmploymentRequestDocumentFileViewModel());
            }
            return maps;
        }

        public static List<DocumentFileDTO> ToDTO(List<EmploymentRequestDocumentFileViewModel> vm)
        {
            Mapper<EmploymentRequestDocumentFileViewModel, DocumentFileDTO>.CreateMap();
            Mapper<KeyValueViewModel, KeyValueDTO>.CreateMap();
            var maps = Mapper<List<EmploymentRequestDocumentFileViewModel>, List<DocumentFileDTO>>.Map(vm);
